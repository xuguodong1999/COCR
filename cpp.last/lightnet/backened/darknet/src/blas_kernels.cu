...
extern "C" void mean_delta_gpu(float *delta, float *variance, int batch, int filters, int spatial, float *mean_delta) {
    mean_delta_kernel<<<cuda_gridsize(filters), BLOCK, 0, get_cuda_stream() >>>(delta, variance, batch, filters,
                                                                                spatial, mean_delta);
    CHECK_CUDA(cudaPeekAtLastError());
}
...
extern "C" void fast_v_cbn_gpu(const float *x, float *mean, int batch, int filters, int spatial, int minibatch_index,
                               int max_minibatch_index, float *m_avg, float *v_avg, float *variance,
                               const float alpha, float *rolling_mean_gpu, float *rolling_variance_gpu,
                               int inverse_variance, float epsilon) {
    fast_v_cbn_kernel << <
...
extern "C" void variance_gpu(float *x, float *mean, int batch, int filters, int spatial, float *variance) {
    variance_kernel<<<cuda_gridsize(filters), BLOCK, 0, get_cuda_stream() >>>(x, mean, batch, filters, spatial,
                                                                              variance);
    CHECK_CUDA(cudaPeekAtLastError());
}
...
extern "C" void gradient_centralization_gpu(int w, int h, int c, int f, float *in) {
    ...
    if (f_size % WARP_SIZE == 0) {
        ...
        get_number_of_blocks(size, BLOCK), BLOCK, 0, get_cuda_stream() >> > (f, f_size, in);
        CHECK_CUDA(cudaPeekAtLastError());
    }
}
...
__global__ void shortcut_multilayer_kernel(int size, int src_outputs, int batch, int n, int *outputs_of_layers_gpu,
                                           float **layers_output_gpu, float *out, float *in, float *weights_gpu,
...
extern "C" void backward_shortcut_multilayer_gpu(int src_outputs, int batch, int n, int *outputs_of_layers_gpu,
    ...
    cuda_gridsize(size), BLOCK, 0, get_cuda_stream() >> > (size, src_outputs, batch, n, outputs_of_layers_gpu,
...
extern "C" void shortcut_gpu(int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float *out) {
    ...
    shortcut_kernel<<<cuda_gridsize(size), BLOCK, 0, get_cuda_stream()>>>(size, minw, minh, minc, stride, sample, batch,
                                                                          w1, h1, c1, add, w2, h2, c2, out);
    CHECK_CUDA(cudaPeekAtLastError());
}
...
softmax_kernel_new_api(float *input, int n, int batch, int batch_offset, int groups, int group_offset, int stride,
    ...
    int b = id / groups;
    int g = id % groups;
    softmax_device_new_api(input + b * batch_offset + g * group_offset, n, temp, stride,
...
softmax_tree_gpu(float *input, int spatial, int batch, int stride, float temp, float *output, tree hier) {
    ...
    */
    ...
    softmax_tree_kernel <<<cuda_gridsize(num), BLOCK, 0, get_cuda_stream() >>>(input, spatial, batch, stride, temp,
...
backward_scale_channels_kernel(float *in_w_h_c_delta, int size, int channel_size, int batch_size, int scale_wh,
    ...
    if (index < size) {
        ...
        if (scale_wh) {
            ...
            atomicAdd(&out_state_delta[osd_index],
                      in_w_h_c_delta[index] * in_from_output[index] / channel_size); // l.delta * from

            out_from_delta[index] +=
                    in_scales_c[osd_index] * in_w_h_c_delta[index]; // input * l.delta  // atomic isn't required here
...
backward_scale_channels_gpu(float *in_w_h_c_delta, int size, int channel_size, int batch_size, int scale_wh,
    ...
    num_blocks, block_size, 0, get_cuda_stream() >> > (in_w_h_c_delta, size, channel_size, batch_size, scale_wh,
...
__global__ void backward_sam_kernel(float *in_w_h_c_delta, int size, int channel_size,
    ...
    if (index < size) {
        out_state_delta[index] +=
                in_w_h_c_delta[index] * in_from_output[index]; // l.delta * from  (should be divided by channel_size?)
...
sway_and_flip_weights_kernel(const float *src_weight_gpu, float *weight_deform_gpu, int nweights, int n,
    ...
    if (i < nweights) {
        ...
        if (stage_id == 0) {
            ...
            for (int y = 0; y < kernel_size; ++y) {
                for (int x = 0; x < kernel_size; ++x) {
                    weight_deform_gpu[(kernel_size - x - 1) + y * kernel_size + i] = src_weight_gpu[x +
                                                                                                    y * kernel_size +
                                                                                                    i];
                }
            }
...
rotate_weights_gpu(const float *src_weight_gpu, float *weight_deform_gpu, int nweights, int n, int size, int reverse) {
    ...
    num_blocks, block_size, 0, get_cuda_stream() >> > (src_weight_gpu, weight_deform_gpu, nweights, n, size, reverse);
...
extern "C" void reduce_and_expand_array_gpu(const float *src_gpu, float *dst_gpu, int size, int groups) {
    ...
    num_blocks, block_size, 0, get_cuda_stream() >> > (src_gpu, dst_gpu, current_size, groups);
...
P_constrastive_f_det_gpu(int *labels, unsigned int feature_size, float temperature, contrastive_params *contrast_p,
    ...
    num_blocks, block_size, 0, get_cuda_stream() >> > (labels, feature_size, temperature, contrast_p, contrast_p_size);
