...
torch::data::Example<> CouchDataSet::get(size_t index) {
    ...
    if (isTrainMode() && prob < max_prob / 4) {
        ...
        if (availableFontFamilies.empty()) {
            ...
            for (auto &f: blackList) {
                availableFontFamilies.removeOne(f);
            }
        }
        cv::Mat c1(batchHeight, batchWidth, CV_8UC1,
                   convertToScalar(ColorName::rgbWhite));
        const auto &s = gbTable[sCastTable[classIndex]];
        auto tmpMat = CvUtil::GetFont(
                std::string(s.begin(), s.end()),
                availableFontFamilies[rand() % availableFontFamilies.size()].toStdString());
        cv::Mat c1_1;
        cv::resize(*tmpMat.getHolder(), c1_1, cv::Size(batchWidth * kx, batchHeight * ky));
...
//        auto item = CouchLoader::GetShapeItem(classIndex, sampleIndex);
        auto bgColor = convertToScalar(ColorName::rgbWhite),
                strokeColor = convertToScalar(ColorName::rgbBlack);
        if (isTrainMode()) {
            // FIXME
//            if (byProb(RC::revertColorProb)) {// 反转颜色
