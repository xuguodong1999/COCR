...
void valid(po::variables_map &vm, DataLoader::ImageFolderBBWithPaths &valid_dataloader, torch::Device &device,
           Loss &criterion, YOLOv2 &model, const std::vector<std::string> class_names, const size_t epoch,
...
// Training Function
// -------------------
void train(po::variables_map &vm, torch::Device &device, YOLOv2 &model, std::vector<transforms_Compose > &transformBB,
           std::vector<transforms_Compose > &transformI, const std::vector<std::string> class_names,
           const std::vector<std::tuple<float, float>> anchors,
    ...
    // (5) Set Detector
    auto detector = YOLODetector(anchors, (long int) vm["class_num"].as<size_t>(), vm["prob_thresh"].as<float>(),
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        show_progress = new progress::display(/*count_max_=*/total_iter, /*epoch=*/{epoch, total_epoch}, /*loss_=*/
                                                             {"coord_xy", "coord_wh", "conf_o", "conf_x", "class", "W",
                                                              "H"});
        ...
        while (dataloader(mini_batch)) {
            ...
            image = F::interpolate(image, F::InterpolateFuncOptions().size(std::vector<long int>({height, width})).mode(
                    torch::kBilinear).align_corners(false));

            // -----------------------------------
            // c3. YOLOv2 Training Phase
            // -----------------------------------
            output = model->forward(image);  // {N,C,H,W} ===> {N,G,G,A*(CN+5)}
            ...
            show_progress->increment(/*loss_value=*/
                    {loss_coord_xy.item<float>(), loss_coord_wh.item<float>(), loss_obj.item<float>(),
                     loss_noobj.item<float>(), loss_class.item<float>(), (float) width, (float) height});
