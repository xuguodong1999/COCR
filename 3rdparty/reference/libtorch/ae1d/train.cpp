...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::Data1dFolderWithPaths &valid_dataloader, torch::Device &device,
           Loss &criterion, AutoEncoder1d &model, const size_t epoch, visualizer::graph &writer);
...
train(po::variables_map &vm, torch::Device &device, AutoEncoder1d &model, std::vector<transforms_Compose > &transform) {
    ...
    dataloader = DataLoader::Data1dFolderWithPaths(dataset, vm["batch_size"].as<size_t>(), /*shuffle_=*/
    ...
    if (vm["valid"].as<bool>()) {
        ...
        valid_dataloader = DataLoader::Data1dFolderWithPaths(valid_dataset,
