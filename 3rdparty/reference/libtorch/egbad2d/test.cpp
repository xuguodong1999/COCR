...
AnomalyScore(torch::Tensor image, torch::Tensor fake_image, torch::Tensor z, GAN_Discriminator &dis,
...
void test(po::variables_map &vm, torch::Device &device, GAN_Encoder &enc, GAN_Generator &gen, GAN_Discriminator &dis,
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_enc.pth";
    ...
    while (dataloader(data)) {
        ...
        anomaly_score_with_alpha = AnomalyScore(image, output, z, dis, vm["test_Lambda"].as<float>());
        ...
        std::cout << '<' << std::get<1>(data).at(0) << "> anomaly_score:" << anomaly_score.item<float>() << " res:"
                  << res_loss.item<float>() << " dis:" << dis_loss.item<float>() << std::endl;
...
AnomalyScore(torch::Tensor image, torch::Tensor fake_image, torch::Tensor z, GAN_Discriminator &dis,
    ...
    torch::Tensor image_feature = dis->forward(image, z).second;
    torch::Tensor fake_image_feature = dis->forward(fake_image, z).second;
