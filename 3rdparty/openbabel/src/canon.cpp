...
namespace OpenBabel {
    ...
    inline bool
    CompareBondPairSecond(const std::pair<OBBond *, unsigned int> &a, const std::pair<OBBond *, unsigned int> &b) {
     ...
     */
    void
    addNbrs(OBBitVec &fragment, OBAtom *atom, const OBBitVec &mask, const std::vector<OBBond *> &metalloceneBonds) {
    ...
    void
    findMetalloceneBonds(std::vector<OBBond *> &bonds, OBMol *mol, const std::vector<unsigned int> &symmetry_classes) {
        FOR_ATOMS_OF_MOL (atom, mol) {
            ...
            FOR_NBORS_OF_ATOM (nbr, &*atom)bonds.push_back(mol->GetBond(&*atom, &*nbr));
        }
    }
    ...
    struct CanonicalLabelsImpl {
        ...
        struct StereoCenter {
             ...
             */
            int getDescriptor(const std::vector<unsigned int> &symmetry_classes,
        ...
        static void CompleteCode(OBMol *mol, FullCode &fullcode, State &state) {
            ...
            if (state.stereoCenters.size()) {
                ...
                for (std::size_t i = 0; i < state.stereoCenters.size(); ++i) {
                    ...
                    if (isInFragment)
                        fullcode.code.push_back(
                                state.stereoCenters[i].getDescriptor(state.symmetry_classes, code.labels));
                }
            }
         ...
         */
        static void LabelFragments(OBAtom *current, std::vector<OBAtom *> &nbrs, unsigned int label, Timeout &timeout,
                                   FullCode &bestCode, State &state) {
            ...
            if (numUnique < nbrs.size()) {
                ...
                for (std::size_t i = 0; i < nbrs.size(); ++i) {
                    ...
                    if (ligandSizes.back() == 1) {
                        ...
                        Orbits orbits;
                        OBBitVec mcr;
                        State lstate(state.symmetry_classes, ligand, state.stereoCenters, identityCodes, orbits, mcr,
                                     state.onlyOne);
...
//      print_orbits("orbits", orbits);
         ...
         * label+1, label+2, ...
         */
        static void CanonicalLabelsRecursive(OBAtom *current, unsigned int label, Timeout &timeout, FullCode &bestCode,
                                             State &state) {
            OBMol *mol = current->GetParent();
            PartialCode &code = state.code;
            ...
            if (state.backtrackDepth) {
                ...
                if (code.atoms.size() == state.backtrackDepth) {
                    ...
                    return;
                } else if (code.atoms.size() < state.backtrackDepth) {
...
//            if (state.mcr.BitIsSet(finalNbrs[0]->GetIdx()))
         ...
         */
        static std::vector<OBAtom *>
        findStartAtoms(OBMol *mol, const OBBitVec &fragment, const std::vector<unsigned int> &symmetry_classes) {
        ...
        static void CalcCanonicalLabels(OBMol *mol, const std::vector<unsigned int> &symmetry_classes,
                                        std::vector<unsigned int> &canonical_labels, const OBStereoUnitSet &stereoUnits,
                                        const OBBitVec &mask,
            ...
            if (stereoFacade) {
                for (std::size_t i = 0; i < stereoUnits.size(); ++i) {
                    ...
                    if (unit.type == OBStereo::Tetrahedral) {
                        ...
                        for (std::size_t j = 0; j < config.refs.size(); ++j) {
                            OBAtom *ref = mol->GetAtomById(config.refs[j]);
                            unsigned int r = (ref && ref->GetAtomicNum() != OBElements::Hydrogen) ? ref->GetIndex()
    ...
    void CanonicalLabels(OBMol *mol, const std::vector<unsigned int> &symmetry_classes,
        ...
        if (!maskCopy.CountBits())
            FOR_ATOMS_OF_MOL (atom, mol)maskCopy.SetBitOn(atom->GetIdx());
        ...
        if (onlyOne) {
            ...
            CanonicalLabelsImpl::CalcCanonicalLabels(mol, symmetry_classes, canonical_labels, OBStereoUnitSet(),
                                                     maskCopy, nullptr, maxSeconds, true);
        } else {
            std::vector<OBBond *> metalloceneBonds;
            findMetalloceneBonds(metalloceneBonds, mol, symmetry_classes);
            ...
            CanonicalLabelsImpl::CalcCanonicalLabels(mol, symmetry_classes, canonical_labels, stereoUnits, maskCopy,
                                                     &newsf, maxSeconds);
        }
