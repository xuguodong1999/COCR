                    ...
                    if (converted == 2 && p1 > 0 && p2 > 0) {
                        if (p1 > pmol->NumAtoms() || p2 > pmol->NumAtoms()) {
                            std::cerr << "ERROR at least one of the atom indices " << p1 << " or " << p2
                                      << " is greater than the number of atoms " << pmol->NumAtoms() << "." << std::endl
                                      << std::flush;
                            return false;
                        }
                        OBBond *b = pmol->GetBond(p1, p2);
                        if (!b) {
                            std::cerr << "ERROR atoms " << p1 << " and " << p2 << " form no bond." << std::endl
                                      << std::flush;
                            return false;
                        }
                        if (!(b->IsRotor())) {
                        ...
                        std::cerr << "ERROR parsing '" << segment
                                  << "' as rotor (must be i-j where i and j are indices >0)" << std::endl << std::flush;
                        return false;
                    }
                }
