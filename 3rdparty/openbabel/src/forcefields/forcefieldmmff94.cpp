        ...
        AddGradient(_vdwcalculations[i].force_b, _vdwcalculations[i].idx_b);
            ...
            IF_OBFF_LOGLVL_HIGH {
                snprintf(_logbuf, BUFF_SIZE, "%2d   %2d     %8.3f  %8.3f  %8.3f  %8.3f\n",
                         ...
                         _vdwcalculations[i].rab, _vdwcalculations[i].R_AB, _vdwcalculations[i].epsilon,
                         _vdwcalculations[i].energy);
                OBFFLog(_logbuf);
            }
...
#pragma omp parallel for reduction(+:energy)
        ...
        AddGradient(_electrostaticcalculations[i].force_b, _electrostaticcalculations[i].idx_b);
            ...
            IF_OBFF_LOGLVL_HIGH {
                snprintf(_logbuf, BUFF_SIZE, "%2d   %2d   %8.3f  %8.3f  %8.3f  %8.3f\n",
                         atoi(_electrostaticcalculations[i].a->GetType()),
                         atoi(_electrostaticcalculations[i].b->GetType()),
...
//      ifs.close();
                ...
                FOR_NBORS_OF_ATOM (nbr, ringatom) {
                    ...
                    if (!nbr->IsAromatic()) {
                        if (ringatom->GetAtomicNum() == OBElements::Carbon && ringatom->IsInRingSize(5)
                            && ringatom->IsInRingSize(6) && nbr->GetAtomicNum() == OBElements::Carbon &&
                            nbr->IsInRingSize(5)
                            && nbr->IsInRingSize(6)) {
                            c60++;
                        } else {
                ...
                if (IsAromatic) {
                    ...
                    for (unsigned int i = 0; i < alphaPos.size(); i++) {
                        if (alphaPos[i]->GetAtomicNum() == OBElements::Sulfur) {
                            ...
                            alphaAtoms.push_back(alphaPos[i]);
                        } else if (alphaPos[i]->GetAtomicNum() == OBElements::Nitrogen &&
                                   (alphaPos[i]->GetExplicitDegree() == 3)) {
                            bool IsNOxide = false;
                    ...
                    for (unsigned int i = 0; i < betaPos.size(); i++) {
                        if (betaPos[i]->GetAtomicNum() == OBElements::Sulfur) {
                            ...
                            betaAtoms.push_back(betaPos[i]);
                        } else if (betaPos[i]->GetAtomicNum() == OBElements::Nitrogen &&
                                   (betaPos[i]->GetExplicitDegree() == 3)) {
                            bool IsNOxide = false;
                    ...
                    if (!alphaAtoms.size() && !betaAtoms.size()) {
                        if (atom->GetAtomicNum() == OBElements::Carbon) {
                            ...
                            FOR_NBORS_OF_ATOM (nbr, atom) {
                                if (!(nbr->GetAtomicNum() == OBElements::Carbon && nbr->IsAromatic() &&
                                      nbr->IsInRingSize(6)))
                                    c60 = false;
                            }
                            if (c60)
                    ...
                    if (alphaAtoms.size() == 2) {
                        if (atom->GetAtomicNum() == OBElements::Carbon && IsInSameRing(alphaAtoms[0], alphaAtoms[1])) {
                            if (alphaAtoms[0]->GetAtomicNum() == OBElements::Nitrogen &&
                                alphaAtoms[1]->GetAtomicNum() == OBElements::Nitrogen) {
                                if ((alphaAtoms[0]->GetExplicitDegree() == 3) &&
                                    (alphaAtoms[1]->GetExplicitDegree() == 3)) {
                                    return 80; // Aromatic carbon between N's in imidazolium (CIM+)
                                }
                            }
                        }
                    ...
                    if (alphaAtoms.size() && betaAtoms.size()) {
                        ...
                        for (unsigned int i = 0; i < alphaAtoms.size(); i++) {
                            if (alphaAtoms[i]->GetAtomicNum() == OBElements::Sulfur ||
                                alphaAtoms[i]->GetAtomicNum() == OBElements::Oxygen) {
                                if (atom->GetAtomicNum() == OBElements::Carbon) {
                                    return 63; // Aromatic 5-ring C, alpha to N:, O:, or S: (C5A)
                                } else if (atom->GetAtomicNum() == OBElements::Nitrogen) {
                                    return 65; // Aromatic 5-ring N, alpha to N:, O:, or S: (N5A)
                        ...
                        for (unsigned int i = 0; i < betaAtoms.size(); i++) {
                            if (betaAtoms[i]->GetAtomicNum() == OBElements::Sulfur ||
                                betaAtoms[i]->GetAtomicNum() == OBElements::Oxygen) {
                                if (atom->GetAtomicNum() == OBElements::Carbon) {
                                    return 64; // Aromatic 5-ring C, beta to N:, O:, or S: (C5B)
                                } else if (atom->GetAtomicNum() == OBElements::Nitrogen) {
                                    return 66; // Aromatic 5-ring N, beta to N:, O:, or S: (N5B)
                        ...
                        if (nbrNbr->GetAtomicNum() == OBElements::Carbon) {
                            ...
                            FOR_NBORS_OF_ATOM (nbrNbrNbr, &*nbrNbr) {
                                ...
                                if (!bond->IsAromatic() && bond->GetBondOrder() == 2) {
                                    ...
                                    if (nbrNbrNbr->GetAtomicNum() == OBElements::Carbon ||
                                        nbrNbrNbr->GetAtomicNum() == OBElements::Nitrogen) {
                ...
                if (nbr->GetAtomicNum() == OBElements::Nitrogen) {
                    ...
                    if (nbr->GetExplicitDegree() == 2) {
                        FOR_NBORS_OF_ATOM (nbrNbr, &*nbr) {
                            ...
                            if (!bond->IsAromatic() && bond->GetBondOrder() == 2) {
                                if (nbrNbr->GetAtomicNum() == OBElements::Carbon ||
                                    nbrNbr->GetAtomicNum() == OBElements::Nitrogen) {
                        ...
                        if (nbrNbr->GetAtomicNum() == OBElements::Carbon) {
                            ...
                            FOR_NBORS_OF_ATOM (nbrNbrNbr, &*nbrNbr) {
                                ...
                                if (!bond->IsAromatic() && bond->GetBondOrder() == 2) {
                                    if (nbrNbrNbr->GetAtomicNum() == OBElements::Carbon ||
                                        nbrNbrNbr->GetAtomicNum() == OBElements::Nitrogen ||
                                        nbrNbrNbr->GetAtomicNum() == OBElements::Oxygen ||
                                        nbrNbrNbr->GetAtomicNum() == OBElements::Sulfur) {
        ...
        if (atom->GetAtomicNum() == 6) {
            ...
            if (atom->GetExplicitDegree() == 3) {
                ...
                if ((N3count >= 2) && (doubleBondTo == 7 || (!(doubleBondTo == 6) && atom->GetExplicitDegree() == 3
                                                             && N3fcharge == 1)) && !N2count && !oxygenCount &&
                    !sulphurCount) {
                    // N3==C--N3
        ...
        if (atom->GetAtomicNum() == 7) {
                ...
                if (atom->GetExplicitValence() == 3) {
                    ...
                    FOR_NBORS_OF_ATOM (nbr, atom) {
                        if (nbr->GetAtomicNum() == OBElements::Sulfur ||
                            nbr->GetAtomicNum() == OBElements::Phosphorus) {
                            oxygenCount = 0;

                            FOR_NBORS_OF_ATOM (nbrNbr, &*nbr) {
                                if (nbrNbr->GetAtomicNum() == OBElements::Oxygen &&
                                    (nbrNbr->GetExplicitDegree() == 1)) {
                                    oxygenCount++;
                                }
                            }
                            if (oxygenCount >= 2) {
                    ...
                    FOR_NBORS_OF_ATOM (nbr, atom) {
                        if (nbr->GetAtomicNum() == OBElements::Carbon) {
                            FOR_NBORS_OF_ATOM (nbrNbr, &*nbr) {
                                bond = _mol.GetBond(&*nbr, &*nbrNbr);
                                if (!bond->IsAromatic() && bond->GetBondOrder() == 2 &&
                                    (nbrNbr->GetAtomicNum() == OBElements::Oxygen ||
                                     nbrNbr->GetAtomicNum() == OBElements::Sulfur)) {
                                    IsAmide = true;
                    ...
                    FOR_NBORS_OF_ATOM (nbr, atom) {
                        if (nbr->GetAtomicNum() == OBElements::Carbon) {
                            ...
                            FOR_NBORS_OF_ATOM (nbrNbr, &*nbr) {
                                ...
                                if (nbrNbr->GetAtomicNum() == OBElements::Nitrogen &&
                                    (nbrNbr->GetExplicitDegree() == 2) &&
                                    (bond->GetBondOrder() == 2 || bond->IsAromatic())) {
                                    N2count++;
                                }
                                if (nbrNbr->IsAromatic()) {
                                    if (nbrNbr->GetAtomicNum() == OBElements::Oxygen) {
                                    ...
                                    if (nbrNbr->GetAtomicNum() == OBElements::Carbon) {
                                        ...
                                        if (!oxygenCount && !sulphurCount && (nitrogenCount == 1)) {
                                            ...
                                            FOR_NBORS_OF_ATOM (nbr2, atom) {
                                                if (nbr2->IsAromatic() && nbr2->GetAtomicNum() == OBElements::Carbon &&
                                                    nbr2->IsInRingSize(6)) {
                                                    bondToAromC = true;
                                                }
                                            }
            ...
            if (atom->GetExplicitDegree() == 2) {
                ...
                if (atom->GetExplicitValence() == 3) {
                    ...
                    FOR_NBORS_OF_ATOM (nbr, atom) {
                        bond = _mol.GetBond(&*nbr, atom);
                        if (nbr->GetAtomicNum() == OBElements::Oxygen && !bond->IsAromatic() &&
                            bond->GetBondOrder() == 2 && (nbr->GetExplicitDegree() == 1)) {
                            return 46; // Nitrogen in nitroso group (N=O)
                        }
                        if ((nbr->GetAtomicNum() == OBElements::Carbon ||
                             nbr->GetAtomicNum() == OBElements::Nitrogen) && !bond->IsAromatic() &&
                            bond->GetBondOrder() == 2) {
    ...
    bool OBForceFieldMMFF94::SetTypes() {
        ...
        FOR_BONDS_OF_MOL (bond, _mol)bond->SetAromatic(false);
        FOR_ATOMS_OF_MOL (atom, _mol)atom->SetAromatic(false);
    ...
    bool OBForceFieldMMFF94::SetupCalculations() {
        ...
        //
        IF_OBFF_LOGLVL_LOW OBFFLog("SETTING UP BOND CALCULATIONS...\n");
        ...
        FOR_BONDS_OF_MOL(bond, _mol) {
            ...
            parameter = GetTypedParameter2Atom(bondtype, atoi(a->GetType()), atoi(b->GetType()),
                                               _ffbondparams); // from mmffbond.par
            if (parameter == nullptr) {
                parameter = GetParameter2Atom(a->GetAtomicNum(), b->GetAtomicNum(),
                                              _ffbndkparams); // from mmffbndk.par - emperical rules
                if (parameter == nullptr) {
                    IF_OBFF_LOGLVL_LOW {
        ...
        //
        IF_OBFF_LOGLVL_LOW OBFFLog("SETTING UP ANGLE & STRETCH-BEND CALCULATIONS...\n");
        ...
        FOR_ANGLES_OF_MOL(angle, _mol) {
            ...
            if (anglecalc.ka == 0.0) {
                IF_OBFF_LOGLVL_LOW {
                    snprintf(_logbuf, BUFF_SIZE,
                             "   USING EMPIRICAL RULE FOR ANGLE BENDING FORCE CONSTANT %d-%d-%d (IDX)...\n",
                             a->GetIdx(), b->GetIdx(), c->GetIdx());
                    OBFFLog(_logbuf);
                }
            ...
            if (parameter == nullptr) {
                ...
                if (parameter == nullptr) {
                    ...
                    IF_OBFF_LOGLVL_LOW {
                        snprintf(_logbuf, BUFF_SIZE,
                                 "    COULD NOT FIND PARAMETERS FOR STRETCH-BEND %d-%d-%d (IDX)...\n", a->GetIdx(),
                                 b->GetIdx(), c->GetIdx());
                        OBFFLog(_logbuf);
                    }
                    return false;
      ...
      */
        ...
        //
        IF_OBFF_LOGLVL_LOW OBFFLog("SETTING UP TORSION CALCULATIONS...\n");
        ...
        FOR_TORSIONS_OF_MOL(t, _mol) {
            ...
            if (order >= 0) {
                ...
                if (parameter == nullptr) // try 3-2-2-5
                    parameter = GetTypedParameter4Atom(torsiontype, EqLvl3(type_a), type_b, type_c, EqLvl5(type_d),
                                                       _fftorsionparams);
                if (parameter == nullptr) // try 5-2-2-3
                    parameter = GetTypedParameter4Atom(torsiontype, EqLvl5(type_a), type_b, type_c, EqLvl3(type_d),
                                                       _fftorsionparams);
                if (parameter == nullptr) // try 5-2-2-5
                    parameter = GetTypedParameter4Atom(torsiontype, EqLvl5(type_a), type_b, type_c, EqLvl5(type_d),
                                                       _fftorsionparams);
            } else {
                // try exact match
                parameter = GetTypedParameter4Atom(torsiontype, type_d, type_c, type_b, type_a, _fftorsionparams);
                if (parameter == nullptr) // try 3-2-2-5
                    parameter = GetTypedParameter4Atom(torsiontype, EqLvl3(type_d), type_c, type_b, EqLvl5(type_a),
                                                       _fftorsionparams);
                if (parameter == nullptr) // try 5-2-2-3
                    parameter = GetTypedParameter4Atom(torsiontype, EqLvl5(type_d), type_c, type_b, EqLvl3(type_a),
                                                       _fftorsionparams);
                if (parameter == nullptr) // try 5-2-2-5
                    parameter = GetTypedParameter4Atom(torsiontype, EqLvl5(type_d), type_c, type_b, EqLvl5(type_a),
                                                       _fftorsionparams);
            }
            ...
            if (parameter) {
                ...
                if (b->GetBond(c)->IsAromatic()) {
                    ...
                    if (((GetMltb(type_b) == 2) && (GetMltb(type_c) == 2)) && !bond->IsAromatic() &&
                        bond->GetBondOrder() == 2)
                        pi_bc = 1.0;
                    else
                        pi_bc = 0.4;
                ...
                if (!found_rule) {
                    ...
                    if (!bond->IsAromatic() && bond->GetBondOrder() == 1 && (
                        ...
                        if (!found_rule)
                            if (((GetMltb(type_b) == 1) || (GetMltb(type_c) == 1)) &&
                                (b->GetAtomicNum() != OBElements::Carbon || c->GetAtomicNum() != OBElements::Carbon)) {
                                pi_bc = 0.4;
                                found_rule = true;
                            }
                ...
                // rule (h) page 632
                if (!found_rule) {
                    if ((b->GetAtomicNum() == OBElements::Oxygen || b->GetAtomicNum() == OBElements::Sulfur) &&
                        (c->GetAtomicNum() == OBElements::Oxygen || c->GetAtomicNum() == OBElements::Sulfur)) {
                        double Wb, Wc;
                        ...
                        IF_OBFF_LOGLVL_LOW {
                            snprintf(_logbuf, BUFF_SIZE,
                                     "   USING EMPIRICAL RULE FOR TORSION FORCE CONSTANT %d-%d-%d-%d (IDX)...\n",
                                     a->GetIdx(), b->GetIdx(), c->GetIdx(), d->GetIdx());
                            OBFFLog(_logbuf);
                        }
        ...
        //
        IF_OBFF_LOGLVL_LOW OBFFLog("SETTING UP OOP CALCULATIONS...\n");
        ...
        FOR_ATOMS_OF_MOL(atom, _mol) {
            ...
            for (unsigned int idx = 0; idx < _ffoopparams.size(); idx++) {
                if (type_b == _ffoopparams[idx].b) {
                    ...
                    if (((type_a == _ffoopparams[idx].a) && (type_c == _ffoopparams[idx].c) &&
                         (type_d == _ffoopparams[idx].d)) ||
                        ((type_c == _ffoopparams[idx].a) && (type_a == _ffoopparams[idx].c) &&
                         (type_d == _ffoopparams[idx].d)) ||
                        ((type_c == _ffoopparams[idx].a) && (type_d == _ffoopparams[idx].c) &&
                         (type_a == _ffoopparams[idx].d)) ||
                        ((type_d == _ffoopparams[idx].a) && (type_c == _ffoopparams[idx].c) &&
                         (type_a == _ffoopparams[idx].d)) ||
                        ((type_a == _ffoopparams[idx].a) && (type_d == _ffoopparams[idx].c) &&
                         (type_c == _ffoopparams[idx].d)) ||
                        ((type_d == _ffoopparams[idx].a) && (type_a == _ffoopparams[idx].c) &&
                         (type_c == _ffoopparams[idx].d))) {
                        found = true;
                    ...
                    if ((_ffoopparams[idx].a == 0) && (_ffoopparams[idx].c == 0) && (_ffoopparams[idx].d == 0) &&
                        !found) // *-XX-*-*
                    {
                        oopcalc.koop = _ffoopparams[idx]._dpar[0];
        ...
        // VDW Calculations
        //
        IF_OBFF_LOGLVL_LOW OBFFLog("SETTING UP VAN DER WAALS CALCULATIONS...\n");
        ...
        FOR_PAIRS_OF_MOL(p, _mol) {
            ...
            if (vdwcalc.aDA == 1) { // hydrogen bond donor
                ...
                if (vdwcalc.bDA == 2) { // hydrogen bond acceptor
                    vdwcalc.epsilon = 0.5 * (181.16 * vdwcalc.Ga * vdwcalc.Gb * vdwcalc.alpha_a * vdwcalc.alpha_b) /
                                      (sqrt_a + sqrt_b) * (1.0 / R_AB6);
                    ...
                    vdwcalc.epsilon =
                            (181.16 * vdwcalc.Ga * vdwcalc.Gb * vdwcalc.alpha_a * vdwcalc.alpha_b) / (sqrt_a + sqrt_b) *
                            (1.0 / R_AB6);
        ...
        //
        IF_OBFF_LOGLVL_LOW OBFFLog("SETTING UP ELECTROSTATIC CALCULATIONS...\n");
            ...
            if (type == 56) {
                ...
                FOR_NBORS_OF_ATOM(nbr, &*atom) {
                    FOR_NBORS_OF_ATOM(nbr2, &*nbr) {
                        ...
                        if (nbr2->GetAtomicNum() == OBElements::Nitrogen && !nbr2->IsAromatic())
                            sulfonamide = true;
                        OBBond *bond = nbr->GetBond(&*nbr2);
                        if (nbr2->GetAtomicNum() == OBElements::Carbon && !bond->IsAromatic() &&
                            bond->GetBondOrder() == 2)
                            sulfone_s_c = true;
                    }
                ...
                for (ri = vr.begin(); ri != vr.end(); ++ri) // for each ring
                    if ((*ri)->IsAromatic() && (*ri)->IsMember(&*atom) && ((*ri)->Size() == 5)) {
                        ...
                        for (rj = (*ri)->_path.begin(); rj != (*ri)->_path.end(); ++rj) // for each ring atom
                            if (_mol.GetAtom(*rj)->GetAtomicNum() == OBElements::Nitrogen &&
                                (_mol.GetAtom(*rj)->GetExplicitDegree() == 3))
                                n_count++;
    ...
    bool OBForceFieldMMFF94::SetPartialCharges() {
        ...
        FOR_ATOMS_OF_MOL (atom, _mol)atom->SetPartialCharge(charges[atom->GetIdx()]);
        ...
        for (unsigned int c = 1;; c++) {
            ...
            for (i = types.begin(); i != types.end(); ++i) {
                ...
                if (atoi(_mol.GetAtom(ni)->GetType()) == (*i))
                    snprintf(_logbuf, BUFF_SIZE, "%2d   %3d  %4d    %3d      %3d          PASSED",
                             _mol.GetAtom(ni)->GetIdx(), _mol.GetAtom(ni)->GetHyb(),
            ...
            for (di = fcharges.begin(); di != fcharges.end(); ++di) {
                ...
                if (fabs((*di) - _mol.GetAtom(ni)->GetPartialCharge()) <= 0.001)
                    snprintf(_logbuf, BUFF_SIZE, "%2d   %7.4f     %7.4f          PASSED", _mol.GetAtom(ni)->GetIdx(),
                             _mol.GetAtom(ni)->GetPartialCharge(), *di);
                else {
                    snprintf(_logbuf, BUFF_SIZE, "%2d   %7.4f     %7.4f      XXX FAILED XXX",
                             _mol.GetAtom(ni)->GetIdx(), _mol.GetAtom(ni)->GetPartialCharge(), *di);
                    failed = true;
                }
            ...
            snprintf(_logbuf, BUFF_SIZE, "Electrostatic          %11.5f    %11.5f   %11.5f", E_Electrostatic(), eeq,
                     delta);
            cout << _logbuf << endl;
    ...
    bool OBForceFieldMMFF94::ValidateGradients() {
        ...
        FOR_ATOMS_OF_MOL (a, _mol) {
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "%2d       (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", a->GetIdx(),
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    bond    (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    angle   (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            if (err.x() > 5.0 || err.y() > 5.0 || err.z() > 5.0)
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    strbnd  (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            if (err.x() > 5.0 || err.y() > 5.0 || err.z() > 5.0)
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    torsion (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            if (err.x() > 5.0 || err.y() > 5.0 || err.z() > 5.0)
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    oop     (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    vdw     (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            if (err.x() > 5.0 || err.y() > 5.0 || err.z() > 5.0)
            ...
            snprintf(_logbuf, BUFF_SIZE,
                     "    electro (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, %5.2f, %5.2f)\n", numgrad.x(),
                     numgrad.y(), numgrad.z(),
                     anagrad.x(), anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
            OBFFLog(_logbuf);
            if (err.x() > 5.0 || err.y() > 5.0 || err.z() > 5.0)
    ...
    int OBForceFieldMMFF94::GetTorsionType(OBAtom *a, OBAtom *b, OBAtom *c, OBAtom *d) {
      ...
      */
        ...
        if (a->IsInRingSize(5) && b->IsInRingSize(5) && c->IsInRingSize(5) && d->IsInRingSize(5)) {
            ...
            if (!((atoi(a->GetType()) == 1) || (atoi(b->GetType()) == 1) || (atoi(c->GetType()) == 1) ||
                  (atoi(d->GetType()) == 1)))
                return 0;
    ...
    */
    ...
    OBFFParameter *
    OBForceFieldMMFF94::GetTypedParameter2Atom(int ffclass, int a, int b, std::vector<OBFFParameter> &parameter) {
        OBFFParameter *par;
    ...
    OBFFParameter *OBForceFieldMMFF94::GetTypedParameter3Atom(int ffclass, int a, int b, int c,
                                                              std::vector<OBFFParameter> &parameter) {
        OBFFParameter *par;

        for (unsigned int idx = 0; idx < parameter.size(); idx++)
            if (((a == parameter[idx].a) && (b == parameter[idx].b) && (c == parameter[idx].c) &&
                 (ffclass == parameter[idx]._ipar[0])) ||
                ((a == parameter[idx].c) && (b == parameter[idx].b) && (c == parameter[idx].a) &&
                 (ffclass == parameter[idx]._ipar[0]))) {
                par = &parameter[idx];
                return par;
    ...
    OBFFParameter *OBForceFieldMMFF94::GetTypedParameter4Atom(int ffclass, int a, int b, int c, int d,
                                                              std::vector<OBFFParameter> &parameter) {
        OBFFParameter *par;
