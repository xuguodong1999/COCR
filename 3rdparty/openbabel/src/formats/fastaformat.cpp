...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *GetMIMEType() { return "chemical/x-fasta"; }
...
#define M_PI 3.1415926535897932384626433832795028841971694
    ...
    HelixParameters RNA_helix = {2.95057, 2 * M_PI / 11.0,
                                 8}; // HTER-OXT-.P-O5'-C5'-C4'-C3'-O3'-.P-O5'-C5'-C4'-C3'-O3'-.HCAP
    HelixParameters DNA_helix = {3.37998, 2 * M_PI / 10.0,
                                 8}; // HTER-OXT-.P-O5'-C5'-C4'-C3'-O3'-.P-O5'-C5'-C4'-C3'-O3'-.HCAP
    ...
    // #define GEN_BONDS 1
    void add_residue(OBMol *pmol, OBResidue *res, double offset_x, double offset_Theta, unsigned long &serial_no,
                     ResidueRecord *res_rec, int resBondFromOffset, ptrAtom &resBondFrom, bool create_bonds,
                     bool bond_orders) {
        ...
        if (create_bonds) {
            ...
            if (resBondFrom && atom_count) { // insert the bond from the previous residue
    ...
    void
    generate_sequence(const std::string &sequence, OBMol *pmol, unsigned long chain_no, const HelixParameters &helix,
                      const char *IUPAC_codes, ResidueRecord *Residues, double &offset_x, double &offset_Theta,
                      unsigned long &serial_no, bool create_bonds, bool bond_orders) {
        ...
        for (std::string::const_iterator sx = sequence.begin(), sy = sequence.end(); sx != sy; ++sx, ++residue_num) {
            ...
            if (is_gap) {
                ...
                if (res_rec->IUPACcode) {
                    ...
                    if (residue_num == 1) { // Add the start terminal
                        add_residue(pmol, res, offset_x, offset_Theta, serial_no, &Residues[IUPAC_Start], -1,
                                    resBondFrom, create_bonds, bond_orders);
                    }

                    add_residue(pmol, res, offset_x, offset_Theta, serial_no, res_rec, helix.bond_connect, resBondFrom,
                                create_bonds, bond_orders);
                }
        ...
        if (res != nullptr) { // Add the end terminal
            add_residue(pmol, res, offset_x - helix.unit_X, offset_Theta - helix.unit_Theta, serial_no,
                        &Residues[IUPAC_End], -2, resBondFrom, create_bonds, bond_orders);
        }
    ...
    */
        ...
        while (!in->eof()) { // We have a new line to fetch into 'line'
            getline(*in, line);
            if (line[0] == '>') { // comment data
                if (pmol->GetTitle()[0] == 0) {
                    pmol->SetTitle(&(line.c_str()[1]));
                }
                if (sequence_type == FASTAFormat::UnknownSequence) { // attempt to determine the sequence type
        ...
        switch (sequence_type) {
            case FASTAFormat::RNASequence:
                generate_sequence(sequence, pmol, 1, RNA_helix, IUPAC_RNA_codes, RNAResidues, offset_x, offset_Theta,
                                  serial_no, create_bonds, bond_orders);
                break;
            case FASTAFormat::ProteinSequence:
                generate_sequence(sequence, pmol, 1, protein_helix, IUPAC_Protein_codes, ProteinResidues, offset_x,
                                  offset_Theta, serial_no, create_bonds, bond_orders);
                break;
            case FASTAFormat::DNASequence: {
                generate_sequence(sequence, pmol, 1, DNA_helix, IUPAC_DNA_codes, DNAResidues, offset_x, offset_Theta,
                                  serial_no, create_bonds, bond_orders);
                if (!singleStrand) {
                    offset_x -= DNA_helix.unit_X;
                    offset_Theta -= DNA_helix.unit_Theta;
                    std::string rsequence;
                    for (std::string::const_reverse_iterator sx = sequence.rbegin(), sy = sequence.rend();
                         sx != sy; ++sx)
                        rsequence.append(1, *sx);
                    generate_sequence(rsequence, pmol, 2, DNA_pair_helix, IUPAC_DNA_codes, DNAPairResidues, offset_x,
                                      offset_Theta, serial_no, create_bonds, bond_orders);
                }
            }
                break;
