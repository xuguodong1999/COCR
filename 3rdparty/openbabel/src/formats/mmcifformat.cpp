...
namespace OpenBabel {
    ...
    public:
        ...
        mmCIFFormat() { // Copied from the Chemical MIME Page at http://www.ch.ic.ac.uk/chemime/
        ...
        virtual const char *SpecificationURL() { return "http://mmcif.pdb.org/"; }; //optional
        // CIF itself is at http://www.iucr.org/iucr-top/cif/index.html

        virtual const char *GetMIMEType() { return "chemical/x-mmcif"; };
    ...
    public:
        ...
        struct Token {
            ...
            double as_number() const { return strtod(as_text.c_str(), nullptr); }

            unsigned long as_unsigned() const { return strtoul(as_text.c_str(), nullptr, 10); }
        };
    ...
    CIFTagID::CIFDataName CIFLexer::lookup_tag(const string &tag_name) {
        if (CIFtagLookupTable.empty()) {
            for (size_t idx = 0; CIFTagsRead[idx].tagid != CIFTagID::unread_CIFDataName; ++idx) {
                CIFtagLookupTable.insert(
                        CIFtagmap::value_type(string(CIFTagsRead[idx].tagname), CIFTagsRead[idx].tagid));
            }
        }
    ...
    bool mmCIFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        while (lexer.next_token(token) && token.type != CIFLexer::KeyDataToken);
        if (token.type == CIFLexer::KeyDataToken) { // we have found the next data block:
            pmol->BeginModify();
            ...
            while (!finished && (token_peeked || lexer.next_token(token))) {
                ...
                switch (token.type) {
                    case CIFLexer::KeyGlobalToken:
                        ...
                        if (pmol->NumAtoms() > 0) { // Found a molecule, so finished
                            finished = true;
                    ...
                    case CIFLexer::KeyDataToken:
                        ...
                        if (pmol->NumAtoms() > 0) { // Found a molecule, so finished
                            ...
                            pmol->SetTitle(token.as_text);
                        break;
                    case CIFLexer::KeySaveToken: { // Simply eat tokens until the save_ ending token
                    ...
                    case CIFLexer::KeyLoopToken: {
                        ...
                        CIFTagID::CIFCatName catid = CIFTagID::unread_CIFCatName;
                        while ((token_peeked = lexer.next_token(token)) == true &&
                               token.type == CIFLexer::TagToken) { // Read in the tags
                        ...
                        switch (catid) {
                            case CIFTagID::atom_site: {
                                ...
                                for (CIFColumnList::const_iterator colx = columns.begin(), coly = columns.end();
                                     colx != coly; ++colx) {
                                    switch (*colx) {
                                ...
                                if (use_cartn) {
                                    for (CIFColumnList::iterator colx = columns.begin(), coly = columns.end();
                                         colx != coly; ++colx)
                                        if ((*colx) >= CIFTagID::_atom_site_fract_x &&
                                            (*colx) <= CIFTagID::_atom_site_fract_z)
                                    ...
                                    switch (columns[column_idx]) {
                                        case CIFTagID::_atom_site_label: // The atomic label within the molecule
                                            ...
                                            if (string::npos != token.as_text.find_first_of("0123456789")) {
                                                token.as_text.erase(token.as_text.find_first_of("0123456789"),
                                                                    token.as_text.size());
                                            }
                                        case CIFTagID::_atom_site_type_symbol:
                                            ...
                                            if (tmpSymbol.size() > nbc) {// Try to find a formal charge in the symbol
                                                int charge = 0;
                                                int sign = 0;
                                                for (unsigned int i = nbc; i <
                                                                           tmpSymbol.size(); ++i) {// Use first number found as formal charge
                                            ...
                                            atom->SetAtomicNum(
                                                    atomicNum); //set atomic number, or '0' if the atom type is not recognized
                                        ...
                                        case CIFTagID::_atom_site_label_atom_id: // The atomic label within the residue
                                            ...
                                            if (atom_type_tag == CIFTagID::_atom_site_label_atom_id) {
                                                for (string::iterator posx = token.as_text.begin(), posy = token.as_text.end();
                                                     posx != posy; ++posx) {
                                        ...
                                        case CIFTagID::_atom_site_occupancy: // The occupancy of the site.
                                        {
                                            ...
                                            double occupancy = std::max(0.0, std::min(1.0,
                                                                                      token.as_number())); // clamp occupancy to [0.0, 1.0] bugfix
            ...
            if (pmol->NumAtoms() > 0) {
                if (use_cell >= 6) {
                    ...
                    pmol->SetData(pCell);
                    if (use_fract) {
                        for (OBAtomIterator atom_x = pmol->BeginAtoms(), atom_y = pmol->EndAtoms();
                             atom_x != atom_y; ++atom_x) {
                            OBAtom *atom = (*atom_x);
                            if (wrap_coords)
    ...
    bool mmCIFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        ofs << "# to comply with the Macromolecular CIF Dictionary  (cif_mm.dic) version  2.0.11 http://mmcif.pdb.org/"
            << endl;
        ...
        for (OBAtomIterator atom_x = pmol->BeginAtoms(), atom_y = pmol->EndAtoms();
            ...
            if (has_residues) {
                ...
                if (atomname.empty()) {
                    snprintf(buffer, BUFF_SIZE, "%s%lu", OBElements::GetSymbol(atom->GetAtomicNum()),
                             (unsigned long) site_id);
                    atomname.assign(buffer);
                }
                if (resname.empty())
                    resname.assign("UNK");
                ofs << '\t' << atomname << '\t' << resname << '\t' << pRes->GetChainNum() << '\t' << pRes->GetNum()
                    << endl;
            }
