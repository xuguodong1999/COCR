...
void
TranspositionGetMcrAndFixSetAndUnorderedPartition(Transposition *gamma, NodeSet *McrSet, NodeSet *FixSet, int n, int l,
...
int CtPartCompare(ConTable *Ct1, ConTable *Ct2, S_CHAR *cmp,
    ...
    */
    ...
    /*************************************************************************/
    nLayer++;
    for (i = midCt; i < endCt1 && Ct1->Ctbl[i] == Ct2->Ctbl[i]; i++); /* compare tautomeric groups part of CT */
    if (i < endCt1) {
        diff = (int) Ct1->Ctbl[i] - (int) Ct2->Ctbl[i];
        goto done;
    }
    if (Ct1->NumH && Ct2->NumH) {
        for (i = midNumH; i < lenNumH && Ct1->NumH[i] == Ct2->NumH[i]; i++); /* compare tautomeric H */
        if (i < lenNumH) {
            diff = (int) Ct1->NumH[i] - (int) Ct2->NumH[i];
            i += endCt1 - midCt;
            goto done;
...
int CtFullCompare(ConTable *Ct1, ConTable *Ct2, int bOnlyCommon, int bSplitTautCompare) {
    ...
    if (Ct1->NumH && Ct2->NumH) {
        for (i = midNumH; i < lenNumH1 && Ct1->NumH[i] == Ct2->NumH[i]; i++); /* compare tautomeric H */
        if (i < lenNumH1) {
            diff = (int) Ct1->NumH[i] - (int) Ct2->NumH[i];
            goto done;
        }
...
/*L3:*/
    ...
    if (hz_zeta == k || hz_rho == k || (lab && qzb_rho >= 0 && QZFIX_OK(qzb_rho_fix))) {
        ...
        PartitionGetFirstCell(&pi[k - 1], W, k, n);
        v[k - 1] = CellGetMinNode(&pi[k - 1], &W[k - 1], 0, pCD1);
        if (!dig && PartitionSatisfiesLemma_2_25(&pi[k - 1], n)) { ; /* found additional isomprphism */
        } else {
            t_Lemma = k + 1;
        }
...
int SetInitialRanks2(int num_atoms, ATOM_INVARIANT2 *pAtomInvariant2, AT_RANK *nNewRank, AT_RANK *nAtomNumber) {
    ...
    for (i = num_atoms - 1, nCurrentRank = nNewRank[nAtomNumber[i]] = (AT_RANK) num_atoms, nNumDiffRanks = 1;
         0 < i; i--) {
...
int GetBaseCanonRanking(int num_atoms, int num_at_tg, sp_ATOM *at[],
    ...
    */
    ...
    if (i < num_atoms) {
        ...
        ret = CanonGraph03(num_atoms, num_atoms, num_max, 1 /* digraph?? was 0 */, NeighList[TAUT_NON],
                           (Partition *) pBCN->pRankStack,
                           nSymmRankNoTautH, nCanonRankNoTautH, nAtomNumberCanonNoTautH, pCD + iOther, pCC, &Ct_NoH,
                           &Ct_NoTautH);
        if (ret < 0) {
            goto exit_error;
        }
    ...
    if (s[iOther].num_isotopic_atoms && !s[iOther].bIgnoreIsotopic && !bReqTaut && bReqNonTaut) {
        ...
        iso_sort_key_NoTautH = (AT_ISO_SORT_KEY *) inchi_calloc(maxlen_iso_sort_key_NoTautH,
                                                                sizeof(iso_sort_key_NoTautH[0]));
        ...
        for (i = 0; i < num_atoms; i++) {
            if (at_base[i].endpoint) {
                ...
                iso_sort_key = make_iso_sort_key(at_base[i].iso_atw_diff, at_base[i].num_iso_H[0],
                                                 at_base[i].num_iso_H[1], at_base[i].num_iso_H[2]);
            }
            if (iso_sort_key) {
        ...
        if (i < num_atoms) {
            ...
            ret = CanonGraph04(num_atoms, num_atoms, num_max, 1 /* digraph?? was 0 */, NeighList[TAUT_NON],
                               (Partition *) pBCN->pRankStack,
                               nSymmRankNoTautHIso, nCanonRankNoTautHIso, nAtomNumberCanonNoTautHIso, pCD + iOther, pCC,
                               &Ct_NoTautH, &Ct_NoTautHIso);
            if (ret < 0) {
                goto exit_error;
            }
            /* in case of non-tautomeric structure the final results are in:
            ...
            */
            ...
            memcpy(nAtomNumberCanonNoTautHIso, nAtomNumberCanonNoTautH,
                   num_atoms * sizeof(nAtomNumberCanonNoTautHIso[0]));
        }
        /* in case of non-tautomeric component this is the final result */
    ...
    if (bReqTaut) {
        ...
        for (i = k = num_atoms; i < num_at_tg; i++) {
            ...
            for (j = 0; j < T_NUM_NO_ISOTOPIC; j++) {
                ...
                numH[k++] = t_group_info->t_group[m].num[j] ? t_group_info->t_group[m].num[j] + BASE_H_NUMBER
                                                            : EMPTY_H_NUMBER;
            }
        }
        /* pointers */
        ...
        ret = CanonGraph05(num_atoms, num_at_tg, num_max, 1 /* digraph*/, NeighList[TAUT_YES],
                           (Partition *) pBCN->pRankStack,
                           nSymmRankBase, nCanonRankBase, nAtomNumberCanonBase, pCD + iBase, pCC, &Ct_NoTautH,
                           &Ct_Base);
        if (ret < 0) {
            goto exit_error;
        }
        ...
        if ((s[iBase].num_isotopic_atoms && !s[iBase].bIgnoreIsotopic) ||
            ...
            if (!iso_exchg_atnos && !iso_sort_keyBase) {
                ...
                inchi_free(nSymmRankBaseIso);
                nSymmRankBaseIso = NULL;
                inchi_free(nCanonRankBaseIso);
                nCanonRankBaseIso = NULL;
                inchi_free(nAtomNumberCanonBaseIso);
                ...
                if (num_iso_NoTautH || num_iso_Base || num_iso_NoAuxBase) {
                    ...
                    for (i = 0; i < num_at_tg; i++) {
                        if ((iso_sort_keyBase ? (iso_sort_keyBase[i] !=
                                                 iso_sort_keyBase[nTempRank[nSymmRankBase[i] - 1]]) : 0) ||
                            (iso_exchg_atnos ? (iso_exchg_atnos[i] != iso_exchg_atnos[nTempRank[nSymmRankBase[i] - 1]])
                                             : 0)) {
                ...
                if (i < num_at_tg) {
                    ...
                    for (i = 0; i < num_at_tg; i++) {
                        ...
                        pAtomInvariantAux[i].iso_sort_key = iso_sort_keyBase ? iso_sort_keyBase[i]
                                                                             : 0; /* additional differentiation */
                    ...
                    nNumCurrRanks = DifferentiateRanks2(num_at_tg, NeighList[TAUT_YES],
                                                        nNumCurrRanks, nRankAux,
                                                        nTempRank, nAtomNumberAux, &lCount,
                                                        0 /* 0 means first use qsort */ );
                    /* to accelerate do not call CanonGraph() to find really equivalent atoms */
                    pCD[iBase].nAuxRank = nRankAux;
#endif


                    ret = CanonGraph06(num_atoms, num_at_tg, num_max, 1 /* digraph */, NeighList[TAUT_YES],
                                       (Partition *) pBCN->pRankStack,
                                       nSymmRankBaseIso, nCanonRankBaseIso, nAtomNumberCanonBaseIso, pCD + iBase, pCC,
                                       &Ct_Base, &Ct_BaseIso);
                    if (ret < 0) {
                        goto exit_error;
                    }
                    ...
                    */
                    ...
                    memcpy(nAtomNumberCanonBaseIso, nAtomNumberCanonBase,
                           num_at_tg * sizeof(nAtomNumberCanonBaseIso[0]));
                }
                /* in case of non-tautomeric component this is the final result */
    ...
    if (bReqTaut && bReqNonTaut) {
        ...
        ret = CanonGraph07(num_atoms, num_atoms, num_max, 0, NeighList[TAUT_NON], (Partition *) pBCN->pRankStack,
                           nSymmRankFixH, nCanonRankFixH, nAtomNumberCanonFixH, pCD + iOther, pCC, &Ct_NoTautH,
                           &Ct_FixH);
        if (ret < 0) {
            goto exit_error;
        }
        ...
        if (iflag) {
            ...
            for (i = 0; i < num_atoms; i++) {
                ...
                if (bFixIsoFixedH) /* #if ( FIX_ISO_FIXEDH_BUG == 1 )  */
                {
                    /* fix bug when iso H was removed as a proton and fixed-H isotopic layer is missing -  2008-09-24 DT*/
                    if (at_other) {
                        iso_sort_key = make_iso_sort_key(at_other[i].iso_atw_diff, at_other[i].num_iso_H[0],
                                                         at_other[i].num_iso_H[1], at_other[i].num_iso_H[2]);
                    } else {
                        iso_sort_key = make_iso_sort_key(at_base[i].iso_atw_diff, at_base[i].num_iso_H[0],
                                                         at_base[i].num_iso_H[1], at_base[i].num_iso_H[2]);
                    }
            ...
            if (i < num_atoms) {
                ...
                ret = CanonGraph08(num_atoms, num_atoms, num_max, 1 /* digraph?? was 0 */, NeighList[TAUT_NON],
                                   (Partition *) pBCN->pRankStack,
                                   nSymmRankFixHIso, nCanonRankFixHIso, nAtomNumberCanonFixHIso, pCD + iOther, pCC,
                                   &Ct_FixH, &Ct_FixHIso);
                if (ret < 0) {
                    goto exit_error;
                }
    ...
    ret = 0;
    ret |= (Ct_NoH->lenCt != Ct_NoTautH->lenCt) ||
           memcmp(Ct_NoH->Ctbl, Ct_NoTautH->Ctbl, Ct_NoH->lenCt * sizeof(Ct_NoH->Ctbl[0]));
    if (bReqTaut) {
        if (Ct_FixH) {
            ret |= (Ct_NoTautH->lenCt != Ct_FixH->lenCt) ||
                   memcmp(Ct_NoTautH->Ctbl, Ct_FixH->Ctbl, Ct_NoTautH->lenCt * sizeof(Ct_NoTautH->Ctbl[0]));
            ret |= (Ct_NoTautH->lenNumH != Ct_FixH->lenNumH) ||
                   memcmp(Ct_NoTautH->NumH, Ct_FixH->NumH, Ct_NoTautH->lenNumH * sizeof(Ct_Base->NumH[0]));
        }
        ret |= (Ct_NoTautH->lenCt > Ct_Base->lenCt) ||
               memcmp(Ct_NoTautH->Ctbl, Ct_Base->Ctbl, Ct_NoTautH->lenCt * sizeof(Ct_NoTautH->Ctbl[0]));
        ret |= (Ct_NoTautH->lenNumH > Ct_Base->lenNumH) ||
               memcmp(Ct_NoTautH->NumH, Ct_Base->NumH, Ct_NoTautH->lenNumH * sizeof(Ct_Base->NumH[0]));
    }

    /* isotopic canonicalization */
    if (Ct_NoTautHIso) {
        ret |= (Ct_NoH->lenCt != Ct_NoTautHIso->lenCt) ||
               memcmp(Ct_NoH->Ctbl, Ct_NoTautHIso->Ctbl, Ct_NoH->lenCt * sizeof(Ct_NoH->Ctbl[0]));
        ret |= (Ct_NoTautH->lenNumH != Ct_NoTautHIso->lenNumH) ||
               memcmp(Ct_NoTautH->NumH, Ct_NoTautHIso->NumH, Ct_NoTautH->lenNumH * sizeof(Ct_Base->NumH[0]));
    } else if (Ct_BaseIso) {
        ret |= (Ct_BaseIso->lenCt != Ct_Base->lenCt) ||
               memcmp(Ct_BaseIso->Ctbl, Ct_Base->Ctbl, Ct_BaseIso->lenCt * sizeof(Ct_BaseIso->Ctbl[0]));
        ret |= (Ct_BaseIso->lenNumH != Ct_Base->lenNumH) ||
               memcmp(Ct_BaseIso->NumH, Ct_Base->NumH, Ct_BaseIso->lenNumH * sizeof(Ct_BaseIso->NumH[0]));
        if (Ct_FixHIso) {
            ret |= (Ct_FixHIso->lenCt > Ct_BaseIso->lenCt) ||
                   memcmp(Ct_FixHIso->Ctbl, Ct_BaseIso->Ctbl, Ct_FixHIso->lenCt * sizeof(Ct_FixHIso->Ctbl[0]));
            ret |= (Ct_FixHIso->lenNumH > Ct_BaseIso->lenNumH) ||
                   memcmp(Ct_FixHIso->NumH, Ct_BaseIso->NumH, Ct_FixHIso->lenNumH * sizeof(Ct_BaseIso->NumH[0]));
        }
    }
