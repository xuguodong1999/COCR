...
int Eql_INChI_Stereo(INChI_Stereo *s1, int eql1, INChI_Stereo *s2, int eql2, int bRelRac) {
    ...
    if (EQL_SP2 == eql1) {
        ...
        return 0;
    } else if ((eql1 == EQL_SP3 || (inv1 = (eql1 == EQL_SP3_INV))) &&
                ...
                if (t_parity2 && nNumber2) {
                    if (inv1 ^ inv2) {
                        ...
                        for (i = 0, num_inv = 0; i < len; i++) {
                            ...
                            } else if (t_parity1[i] != t_parity2[i]) {
                                break;
                            }
                        }
                ...
                if (bCountRingClosures) {
                    if (nDelim == '-' && i > 3 && bNoNum_H) {
                        if (!nNumRingClosures) {
                            ...
                            if (nNumRingClosures) {
                                len += MakeDecNumber(szValue + len, (int) sizeof(szValue) - len, NULL,
                                                     nNumRingClosures);
                            }
                            nNumRingClosures--;
                        } else {
    ...
    if (!bOvfl) {
        for (i = 0; i < nLenCT && nLen < nLen_szLinearCT; i++) {
            ...
            if (!(nCtMode & CT_MODE_NO_ORPHANS) || ((bLessThanPrev = LinearCT[i] < nMax) ||
                ...
                nValue = LinearCT[i];
                if (nCtMode & CT_MODE_ABC_NUMBERS) {
                    len = MakeAbcNumber(szValue, (int) sizeof(szValue), (!bNext && bAddDelim) ? ITEM_DELIMETER : NULL,
                                        nValue);
                } else if (nCtMode & CT_MODE_NO_ORPHANS) {  /*  censored CT */
...
int MakeTautString(AT_NUMB *LinearCT, int nLenCT, int bAddDelim,
    ...
    if (!bOvfl) {
        for (i = nGroupLen = iGroupOutputCount = 0; i < nLenCT && nLen < nLen_szLinearCT; i++) {
            ...
            if (nGroupLen == iGroupOutputCount) {
            ...
            } else if (bCompressed && iGroupOutputCount >= INCHI_T_NUM_MOVABLE) {
                ...
                if (bCompressed) {
                    ...
                    switch (iGroupOutputCount) {
                        case 0:
                            len = MakeDecNumber(szValue, (int) sizeof(szValue), (i == 1) ? ITEM_DELIMETER : NULL,
                                                nValue);
                            break;
                        case 1:
                            p = "-";
    ...
    if (!bOvfl) {
        for (i = 0; i < nNumberOfIsotopicAtoms && nLen < nLen_szLinearCT; i++) {
            ...
            for (j = 0; j < 5; j++) {
                ...
                if (!j) {
                ...
                } else if (bAbcNumbers) { /*  Abc output */
                    switch (j) {
                            ...
                            if (nValue) {
                                if ((int) sizeof(szValue) - tot_len > 1) {
                                    ...
                                    if (1 == nValue) {
                                        ...
                                        ret = MakeDecNumber(p + len, (int) sizeof(szValue) - tot_len - len, NULL,
                                                            nValue);
                    ...
                    if (j == 1) { /*  Decimal output */
                        ...
                        len = MakeDecNumber(p, (int) sizeof(szValue) - tot_len, sign[nValue >= 0],
                                            abs(nValue - subtract));
                    } else {
                        /*  hydrogen isotope */
                        if (nValue != 1) {
                            len = MakeDecNumber(p, (int) sizeof(szValue) - tot_len, h[j - 2], nValue);
                        } else if ((int) sizeof(szValue) - tot_len > 1) {
                            strcpy(p, h[j - 2]);
                            len = 1;
                        } else {
                            len = -1; /*  overflow */
        ...
        for (i = 0; i < nLenCT && nLen < nLen_szLinearCT; i++) {
            ...
            for (j = 0; j < 3; j++) {
                ...
                if (nCtMode & CT_MODE_ABC_NUMBERS) {
                    ...
                    if (j < 2) {
                        len = MakeDecNumber(p, (int) sizeof(szValue) - tot_len,
                                            tot_len ? "-" : (i || bAddDelim) ? ITEM_DELIMETER : NULL, nValue);
                    } else if (tot_len + 1 < (int) sizeof(szValue)) {
