...
/* for execution outside the VC++ debugger uncomment one of the following two */
    ...
    if (output_file->s.pStr && output_file->s.nUsedLength > 0 && out) {
        ...
        out->szAuxInfo = NULL;
        if (!(INCHI_OUT_SDFILE_ONLY &
              ip->bINChIOutputOptions)) /* do not remove last LF from SDF output - 2008-12-23 DT */
            for (p = strchr(out->szInChI, '\n'); p; p = strchr(p + 1, '\n')) {
                if (!memcmp(p, "\nAuxInfo", 8)) {
                    *p = '\0';            /* remove LF after INChI */
                    out->szAuxInfo = p + 1; /* save pointer to AuxInfo */
                } else if (out->szAuxInfo || !p[1]) {   /* remove LF after aux info or from the last char */
                    *p = '\0';
                    break;
                }
            }
...
int SetBondProperties(inp_ATOM *at, inchi_Atom *ati, int a1, int j,
    ...
    } else if ((p1 || p2) && (p1 || at[a1].valence < MAXVAL) && (p2 || at[a2].valence < MAXVAL)) {
...
int InpAtom0DToInchiAtom(inp_ATOM *at, int num_atoms, inchi_OutputStruct *outStruct) {
    ...
    for (i = 0; i < num_atoms; i++) {
        ...
        if (at[i].iso_atw_diff) {
            outStruct->atom[i].isotopic_mass =
                    ISOTOPIC_SHIFT_FLAG + (at[i].iso_atw_diff > 0 ? at[i].iso_atw_diff - 1 : at[i].iso_atw_diff);
        }
                    ...
                    for (m2 = 0; m2 < MAX_NUM_STEREO_BONDS && at[cur].sb_parity[m2]; m2++) {
                        if (chain[len - 2] == at[cur].neighbor[(int) at[cur].sb_ord[m2]]) {
                            if (n < num_stereo0D) {
                                ...
                                outStruct->stereo0D[n].type =
                                        len == 3 ? INCHI_StereoType_Allene : INCHI_StereoType_DoubleBond;
...
EXPIMP_TEMPLATE INCHI_API int INCHI_DECL
GetStructFromStdINCHI(inchi_InputINCHI *inpInChI, inchi_OutputStruct *outStruct) {
    if ((inpInChI) &&
        (inpInChI->szInChI) &&
