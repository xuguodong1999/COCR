...
int needed_unusual_el_valence(int nPeriodicNum, int charge, int radical, int bonds_valence,
    ...
    */
    ...
    return (exact_found && 1 == num_found && 1 == num_found_known) ? 0 : chem_valence ? chem_valence
                                                                                      : -1 /* needs zero */;
}
...
int mystrncpy(char *target, const char *source,
              unsigned maxlen) {   /*  protected from non-zero-terminated source and overlapped target/source. 7-9-99 DCh. */
    const char *p;
    unsigned len;
