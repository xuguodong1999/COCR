...
int DisplayTheWholeStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
                             ...
                             ORIG_ATOM_DATA *orig_inp_data, long num_inp, int iINChI, int bShowStruct,
                             int bINCHI_LIB_Flag);
    ...
    if (sd->nErrorType == _IS_FATAL) {
        if (nLogMask & LOG_MASK_FATAL)
            inchi_ios_eprint(log_file, "Fatal Error %d (aborted; %s) inp structure #%ld.%s%s%s%s\n",
                             sd->nStructReadError, sd->pStrErrStruct, *num_inp,
                             SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    ...
    if (sd->nErrorType == _IS_ERROR) {  /*  70 => too many atoms */
        if (nLogMask & LOG_MASK_ERR)
            inchi_ios_eprint(log_file, "Error %d (no %s; %s) inp structure #%ld.%s%s%s%s\n",
                             sd->nStructReadError,
                             (ip->bINChIOutputOptions & INCHI_OUT_SDFILE_ONLY) ? "Molfile" : INCHI_NAME,
        ...
        if (sd->nErrorType != _IS_OKAY && sd->nErrorType != _IS_WARNING) {
            sd->nErrorType =
                    ProcessStructError(output_file, log_file, /*sd->nStructReadError,*/
                                       sd->pStrErrStruct, sd->nErrorType, &sd->bXmlStructStarted, *num_inp, ip, pStr,
                                       nStrLen);
        }
    }
    exit_function:
...
int GetOneComponent(STRUCT_DATA *sd, INPUT_PARMS *ip, INCHI_IOSTREAM *log_file, INCHI_IOSTREAM *output_file,
    ...
    inp_cur_data->num_at = ExtractConnectedComponent(orig_inp_data->at, orig_inp_data->num_inp_atoms, i + 1,
                                                     inp_cur_data->at);
    sd->ulStructTime += InchiTimeElapsed(&ulTStart);
    ...
    if (inp_cur_data->num_at <= 0 || orig_inp_data->nCurAtLen[i] != inp_cur_data->num_at) {
        ...
        inchi_ios_eprint(log_file, "%s #%d structure #%ld.%s%s%s%s\n", sd->pStrErrStruct, i + 1, num_inp,
                         SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
        sd->nErrorCode =
                inp_cur_data->num_at < 0 ? inp_cur_data->num_at : (orig_inp_data->nCurAtLen[i] != inp_cur_data->num_at)
                                                                  ? CT_ATOMCOUNT_ERR : CT_UNKNOWN_ERR;
        /* num_err ++; */
...
/*******************************************************************************************/
int
CreateOneComponentINChI(STRUCT_DATA *sd, INPUT_PARMS *ip, INP_ATOM_DATA *inp_cur_data, ORIG_ATOM_DATA *orig_inp_data,
...
int TreatCreateOneComponentINChIError(STRUCT_DATA *sd, INPUT_PARMS *ip, ORIG_ATOM_DATA *orig_inp_data,
    ...
    if (sd->nErrorCode) {
        ...
        sd->nErrorType = (sd->nErrorCode == CT_OUT_OF_RAM || sd->nErrorCode == CT_USER_QUIT_ERR) ? _IS_FATAL
                                                                                                 : _IS_ERROR;
        if ((ip->bINChIOutputOptions & INCHI_OUT_XML)
#ifdef TARGET_LIB_FOR_WINCHI
            || (ip->bINChIOutputOptions & INCHI_OUT_WINCHI_WINDOW) && (ip->bINChIOutputOptions & INCHI_OUT_PLAIN_TEXT)
            ...
            if (prb_file->f && 0L <= sd->fPtrStart && sd->fPtrStart < sd->fPtrEnd &&
                !ip->bSaveAllGoodStructsAsProblem) {
                CopyMOLfile(inp_file->f, sd->fPtrStart, sd->fPtrEnd, prb_file->f, num_inp);
            }
        } else {
            /*  save the problem structure */
            if (sd->nErrorCode && prb_file->f && 0L <= sd->fPtrStart && sd->fPtrStart < sd->fPtrEnd &&
                !ip->bSaveAllGoodStructsAsProblem) {
                CopyMOLfile(inp_file->f, sd->fPtrStart, sd->fPtrEnd, prb_file->f, num_inp);
            }
        }
...
/***************************************************************************/
int
PreprocessOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, ORIG_ATOM_DATA *orig_inp_data, ORIG_ATOM_DATA *prep_inp_data) {
    int i;
    ...
    if (0 < fix_odd_things(prep_inp_data->num_inp_atoms, prep_inp_data->at, /*0*/ip->bTautFlags & TG_FLAG_FIX_SP3_BUG,
                           ip->bFixNonUniformDraw)) { /* changed 2010-03-17 DT */
    ...
    } else {   /* remove "disconnected structure parities" from the structure */
        int k, m, p;
        inp_ATOM *at = (prep_inp_data)->at;
        int num_at = (prep_inp_data)->num_inp_atoms;
        for (k = 0; k < num_at; k++) {
...
int ProcessOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
    ...
    COMP_ATOM_DATA composite_norm_data[INCHI_NUM][
            TAUT_NUM + 1]; /* [0]:non-taut, [1]:taut, [2]:intermediate taut struct */
...
/*^^^ #if ( !defined( TARGET_LIB_FOR_WINCHI ) && !defined( TARGET_API_LIB ) ) */
        ...
        ret2a = WriteOrigAtomDataToSDfile(orig_inp_data, output_file, szNumber, NULL,
                                          ...
                                          (ip->bINChIOutputOptions & INCHI_OUT_SDFILE_ATOMS_DT) ? 1 : 0, ip->pSdfLabel,
                                          ip->pSdfValue);
        goto exit_function;
    }
...
exit_loop:;
    ...
    if (nRet != _IS_FATAL && nRet != _IS_ERROR) {
         ...
         */
        if (prb_file && prb_file->f && 0L <= sd->fPtrStart && sd->fPtrStart < sd->fPtrEnd &&
            ip->bSaveAllGoodStructsAsProblem) {
...
int CreateOneStructureINChI(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
    ...
    */
    ...
    for (i = 0, nRet = 0; !sd->bUserQuitComponent && i < cur_prep_inp_data->num_components; i++) {
...
#if (bREUSE_INCHI == 1)
        if (iINChI == INCHI_REC &&
            ((!ip->bDisplay && !ip->bDisplayCompositeResults && !(ip->bCompareComponents & CMP_COMPONENTS)) ||
                                    ...
                                    switch (k) {
                                        ...
                                        case TAUT_YES:
                                            if (pINChI[i][k]->lenTautomer > 0) {
                                                sd->num_taut[iINChI]++;
                                            } else if (!pINChI[i][TAUT_NON] || !pINChI[i][TAUT_NON]->nNumberOfAtoms) {
                                                sd->num_non_taut[iINChI]++;
                                            }
                                            break;
                                    }
        ...
        /*****************************************************/
        nRet = GetOneComponent(sd, ip, log_file, output_file, inp_cur_data, cur_prep_inp_data, i, num_inp, pStr,
                               nStrLen);
        if (ip->msec_MaxTime) {
            ip->msec_LeftTime -= InchiTimeElapsed(&ulTStart);
        }
        ...
        /*******************************************************************************/
        nRet = CreateOneComponentINChI(sd, ip, inp_cur_data, orig_inp_data, pINChI/*2[iINChI]*/,
                                       pINChI_Aux/*2[iINChI]*/, iINChI,
