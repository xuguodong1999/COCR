...
*/
        ...
        at_rank1 = pRankStack1[0][at_from1 = nAtomNumberCanonFrom[(int) at_rank_canon1 -
                                                                  1]]; /* atom 1 rank "from" for mapping */
        at_rank2 = pRankStack1[0][at_from2 = nAtomNumberCanonFrom[(int) at_rank_canon2 -
                                                                  1]]; /* atom 2 rank "from" for mapping */
        ...
        */
        ...
        for (j1 = 0; j1 <= iMax && at_rank1 == pRankStack2[0][at_to1 = pRankStack2[1][iMax - j1]]; j1++) {
            ...
            for (j2 = 0; j2 < MAX_NUM_STEREO_BONDS && (at_to2 = at[at_to1].stereo_bond_neighbor[j2]); j2++) {
                ...
                } else if (PARITY_CALCULATE(at[at_to1].stereo_bond_parity[j2])) {
                    if (stereo_bond_parity != sb_parity_calc) {
                ...
                if (!bAllParitiesIdentical) {
                    ...
                    if (pRankStack1[nStackPtr[istk]][at_from2] != pRankStack2[nStackPtr[istk]][at_to2]) {
                        ...
                        } else if (sb_parity_calc == BEST_PARITY) {
                ...
                if (stereo_bond_parity != sb_parity_calc) /*  parity is known */
                {
                    ...
                    if (c < 0 && !pCS->bStereoIsBetter) {
                        ...
                        ret2 = map_stereo_bonds4(at, num_atoms, num_at_tg, num_max, bAllene, nCanonRankFrom,
                                                 nAtomNumberCanonFrom, nCanonRankTo,
                                                 nSymmRank, pRankStack1 + nStackPtr[istk],
                                                 pRankStack2 + nStackPtr[istk],
                ...
                for (i = 0; i < num1; i++) {
                    ...
                    if (num1 == 2) {
                        ...
                        */
                        ...
                        parity1 = parity_of_mapped_half_bond(at_from1, at_to1, at_from2, at_to2, at, &EN1[1],
                                                             nCanonRankFrom, pRankStack1[nStackPtr[istk2]],
                                                             pRankStack2[nStackPtr[istk2]]);
                        if (parity1 <= 0)
                            return CT_STEREOCOUNT_ERR;  /*  program error */ /*   <BRKPT> */
                    } else {
                    ...
                    for (j = 0; j < num2; j++) {
                        ...
                        if (num2 == 2) {
                            ...
                            */
                            ...
                            parity2 = parity_of_mapped_half_bond(at_from2, at_to2, at_from1, at_to1, at, &EN2[1],
                                                                 nCanonRankFrom, pRankStack1[nStackPtr[istk3]],
                                                                 pRankStack2[nStackPtr[istk3]]);
                            if (parity2 <= 0) {
                                return CT_STEREOCOUNT_ERR;  /*  program error */ /*   <BRKPT> */
                            }
                        ...
                        if (sb_parity_calc != bond_parity ||
                            ...
                            ret2 = map_stereo_bonds4(at, num_atoms, num_at_tg, num_max, bAllene, nCanonRankFrom,
                                                     nAtomNumberCanonFrom, nCanonRankTo,
                                                     nSymmRank, pRankStack1 + nStackPtr[istk3],
                                                     pRankStack2 + nStackPtr[istk3],
        ...
        */                                      /*  nRankTo                 nAtomNumberTo */
        for (j1 = 0; j1 <= iMax && at_rank1 == pRankStack2[0][at_to1 = pRankStack2[1][iMax - j1]]; j1++) {
            ...
            */
            ...
            } else if (PARITY_CALCULATE(at[at_to1].stereo_atom_parity)) {
                if (stereo_center_parity != sb_parity_calc) {
                ...
                /*  known in advance parity = stereo_center_parity */
            {
                ...
                if (c < 0 && !pCS->bStereoIsBetter) {
                    ...
                    ret = map_stereo_atoms4(at, num_atoms, num_at_tg, num_max, nCanonRankFrom, nAtomNumberCanonFrom,
                                            nCanonRankTo,
