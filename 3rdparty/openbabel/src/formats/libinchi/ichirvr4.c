...
int FillOutCMP2MHINCHI(StrFromINChI *pStruct, ALL_TC_GROUPS *pTCGroups, inp_ATOM *at2,
    ...
    for (i = 0; i < pStruct->ti.num_t_groups; i++) {
        ...
        for (j = 0; j < jNum; j++) {
            ...
            } else if (at2[iat].chem_bonds_valence == at2[iat].valence + 1) {
                /* donor */
                if (is_N) {
                    /* N */
    ...
    for (i = j = 0; i < pStruct->num_atoms; i++) {
        ...
        if (!pVA[i].cMetal) {
            pc2i->nChargeMobHRevrsNonMetal += (at_Mobile_H_Revrs && !at_Mobile_H_Revrs[i].endpoint)
                                              ? at_Mobile_H_Revrs[i].charge : 0;
        }
...
int NormalizeAndCompare(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, BN_STRUCT *pBNS, BN_DATA *pBD,
    ...
    if (pStruct->bMobileH == TAUT_NON) {
        ...
        iRevrInChI = (pStruct->pOneINChI[1] && pStruct->pOneINChI[1]->nNumberOfAtoms &&
                      !pStruct->pOneINChI[1]->bDeleted) ? 1 : 0;
    } else {
        iOrigInChI = 0;
        iRevrInChI = 0;
    }
    ...
    cmpInChI = CompareReversedINChI2(pStruct->pOneINChI[iRevrInChI], pInChI[iOrigInChI],
                                     pStruct->pOneINChI_Aux[iRevrInChI], NULL /*INChI_Aux *a2*/, &icr, &err);
    ...
    if ((cmpInChI & IDIF_LESS_H) && at_prep && 0 < (nDeltaCur = icr.tot_num_H2 - icr.tot_num_H1)) {
        do {
            ...
            if (ret) {
                ...
                cmpInChI = CompareReversedINChI2(pStruct->pOneINChI[iRevrInChI], pInChI[iOrigInChI],
                                                 pStruct->pOneINChI_Aux[iRevrInChI], NULL, &icr, &err);
    ...
    if (pStruct->bMobileH == TAUT_NON) {
        ...
        do {
            if (0 > (ret = FixFixedHRestoredStructure(ip, sd, pBNS, pBD, pStruct, at, at2, at3, pVA, pTCGroups,
                                                      ...
                                                      num_inp, bHasSomeFixedH, pnNumRunBNS, pnTotalDelta,
                                                      forbidden_edge_mask,
    ...
    /**********************************************************************************************/
    /* stereo */
    cmpInChI = CompareReversedINChI2(pStruct->pOneINChI[0], pInChI[0], pStruct->pOneINChI_Aux[0],
                                     NULL /*INChI_Aux *a2*/, &icr, &err);
    if (cmpInChI & IDIF_PROBLEM) {
        ret = RI_ERR_PROGR; /* severe restore problem */
        goto exit_function;
    }
    ...
    if (iRevrInChI || iOrigInChI) {
        ...
        cmpInChI2 = CompareReversedINChI2(pStruct->pOneINChI[iRevrInChI], pInChI[iOrigInChI],
                                          pStruct->pOneINChI_Aux[iRevrInChI], NULL /*INChI_Aux *a2*/, &icr2, &err);
        if (cmpInChI & IDIF_PROBLEM) {
            ret = RI_ERR_PROGR; /* severe restore problem */
            goto exit_function;
        }
...
int MoveChargeToRemoveCenerpoints(BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct,
    ...
    int bPossiblyIgnore = pStruct->charge >= 0 &&
                          (!pTCGroups->num_tgroups || (pStruct->iMobileH == TAUT_NON && pStruct->ti.num_t_groups));
        ...
        if (pVA[i].cNumValenceElectrons != 4 && /* not C, Si, Ge */
            ...
            for (j = 0, num_endpoints = 0; j < at2[i].valence; j++) {
                ...
                if ((at2[neigh].endpoint ||
                     (pStruct->iMobileH == TAUT_NON && pStruct->endpoint && pStruct->endpoint[neigh])) ||
                    at2[neigh].charge > 0) {
                    num_known_endpoints++;
                    continue;
                }
            ...
            if (pStruct->iMobileH == TAUT_NON && num_donors == numDoubleBondNotONeigh) {
                ...
                if (!ChargeListAllExcept_DB_O.num_edges) {
                    ...
                    for (k = 0; k < num_at; k++) {
                        ...
                        } else if ((e = pVA[k].nCMinusGroupEdge - 1) >= 0 && !pBNS->edge[e].flow &&
            ...
            } else if (!bPossiblyIgnore ||
                       (!num_known_endpoints && !num_wrong_neigh && (num_acceptors_O + num_donors_O) >= 3)) {
                ...
                for (j = 0; j < numMobileChargeNeigh; j++) {
                    ...
                    } else if (IS_BNS_VT_C_GR(pv2m->type)) {
                        /* irreversible change to ChargeStruct */
    ...
    for (i = 0; i < num_at; i++) {
        if (bIsMetalSalt(at2, i)) {
            ...
            for (k = 0; k < at2[i].valence; k++) {
                ...
                } else if (NO_VERTEX != (vPlusMinus = GetPlusMinusVertex(pBNS, pTCGroups, 1, 1))) {
...
int AddRemProtonsInRestrStruct(ICHICONST INPUT_PARMS *ip_inp, STRUCT_DATA *sd, long num_inp,
                               ...
                               int *recmet_change_balance) {   /* on entry and exit, all at[i].num_H do not include isotopic H  and explicit terminal H are connected */
    int iComp, q, ret = 0;
...
int AddRemIsoProtonsInRestrStruct(ICHICONST INPUT_PARMS *ip_inp, STRUCT_DATA *sd, long num_inp, int bHasSomeFixedH,
                                  ...
                                  NUM_H recmet_change_balance[]) {   /* on entry and exit, all at[i].num_H do not include isotopic H and explicit terminal H are connected */
