...
int
GetNeutralRepsIfNeeded(AT_NUMB *pri, AT_NUMB *prj, inp_ATOM *at, int num_atoms, T_ENDPOINT *EndPoint, int nNumEndPoints,
                       C_GROUP_INFO *cgi);
...
/*****************************************************************************/
int
AddAtom2num(AT_RANK num[], inp_ATOM *atom, int at_no, int bSubtract) {  /*  bSubtract: 0=> add, 1=>subtract, 2=> fill */
...
/*****************************************************************************/
void AddAtom2DA(AT_RANK num_DA[], inp_ATOM *atom, int at_no,
                int bSubtract) {   /*  bSubtract: 0=> add, 1=>subtract, 2=> fill */
    inp_ATOM *at = atom + at_no;
...
int nGetEndpointInfo(inp_ATOM *atom, int iat, ENDPOINT_INFO *eif) {
    ...
    if (atom[iat].charge == -1 || atom[iat].charge == 0) {
        ...
        if (cChargeSubtype & C_SUBTYPE_H_ACCEPT) {
            eif->cDonor = 0;
            eif->cAcceptor = 1;
        } else if (cChargeSubtype & C_SUBTYPE_H_DONOR) {
            eif->cDonor = 1;
            eif->cAcceptor = 0;
        } else {
            return 0;
...
int RegisterEndPoints(T_GROUP_INFO *t_group_info,
    ...
    */
    ...
    for (i = j = k = 0; i < nNumEndPoints; i++) {
        ...
        k += (prev_eqnum ==
              EndPoint[i].nEquNumber);   /* count endpoints that belongo to a group equivalent to the 1st group */
    ...
    } else if (nNumZeroEqu == nNumEndPoints) {
    ...
    for (i = 0, nNumFoundEqu = 0; i < nNumTgroupNumbers; i++) {
        for (j = i + 1; j < nNumTgroupNumbers; j++) {
            if ((nTGroupEqu[i] != nTGroupEqu[j] && (i >= nNumDiffTGroupNumbers || j >= nNumDiffTGroupNumbers)) ||
                ...
                } else if (nTGroupEqu[i]) { /* extend existing t-group */
                    nTGroupEqu[j] = nTGroupEqu[i];
                    EndPoint[nTGEndPointNo[j]].nEquNumber = nTGroupEqu[i];

                } else if (nTGroupEqu[j]) { /* extend existing t-group */
                    nTGroupEqu[i] = nTGroupEqu[j];
...
int bCanBeACPoint(inp_ATOM *at, S_CHAR cCharge, S_CHAR cChangeValence, S_CHAR neutral_bonds_valence,
    ...
    if (at->charge == cCharge && at->valence == at->chem_bonds_valence && at->num_H) {
        ...
        return 0;
    } else if (at->charge == cCharge && at->valence < at->chem_bonds_valence) {
        ...
        if (nBondsValence == neutral_bonds_valence) {
            if (nNumBonds == neutral_valence) {
                ...
                } else if (at->valence < neutral_valence /*&& nEndpointValence */ ) {
                    /* num_H > 0: >NH -NH2 -OH */
    ...
    if (at[point1].c_point == at[point2].c_point) {
        ...
        if (num_c < max_num_c) {
            ...
            *pnum_c = num_c + 1;
            ...
            if (at[point1].num_H) {
                c_group[num_c].num[1]++;
            } else if (at[point2].num_H) {
                c_group[num_c].num[1]++;
            } else if ((at[point1].endpoint || at[point2].endpoint) && t_group_info && t_group_info->t_group &&
        ...
        *pnum_c = num_c;
        ...
        for (i = 0; i < num_atoms; i++) {
            if (at[i].c_point > nGroupNumber) {
                at[i].c_point--;
            } else if (at[i].c_point == nGroupNumber) {
        ...
        for (j = 0; j < at[at_no].valence; j++) {
            ...
            if (((eif.cAcceptor && (bond_type == BOND_DOUBLE ||
                ...
                (at[centerpoint].chem_bonds_valence > at[centerpoint].valence ||
                 ...
                 (at[centerpoint].chem_bonds_valence == at[centerpoint].valence &&
                  (at[centerpoint].endpoint ||
                   at[centerpoint].c_point)) /* tautomerism or charge may increment at[centerpoint].chem_bonds_valence*/ ) &&
    ...
    if (1 || (s_group_info->num_candidates < 0)) {
        ...
        for (i = 0, nNumCandidates = nNumCandidates2 = 0; i < num_atoms; i++) {
            ...
            if (0 == (s_type = GetSaltChargeType(at, i, t_group_info, &s_subtype)) ||
                ...
                } else if (bDoNotMergeNonTautAtom(at, i)) {
...
int MakeIsotopicHGroup(inp_ATOM *at, int num_atoms, S_GROUP_INFO *s_group_info,
    ...
    if (1 || (s_group_info->num_candidates < 0)) {
        ...
        if (nNumCandidates > 0) {
            t_group_info->nIsotopicEndpointAtomNumber = (AT_NUMB *) inchi_calloc(nNumNonTautCandidates + 1,
                                                                                 sizeof(t_group_info->nIsotopicEndpointAtomNumber[0]));
                ...
                if ((bond_type == BOND_DOUBLE ||
                    ...
                    for (k = 0, nNumEndPoints = 0, nNumBondPos = 0; k < at[centerpoint].valence; k++) {
                        ...
                        if (!ALLOWED_EDGE(pBNS, centerpoint, k)) {
                            continue;
                        } else if (bond_type == BOND_ALTERN || bond_type == BOND_ALT12NS || bond_type == BOND_TAUTOM) {
                            bTautBond = 1;
#if (REPLACE_ALT_WITH_TAUT == 1)
                            bAltBond = (bond_type == BOND_ALTERN || bond_type == BOND_ALT12NS);
#endif
                        } else if (bond_type == BOND_SINGLE || bond_type == BOND_DOUBLE)
                            bNonTautBond = 1;
                        else
                            continue;
                    ...
                    if (nNumEndPoints > 1 && nNumPossibleMobile && nNumDonor && nNumAcceptor) {
                        ...
                        if (nNumEndPoints > 0) {
                            if (!nGroupNumber || bDiffGroups > 0) {
                                num_changes = RegisterEndPoints(t_group_info, EndPoint, nNumEndPoints, at, num_atoms,
                                                                c_group_info, pBNS);
                                if (num_changes == -1) {
                ...
                for (j = 0; j < at[i].valence; j++) {
                    ...
                    if ((bond_type == BOND_DOUBLE ||
                        ...
                        for (k = 0, nNumEndPoints = 0, nNumBondPos = 0; k < at[centerpoint].valence; k++) {
                            ...
                            if (!ALLOWED_EDGE(pBNS, centerpoint, k)) {
                                continue;
                            } else if (bond_type == BOND_ALTERN || bond_type == BOND_ALT12NS ||
                                       bond_type == BOND_TAUTOM) {
                                bTautBond = 1;
#if (REPLACE_ALT_WITH_TAUT == 1)
                                bAltBond = (bond_type == BOND_ALTERN || bond_type == BOND_ALT12NS);
                        ...
                        if (nNumEndPoints > 1 && nNumPossibleMobile && nNumDonor && nNumAcceptor && num_O == 1 &&
                            num_C) {
                            /*
                             * a tautomeric group has been found
                             *
                            ...
                            if (nNumEndPoints > 0) {
                                if (!nGroupNumber || bDiffGroups > 0) {
                                    num_changes = RegisterEndPoints(t_group_info, EndPoint, nNumEndPoints, at,
            ...
            if (2 == at[i1].valence &&
                ...
                for (j = 0; j < at[i1].valence; j++) {
                    ...
                    ret = nGet12TautIn5MembAltRing(at, i1, j, nDfsPathPos,
                                                   ...
                                                   EndPoint, sizeof(EndPoint) / sizeof(EndPoint[0]),
                                                   BondPos, sizeof(BondPos) / sizeof(BondPos[0]),
                                                   &nNumEndPoints, &nNumBondPos, pBNS, pBD, num_atoms);
                    if (ret > 0) {
                        if (nNumEndPoints) {
                            num_changes = RegisterEndPoints(t_group_info, EndPoint, nNumEndPoints, at, num_atoms,
                                                            c_group_info, pBNS);
...
#endif
                            nMobile2 =
                                    at[endpoint2].num_H + (at[endpoint2].charge == -1);  /*  number of mobile groups */
...
int CountTautomerGroups(sp_ATOM *at, int num_atoms, T_GROUP_INFO *t_group_info) {
    ...
    if (max_t_group &&
        !(nTautomerGroupNumber = (AT_NUMB *) inchi_calloc(max_t_group + 1,
                                                          sizeof(nTautomerGroupNumber[0])) /*temp*/ )) {
    ...
    for (i = 0, nNewGroupNumber = 0; i < num_t; /*i ++*/ ) {
        ...
        if (t_group[i].nNumEndpoints != nTautomerGroupNumber[(int) nGroupNumber]
            #if (IGNORE_TGROUP_WITHOUT_H == 1)
            || (bNoH = (t_group[i].num[0] ==
                        t_group[i].num[1]))  /* only for (H,-) t-groups; (+) t-groups are not removed */
#endif
                ) {
    ...
    if (!ret && ((t_group_info->tni.bNormalizationFlags & FLAG_NORM_CONSIDER_TAUT) ||
                 (t_group_info->nNumIsotopicEndpoints > 1 && (t_group_info->bTautFlagsDone &
