...
// Derive directly from OBFormat for objects which are not molecules.
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.lpmd.cl/index.php/documentation/the-lpmd-format"; };
    ...
    bool LpmdFormat::ReadHeader(std::istream &ifs, OBMol &mol) {
        ...
        if (tokens.at(0).compare("LPMD") != 0 || tokens.at(1).compare("2.0") != 0) {
            obErrorLog.ThrowError(__FUNCTION__, "The start line, doesn't identify this file like a lpmd 2.0 file",
                                  obError);
            return false;
        }
    ...
    bool LpmdFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int i = 1; i < headers.size(); ++i) //the first element is HDR
            {
                ...
                //Accelerations
                if (headers.at(i).compare("AX") == 0) {
                    from_string<double>(AX, tokens.at(i - 1), std::dec);
                    ma++;
                }
                if (headers.at(i).compare("AY") == 0) {
                    from_string<double>(AY, tokens.at(i - 1), std::dec);
                    ma++;
                }
                if (headers.at(i).compare("AZ") == 0) {
                    from_string<double>(AZ, tokens.at(i - 1), std::dec);
                    ma++;
                }
                //Forces
                if (headers.at(i).compare("FX") == 0) {
                    from_string<double>(FX, tokens.at(i - 1), std::dec);
                    mf++;
                }
                if (headers.at(i).compare("FY") == 0) {
                    from_string<double>(FY, tokens.at(i - 1), std::dec);
                    mf++;
                }
                if (headers.at(i).compare("FZ") == 0) {
                    from_string<double>(FZ, tokens.at(i - 1), std::dec);
                    mf++;
                }
                //Charges
...
////////////////////////////////////////////////////////////////
    ...
    bool LpmdFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        for (int i = 0; i < mol.NumAtoms(); ++i) {
            ...
            if (level >= 2 && mode == 0) {
                double mass = atom->GetAtomicMass();
                vector3 a = vector3(forceslist[0][i].x() / mass, forceslist[0][i].y() / mass,
                                    forceslist[0][i].z() / mass);
