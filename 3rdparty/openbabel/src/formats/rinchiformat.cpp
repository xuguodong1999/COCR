...
#define M_AGENTS 2
        ...
        for (int part = 0; part <= 2; ++part) {
            ...
            switch (part) {
                case M_REACTANTS:
                    N = facade.NumComponents(REACTANT);
                    break;
                case M_PRODUCTS:
                    N = facade.NumComponents(PRODUCT);
                    break;
                case M_AGENTS:
                    N = facade.NumComponents(AGENT);
                    break;
            }
            for (unsigned int i = 0; i < N; ++i) {
                ...
                switch (part) {
                    case M_REACTANTS:
                        facade.GetComponent(&mol, REACTANT, i);
                        break;
                    case M_PRODUCTS:
                        facade.GetComponent(&mol, PRODUCT, i);
                        break;
                    case M_AGENTS:
                        facade.GetComponent(&mol, AGENT, i);
                        break;
                }
        ...
        if (rsize > 0 || psize > 0 || !inchis[M_AGENTS].empty()) {
            ...
            if (!inchis[M_AGENTS].empty()) {
                ofs << "<>";
                for (std::vector<std::string>::const_iterator vit = inchis[M_AGENTS].begin();
                     vit != inchis[M_AGENTS].end(); ++vit) {
