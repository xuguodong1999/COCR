...
namespace OpenBabel {
    ...
    SVGPainter::SVGPainter(ostream &ofs, std::set<ColorGradient> *gradients, bool withViewBox,
            ...
            : m_ofs(ofs), m_withViewBox(withViewBox), m_width(width), m_height(height),
              m_Pencolor("black"), m_Fillcolor("white"), m_Gradientcolor(make_pair(OBColor("white"), OBColor("white"))),
              m_PenWidth(1),
    ...
    void SVGPainter::NewCanvas(double width, double height) {
        ...
        if (!m_withViewBox &&
            m_Fillcolor.alpha != 0.0)//Background color for single molecule. Handled by outer svg when table.
