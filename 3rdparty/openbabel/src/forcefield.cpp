  ...
  **/
    ...
    */
    ...
    int OBForceField::FastRotorSearch(bool permute) {
        ...
        double *bestconf = new double[_mol.NumAtoms() *
                                      3]; // store the best conformer to date in the current permutation
        ...
        for (int N = 0; N < num_permutations; ++N) {
            ...
            for (int i = 0; i < reordered_rotors.size(); ++i) {
                ...
                if (i == 4) { // Check whether this rotorKey has already been chosen
                    ...
                    unsigned int hash =
                            rotorKey[1] + rotorKey[2] * 12 + rotorKey[3] * 12 * 12 + rotorKey[4] * 12 * 12 * 12;
    ...
    void OBForceField::DistanceGeometry() {
                            ...
                            if (&*nbr3 == &*b) {
                                ...
                                if (i > j) // minimum distance (torsion angle = 0)
                                    ...
                                    delta_y = 1.3 * sin(DEG_TO_RAD * (180.0 - 120.0)) +
                                              1.3 * sin(DEG_TO_RAD * (180.0 - 120.0));
        ...
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
                G[i][j] =
                        0.5 * (matrix[0][i] * matrix[0][i] + matrix[0][j] * matrix[0][j] - matrix[i][j] * matrix[i][j]);
            }
        }
