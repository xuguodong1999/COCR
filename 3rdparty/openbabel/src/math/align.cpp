...
namespace OpenBabel {
    ...
    OBAlign::OBAlign(const OBMol &refmol, const OBMol &targetmol, bool includeH, bool symmetry) : _method(
            OBAlign::Kabsch) {
        _symmetry = symmetry;
        _includeH = includeH;
        SetRefMol(refmol);
...
/* Evaluates the Newton-Raphson correction for the Horn quartic.
   only 11 FLOPs */
    ...
    vector<double> CalcQuarticCoeffs(const Eigen::Matrix3d &M) {
        ...
        coeff[1] = 8.0 * (Sxx * Syz * Szy + Syy * Szx * Sxz + Szz * Sxy * Syx - Sxx * Syy * Szz - Syz * Szx * Sxy -
                          Szy * Syx * Sxz);
    ...
    bool OBAlign::Align() {
        ...
        if (_pref->size() != N) {
            obErrorLog.ThrowError(__FUNCTION__, "Cannot align the reference and target as they are of different size",
                                  obError);
            return false;
        }
    ...
    matrix3x3 OBAlign::GetRotMatrix() {
        ...
        for (int row = 0; row < 3; ++row)
            for (int col = 0; col < 3; ++col)
                rot[col][row] = _rotMatrix(row,
                                           col); // Return in form suitable for use in expressions like "result *= rotMatrix";
    ...
    bool OBAlign::UpdateCoords(OBMol *target) {
        ...
        if (newcoords.size() != target->NumAtoms()) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Cannot update the target molecule with the alignment coordinates as they are of different size",
                                  obError);
            return false;
        }
