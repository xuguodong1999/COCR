...
namespace OpenBabel {
    ...
    const SpaceGroup *SpaceGroup::GetSpaceGroup(const string &name_in) {
        ...
        const SpaceGroup *match =
                _SpaceGroups.sgbn.find(name_in) != _SpaceGroups.sgbn.end() ? _SpaceGroups.sgbn[name_in] : nullptr;
        ...
        if (!match) {
            ...
            string search = name;
            bool hasMirror = (name.find('m') != string::npos || name.find('d') != string::npos ||
                              name.find('n') != string::npos || name.find('c') != string::npos);
    ...
    const SpaceGroup *SpaceGroup::Find(SpaceGroup *group) {
        ...
        if (group->m_Hall.length() > 0 && _SpaceGroups.sgbn.find(group->m_Hall) != _SpaceGroups.sgbn.end()) {
            ...
            if (!found)
                obErrorLog.ThrowError(__FUNCTION__, "Unknown space group (Hall symbol:" + group->m_Hall +
                                                    ") error, please file a bug report.", obError);
            if (group->m_transforms.size() && *found != *group) {
                unsigned id = group->GetId();
                if (id != 3 && id != 68) // these groups have duplicates
                {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Space group error (Hall symbol and list of transforms do not match), please file a bug report.",
                                          obWarning);
                    return found;
                }
            } else
        ...
        if (stripped_hm.length() > 0 &&
            ...
            if (group->m_transforms.size()) {// If transforms (symmetry operations) are listed, make sure they match the tabulated ones
                ...
                for (i = _SpaceGroups.sgbi[found->m_id - 1].begin(); i != end; ++i)
                    if ((**i) == *group)
                        return *i;
                obErrorLog.ThrowError(__FUNCTION__, "Unknown space group error (H-M symbol:" + group->m_HM +
                                                    "), cannot match the list of transforms, please file a bug report.",
                                      obError);
                return nullptr;
            } else if (group->m_transforms.size() ==
                       0) {// No transforms (symmetry operations) are listed, warn if HM symbol can match several spacegroups
                int n = 0;
                ...
                if (n > 1)
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Ambiguous space group: HM symbol corresponds to several space groups.",
                                          obWarning);
                return found;
            }
            /* even if there is an error (this should not occur) return the found group, since
            ...
            if (group->m_transforms.size()) {
                ...
                if (_SpaceGroups.sgbi[group->m_id - 1].size() > 1)
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Ambiguous space group: sg number corresponds to several space groups.",
                                          obWarning);
