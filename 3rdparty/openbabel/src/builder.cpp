...
namespace OpenBabel {
    ...
    **/
    ...
    void OBBuilder::AddRingFragment(OBSmartsPattern *sp, const std::vector<vector3> &coords) {
        ...
        if (hasAllZeroCoords) {
            std::stringstream ss;
            ss << "Ring fragment " << sp->GetSMARTS()
               << " in ring-fragments.txt has all zero coordinates. Ignoring fragment.";
        ...
        if (dimension != 2) {
            ...
            if (atom->GetExplicitDegree() == 1) {
                ...
                if (bond2 == VZero || v1 == VZero) {
                    ...
                    v1 = cross(bond1,
                               vrand); // so find a perpendicular, given the random vector (this doesn't matter here)
                }
                v2 = cross(bond1, v1);
                v2 = v2.normalize();
            ...
            if (_rigid_fragments_index.count(fragment_smiles) > 0) {
                ...
                if (!sp.Init(fragment_smiles)) {
                    ...
                    for (j = mlist.begin(); j != mlist.end(); ++j) {
                        ...
                        for (k = j->begin(), counter = 0;
                             k != j->end(); ++k, ++counter) { // for all atoms of the fragment
    ...
    void OBBuilder::FlipSpiro(OBMol &mol, int idx) {
        ...
        vector<int> nbrs;
        FOR_NBORS_OF_ATOM(nbr, p)nbrs.push_back(nbr->GetIdx());
        //assert(nbrs.size() == 4);
    ...
    bool OBBuilder::CorrectStereoAtoms(OBMol &mol, bool warn) {
        ...
        for (origth = nonringtetra.begin(), newth = nonringnewtetra.begin();
             origth != nonringtetra.end(); ++origth, ++newth) {
            OBTetrahedralStereo::Config config = (*newth)->GetConfig(OBStereo::Clockwise, OBStereo::ViewFrom);
