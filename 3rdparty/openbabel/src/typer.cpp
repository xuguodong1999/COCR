    ...
    */
    ...
    void OBAtomTyper::ParseLine(const char *buffer) {
        ...
        if (EQn(buffer, "INTHYB", 6)) {
            tokenize(vs, buffer);
            if (vs.size() < 3) {
                obErrorLog.ThrowError(__FUNCTION__, " Could not parse INTHYB line in atom type table from atomtyp.txt",
                                      obInfo);
                return;
            }
            ...
            tokenize(vs, buffer);
            if (vs.size() < 3) {
                obErrorLog.ThrowError(__FUNCTION__, " Could not parse EXTTYP line in atom type table from atomtyp.txt",
                                      obInfo);
                return;
            }
            sp = new OBSmartsPattern;
    ...
    */
    ...
    void OBRingTyper::ParseLine(const char *buffer) {
        ...
        if (EQn(buffer, "RINGTYP", 7)) {
            tokenize(vs, buffer);
            if (vs.size() < 3) {
                obErrorLog.ThrowError(__FUNCTION__, " Could not parse RING line in ring type table from ringtyp.txt",
                                      obInfo);
                return;
            }
            sp = new OBSmartsPattern;
    ...
    enum ExocyclicAtom {
        NO_EXOCYCLIC_ATOM, EXO_OXYGEN, EXO_NONOXYGEN
    };
