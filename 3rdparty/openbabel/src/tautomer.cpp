...
//#define DEBUG 1
    ...
    struct TautomerImpl {
        ...
        public:
            AssignDonorRAII(std::vector<Type> &atomTypes, int &hydrogenCounter, OBAtom *atom) : m_atomTypes(atomTypes),
                                                                                                m_atom(atom),
                                                                                                m_hydrogenCounter(
                                                                                                        hydrogenCounter) {
                redo(atom);
        ...
        bool IsPropagationValid(OBMol *mol, const std::vector<Type> &atomTypes, const std::vector<Type> &bondTypes,
                                int depth) {
            FOR_ATOMS_OF_MOL (atom, mol) {
        ...
        void AssignmentPropagation(OBMol *mol, std::vector<Type> &atomTypes, std::vector<Type> &bondTypes,
            ...
            if (IsLeafNode(atomTypes)) {
                ...
                if (!numHydrogens) {
                    ...
                    FOR_ATOMS_OF_MOL (atom, mol)atom->SetAromatic(false);
        ...
        void Enumerate(OBMol *mol, TautomerFunctor &functor, bool canonical) {
            ...
            FOR_ATOMS_OF_MOL(atom, mol)hcounts.push_back(atom->GetImplicitHCount());
#ifdef DEBUG
            ...
            FOR_BONDS_OF_MOL (bond, mol)bondOrders.push_back(bond->GetBondOrder());
            ...
            if (canonical) {
                ...
                FOR_ATOMS_OF_MOL (atom, mol)m_canonAtoms.push_back(&*atom);
            }
