...
namespace OpenBabel {
    ...
    static double UnitNameToConversionFactor(const char *unit) {
        ...
        switch (p[0]) {
            case 'e':
                ...
                if (p[1] == 'l' && p[2] == 'e' && p[3] == 'c' && p[4] == 't' && p[5] == 'r' && p[6] == 'o' &&
                    p[7] == 'n' &&
            ...
            case 'k':
                ...
                if (p[1] == 'c' && p[2] == 'a' && p[3] == 'l' && p[4] == '/' && p[5] == 'm' && p[6] == 'o' &&
                    p[7] == 'l' && p[8] == '\0')
                    return 1.0; // kcal/mol
                break;
            case 'H':
                if (p[1] == 'a' && p[2] == 'r' && p[3] == 't' && p[4] == 'r' && p[5] == 'e' && p[6] == 'e' &&
                    p[7] == '\0')
                    return HARTEE_TO_KCALPERMOL; // Hartree
                break;
            case 'J':
                if (p[1] == '/' && p[2] == 'm' && p[3] == 'o' && p[4] == 'l' && p[5] == ' ' && p[6] == 'K' &&
                    p[7] == '\0')
                    return KJPERMOL_TO_KCALPERMOL; // J/mol K
                break;
            case 'R':
                if (p[1] == 'y' && p[2] == 'd' && p[3] == 'b' && p[4] == 'e' && p[5] == 'r' && p[6] == 'g' &&
                    p[7] == '\0')
    ...
    */
    ...
    bool OBTypeTable::Translate(string &to, const string &from) {
        ...
        obErrorLog.ThrowError(__FUNCTION__, "Cannot perform atom type translation: table cannot find requested types.",
                              obWarning);
        to = from;
        return (false);
    }
    ...
    void OBGlobalDataBase::Init() {
        ...
        string fn_open = OpenDatafile(ifs, _filename);
