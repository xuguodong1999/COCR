...
int mol_copy_check_empty(char *dest, char *source, int len, char **first_space) {
    ...
    for (i = (len - 1); i >= 0 && 0 != (c = source[i]) && isspace(UCINT c);
    i-- );
 ...
 */
    ...
    switch (data_type) {
        case MOL_STRING_DATA:
            for (i = 0; i < field_len && 0 != (c = p[i]) && isspace(UCINT c);
            i++ ) /* pass by all leading spaces */
            ;
        ...
        case MOL_LONG_INT_DATA: { /* block start */
            char str[MOL_MAX_VALUE_LEN + 1];
            ldata = 0L;
            if (field_len > MOL_MAX_VALUE_LEN) {
                ret = -1;
            } else if (field_len > 0) { /* fixed length */
                *line_ptr += (len = mol_copy_check_empty(str, p, field_len, &q));
                *q = '\0';
                if (!len || !(q - str)) { /* empty string */
                    ret = 0;
                } else if ((ldata = strtol(str, &p_end, 10), p_end != q)) { /* wrong data: incompletely interpreted */
                    ret = -1;
                }
        ...
        case MOL_FLOAT_DATA: { /* block start */
            char str[MOL_MAX_VALUE_LEN + 1];
            if (field_len > MOL_MAX_VALUE_LEN) {
                ret = -1;
                ddata = 0.0;
                ...
                *q = '\0';
                if (!len || !(q - str)) { /* empty string */
                    ddata = 0.0;
                    ret = 0;
                } else if ((ddata = strtod(str, &p_end), p_end != q)) { /* wrong data */
                    ret = -1;
                }
            } else { /* free format */
    ...
    for (i = 0; i < ctab->nNumberOfAtoms; i++) {
        ...
        if (NULL != ctab->MolAtom) {
            if (0 > mol_read_datum(&ctab->MolAtom[i].fX, 10, MOL_DOUBLE_DATA, &p)
                ...
                || 0 == mol_read_datum(&ctab->MolAtom[i].szAtomSymbol, 3, MOL_STRING_DATA,
                                       &p) /* was sizeof(ctab->MolAtom[0].szAtomSymbol)-1 */
            ...
            ctab->MolAtom[i].szAtomSymbol[1] = (char) tolower(UCINT
            ctab->MolAtom[i].szAtomSymbol[1]); /* 5-4-99 DCh*/

            if ((chg = (S_SHORT) ctab->MolAtom[i].cCharge) < 0 ||
                chg >= (int) (sizeof(charge_val) / sizeof(charge_val[0]))) {
                /* ctab->MolAtom[i].cCharge = 0; */ /* error; ignore for now */
                ctab->MolAtom[i].cCharge = (S_CHAR)(
                        4 - chg); /*  allow greater charges to accommodate NCI structures. 8-20-2002 */
...
int read_properties_block(MOL_CTAB *ctab, MOL_HEADER_BLOCK *pHdr, FILE *inp, int err, char *pStrErr) {
    enum {
        MULTI_LINE_MODE_NO_MODE, MULTI_LINE_MODE_ISIS_ALIAS
    };
    char *p;
    char line[MOLFILEINPLINELEN];
    const int line_len = sizeof(line);
     ...
     */
    for (i = 0; ctab->csCurrentCtabVersion[0] ? 1 : (i <
                                                     ctab->nNumberOfPropertyLines); i++) { /* the last line should be M END */
            ...
            for (j = 0; j < num_entries; j++) {
                ...
                if (1 /* !ctab->MolAtom[atoms[j]-1].cAtomAliasedFlag */) {
                    ...
                    if (at[1] || at[0] != 'D' && at[0] != 'T') {  /*  D & T cannot have ISO */
                        ...
                        int atw, atw_diff;
                        /*^^^ 
                        NB: According to MDL specification, difference should be in 
...
MOL_DATA *read_mol_file(FILE *inp, MOL_HEADER_BLOCK *OnlyHeaderBlock, MOL_CTAB *OnlyCtab,
    ...
    *err = 0;
    ...
    if (bReadAll) {
        if (NULL == (mol_data->ctab.MolAtom = (MOL_ATOM *) inchi_calloc(inchi_max(mol_data->ctab.nNumberOfAtoms, 1),
                                                                        sizeof(MOL_ATOM)))) {
            ret = 2; /* can't allocate MolAtom structure */
            MOLFILE_ERR_FIN (ret, 2, err_fin, "Out of RAM");
        }
        if (bGetOrigCoord &&
            NULL == (mol_data->ctab.szCoord = (MOL_COORD *) inchi_calloc(inchi_max(mol_data->ctab.nNumberOfAtoms, 1),
                                                                         sizeof(MOL_COORD)))) {
            ret = 2; /* can't allocate MolAtom structure */
            MOLFILE_ERR_FIN (ret, 2, err_fin, "Out of RAM");
        }
    ...
    if (bReadAll && ret < 30) {
        if (!bEndOfData && NULL == (mol_data->ctab.MolBond = (MOL_BONDS *) inchi_calloc(
                inchi_max(mol_data->ctab.nNumberOfBonds, 1), sizeof(MOL_BONDS)))) {
...
enum {
    ...
    SDF_DATA_LINE,
    SDF_END_OF_DATA_ITEM,
    SDF_EMPTY_LINE,
    SDF_END_OF_DATA_BLOCK
};
...
int identify_sdf_label(char *inp_line, const char *pSdfLabel) {
    ...
    if ((p = strchr(inp_line, '<')) &&
        ...
        memcpy(line, p + 1, len);
        line[len] = '\0';
        for (i = 0; isspace(UCINT line[i]);
        i++ );
        for (j = len - 1; j >= i && isspace(UCINT line[i]);
        j-- );
        len = j - i + 1;
        p = line + i;
...
int bypass_sdf_data_items(FILE *inp, long *cas_reg_no, char *comment,
        ...
        switch (current_state) {
            ...
            case SDF_EMPTY_LINE:              /* Added 9-25-97 DCh */
                ...
                if (0 == strcmp(line, SDF_END_OF_DATA)) {
                    ...
                    current_state = (bNeedsName || bNeedsComm || bNeedsCASrn || bNeedsUser) ? identify_sdf_label(line,
                                                                                                                 pSdfLabel)
                                                                                            : SDF_DATA_HEADER;
    ...
    if (0 == err && SDF_END_OF_DATA_BLOCK != current_state &&
        NULL == p); /* err = 4; */ /* unexpected end of file: missing $$$$ */
