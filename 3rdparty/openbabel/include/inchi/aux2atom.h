                    ...
                    while (1) {
                        ...
                        if (isdigit(UCINT *p)) {
                            ...
                            if (!AT_BONDS_VAL(atom, i))
                                AT_BONDS_VAL(atom,
                                             i) = ISOLATED_ATOM; /* same convention as in MOLfile, found zero bonds valence */
                            p = q;
                        }
                        /* charge */
                        ...
                        *at = atom;
                        ...
                        for (bNonMetal = 0; bNonMetal < 1; bNonMetal++) {
                            for (a1 = 0; a1 < num_atoms; a1++) {
                                ...
                                for (n1 = 0; MIN_INPUT_BOND_TYPE + n1 <= 3 &&
                                             MIN_INPUT_BOND_TYPE + n1 <= MAX_INPUT_BOND_TYPE; n1++) {
                                        ...
                                        default:
                                            ...
                                            for (n1 = 0; n1 < AT_NUM_BONDS(atom[a1]); n1++) {
                                                if (atom[a1].bond_type[n1] == INCHI_BOND_TYPE_ALTERN) {
                                                    ...
                                                    p1 = IN_NEIGH_LIST(atom[a2].neighbor, (ATOM_NUMBER) a1,
                                                                       AT_NUM_BONDS(atom[a2]));
                                                    if (p1) {
                                                        atom[a1].bond_type[n1] =
                                                        atom[a2].bond_type[p1 -
                                                                           atom[a2].neighbor] = INCHI_BOND_TYPE_SINGLE;
                                                    } else {
                                                        *err = -2;  /*  Program error */
                                                        MOLFILE_ERR_SET (*err, 0,
                                 ...
                                 */
                                {
                                    ...
                                    } else if (valence && valence >= chem_bonds_valence) {
                                        atom[a1].num_iso_H[0] = valence - chem_bonds_valence;
                                    } else if (valence || bDoNotAddH) {
                                        atom[a1].num_iso_H[0] = 0;
                                    } else if (!bDoNotAddH) {
...
#define MAX_CHAIN_LEN 20
                                    ...
                                    if (bEnd2 + bEnd1 == 1) {
                                        ...
                                        chain[len++] = next;
                                        while (len <
                                               MAX_CHAIN_LEN) { /* arbitrary very high upper limit to prevent infinite loop */
                                            prev = cur;
                                            cur = next;
                                            /* follow double bond path && avoid going back */
                                            if (AT_NUM_BONDS(atom[cur]) == 2 &&
                                                atom[cur].bond_type[0] + atom[cur].bond_type[1] ==
                                                2 * INCHI_BOND_TYPE_DOUBLE &&
                                        ...
                                        if (len > 2 &&
                                            (p2 = IN_NEIGH_LIST(atom[cur].neighbor, (ATOM_NUMBER) prev,
                                                                AT_NUM_BONDS(atom[cur])))) {
        ...
        while (0 < (res = inchi_ios_gets(szLine, sizeof(szLine) - 1, inp_molfile, &bTooLongLine))) {
            ...
            if (!memcmp(szLine, sStructHdrXml, sizeof(sStructHdrXml) - 1)) {
                ...
                bErrorMsg = bRestoreInfo = 0;
                        ...
                        (bFatal = 0, len = sizeof(sStructMsgXmlErr) - 1, !memcmp(szLine, sStructMsgXmlErr, len))) ||
                       (bHeaderRead &&
                        (len = sizeof(sStructMsgXmlErrFatal) - 1, !memcmp(szLine, sStructMsgXmlErrFatal, len)) &&
                        (bFatal = 1))) {
                p = szLine + len;
                q = strchr(p, '\"');
                if (q && !bFindNext) {
            ...
            } else if (bHeaderRead && bInTheAuxInfo &&
                       !memcmp(szLine, sStructRevXmlRevHdr, sizeof(sStructRevXmlRevHdr) - 1)) {
                /***********************  atoms xml ***************************/
                num_struct = 1;
                ...
                if ((pszCoord = (MOL_COORD *) inchi_malloc(inchi_max(num_atoms, 1) * sizeof(MOL_COORD)))) {
                    ...
                    while (i < num_atoms) {
                        ...
                        if (!bItemIsOver && (int) sizeof(szLine) - res + pos > (int) sizeof(szNextLine)) {
                            ...
                            res2 = inchi_ios_gets(szNextLine, sizeof(szNextLine) - 1, inp_molfile, &bTooLongLine2);
                            if (res2 > 0 &&
                                memcmp(szNextLine, sStructRevXmlRevXYZEnd, sizeof(sStructRevXmlRevXYZEnd) - 1)) {
                                if (pos) {
                                    res -= pos;  /* number of chars left to process in szLine */
                                    memmove(szLine, p,
                                            res * sizeof(szLine[0])); /* move them to the start of the line */
                                }
                                memcpy(szLine + res, szNextLine, (res2 + 1) * sizeof(szNextLine[0]));
                                res += res2;
                                szLine[res] = '\0';
...
/*exit_cycle:;*/
        ...
        while (num_struct > 0 && 0 < inchi_ios_gets(szLine, sizeof(szLine) - 1, inp_molfile, &bTooLongLine)) {
            if (!memcmp(szLine, sStructHdrXmlEnd, sizeof(sStructHdrXmlEnd) - 1))
                num_struct--;
            else if (!memcmp(szLine, sStructHdrXml, sizeof(sStructHdrXml) - 1))
                num_struct++;
        }
        return num_atoms;
