...
//! changing bonds, etc.) use the OBChemTsfm class.
    ...
    public:
        ...
        int NumReactants() const { return static_cast<int> (_reactants.size()); }

        int NumProducts() const { return static_cast<int> (_products.size()); }

        int NumAgents() const {
            return static_cast<int> (_agents.size());
        }

        void AddReactant(const std::shared_ptr<OBMol> sp) { _reactants.push_back(sp); }

        void AddProduct(const std::shared_ptr<OBMol> sp) { _products.push_back(sp); }

        void SetTransitionState(const std::shared_ptr<OBMol> sp) { _ts = sp; }

        void AddAgent(const std::shared_ptr<OBMol> sp) { _agents.push_back(sp); }
        ...
        std::shared_ptr<OBMol> GetTransitionState() const { return _ts; }
