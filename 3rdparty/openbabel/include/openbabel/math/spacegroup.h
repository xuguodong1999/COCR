...
namespace OpenBabel {
    ...
    */
    ...
    public:
        ...
        void SetHallName(const char *name) { m_Hall = name; }

        void SetId(unsigned n) { m_id = n; }

        void AddTransform(const std::string &s);

        const std::string &GetHMName() const { return m_HM; }

        const std::string &GetHallName() const { return m_Hall; }

        unsigned GetId() const { return m_id; }

        unsigned int GetOriginAlternative() const { return m_OriginAlternative; }
