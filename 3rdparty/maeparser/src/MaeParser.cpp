...
// make it behave like qi::parse(save, buffer.current, qi::double_, value)
// feature 1: don't allow white space before
// feature 2: move first iterator to last non-white-space position
template<typename Iterator>
inline bool parse_double(Iterator &first, Iterator last, double &value) {
    char *parseEnd;
    value = std::strtod(first, &parseEnd);
    first = parseEnd;
    // FIXME: check if really failed
 ...
 */
    template<>
    MAEPARSER_EXPORT int parse_value<int>(Buffer &buffer) {
        int value = 0;
        int sign = 1;
    ...
    template<>
    MAEPARSER_EXPORT double parse_value<double>(Buffer &buffer) {
        char *save = buffer.current;
        ...
        double value = 0;
        if (!parse_double(save, buffer.current, value) ||
            save != buffer.current) {
    ...
    template<>
    MAEPARSER_EXPORT std::string parse_value<std::string>(Buffer &buffer) {
        char *save = buffer.current;
        if (*buffer.current != '"') {
            while (buffer.current < buffer.end || buffer.load(save)) {
                switch (*buffer.current) {
    ...
    template<>
    MAEPARSER_EXPORT BoolProperty parse_value<BoolProperty>(Buffer &buffer) {
        bool value = false;
        if (*buffer.current == '1') {
            value = true;
                ...
                case 'r': {
                    ...
                    for (size_t ix = prop_indx; ix < value_count; ix += col_count) {
                        ...
                        if (data[0] == '<' && data[1] == '>') {
                            ...
                            if (!parse_double(data, end, value) ||
                                data != end) {
