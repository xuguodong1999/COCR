  ...
  bool PDBFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    if (!compndWritten) {
      if (strlen(mol.GetTitle()) > 0)
        ofs << fmt::format("COMPND    {:s} ", mol.GetTitle()) << endl;
      else
        ofs << "COMPND    UNNAMED" << endl;
    }

    if (!authorWritten) {
      ofs << fmt::format("AUTHOR    GENERATED BY OPEN BABEL {:s}", BABEL_VERSION) << endl;
    }
    ...
    if (pmol->HasData(OBGenericDataType::UnitCell))
      {
        ...
        if(pUC->GetSpaceGroup()){
          ...
          ofs << fmt::format("CRYST1{:9.3f}{:9.3f}{:9.3f}{:7.2f}{:7.2f}{:7.2f} {:<11s} 1",
    ...
    for (i = 1; i <= mol.NumAtoms(); i++)
      {
        atom = mol.GetAtom(i);
        std::string type_name = OBElements::GetSymbol(atom->GetAtomicNum());
        ...
        if (type_name.size() > 1)
          ...
          {
            type_name = fmt::format(" {:<3s}", type_name);
          }
        std::string the_res;
        ...
        ofs << fmt::format("{:s}{:5d} {:<4s} {:<3s} {:c}{:4d}{:c}   {:8.3f}{:8.3f}{:8.3f}{:6.2f}  0.00          {:2s}{:2s}\n",
                 het?"HETATM":"ATOM  ",
                 i,
                 type_name,
                 the_res,
        ...
        for (nbr = atom->BeginNbrAtom(k);nbr;nbr = atom->NextNbrAtom(k))
          {
            ...
            for(unsigned bo = 0; bo < bondorder; bo++) {
              if ((currentValence % 4) == 0) {
                ...
                ofs << fmt::format("CONECT{:5d}", i);
              }
              currentValence++;
              ofs << fmt::format("{:5d}", nbr->GetIdx());
            }
          }
    ...
    ofs << "MASTER        0    0    0    0    0    0    0    0 ";
    ofs << fmt::format("{:4d}    0 {:4d}    0\n", mol.NumAtoms(),mol.NumAtoms());
