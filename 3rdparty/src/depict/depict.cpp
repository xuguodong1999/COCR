...
namespace OpenBabel {
    ...
    private:
        void DrawAromaticRingBond(OBAtom *prevAtom, OBAtom *beginAtom, OBAtom *endAtom, OBAtom *nextAtom,
                                  const vector3 &center, double dist);
...
*/
    ...
    unsigned int GetAtomSymClass(OBAtom *atom) {
        ...
        if (pd) {
            ...
            vector<unsigned int>::iterator end_pos = unique(copy_sym.begin(),
                                                            copy_sym.end()); // Requires sorted elements
    ...
    inline void OBDepictPrivateBallAndStick::DrawAromaticRingBond(OBAtom *prevAtom, OBAtom *beginAtom, OBAtom *endAtom,
                                                                  OBAtom *nextAtom, const vector3 &center,
                                                                  double dist) {
        ...
        static const vector<double> pat = vector<double>(dashpattern,
                                                         dashpattern + sizeof(dashpattern) / sizeof(double));
        painter->DrawLine(begin.x() + offset.x(), begin.y() + offset.y(),
