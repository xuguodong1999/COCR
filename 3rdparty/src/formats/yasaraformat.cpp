...
/* DECODE A SIGNED INT32 STORED IN LITTLE ENDIAN FORMAT
   ==================================================== */
    ...
    int int32le(int32 value) { return (int32lemem((char *) &value)); }
...
/* DECODE AN UNSIGNED INT32 STORED IN LITTLE ENDIAN FORMAT
   ======================================================= */
    ...
    unsigned int uint32le(uint32 value) { return (uint32lemem((char *) &value)); }
...
/* GET SIZE OF ATOM
   ================ */
    int mob_atomlen(struct mobatom *atom) { return (4 + mob_links(atom) + atom->header[MOB_IDLEN]); }

    int mob_atomsize(struct mobatom *atom) { return (mob_atomlen(atom) * sizeof(mobdata)); }

/* MOVE POINTER TO NEXT ATOM OF YASARA OBJECT STRUCTURE
   ==================================================== */
    struct mobatom *mob_next(struct mobatom *atom) { return ((struct mobatom *) ((char *) atom + mob_atomsize(atom))); }

    void mob_setnext(struct mobatom **atomadd) { *atomadd = mob_next(*atomadd); }
...
/* READ YASARA OBJECT
   ...
   IF options CONTAINS 'f' (COMMAND LINE -af), ATOM NAMES WILL STAY FIXED (I.E. LEAVING THE SPACE IN THE FIRST COLUMN) */
    bool YOBFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        res = nullptr;
        charged = 0;
        for (i = 0; i < atoms; i++) { /* GET ELEMENT, TYPE AND POSITION */
            ...
            if (!mob_hasres(srcatom, &id)) { /* NEW RESIDUE FOUND */
            ...
            *((int32 *) atomname) = id.atom;
            ...
            if (atomname[0] == ' ' && (!pConv->IsOption("f", OBConversion::INOPTIONS)))
                memmove(atomname, atomname + 1, 4);
    ...
    bool YOBFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        for (i = 1; i <= atoms; i++) {
            ...
            if (srcatom->HasResidue()) {
                ...
                pos = 4;
                if (!pConv->IsOption("f", OBConversion::OUTOPTIONS)) { /* GUESS IF THE ATOM NAME NEEDS TO BE SHIFTED */
                    if (strlen(mob_elementsym[element]) == 1 || strncasecmp(mob_elementsym[element], atomname, 2))
                        pos = 5;
                }
