...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.povray.org/"; }; //optional
    ...
    void OutputCSTBonds(ostream &ofs, OBMol &mol, string prefix) {
        ...
        for (unsigned int i = 0; i < mol.NumBonds(); ++i) {
            ...
            bond_type.erase(
                    remove_if(bond_type.begin(), bond_type.end(), bind(equal_to<char>(), placeholders::_1, '.')),
                    bond_type.end());
        ...
        if (tmp) {
            ...
            if (model_type != "BAS" && model_type != "SPF" && model_type != "CST") {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Unknown model type specified. Using the default instead (\"BAS\", ball-and-stick).\n",
                                      obWarning);
                model_type = "BAS";
            }
        }
