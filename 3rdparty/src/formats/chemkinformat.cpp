    ...
    private:
        ...
        virtual bool ReadChemObject(OBConversion *pConv) {
            ...
            if (ret) //Do transformation and return molecule
                return pConv->AddChemObject(
                        pReact->DoTransformations(pConv->GetOptions(OBConversion::GENOPTIONS), pConv)) != 0;
            else
...
/////////////////////////////////////////////////
    void ChemKinFormat::Init() {    //initialize the member variables used during input
        ln.clear();
        AUnitsFactor = 1.0;
        EUnitsFactor = 1.0;
        SpeciesListed = false;
                ...
                */
                ...
                if (isalpha(firstr[0][0])) {
                    ...
                    if (pConv->IsOption("L", OBConversion::INOPTIONS)//this option mandates a label
                        ...
                        || (SpeciesListed &&
                            !IMols.count(*itr)))    // there is a species list and it is not a species name
                    {
...
//////////////////////////////////////////////////////////////////
    bool ChemKinFormat::WriteHeader(OBConversion *pConv) {
        ...
        for (itr = OMols.begin(); itr != OMols.end(); ++itr) {
            ...
            FOR_ATOMS_OF_MOL(atom, itr->get())elements.insert(OBElements::GetSymbol(atom->GetAtomicNum()));
        }
        if (!elements.empty()) {
            ofs << "ELEMENTS\n";
