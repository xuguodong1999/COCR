...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.wwpdb.org/docs.html"; };

        virtual const char *GetMIMEType() { return "chemical/x-pdb"; };
    ...
    bool PDBFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        FOR_ATOMS_OF_MOL(matom, mol)OBAtomAssignTypicalImplicitHydrogens(&*matom);
    ...
    bool PDBFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        for (i = 1; i <= mol.NumAtoms(); i++) {
            ...
            if (atom->HasData("_atom_site_occupancy")) {
                OBPairFloatingPoint *occup_fp = dynamic_cast<OBPairFloatingPoint *> (atom->GetData(
        ...
        if (res == nullptr
            ...
            for (res = mol.BeginResidue(ri); res; res = mol.NextResidue(ri))
                if (res->GetName() == resname
                    ...
                    if (insertioncode)
                        fprintf(stderr, "I: identified residue wrt insertion code: '%c'\n", insertioncode);
                    break;
                }
