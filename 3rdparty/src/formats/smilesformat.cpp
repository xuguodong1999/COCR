...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.daylight.com/smiles/"; };
  ...
  */
    ...
    char OBSmilesParser::SetRingClosureStereo(StereoRingBond rcstereo, OBBond *dbl_bond) {
        ...
        if (rcstereo.updown[0] == BondUpChar ||
            rcstereo.updown[0] == BondDownChar) { // Is there a stereo symbol at the opening?
            bool on_dbl_bond = (rcstereo.atoms[0] == dbl_bond->GetBeginAtom() ||
                                rcstereo.atoms[0] == dbl_bond->GetEndAtom());
            updown = (rcstereo.updown[0] == BondUpChar) ^ on_dbl_bond;
            found = true;
        }
        if (rcstereo.updown[1] == BondUpChar ||
            rcstereo.updown[1] == BondDownChar) { // Is there a stereo symbol at the closing?
            bool on_dbl_bond = (rcstereo.atoms[1] == dbl_bond->GetBeginAtom() ||
                                rcstereo.atoms[1] == dbl_bond->GetEndAtom());
            ...
            if (!found) {
                ...
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Ignoring the cis/trans stereochemistry specified for the ring closure\n  as it is inconsistent.",
                                      obWarning);
                found = false;
            }
        }
    ...
    void OBSmilesParser::CreateCisTrans(OBMol &mol) {
        ...
        FOR_BONDS_OF_MOL(dbi, mol) {
            ...
            for (int i = 0; i < 2; ++i) { // Loop over each end of the double bond in turn
                ...
                FOR_BONDS_OF_ATOM(bi, dbl_bond_atoms[i]) {
                    ...
                    if (stereo_bond[i] == nullptr) { // This is a first stereo bond
                        ...
                        if (stereo !=
                            bond_stereo[i]) { // Verify that the other stereo bond (on the same atom) has opposite stereo
                            other_bond[i] = b; // Use this for the 'other' bond
                        } else {
                            obErrorLog.ThrowError(__FUNCTION__,
            ...
            unsigned int second = (other_bond[0] == nullptr) ? OBStereo::ImplicitRef : other_bond[0]->GetNbrAtom(
                    a1)->GetId();
            unsigned int fourth = (other_bond[1] == nullptr) ? OBStereo::ImplicitRef : other_bond[1]->GetNbrAtom(
                    a2)->GetId();
            ...
            switch (insertpos) {
                case -1:
                    if (ChiralSearch->second->refs[0] != OBStereo::NoRef)
                        obErrorLog.ThrowError(__FUNCTION__, "Warning: Overwriting previous from reference id.",
                                              obWarning);
                ...
                case 3:
                    if (ChiralSearch->second->refs[insertpos] != OBStereo::NoRef)
                        obErrorLog.ThrowError(__FUNCTION__, "Warning: Overwriting previously set reference id.",
                                              obWarning);
                    (ChiralSearch->second)->refs[insertpos] = id;
                    break;
                default:
    ...
    bool OBSmilesParser::ParseComplex(OBMol &mol) {
            ...
            switch (*_ptr) {
                case '@':
                    ...
                    if (*_ptr == 'S' && _ptr[1] == 'P') { // @SP1/2/3
                        ...
                        if (_squarePlanarMap.find(atom) ==
                            _squarePlanarMap.end()) // Prevent memory leak for malformed smiles (PR#3428432)
                        ...
                        chiralWatch = true;
                        if (_tetrahedralMap.find(atom) ==
                            _tetrahedralMap.end()) // Prevent memory leak for malformed smiles (PR#3428432)
        ...
        if (charge) {
            ...
            if (abs(charge) > 10 ||
                (element && charge > element)) { // if the charge is +/- 10 or more than the number of electrons
    ...
    bool OBSmilesParser::ParseRingBond(OBMol &mol) {
        ...
        if (*_ptr == '%') {
            ...
            if (*_ptr == '(') { // %(NNN) extension to OpenSMILES
                ...
                while (isdigit(*_ptr)) {
                    ...
                    if (_ptr - start > 5) {
                        obErrorLog.ThrowError(__FUNCTION__,
                                              "Ring closure numbers with more than 5 digits are not supported",
                                              obWarning);
                        return false;
                    }
                }
                if (*_ptr != ')') {
                    obErrorLog.ThrowError(__FUNCTION__, "Matching close parenthesis not found for ring closure number",
                                          obWarning);
                    return false;
                }
        ...
        for (bond = _rclose.begin(); bond != _rclose.end(); ++bond) {
            if (bond->digit == digit) {
                ...
                if (bond->prev == _prev) {
                    obErrorLog.ThrowError(__FUNCTION__, "Invalid SMILES: Ring closures imply atom bonded to itself.",
                                          obWarning);
                    return false;
                }
                upDown = (_updown > bond->updown) ? _updown : bond->updown;
                ...
                mol.AddBond(bond->prev, _prev, bondOrder == 0 ? 1 : bondOrder, aromatic_bond ? OB_AROMATIC_BOND : 0,
                            bond->numConnections);
                // store up/down
                if (upDown == BondUpChar || upDown == BondDownChar)
                    _upDownMap[mol.GetBond(bond->prev, _prev)] = upDown;
   ...
   ----------------------------------------------------------------------*/
    ...
    struct OutOptions {
        ...
        OutOptions(bool _isomeric, bool _kekulesmi, bool _showatomclass, bool _showexplicitH, bool _smarts,
                ...
                isomeric(_isomeric), kekulesmi(_kekulesmi), showatomclass(_showatomclass),
                showexplicitH(_showexplicitH),
                smarts(_smarts),
                ordering(_ordering) {}
    };
   ...
   ***************************************************************************/
    ...
    char OBMol2Cansmi::GetCisTransBondSymbol(OBBond *bond, OBCanSmiNode *node) {
        ...
        if (_isup.find(bond) == _isup.end()) // No it hasn't
        {
            ...
            if (dbl_bond_first) {
                if (atom->IsAromatic())
                    FOR_BONDS_OF_ATOM (bond, atom)if (bond->IsAromatic() && bond->GetBondOrder() == 2)
                            return 0;
                endatom = nbr_atom->GetId();
                centeratom = atom->GetId();
            } else {
                if (nbr_atom->IsAromatic())
                    FOR_BONDS_OF_ATOM (bond, nbr_atom)if (bond->IsAromatic() && bond->GetBondOrder() == 2)
                            return 0;
                endatom = atom->GetId();
                centeratom = nbr_atom->GetId();
            }

            for (ChiralSearch = _unvisited_cistrans.begin();
                 ChiralSearch != _unvisited_cistrans.end(); ++ChiralSearch) {
                OBCisTransStereo::Config cfg = ChiralSearch->GetConfig(OBStereo::ShapeU);
                lookup = std::find(cfg.refs.begin(), cfg.refs.end(), endatom);
                if (lookup != cfg.refs.end() && (cfg.begin == centeratom || cfg.end ==
                                                                            centeratom)) { // Atoms endatom and centeratom are in this OBCisTransStereo
    ...
    bool OBMol2Cansmi::GetSmilesElement(OBCanSmiNode *node,
        ...
        if (atom->GetAtomicNum() != OBElements::Hydrogen && !options.showexplicitH) {
            FOR_NBORS_OF_ATOM(nbr, atom) {
                if (nbr->GetAtomicNum() == OBElements::Hydrogen && (!options.isomeric || nbr->GetIsotope() == 0) &&
            ...
            if (!bracketElement) {
                if (element == 0) { // asterisk is always hypervalent but we don't bracket it unless has Hs
                    ...
                    if (implicitValence == 0 // hypervalent
                        ...
                        || (!options.kekulesmi && element != 6 && atom->IsAromatic() &&
                            numImplicitHs != 0)) // aromatic nitrogen/phosphorus
                        bracketElement = true;
                }
            }
        }
        ...
        if (atom->GetFormalCharge() != 0 // charged elements
            ...
            || (options.showatomclass && atom->HasData(
                "Atom Class"))) // If the molecule has Atom Class data and -xa option set and atom has data
            bracketElement = true;
        ...
        if (!bracketElement) {
            ...
            if (element) {
                ...
                vector<pair<int, pair<OBAtom *, OBBond *> > > *externalBonds =
                        (vector<pair<int, pair<OBAtom *, OBBond *> > > *) ((OBMol *) atom->GetParent())->GetData(
                                "extBonds");
        ...
        if (hcount > 0 &&
            (atom == _endatom || (atom == _startatom && !options.ordering))) // Leave a free valence for attachment
            hcount--;
        if (hcount > 0) {
   ...
   ***************************************************************************/
    ...
    const char *OBMol2Cansmi::GetTetrahedralStereo(OBCanSmiNode *node,
        ...
        for (vector<OBAtom *>::const_iterator atom_it = chiral_neighbors.begin() + 1;
             atom_it != chiral_neighbors.end(); ++atom_it) {
            if (*atom_it)
   ...
   ***************************************************************************/
    ...
    const char *OBMol2Cansmi::GetSquarePlanarStereo(OBCanSmiNode *node,
        ...
        OBStereo::Refs canonRefs = OBStereo::MakeRefs(chiral_neighbors[0]->GetId(),
                                                      chiral_neighbors[1]->GetId(), chiral_neighbors[2]->GetId(),
   ...
   ***************************************************************************/
    ...
    OBMol2Cansmi::GetCanonClosureDigits(OBAtom *atom,
        ...
        for (bi = vbonds.begin(); bi != vbonds.end(); ++bi) {
            ...
            int bo = (bond1->IsAromatic()) ? 1
                                           : bond1->GetBondOrder();  // CJ: why was this line added?  bo is never used?
                ...
                if (!bci->is_open) { // Ring closure
                    ...
                    if (bs[0])
                        ...
                        switch (bci->bond->GetBondOrder()) {
                            case 1:
                                if (!bci->bond->IsAromatic() && bci->bond->IsInRing() &&
            ...
            switch (bond->GetBondOrder()) {
                case 1:
                    ...
                    if (cc[0] != '\0') {
                        ...
                        if (!bond->IsAromatic() && bond->IsInRing() && bond->GetBeginAtom()->IsAromatic() &&
    ...
    void StandardLabels(OBMol *pMol, OBBitVec *frag_atoms,
        ...
        FOR_ATOMS_OF_MOL(atom, *pMol) {
            if (frag_atoms->BitIsSet(atom->GetIdx())) {
                ...
                labels.push_back(
                        OBStereo::ImplicitRef); //to match situation when canonical ordering. Just a big number?
            ...
            for (vector<string>::iterator it = s_components.begin(); it != s_components.end(); ++it) {
                ...
                if (*(it->rbegin()) == 'm') {
                    ...
                    new_canonical_labels.insert(new_canonical_labels.end(),
                                                canonical_labels.begin() + total,
                                                canonical_labels.begin() + total + mult);
                    total += mult;
                } else {
                    tokenize(s_atoms, *it, ",");
                    vector<int> atoms;
            ...
            if (_pconv->IsOption("C")) {      // "C" == "anti-canonical form"
                ...
                for (int i = 0; i < canonical_order.size(); ++i)
                    if (canonical_order[i] == 0) { // Explicit hydrogens
                        if (mol.GetAtom(i + 1)->GetAtomicNum() == OBElements::Hydrogen &&
                            mol.GetAtom(i + 1)->GetIsotope() != 0) { // [2H] or [3H]
        ...
        while (1) {
            ...
            if (root_atom == nullptr) {
                ...
                if ((_pconv->IsOption("I") || universal_smiles)
                    ...
                    && root_atom->HasSingleBond() && (root_atom->GetAtomicNum() == OBElements::Oxygen ||
                                                      root_atom->GetAtomicNum() == OBElements::Sulfur)) {
                    ...
                    FOR_NBORS_OF_ATOM(nbr, central) {
                        ...
                        if (nbr->GetAtomicNum() == root_atom->GetAtomicNum() && nbr->GetExplicitDegree() == 1 &&
                            nbr->HasDoubleBond()) {
                            root_atom = &*nbr;
                            break;
                        }
