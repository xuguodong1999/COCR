...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.cec.mpg.de/forum/portal.php"; } //optional
...
//        if(strstr(buffer,"ORBITAL ENERGIES") != NULL) {
            ...
            if (checkKeywords.find("MULLIKEN ATOMIC CHARGES") != notFound) {
                ...
                while (vs.size() == 4) { // atom number, atomic symbol,:,  charge
        ...
        if (UVWavelength.size() > 0 || CDWavelength.size() > 0) {
            ...
            if (UVWavelength.size() > 0) {
                ...
                if (CDWavelength.size() == UVWavelength.size()) {
                    ...
                    etd->SetRotatoryStrengthsVelocity(
                            CDVelosity); // just vector with 0.0 because ORCA doesn't calculate these values
                }
            } else {
                // only CD spectrum has been found
                etd->SetData(CDWavelength,
                             CDVelosity); // ony wavelengths information are known , 2nd vector just contains 0.0
                etd->SetRotatoryStrengthsLength(CDStrengthsLength);
                etd->SetRotatoryStrengthsVelocity(
                        CDVelosity); // just vector with 0.0 because ORCA doesn't calculate these values
            }
