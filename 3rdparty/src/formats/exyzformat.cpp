...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://openbabel.org/wiki/EXYZ"; }; //optional

        virtual const char *GetMIMEType() { return "chemical/x-exyz"; };
    ...
    bool EXYZFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (sscanf(buffer, "%d", &natoms) == 0 || !natoms) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problems reading an E-XYZ file: The first line must contain the number of atoms.",
                                  obWarning);
            return (false);
        }
        ...
        if (!ifs.getline(buffer, BUFF_SIZE)) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problems reading an EXYZ file: Could not read the second line (title/comments/keywords).",
                                  obWarning);
            return (false);
        }
        ...
        location = readKeywords.find(
                "%VIRTUAL");   // file contains information about virtual atoms in the column next to the x,y,z values
