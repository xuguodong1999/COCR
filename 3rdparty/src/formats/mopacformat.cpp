...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *GetMIMEType() { return "chemical/x-mopac-out"; };
    ...
    bool MOPACFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        while (ifs.getline(buffer, BUFF_SIZE)) {
            ...
            else if (strstr(buffer, "(ANGSTROMS)") != nullptr && strstr(buffer, "(DEGREES)") ==
                                                                 nullptr) { // newer versions don't print CARTESIAN for final geometry
                mol.Clear();
                ifs.getline(buffer, BUFF_SIZE);    // blank
                ifs.getline(buffer, BUFF_SIZE);
                tokenize(vs, buffer);
            ...
            } else if (strstr(buffer, "NC:NB:NA:I") != nullptr) // z-matrix
            {
                ...
                if (!ifs.getline(buffer, BUFF_SIZE))
                    break;
            } else if (strstr(buffer, "MASS-WEIGHTED COORDINATE ANALYSIS") !=
                       nullptr) { // the correct vibrations -- earlier bits aren't mass-weighted
    ...
    public:
        ...
        virtual const char *GetMIMEType() { return "chemical/x-mopac-input"; };
    ...
    bool MOPACCARTFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        while (ifs.getline(buffer, BUFF_SIZE)) {
            ...
            if (vs.size() > 3) //Only one label allowed per line
            {
                ...
                cerr
                        << "Invalid format in geometry specification: There appears to be more than one atom label specified!\n";
                return false;
