...
int CompINChILayers(const INCHI_SORT *p1, const INCHI_SORT *p2,
    ...
    if (IsoStereo2 && IsoStereo2->nNumberOfStereoBonds) {
        ...
        */
        if (Stereo2 && Stereo2->nNumberOfStereoBonds) {
            *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
        } else if (IsoStereo1 && IsoStereo1->nNumberOfStereoBonds &&
    ...
    } else if (SP3_ANY & bRelRac[DIFL_M]) {
        *psDifSegs |= i2 ? DIFV_IS_EMPTY : DIFV_EQL2PRECED;
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    if ((SP3_ANY & bRelRac[DIFL_F])) {
        *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
    } else if ((SP3_ANY & bRelRac[DIFL_MI]) && !(SP3_ANY & bRelRac[DIFL_M])) {
        *psDifSegs |= i2 ? DIFV_IS_EMPTY : DIFV_EQL2PRECED;
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    } else if (bRelRac[DIFL_M] & SP3_ABS) {
        *psDifSegs |= i2 ? DIFV_IS_EMPTY : DIFV_EQL2PRECED;
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    } else if (SP3_ABS & bRelRac[DIFL_M]) {
        *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    if ((SP3_ABS & bRelRac[DIFL_F])) {
        *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
    } else if ((SP3_ABS & bRelRac[DIFL_MI]) && !(SP3_ABS & bRelRac[DIFL_M])) {
        *psDifSegs |= i2 ? DIFV_IS_EMPTY : DIFV_EQL2PRECED;
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    } else if (bRelRac[DIFL_M] & SP3_TYPE) {
        *psDifSegs |= i2 ? DIFV_IS_EMPTY : DIFV_EQL2PRECED;
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    } else if (SP3_TYPE & bRelRac[DIFL_M]) {
        *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    if ((SP3_TYPE & bRelRac[DIFL_F])) {
        *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
    } else if ((SP3_TYPE & bRelRac[DIFL_MI]) && !(SP3_TYPE & bRelRac[DIFL_M])) {
        *psDifSegs |= i2 ? DIFV_IS_EMPTY : DIFV_EQL2PRECED;
    } else {
        *psDifSegs |= DIFV_BOTH_EMPTY;
    }
    ...
    if (i2 && !i2->bDeleted) {
            ...
            if (!i1 || i1->bDeleted ||
                ...
                *psDifSegs |= DIFV_NEQ2PRECED;
                ...
                if (!diff) {
                    num = i1->nNumberOfIsotopicTGroups;
                    for (i = 0; i < num; i++) {
                        if ((diff = (int) i2->IsotopicTGroup[i].nTGroupNumber -
                                    (int) i1->IsotopicTGroup[i].nTGroupNumber))
                            break;
        ...
        } else if (i1 && !i1->bDeleted && (i1->nNumberOfIsotopicAtoms || i1->nNumberOfIsotopicTGroups)) {
            *psDifSegs |= DIFV_IS_EMPTY;
        }
    } else if (!i2) {
        if (i1 && !i1->bDeleted && (i1->nNumberOfIsotopicAtoms || i1->nNumberOfIsotopicTGroups)) {
...
#define nFirstIsoSegm   DIFS_i_IATOMS
    ...
    } else if (sDifSegs[nLayer][nFirstSegm] == DIFV_BOTH_EMPTY ||
...
int CompareInchiStereo(INChI_Stereo *Stereo1, INCHI_MODE nFlags1, INChI_Stereo *Stereo2, INCHI_MODE nFlags2) {
    ...
    if (Stereo2 && Stereo1) {
    ...
    } else if (Stereo2 && (Stereo2->nNumberOfStereoBonds > 0 ||
        ...
        return 1;
    } else if (Stereo1 && (Stereo1->nNumberOfStereoBonds > 0 ||
    ...
    */
    ...
    do {
            ...
            /* replaced due to missing sequence point; undefined behavior, pointed by Geoffrey Hutchison */
        {
            ...
            } else if (nTopStackAtom && u != (int) nStackAtom[nTopStackAtom - 1] &&
...
/*************************************************************************************/
int CompareReversedStereoINChI2(INChI_Stereo *s1/* InChI from reversed struct */, INChI_Stereo *s2 /* input InChI */,
                                ICR *picr) {
    int ret = 0;
    ...
    if ((nNumSc1 || nNumSc2) &&
        ...
        for (j1 = j2 = 0; j1 < nNumSc1 && j2 < nNumSc2;) {
            if (s1->nNumber[j1] == s2->nNumber[j2]) {
                ...
                j1++;
                j2++;
            } else if (s1->nNumber[j1] < s2->nNumber[j2]) {
                num_in1_only++;
    ...
    if ((nNumSb1 || nNumSb2) &&
        ...
        for (j1 = j2 = 0; j1 < nNumSb1 && j2 < nNumSb2;) {
            if (s1->nBondAtom1[j1] == s2->nBondAtom1[j2] &&
                ...
                j1++;
                j2++;
            } else if (s1->nBondAtom1[j1] < s2->nBondAtom1[j2] ||
...
/*************************************************************************************/
int CompareReversedINChI(INChI *i1 /* InChI from reversed struct */, INChI *i2 /* input InChI */, INChI_Aux *a1,
                         INChI_Aux *a2) {
    int ret;
    if (i1 == NULL && i2 == NULL)
        return 0;
            ...
            if (bHasFixedH1 && !bHasFixedH2) {
                for (i = 0; i < i1->nNumberOfAtoms; i++) {
                    if (i1->nNum_H_fixed[i] > 0) {
                        j1++;
                    } else if (i1->nNum_H_fixed[i] < 0) {
                        j2++;
                    }
                }

                return 18; /* Diff: Extra Fixed-H */
            } else if (!bHasFixedH1 && bHasFixedH2) {
                for (i = j1 = j2 = 0; i < i1->nNumberOfAtoms; i++) {
                    if (0 > i2->nNum_H_fixed[i]) {
                        j1++;
                    } else if (0 < i2->nNum_H_fixed[i]) {
                        j2++;
                    }
                }
                ...
                for (i = j1 = j2 = 0; i < i1->nNumberOfAtoms; i++) {
                    if (i1->nNum_H_fixed[i] > i2->nNum_H_fixed[i]) {
                        j1++;
                    } else if (i1->nNum_H_fixed[i] < i2->nNum_H_fixed[i]) {
                        j2++;
                    }
                }
            }
    ...
    if (i1->nNumberOfIsotopicAtoms > 0 &&
        memcmp(i1->IsotopicAtom, i2->IsotopicAtom, i1->nNumberOfIsotopicAtoms * sizeof(i1->IsotopicAtom[0])))
...
*/
    ...
    if (!i2->StereoIsotopic && i2->Stereo && i1->StereoIsotopic &&
        ...
        ;
    } else if ((ret = CompareReversedStereoINChI(i1->StereoIsotopic, i2->StereoIsotopic))) {
        return ret + 40;
    }
...
int CompareIcr(ICR *picr1, ICR *picr2, INCHI_MODE *pin1, INCHI_MODE *pin2, INCHI_MODE mask) {
    ...
    if (nNumExtraBits1 && !nNumExtraBits2) {
        ret = 1;
    } else if (!nNumExtraBits1 && nNumExtraBits2) {
        ret = -1;
    } else if (!in1 && !in2) {
        ret = 0;
    } else {
...
INCHI_MODE CompareReversedINChI2(INChI *i1 /* InChI from reversed struct */, INChI *i2 /* input InChI */,
    ...
    *err = 0;
    ...
    if (i1->lenConnTable != i2->lenConnTable) {
        ret |= IDIF_CON_LEN;
        goto exit_function;
    } else if (i1->lenConnTable > 0 &&
               memcmp(i1->nConnTable, i2->nConnTable, i1->lenConnTable * sizeof(i1->nConnTable[0]))) {
        ret |= IDIF_CON_TBL;
        goto exit_function;
    }
    /* output special cases: different number of t-groups, different sizes of t-groups, different endpoints */
        ...
        */
        ...
        for (j1 = j2 = 0; j1 < num1 && j2 < num2;) {
            if (pe1[j1] == pe2[j2]) {
                ...
                j2++;
                num_eq++;
            } else if (pe1[j1] < pe2[j2]) { /* BC: fixed, was pe2[j1] 2006-03-27 */
        ...
        if (!num_in1_only && !num_in2_only && num_eq) { ; /* same t-groups endpoints */
        } else {
...
/*^^^ */
    ...
    if (out_norm_data[TAUT_YES]->at && at[TAUT_YES]) {
        ...
        ret = mark_alt_bonds_and_taut_groups(out_norm_data[TAUT_YES]->at, out_norm_data[TAUT_YES]->at_fixed_bonds,
                                             num_atoms,
    ...
    if (out_norm_data[TAUT_NON]->at && out_norm_data[TAUT_YES]->at && at[TAUT_NON] &&
        ...
        ret = mark_alt_bonds_and_taut_groups(out_norm_data[TAUT_NON]->at, NULL, num_atoms, NULL, &bTautFlags,
                                             &bTautFlagsDone);
        if (ret < 0) {
            goto exit_function;  /*  out of RAM or other normalization problem */
        }
...
/*^^^ */
    ...
    if ((nUserMode & REQ_MODE_BASIC) &&
        (!out_norm_data[TAUT_NON]->at || !ppINChI[TAUT_NON] || !ppINChI_Aux[TAUT_NON] || !at[TAUT_NON])) {
        nUserMode ^= REQ_MODE_BASIC;
    }
    if ((nUserMode & REQ_MODE_TAUT) &&
        (!out_norm_data[TAUT_YES]->at || !ppINChI[TAUT_YES] || !ppINChI_Aux[TAUT_YES] || !at[TAUT_YES])) {
    ...
    for (i = n2; i >= n1 && !RETURNED_ERROR(ret); i--) {
        ...
        switch (i) {
            case TAUT_NON: /*  non-tautomeric */
                ...
                nMode |= (bHasIsotopicAtoms && s[TAUT_NON].bMayHaveStereo && (nUserMode & REQ_MODE_ISO_STEREO))
                         ? CANON_MODE_ISO_STEREO : 0;
            ...
            case TAUT_YES: /*  tautomeric */
                ...
                nMode |= (bHasIsotopicAtoms && s[TAUT_YES].bMayHaveStereo && (nUserMode & REQ_MODE_ISO_STEREO))
                         ? CANON_MODE_ISO_STEREO : 0;
                nMode |= (nUserMode & REQ_MODE_NOEQ_STEREO) ? CMODE_NOEQ_STEREO : 0;
                nMode |= (nUserMode & REQ_MODE_REDNDNT_STEREO) ? CMODE_REDNDNT_STEREO : 0;
                nMode |= (nUserMode & REQ_MODE_NO_ALT_SBONDS) ? CMODE_NO_ALT_SBONDS : 0;
        ...
        if (pBCN) {
            ...
            /* old way */
            pCS->NeighList = CreateNeighList(num_atoms, i ? num_at_tg : num_atoms, at[i], pCS->bDoubleBondSquare,
                                             pCS->t_group_info);
            pCS->pBCN = NULL;
            ret = Canon_INChI(num_atoms, i ? num_at_tg : num_atoms, at[i], pCS, nMode, i);
        }
