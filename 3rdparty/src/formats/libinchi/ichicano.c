...
long InchiTimeMsecDiff(inchiTime *TickEnd, inchiTime *TickStart) {
    if (FullMaxClock > 0) {
        ...
        if ((TickEnd->clockTime >= 0 && TickStart->clockTime >= 0) ||
            ...
            delta = TickEnd->clockTime - TickStart->clockTime;
        } else if (TickEnd->clockTime >= HalfMaxPositiveClock &&
            ...
            delta = -delta;
        } else if (TickEnd->clockTime <= HalfMinNegativeClock &&
...
int bInchiTimeIsOver(inchiTime *TickStart) {
    if (FullMaxClock > 0) {
        ...
        } else if (clockCurrTime >= HalfMaxPositiveClock &&
                   TickStart->clockTime <= HalfMinNegativeClock) {
            /* curr is earlier than start */
            return 0;
        } else if (clockCurrTime <= HalfMinNegativeClock &&
                   TickStart->clockTime >= HalfMaxPositiveClock) {
...
int GetCanonLengths(int num_at, sp_ATOM *at, ATOM_SIZES *s,
                    T_GROUP_INFO *t_group_info) { /* include taut. groups as additional "atoms" to the connection table 07-22-2002 */
...
int FillIsotopicAtLinearCT(int num_atoms, sp_ATOM *at,
    ...
    for (rank = 1; rank <= num_atoms; rank++) {
        ...
        *****************************************************/
        ...
        k = (!at[i].endpoint && !(at[i].cFlags & AT_FLAG_ISO_H_POINT) &&
             (at[i].num_iso_H[0] || at[i].num_iso_H[1] || at[i].num_iso_H[2]));
...
int FillTautLinearCT2(int num_atoms, int num_at_tg, int bIsoTaut,
    ...
    const static int max_num_iso =
            sizeof(LinearCTIsotopicTautomer->num) / sizeof(LinearCTIsotopicTautomer->num[0]) + T_NUM_NO_ISOTOPIC;
...
*****************************************************************/
    ...
    int bTaut = (num_at_tg > num_atoms) && pCS->t_group_info && pCS->t_group_info->num_t_groups &&
                pCS->t_group_info->t_group;
    ...
    int bIsoXchgH = pCS->t_group_info && pCS->t_group_info->nNumIsotopicEndpoints > 1 &&
                    ...
                    pCS->t_group_info->nIsotopicEndpointAtomNumber[0] &&
                    (pCS->t_group_info->bTautFlagsDone &
    ...
    /* isotopic canonicalization */
    int bCanonIsotopic = bHasIsotopicCanonData && (nMode & CMODE_ISO) &&
                         (pCS->LinearCTIsotopic || pCS->LinearCTIsotopicTautomer || bIsoXchgH);
    ...
    int bCanonIsoStereo = bHasIsotopicCanonData && (nMode & CMODE_ISO_STEREO) &&
                          (pCS->LinearCTIsotopicStereoDble || pCS->LinearCTIsotopicStereoCarb) && bCanonIsotopic;
    ...
    *pCS2 = *pCS;  /* save input information and pointers to allocated memory */
    ...
    if (bCanonIsotopic) {
        ...
        if (pCS->nCanonOrdIsotopic) {
            memcpy(pCS->nCanonOrdIsotopic, ftcn->PartitionCtIso.AtNumber,
                   num_at_tg * sizeof(pCS->nCanonOrdIsotopic[0]));
    ...
    if (bTaut) {
        ...
        nRet = FillTautLinearCT2(num_atoms, num_at_tg, bIsoTaut,
                                 ...
                                 pCS->LinearCTIsotopicTautomer, pCS->nMaxLenLinearCTIsotopicTautomer,
        ...
        if (nRet <= 0) {
            ...
            if (bCanonIsotopic /*&& pCS->nLenLinearCTIsotopicTautomer*/ ) {
                ...
                if (pCS->nSymmRankIsotopicTaut) {
                    memcpy(pCS->nSymmRankIsotopicTaut, tiSymmRank,
                           num_t_groups * sizeof(pCS->nSymmRankIsotopicTaut[0]));
                }
    ...
    *pCS2 = *pCS;  /* save input information and pointers to allocated memory */
        ...
        */
        ...
        if (RETURNED_ERROR(nRet)) {
            ...
            if (bTaut && pCS->nCanonOrdStereoTaut) {
                if (0 <
                    (nRet = SortTautomerGroupsAndEndpoints(t_group_info1, num_atoms, num_at_tg, nCanonRankStereo))) {
        ...
        if (pCS->nLenLinearCTStereoDble > 0 || pCS->nLenLinearCTStereoCarb > 0) {
            ...
            memcpy(pCS->LinearCTStereoDble, pCS2->LinearCTStereoDble,
                   pCS->nLenLinearCTStereoDble * sizeof(pCS->LinearCTStereoDble[0]));
            memcpy(pCS->LinearCTStereoCarb, pCS2->LinearCTStereoCarb,
                   pCS->nLenLinearCTStereoCarb * sizeof(pCS->LinearCTStereoCarb[0]));
        }
    ...
    if (bCanonIsoStereo && bCanonIsotopic) {
        ...
        ***************************************************************************************/
        ...
        if (RETURNED_ERROR(nRet)) {
            ...
            if (!nRet) {
                ...
                if (pCS->nCanonOrdIsotopicStereoTaut) {
                    if (0 < (nRet = SortTautomerGroupsAndEndpoints(t_group_info1, num_atoms, num_at_tg,
                                                                   nCanonRankIsotopicStereo))) {
                        /*non-isotopic contains symmetry ranks */
                        int num_t_groups = t_group_info1->num_t_groups;
                        AT_NUMB *tGroupNumber = t_group_info1->tGroupNumber;
                        /*AT_NUMB *tiSymmRank        = tGroupNumber + TGSO_SYMM_IRANK*num_t_groups; */
                        memcpy(pCS->nCanonOrdIsotopicStereoTaut, tGroupNumber,
                               num_t_groups * sizeof(pCS->nCanonOrdIsotopicStereoTaut[0]));
        ...
        if (pCS->nLenLinearCTStereoDble > 0 || pCS->nLenLinearCTStereoCarb > 0) {
            /* copy previous results, the canonical stereo CT */
            memcpy(pCS->LinearCTStereoDble, pCS2->LinearCTIsotopicStereoDble,
                   pCS->nLenLinearCTStereoDble * sizeof(pCS->LinearCTStereoDble[0]));
            memcpy(pCS->LinearCTStereoCarb, pCS2->LinearCTIsotopicStereoCarb,
                   pCS->nLenLinearCTStereoCarb * sizeof(pCS->LinearCTStereoCarb[0]));
        }
        ...
        if (pCS->nCanonOrdIsotopicStereoInv && pCS->nCanonOrdIsotopicStereo) {
            /* in case there is nothing to invert */
            memcpy(pCS->nCanonOrdIsotopicStereoInv, pCS->nCanonOrdIsotopicStereo,
                   num_at_tg * sizeof(pCS->nCanonOrdIsotopicStereoInv[0]));
        }
    ...
    if (pCS->LinearCT2) {
        ...
        inchi_swap((char *) &pCS->nLenLinearCTAtOnly, (char *) &pCS->nLenLinearCTAtOnly2,
                   sizeof(pCS->nLenLinearCTAtOnly));
    }
