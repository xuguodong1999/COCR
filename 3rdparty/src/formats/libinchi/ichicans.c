...
/**********************************************************************************/
int
SetOneStereoBondIllDefParity(sp_ATOM *at, int jc, /* atom number*/ int k /* stereo bond ord. number*/, int new_parity) {
                                ...
                                if (k2 - k1 > 1) {
                                    ...
                                    for (kn = k1, num_neighbors_with_parity = 0; kn < k2; kn++) {
                                        ...
                                        num_neighbors_with_parity +=
                                                find_atoms_with_parity(at, visited, jc,
                                                                       (int) at[jc].neighbor[nNeighborNumber[kn]]);
                                    }
                                }
                    ...
                    if (num_no_parity_atoms == i2 - i1) {
                        ...
                        for (ic = i1; ic < i2; ic++) {
                            ...
                            for (k = 0; k < MAX_NUM_STEREO_BOND_NEIGH && (jn = at[jc].stereo_bond_neighbor[k]); k++) {
                                ...
                                /*  opposite end */
                                for (k1 = 0;
                                     k1 < MAX_NUM_STEREO_BOND_NEIGH && (kn = at[jn].stereo_bond_neighbor[k1]); k1++) {
                                    if (kn - 1 == jc) {
 ...
 **********************************************************************************/
int
FillSingleStereoDescriptors(sp_ATOM *at, int i, int num_trans, const AT_RANK *nRank, AT_STEREO_CARB *LinearCTStereoCarb,
                            int *nStereoCarbLen, int nMaxStereoCarbLen, AT_STEREO_DBLE *LinearCTStereoDble,
                            int *nStereoDbleLen, int nMaxStereoDbleLen, int bAllene) {
    ...
    if (at[i].parity || at[i].stereo_bond_neighbor[0]) {
        ...
        if (ATOM_PARITY_WELL_DEF(at[i].parity) && num_trans < 0) {
            ...
            pn_RankForSort = nRank;
            num_trans = insertions_sort(nNeighborNumber2, num_neigh, sizeof(nNeighborNumber2[0]),
...
void SwitchAtomStereoAndIsotopicStereo(sp_ATOM *at, int num_atoms, int *bSwitched) {
    ...
    for (i = 0; i < num_atoms; i++) {
        ...
        inchi_swap((char *) &at[i].stereo_atom_parity, (char *) &at[i].stereo_atom_parity2,
                   sizeof(at[i].stereo_atom_parity));
        inchi_swap((char *) &at[i].bHasStereoOrEquToStereo, (char *) &at[i].bHasStereoOrEquToStereo2,
                   sizeof(at[i].bHasStereoOrEquToStereo));

        inchi_swap((char *) at[i].stereo_bond_neighbor, (char *) at[i].stereo_bond_neighbor2,
                   sizeof(at[i].stereo_bond_neighbor));
        inchi_swap((char *) at[i].stereo_bond_ord, (char *) at[i].stereo_bond_ord2, sizeof(at[i].stereo_bond_ord));
        inchi_swap((char *) at[i].stereo_bond_z_prod, (char *) at[i].stereo_bond_z_prod2,
                   sizeof(at[i].stereo_bond_z_prod));
        inchi_swap((char *) at[i].stereo_bond_parity, (char *) at[i].stereo_bond_parity2,
                   sizeof(at[i].stereo_bond_parity));
    }
    *bSwitched = !*bSwitched;
}
...
/**********************************************************************************/
int FillAllStereoDescriptors(sp_ATOM *at, int num_atoms, const AT_RANK *nCanonRank, const AT_RANK *nAtomNumberCanon,
                             CANON_STAT *pCS) {
    int ret = 0, i;
    ...
    for (i = 0; !ret && i < num_atoms; i++) {
        ret = FillSingleStereoDescriptors(at, (int) nAtomNumberCanon[i], -1, nCanonRank, pCS->LinearCTStereoCarb,
                                          &pCS->nLenLinearCTStereoCarb, pCS->nMaxLenLinearCTStereoCarb,
                                          pCS->LinearCTStereoDble, &pCS->nLenLinearCTStereoDble,
                                          pCS->nMaxLenLinearCTStereoDble, 0 /* bAllene */ );
    }
    for (i = 0; !ret && i < num_atoms; i++) {
        ret = FillSingleStereoDescriptors(at, (int) nAtomNumberCanon[i], -1, nCanonRank, pCS->LinearCTStereoCarb,
                                          &pCS->nLenLinearCTStereoCarb, pCS->nMaxLenLinearCTStereoCarb,
                                          pCS->LinearCTStereoDble, &pCS->nLenLinearCTStereoDble,
                                          pCS->nMaxLenLinearCTStereoDble, 1 /* bAllene */);
    }
...
int SetKnownStereoBondParities(sp_ATOM *at, int num_atoms, const AT_RANK *nCanonRank,
    ...
    for (i1 = 0, num_set = 0; i1 < num_atoms; i1++) {
        ...
        for (n1 = 0; n1 < MAX_NUM_STEREO_BONDS && (i2 = (int) at[i1].stereo_bond_neighbor[n1]); n1++) {
            ...
            for (n2 = 0, m = 0; n2 < MAX_NUM_STEREO_BONDS && (m = (int) at[i2].stereo_bond_neighbor[n2]) &&
                                m - 1 != i1; n2++); /* locate stereo bond (#n2) at the opposite atom at[i2] */
            ...
            for (j1 = 0; j1 <= iMax1 && nAtomRank1 == nRank[k1 = (int) nAtomNumber[iMax1 - j1]]; j1++) {
                ...
                for (m1 = 0; m1 < num_neigh1; m1++) {
                    ...
                    if (prev_trans < 0) {
                        prev_trans = trans_k;
                    } else if (prev_trans != trans_k) { /* was != trans_k1, changed 9-23-2003 */
    ...
    for (i1 = 0, num_set = 0; i1 < num_atoms; i1++) {
        ...
        for (n1 = 0, i2 = 0; n1 < MAX_NUM_STEREO_BONDS && (i2 = (int) at[i1].stereo_bond_neighbor[n1]); n1++) {
            ...
            */
            ...
            /*num_sb2 = n2;*/
            for (n2 = 0, m = 0;
                 n2 < MAX_NUM_STEREO_BONDS && (m = (int) at[i2].stereo_bond_neighbor[n2]) && m - 1 != i1; n2++);
            if (m - 1 != i1 || at[i1].stereo_bond_parity[n1] != at[i2].stereo_bond_parity[n2]) {
            ...
            for (j1 = 0; j1 <= iMax1 && nAtomRank1 == nRank[k1 = (int) nAtomNumber[iMax1 - j1]]; j1++) {
                ...
                */
                for (m1 = 0; m1 < num_neigh1; m1++) {
                    ...
                    for (s1 = 0, m = 0;
                         s1 < MAX_NUM_STEREO_BONDS && (m = (int) at[k1].stereo_bond_neighbor[s1]) && m - 1 != k2; s1++);
                    if (m - 1 != k2) {
                    ...
                    */
                    for (s2 = 0, m = 0;
                         s2 < MAX_NUM_STEREO_BONDS && (m = (int) at[k2].stereo_bond_neighbor[s2]) && m - 1 != k1; s2++);
                    if (m - 1 != k1) {
                        /*
                        bDifferentParities = 1; // cannot find the stereo bond
                        continue;
            ...
            if (0 == bDifferentParities && PARITY_KNOWN(stereo_bond_parity)) {
                for (j1 = 0; j1 <= iMax1 && nAtomRank1 == nRank[k1 = (int) nAtomNumber[iMax1 - j1]]; j1++) {
                    /*  at[k1] is constitutionally equivalent to at[i1] */
                    for (s1 = 0, k2 = 0;
                         s1 < MAX_NUM_STEREO_BONDS && (k2 = (int) at[k1].stereo_bond_neighbor[s1]); s1++) {
                        k2--;
...
AT_RANK PathsHaveIdenticalKnownParities(sp_ATOM *at, AT_RANK prev1, AT_RANK cur1, AT_RANK prev2, AT_RANK cur2,
    ...
    for (k = 1, n1 = MAX_ATOMS + 1, n2 = MAX_ATOMS + 1; k < at[cur1].valence; k++) {
        ...
        if (!nVisited1[n1]) { /*  recursion */
            if (!(nLength = PathsHaveIdenticalKnownParities(at, cur1, n1, cur2, n2, nVisited1, nVisited2, nRank,
                                                            nCanonRank, nLength))) {
                return 0;
            }
        }
...
int RemoveKnownNonStereoBondParities(sp_ATOM *at, int num_atoms, const AT_RANK *nCanonRank,
    ...
    for (i1 = 0; i1 < num_atoms; i1++) {
        ...
        for (n1 = 0; n1 < MAX_NUM_STEREO_BONDS && (s2 = at[i1].stereo_bond_neighbor[n1]); n1++) {
            ...
            nVisited[i1] = 1;
            if (PathsHaveIdenticalKnownParities(at, (AT_RANK) i1, neigh[0], (AT_RANK) i1, neigh[1], nVisited, nVisited,
                                                nRank, nCanonRank, 1)) {
...
int SetKnownStereoCenterParities(sp_ATOM *at, int num_atoms, const AT_RANK *nCanonRank,
    ...
    for (i = 0, num_set = 0; i < num_atoms; i++) {
        ...
        for (iMax = (int) nAtomRank - 1, j1 = 0;
             j1 <= iMax && nAtomRank == nRank[k = (int) nAtomNumber[iMax - j1]]; j1++) {
