...
int ExtractOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
                        inchi_Input *pInp, INCHI_IOSTREAM *log_file, INCHI_IOSTREAM *output_file,
                    ...
                    for (itaut = 0; itaut < 2; itaut++) {
                        if (NULL != inp_norm_data[itaut]) {
                            if (inp_norm_data[itaut]->bTautomeric) {
                                if (inp_norm_data[itaut]->bNormalizationFlags &
                                    (FLAG_NORM_CONSIDER_TAUT & ~FLAG_PROTON_CHARGE_CANCEL))
                                    if (warn_prot == 0) {
                                        warn_prot++;
...
/*****************************/
    ...
    if (output_file->s.pStr && output_file->s.nUsedLength > 0 && pGenData) {
        ...
        if (!(INCHI_OUT_SDFILE_ONLY &
            ...
            for (p = strchr(pResults->szInChI, '\n'); p; p = strchr(p + 1, '\n')) {
                ...
                } else if (pResults->szAuxInfo || !p[1]) {
    ...
    if (output_file->s.pStr) {
        inchi_free(output_file->s.pStr);
        output_file->s.pStr = NULL;
    }
    if (log_file->s.pStr) {
