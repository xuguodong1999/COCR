...
int bSetBondsAfterCheckOneBond(BN_STRUCT *pBNS, BNS_FLOW_CHANGES *fcd, int nTestFlow, inp_ATOM *at, int num_atoms,
                               int bChangeFlow);

int BnsTestAndMarkAltBonds(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at, int num_atoms, BNS_FLOW_CHANGES *fcd,
                           int bChangeFlow, int nBondTypeToTest);
...
int bAddStCapToAVertex(BN_STRUCT *pBNS, Vertex v1, Vertex v2, VertexFlow *nOldCapVertSingleBond, int *nDots,
                       int bAdjacentDonors);
...
int SetAtomBondType(BNS_EDGE *edge, U_CHAR *bond_type12, U_CHAR *bond_type21, int delta, int bChangeFlow) {
    ...
    if ((bChangeFlow & BNS_EF_CHNG_BONDS) && (bChangeFlow & BNS_EF_ALTR_NS) != BNS_EF_ALTR_NS) {
    ...
    } else if (bChangeFlow & BNS_EF_ALTR_BONDS) {
        if (flow1 == flow2) {
            goto exit_function;
        }
        ...
        switch (bond_type = (*bond_type12 & BOND_TYPE_MASK)) {
            ...
            case BOND_TRIPLE:
                ...
                } else if (flow1 == 0 && flow2 == 2) {
                    bond_mark = BOND_MARK_ALT13;
                    bond_type = BOND_ALT_13;
                } else if (flow1 == 1 && flow2 == 2) {
            ...
            if (v1 < num_atoms && (v0 >= num_atoms || v2 >= num_atoms)) {
                ...
                if (v0 >= num_atoms) {
                    ...
                    if (pBNS->vert[v0].type & BNS_VERT_TYPE_TGROUP) {
                        nDeltaH -= delta;
                    } else if (pBNS->vert[v0].type & BNS_VERT_TYPE_C_GROUP) {
                        nDeltaCharge += delta;
                    }
                }
                if (v2 >= num_atoms) {
                    if (pBNS->vert[v2].type & BNS_VERT_TYPE_TGROUP) {
                        nDeltaH += delta;
                    } else if (pBNS->vert[v2].type & BNS_VERT_TYPE_C_GROUP) {
                        nDeltaCharge -= delta;
                    }
                }
...
int SetBondsFromBnStructFlow(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms, int bChangeFlow0) {
    ...
    */
    for (pass = pBNS->num_altp - 1, ret = 0; 0 <= pass; pass--) {
        ...
        } else if ((bChangeFlow & BNS_EF_SAVE_ALL) == BNS_EF_SAVE_ALL) {
            if (v1 < num_atoms) {
        ...
        for (i = 0; i < n; i++, delta = -delta, v0 = v1, v1 = v2) {
            ...
            */
            ...
            if ((bChangeFlow & BNS_EF_CHNG_BONDS) && v1 < num_atoms) {
                ...
                } else if (v0 < num_atoms && v2 >= num_atoms && v0 != NO_VERTEX) {
            ...
            if (v1 < num_atoms && ineigh1 < at[v1].valence &&
                ...
                ret_val = SetAtomBondType(edge, &at[v1].bond_type[ineigh1], &at[v2].bond_type[ineigh2], delta,
                                          bChangeFlow | bChangeFlowAdd);
                if (ret_val < 0) {
                    err = BNS_PROGRAM_ERR;
                } else {
                    ret |= (ret_val > 0);
        ...
        if (v2 != vLast) {
            err = BNS_PROGRAM_ERR;
        } else if ((bChangeFlow & BNS_EF_CHNG_RSTR) == BNS_EF_CHNG_RSTR) {
        ...
        for (ineigh = 0; ineigh < at[iat].valence && !bError; ineigh++) {
            ...
            if (nMinFlow == nMaxFlow) {
                if (nMaxFlow && bTestForNonStereoBond) {
                    nTestFlow =
                            nMaxFlow - (int) (pBNS->tot_st_cap - pBNS->tot_st_flow); /* temporary use of nTestFlow */
        ...
        if (!a->charge && !a->radical &&
            ...
            */
            ...
            if (1 == num_N && 1 == num_O && 1 == num_OH + num_OM) {
                ...
                if (bDoNotFixAnyBond) { ; /* do nothing */
                } else if (bFound) {
                    if (1 == bFound &&
...
/***************************************************************************************/
int IsZOX(inp_ATOM *atom, int at_x, int ord) {  /* detect O==Z--X, O=O,S,Se,Te */
...
int GetAtomChargeType(inp_ATOM *atom, int at_no, int nAtTypeTotals[], int *pMask, int bSubtract) {
...
#if (FIX_NORM_BUG_ADD_ION_PAIR == 1)
    int i, neigh, mask, bit, type, num_z, num_m, num_o, delta =
            bSubtract > 0 ? -1 : 1; /* 0 or -2 => add, 1 or 2 => subtract */
    ...
    if (at->el_number == el_number_O ||
        ...
        } else if (at->valence && !(num_z || num_o)) {
            if (num_m == at->valence) {
        ...
        if (at->valence) {
            ...
            if (!atom[neigh].charge && atom[neigh].el_number == el_number_C &&
            ...
            } else if ((atom[neigh].el_number == el_number_N) &&
                ...
                } else if (atom[neigh].charge == 1 || atom[neigh].charge == 0) {
            ...
            } else if (atom[neigh].el_number != el_number_C && atom[neigh].el_number != el_number_O &&
            ...
            } else if (at->charge == -1 && !is_el_a_metal(atom[neigh].el_number)) {
                /* >Z-O(-); O=O,S,Se,Te */
...
#if (FIX_NP_MINUS_BUG == 1)
                    else if (at->valence == 1 && at->chem_bonds_valence >= 2 && (at->bond_type[0] & BOND_MARK_ALL))
...
int bIsNegAtomType(inp_ATOM *at, int endpoint, int *cSubType) {
    ...
    if (bIsAtomTypeHard(at, endpoint, PR_HARD_TYP_NEG, PR_HARD_MSK_NEG, -1)) {
        ...
        if (neutral_valence != 2 /* O, S, Se, Te */ &&
            ...
            int num_bonds = at[endpoint].valence;
            int edge_cap = neutral_valence - num_bonds -
                           at[endpoint].num_H; /* does not allow to reduce -NH3(+) to #N or -OH(+)- to -O- */
            edge_flow = inchi_min(edge_flow, edge_cap);
            /* what this means: */
            if (edge_cap) {
                if (edge_cap > edge_flow)
...
*******************************************************************************************/
int HardRemoveAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG, int num2remove, int *nNumCanceledCharges,
    ...
    if (tg_H_Other >= num_atoms && tg_H_Acid >= num_atoms) {
        ...
        do {
            ...
            ret = bExistsAltPath(pBNS, pBD, pAATG, at, num_atoms,
                                 tg_H_Other /*nVertDoubleBond*/, tg_H_Acid /*nVertSingleBond*/,
        ...
        if ((nNumMoved2AcidH /*|| bCancelChargesAlways*/) && cg_Minus >= num_atoms && cg_Plus >= num_atoms &&
            ...
            do {
                nPrevNumCharges = pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES];
                ret = bExistsAltPath(pBNS, pBD, pAATG, at, num_atoms,
                                     cg_Minus /*nVertDoubleBond*/, cg_Plus /*nVertSingleBond*/,
                                     ALT_PATH_MODE_REM_PROTON);
                if (IS_BNS_ERROR(ret)) {
                    return ret;
                }
...
/******************************************************************************************/
int HardAddAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG, int num2add, int *nNumCanceledCharges,
                         BN_STRUCT *pBNS, BN_DATA *pBD) {
    int cg_Plus = 0;
    int cg_Minus_CO = 0;
    ...
    */
    ...
    if (cg_Minus_Other >= num_atoms && cg_Minus_CO >= num_atoms) {
        ...
        do {
            ...
            nPrevNumCharges = pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES];
            ret = bExistsAltPath(pBNS, pBD, pAATG, at, num_atoms,
                                 cg_Minus_Other /*nVertDoubleBond*/, cg_Minus_CO /*nVertSingleBond*/,
                                 ALT_PATH_MODE_REM_PROTON);
            if (IS_BNS_ERROR(ret)) {
                return ret;
            }
        ...
        if ((nNumMoved2AcidMinus /*|| bCancelChargesAlways*/) && cg_Minus_Other >= num_atoms && cg_Plus >= num_atoms &&
            ...
            do {
                nPrevNumCharges = pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES];
                ret = bExistsAltPath(pBNS, pBD, pAATG, at, num_atoms,
                                     cg_Minus_Other /*nVertDoubleBond*/, cg_Plus /*nVertSingleBond*/,
                                     ALT_PATH_MODE_REM_PROTON);
                if (IS_BNS_ERROR(ret)) {
                    return ret;
                }
...
int RemoveNPProtonsAndAcidCharges(inp_ATOM *at, int num_atoms, BN_AATG *pAATG, BN_STRUCT *pBNS, BN_DATA *pBD) {
    ...
    if (!pAATG->nMarkedAtom &&
        (pAATG->nMarkedAtom = (S_CHAR *) inchi_malloc(num_atoms * sizeof(pAATG->nMarkedAtom[0])))) {
    ...
    if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] > 0) {
        ...
        if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] > 0 && bAllowHardRemove) {
            ...
            ret = HardRemoveAcidicProtons(at, num_atoms, pAATG, pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE],
                                          &nNumCanceledCharges, pBNS, pBD);
            if (IS_BNS_ERROR(ret)) {
                bError = ret;
                goto exit_function;
            }
    ...
    } else if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] < 0) {
        ...
        */
        ...
        if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] < 0 && bAllowHardRemove) {
            ret = HardAddAcidicProtons(at, num_atoms, pAATG, -pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE],
                                       &nNumCanceledCharges, pBNS, pBD);
            if (IS_BNS_ERROR(ret)) {
                bError = ret;
                goto exit_function;
...
int mark_alt_bonds_and_taut_groups(inp_ATOM *at, inp_ATOM *at_fixed_bonds_out, int num_atoms,
                                   T_GROUP_INFO *t_group_info, INCHI_MODE *inpbTautFlags,
...
*/
    ...
    if ((pBNS = AllocateAndInitBnStruct(at, num_atoms, BNS_ADD_ATOMS, BNS_ADD_EDGES, max_altp, &num_changed_bonds)) &&
        ...
        do {
            ...
            do {
                taut_pass++;
                for (taut_found = 0; 0 < (ret = MarkTautomerGroups(at, num_atoms, t_group_info, c_group_info, pBNS,
                                                                   pBD)); taut_found++);
                if (ret < 0) {
                    bError = ret;
                }
                ...
                if (*pbTautFlags & TG_FLAG_TEST_TAUT2_SALTS) {
                    ...
                    if (salt_step /*||
                        ...
                        if (salt_found < 0) {
                            bError = salt_found;
                            break;
                        } else if (salt_found == 1 || salt_found == 5) {
                    ...
                    if (!nChanges && salt_pass) { /* one attachment migration */
                        break;
...
/*#endif*/
    ...
    if (!bError && num_atoms == 1 && at[0].at_type == ATT_PROTON && t_group_info &&
...
int bSetBondsAfterCheckOneBond(BN_STRUCT *pBNS, BNS_FLOW_CHANGES *fcd, int nTestFlow, inp_ATOM *at, int num_atoms,
    ...
    for (ifcd -= 1; 0 <= ifcd; ifcd--) {
        ...
        if (v1 < num_atoms && v2 < num_atoms && bChangeFlow && new_flow != pEdge->flow0) {
            ...
            ret_val = SetAtomBondType(pEdge, &at[v1].bond_type[ineigh1], &at[v2].bond_type[ineigh2],
                                      new_flow - pEdge->flow0, bChangeFlow);
...
int bSetFlowToCheckOneBond(BN_STRUCT *pBNS, int iedge, int flow, BNS_FLOW_CHANGES *fcd) {
    ...
    if (f12 < flow) {
        ...
        if (f12 > 0) {
            ...
            pBNS->vert[v1].st_edge.cap = ((pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - f12) |
                                         (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
            pBNS->vert[v2].st_edge.cap = ((pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - f12) |
                                         (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
            pBNS->vert[v1].st_edge.flow = ((pBNS->vert[v1].st_edge.flow & EDGE_FLOW_ST_MASK) - f12) |
                                          (pBNS->vert[v1].st_edge.flow & ~EDGE_FLOW_ST_MASK);
            pBNS->vert[v2].st_edge.flow = ((pBNS->vert[v2].st_edge.flow & EDGE_FLOW_ST_MASK) - f12) |
                                          (pBNS->vert[v2].st_edge.flow & ~EDGE_FLOW_ST_MASK);
        ...
        st_edge_rescap =
                (pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - (pBNS->vert[v1].st_edge.flow & EDGE_FLOW_ST_MASK);
        while (st_edge_rescap && delta1) {
            st_edge_rescap--;  /* grab the radical */
            delta1--;
            pBNS->vert[v1].st_edge.cap = ((pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - 1) |
                                         (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
            nDots--;
        }
        /* grab the adjacent vertex2 radical (st_edge_rescap) if it exists */
        st_edge_rescap =
                (pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - (pBNS->vert[v2].st_edge.flow & EDGE_FLOW_ST_MASK);
        while (st_edge_rescap && delta2) {
            st_edge_rescap--;  /* grab the radical */
            delta2--;
            pBNS->vert[v2].st_edge.cap = ((pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - 1) |
                                         (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
            nDots--;
        }
        ...
        for (i = 0; delta1 && i < pBNS->vert[v1].num_adj_edges; i++) {
            ...
            if (f) {
                ...
                while (f && delta1) {
                    ...
                    pBNS->vert[v_i].st_edge.flow = ((pBNS->vert[v_i].st_edge.flow & EDGE_FLOW_ST_MASK) - 1) |
                                                   (pBNS->vert[v_i].st_edge.flow & ~EDGE_FLOW_ST_MASK);
                    /* next 2 lines added 01-22-2002 */
                    pBNS->vert[v1].st_edge.cap = ((pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - 1) |
                                                 (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
                    pBNS->vert[v1].st_edge.flow = ((pBNS->vert[v1].st_edge.flow & EDGE_FLOW_ST_MASK) - 1) |
                                                  (pBNS->vert[v1].st_edge.flow & ~EDGE_FLOW_ST_MASK);
        ...
        for (i = 0; delta2 && i < pBNS->vert[v2].num_adj_edges; i++) {
            ...
            if (f) {
                ...
                while (f && delta2) {
                    ...
                    pBNS->vert[v2].st_edge.cap = ((pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - 1) |
                                                 (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
                    pBNS->vert[v2].st_edge.flow = ((pBNS->vert[v2].st_edge.flow & EDGE_FLOW_ST_MASK) - 1) |
                                                  (pBNS->vert[v2].st_edge.flow & ~EDGE_FLOW_ST_MASK);
    ...
    if (f12 >= flow) {
        ...
        */
        ...
        pBNS->vert[v1].st_edge.cap = ((pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - flow) |
                                     (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
        pBNS->vert[v2].st_edge.cap = ((pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - flow) |
                                     (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
...
Vertex GetGroupVertex(BN_STRUCT *pBNS, Vertex v1, AT_NUMB type) {
    if (v1 < pBNS->num_atoms) {
        ...
        else if (type == BNS_VERT_TYPE_C_POINT)
...
/*#endif*/ /*  ( TEST_REMOVE_S_ATOMS == 1 && ALT_PATH_MODE_4_SALT == path_type ) */
            ...
            */
            ...
            for (i = 0, n = bAdjacentDonors = 0; i < pVert1->num_adj_edges; i++) {
                ...
                */
                ...
                bAdjacentDonors += bDonors ? (v == v2) && ((pBNS->edge + pVert1->iedge[i])->flow < MAX_BOND_EDGE_CAP)
                                           : 0;
            ...
            for (i = 0, n = bAdjacentDonors = 0; i < pVert2->num_adj_edges; i++) {
                ...
                bAdjacentDonors += bDonors ? (v == v1) && ((pBNS->edge + pVert2->iedge[i])->flow < MAX_BOND_EDGE_CAP)
                                           : 0;
                /* two donors connected by a single or double bond */
            }
            if (!n && !bAdjacentDonors) {
            ...
            if ((pVert1->type & type) == type) {
                ...
                if (BNS_BOND_ERR == v1t) {
                    v1t = NO_VERTEX;
                } else if (IS_BNS_ERROR(v1t)) {
                    return v1t;
                } else if (v1t != NO_VERTEX) {
                    v1Act = v1t;
                }
            }
            /* find t-group that contains v2 */
            if ((pVert2->type & type) == type) {
                ...
                if (BNS_BOND_ERR == v2t) {
                    v2t = NO_VERTEX;
                } else if (IS_BNS_ERROR(v2t)) {
                    return v2t;
                } else if (v2t != NO_VERTEX) {
                    v2Act = v2t;
                }
            }
            ...
            if ((bDonors != 0) == (v2t != NO_VERTEX) && cap == 1) {
                ...
                bSet_v2 = 1;
                iapc++;
            } else if ((bDonors != 0) == (v2t != NO_VERTEX)) {
                bSet_v2 = 1;
            }
        ...
        *nDots = 0;
        ...
        if (type & BNS_VERT_TYPE_ENDPOINT) {
            ...
            if (t1 == NO_VERTEX) {
                /* add st-cap to v1 */
                n = bAddStCapToAVertex(pBNS, v1, (Vertex) (t2 == NO_VERTEX ? v2 : t2), apc->nOldCapsVert[iapc], nDots,
                                       0);
                apc->bSetOldCapsVert[iapc] = n;
                apc->vOldVert[iapc] = v1;
                iapc++;
            ...
            if (t2 != NO_VERTEX) {
                /* add st-cap to t2 */
                n = bAddStCapToAVertex(pBNS, t2, (Vertex) (t1 == NO_VERTEX ? v1 : t1), apc->nOldCapsVert[iapc], nDots,
                                       0);
                apc->bSetOldCapsVert[iapc] = n;
        ...
        */
        ...
        case ALT_PATH_MODE_REM_PROTON:
           ...
           the (+)-charge group (nVertSingleBond) */
            ...
            bChangeFlow = (BNS_EF_SAVE_ALL | BNS_EF_UPD_H_CHARGE) |
                          BNS_EF_ALTR_NS; /* added BNS_EF_ALTR_NS: set non-stereo altern non-ring bonds 2004-07-02*/
            break;
        default:
            type = 0;
            bChangeFlow = BNS_EF_CHNG_RSTR;
        ...
        if (2 * ret >= nDots) {
            ...
            if (pAATG && pAATG->nMarkedAtom) {
                if (pAATG->nAtTypeTotals && (bChangeFlow & BNS_EF_UPD_H_CHARGE)) {
                    ...
                    SubtractOrChangeAtHChargeBNS(pBNS, at, num_atoms, pAATG->nAtTypeTotals, pAATG->nMarkedAtom, NULL,
                                                 1);
                ...
                } else if (!pAATG->nAtTypeTotals) {
                    bDoMarkChangedBonds = MarkAtomsAtTautGroups(pBNS, num_atoms, pAATG, nVertSingleBond,
                                                                nVertDoubleBond);
...
/*****************************************************************************/
BN_STRUCT *AllocateAndInitBnStruct(inp_ATOM *at, int num_atoms, int nMaxAddAtoms, int nMaxAddEdges, int max_altp,
...
/*********************************************************************************/
int BalancedNetworkSearch(BN_STRUCT *pBNS, BN_DATA *pBD,
                          int bChangeFlow) {   /* N has source s, target t, the mirror network M is constructed */
                ...
                if (b_v == NO_VERTEX) {  /* originally 0 instead of NO_VERTEX */
                    ...
                    */
                    ...
                    n++;
                           ...
                           && (SwitchEdge_Vert1(prim(u)) != prim(v) ||
                               SwitchEdge_Vert2(prim(u)) != prim(u)) /* avoid edges of T' */
                           && b_u != b_v
                    ...
                    w = MakeBlossom(pBNS, ScanQ, &QSize, Pu, Pv, max_len_Pu_Pv, SwitchEdge, BasePtr, u, v, iuv, b_u,
                                    b_v, Tree);
                    ...
                    if (prim(w) == Vertex_t) {
                        ...
                        delta = FindPathCap(pBNS, SwitchEdge, Vertex_s, Vertex_t,
                                            10000); /* compute the residual capacity of P + P' */
                        if (IS_BNS_ERROR(delta)) {
                            pBD->QSize = QSize;
                            return delta; /* error */
                        }
                        ...
                        ret = PullFlow(pBNS, SwitchEdge, Vertex_s, Vertex_t, delta, 0,
                                       bChangeFlow); /* augment on a pair of valid st-paths */
                        pBD->QSize = QSize;
...
Vertex MakeBlossom(BN_STRUCT *pBNS, Vertex *ScanQ, int *pQSize,
    ...
    */
    ...
    if (!TREE_IS_ON_SCANQ(prim(w)) /* !IsInScanQ(prim(w))*/ ) {   /* add w' to the blossom */
...
int PullFlow(BN_STRUCT *pBNS, Edge *SwitchEdge, Vertex x, Vertex y, int delta, S_CHAR bReverse, int bChangeFlow) { /*
  ...
  */
    ...
    if (bReverse) {
        ...
        if (z != y) {
            ret = PullFlow(pBNS, SwitchEdge, prim(y), prim(z), delta, (S_CHAR) (1 - bReverse),
                           bChangeFlow); /* augment between z and y */
        }
        if (!IS_BNS_ERROR(ret)) {
...
********************************************************************************/
int FindPathCap(BN_STRUCT *pBNS, Edge *SwitchEdge, Vertex x, Vertex y, int delta) { /* find the minimum residual capacity of all edges
...
int MarkRingSystemsAltBns(BN_STRUCT *pBNS, int bUnknAltAsNoStereo) {
    ...
    nBondStack = (AT_NUMB *) (num_edges ? inchi_malloc(num_edges * sizeof(nBondStack[0]))
                                        : NULL); /* special case: no bonds 2006-03 */
    ...
    for (start = 0; start < num_atoms; start++) {
        ...
        do {
            ...
            while (i = (int) nStackAtom[nTopStackAtom], j = (int) cNeighNumb[i], (int) at[i].valenceAltBns > j) {
                ...
                } else if (!nTopStackAtom || u != (int) nStackAtom[nTopStackAtom - 1]) { /*  may comment out ? */
            ...
            if (i != start) {
                ...
                } else if (nLowNumber[u] > nLowNumber[i]) {
                    /* inherit */
    ...
    if (num_prot < 0 && (bNormalizationFlags & FLAG_PROTON_AC_HARD_ADDED) && 1 == num_tg &&
        ...
        for (i = 0; i < num_atoms; i++) {
            ...
            if ((type & AA_HARD_TYP_CO) && (mask & AA_HARD_MSK_CO)) {
                nNumAcceptors_DB_O++;
            } else if ((type == ATT_ATOM_N) && (mask == ATBIT_NP_H) && !at[i].charge &&
            ...
            for (i = min_at; i < max_at && 0 < num_protons_to_add[iCurIso]; i++) {
                ...
                if (iCurMode) {
                    if (at[i].endpoint)
                        ...
                        continue;
                } else if (!at[i].endpoint &&
                           1 == bHeteroAtomMayHaveXchgIsoH(at, i)) { /* atom number */
                    j = i;
                } else if (at[i].el_number == el_number_H && at[i].charge == 1 &&
