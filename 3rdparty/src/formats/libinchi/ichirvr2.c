...
/******************************************************************************************************/
int RestoreAtomConnectionsSetStereo(StrFromINChI *pStruct, int iComponent, int iAtNoOffset, INChI *pInChI,
                                    INChI *pInChIMobH) {
    inp_ATOM *at = NULL;
    ...
    */
    if (pStereo || pStereo2) {
        ...
        if (nNumDeletedH) {
            ...
            for (i = 0; i < num_atoms; i++) {
                if (at[i].at_type == 1) {
                    if (0 > (ret = AddExplicitDeletedH(at, i, num_atoms, &iDeletedH, &idelH1, nNumDeletedH,
                                                       pStereo2 != NULL))) {
                        goto exit_function;
                    }
                }
    ...
    if (pStereo) {
        ...
        for (i = 0; i < pStereo->nNumberOfStereoCenters; i++) {
            ...
            if (bInvertedParity) {
                parity = (parity == AB_PARITY_EVEN) ? AB_PARITY_ODD : (parity == AB_PARITY_ODD) ? AB_PARITY_EVEN
                                                                                                : parity;
            }
            ...
            if (at[n_vertex].valence == 2 &&
                ...
                if (at[jv].num_H) {
                    if (0 > (ret = AddExplicitDeletedH(at, jv, num_atoms, &iDeletedH, &idelH1, nNumDeletedH,
                                                       pStereo2 != NULL))) {
                        goto exit_function;
                    }
                } else {
                ...
                if (at[jn].num_H) {
                    ...
                    if (0 > (ret = AddExplicitDeletedH(at, jn, num_atoms, &iDeletedH, &idelH2, nNumDeletedH,
                                                       pStereo2 != NULL))) {
                        goto exit_function;
                    }
                } else {
                ...
                */
                ...
                if (at[n_vertex].num_H) {
                    if (0 > (ret = AddExplicitDeletedH(at, n_vertex, num_atoms, &iDeletedH, &idelH1, nNumDeletedH,
                                                       pStereo2 != NULL))) {
                        goto exit_function;
                    }
                }
    ...
    if (pStereo) {
        ...
        if (num_stereo_bonds) {
            ...
            for (i = 0; i < num_atoms; i++) {
                ...
                } else if (num_bond_type_stereo == 1) {
                ...
                } else if (num_bond_type_stereo + num_bond_type_altern) {
...
/******************************************************************************************************/
int
CopyBnsToAtom(StrFromINChI *pStruct, BN_STRUCT *pBNS, VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups, int bAllowZeroBondOrder) {
    ...
    for (i = num_at; i < pBNS->num_vertices; i++) {
        ...
        if ((charge = pv->st_edge.cap - pv->st_edge.flow)) {
            if (IS_BNS_VT_C_OR_CSUPER_GR(pv->type)) {
                left_charge -= charge;
            } else if (IS_BNS_VT_YVCONNECTOR(pv->type)) {
                left_charge += charge;
            }
        }
    }
    ...
    for (i = 0; i < pBNS->num_t_groups; i++) {
        ...
        if (bMinusFirst) {
            ...
            if ((num_H + num_Minus != pv->st_edge.cap - pv->st_edge.flow) &&
                (num_H || num_Minus || vMinus != NO_VERTEX)) {
                return RI_ERR_PROGR;
            }
        } else {
...
/******************************************************************************************************/
int set_cumulene_0D_parity(inp_ATOM *at, inp_ATOM_STEREO *st, int num_at, int idelH1, int i1, int i2, int idelH2,
                           int parity, int len) {
    AT_NUMB nCumulene[MAX_CUMULENE_LEN + 2];
    AT_NUMB *p1, *p2;
...
int set_atom_0D_parity(inp_ATOM *at, inp_ATOM_STEREO *st, int num_at, int num_deleted_H, int i1, int parity) {
    ...
    if (!st || !at[i1].p_parity) {
        ...
        } else if (tot_num_neigh != MAX_NUM_STEREO_ATOM_NEIGH) {
    ...
    if (num_moved) {
        ...
        for (i = pBNS->num_atoms, pv = pBNS->vert + i; i < pBNS->num_vertices; i++, pv++) {
            if ((delta = pv->st_edge.cap - pv->st_edge.flow)) {
                if (IS_BNS_VT_C_OR_CSUPER_GR(pv->type)) {
                    extra_charge -= delta;
                } else if (BNS_VERT_TYPE__AUX == pv->type) {
                    extra_charge += delta;
                } else {
                    ret = RI_ERR_PROGR;
                    goto exit_function;
    ...
    if (num_candidates > 0) {
        ...
        for (k = 0; k < j; k++) {
            ...
            if (this_atom_add_charge) {
                for (i = pBNS->num_vertices - 1, pv = pBNS->vert + i;
                     this_atom_add_charge && pBNS->num_atoms <= i; i--, pv--) {
                    if ((delta = pv->st_edge.cap - pv->st_edge.flow)) {
                        ...
                        } else if (this_atom_add_charge > 0 && BNS_VERT_TYPE__AUX == pv->type) {
        ...
        for (j = 0; j < at[i].valence; j++) {
            ...
            if (at[neigh].endpoint) {
                ...
                if (!tg_number) {
                    ...
                    num_eql_mobile_gr = 1;
                } else if (tg_number == at[neigh].endpoint) {
                    num_eql_mobile_gr++;
                } else {
                    num_dif_mobile_gr++;
                }
            } else if (bond_type == BOND_TYPE_SINGLE && val) {
                if (at[neigh].endpoint) {
                    has_mobile_H |= 1;
                    has_mobile |= 1;
                } else {
            ...
            *********************************************************/
            ...
            for (j = 0; j < num_endpoints; j++) {
                if ((MobileGr[j].atom_type_pVA & (EL_TYPE_N | EL_TYPE_S)) &&
                    ...
                    jXH = j;
                    n++;
                } else if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_O &&
                           MobileGr[j].num_bonds_non_metal == 1 &&
                           !MobileGr[j].forbidden) {
                    if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE && jO1 < 0) {
                        jO1 = j;
                        n++;
                    } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE && jO2 < 0) {
                        jO2 = j;
                        n++;
                    }
                }
            ...
            *********************************************************/
            ...
            for (j = 0; j < num_endpoints; j++) {
                if ((MobileGr[j].atom_type_pVA & (EL_TYPE_O | EL_TYPE_S)) &&
                    ...
                    jO = j;
                    n++;
                } else if ((MobileGr[j].atom_type_pVA & (EL_TYPE_N | EL_TYPE_P)) &&
                           !MobileGr[j].forbidden) {
                    if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                        (MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N && jNH < 0) {
                        jNH = j;
                        n++;
                    } else if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE && jX < 0) {
                        jX = j;
                        n++;
                    }
                }
        ...
        if (2 == delta_met && 4 == num_bonds_non_metal &&
            ...
            for (j = 0; j < num_endpoints; j++) {
                ...
                if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
                    ...
                    jN = j;
                    n++;
                } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                           at[neigh].endpoint) {
            ...
            *********************************************************/
            ...
            for (j = 0; j < num_endpoints; j++) {
                if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N &&
                    ...
                    jNH = j;
                    n++;
                } else if ((MobileGr[j].atom_type_pVA & (EL_TYPE_O | EL_TYPE_S)) &&
                           !MobileGr[j].forbidden &&
                           MobileGr[j].num_bonds_non_metal == 1) {
                    if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
                        jO < 0) {
            ...
            *********************************************************/
            ...
            for (j = 0; j < num_endpoints; j++) {
                if ((MobileGr[j].atom_type_pVA & (EL_TYPE_O | EL_TYPE_S)) &&
                    ...
                    jO = j;
                    n++;
                } else if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N &&
                           !MobileGr[j].forbidden &&
                           MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                           jNH < 0) {
                    jNH = j;
...
#define INC_EDGE_LIST 16
    ...
    for (i = 0; i < num_at && 0 <= ret; i++) {
        ...
        if ((eNMinusEdge = pVA[i].nCMinusGroupEdge - 1) >= 0 &&
            ...
            } else if (!pVA[i].cMetal && !at2[i].endpoint && at2[i].charge != -1) {
        ...
        if ((eNPlusEdge = pVA[i].nCPlusGroupEdge - 1) >= 0 &&
            ...
            } else if (!pVA[i].cMetal && !at2[i].endpoint) {
        ...
        if (pVA[i].cNumValenceElectrons == 5 && at2[i].valence == 3 &&
            ...
            for (j = 0; j < pv0->num_adj_edges; j++) {
                ...
                if (pe->flow == 1 && v2 == NO_VERTEX) {
                    ...
                    ie02 = ie;
                    pe02 = pe;
                } else if (pe->flow == 0 && v1 == NO_VERTEX) {
                    /* 0 - 1, edge 01 */
                    v1 = pe->neighbor12 ^ v0;
                    pv1 = pBNS->vert + v2;
                    ie01 = ie;
        ...
        for (itg = 0; itg < pStruct->ti.num_t_groups; iEndpoint += num_endpoints, itg++) {
            ...
            if (i < num_endpoints) {
                ...
                for (j = 0; j < pStruct->ti.nNumEndpoints; j++) {
                    ...
                    if (((e1 = pVA[endpoint1].nCMinusGroupEdge - 1) < 0 || !pBNS->edge[e1].flow) ||
                        pBNS->edge[e1].forbidden) {
    ...
    for (i = 0; i < num_at; i++) {
        if (pVA[i].cNumValenceElectrons != 4 && /* not C, Si, Ge */
            ...
            if (j == at2[i].valence && num_endpoints > 1) {
                ...
                } else if (pEdgeMinus && (delta == pEdgeMinus->flow) && pEdgePlus->flow == 0) {
