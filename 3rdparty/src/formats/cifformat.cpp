...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.iucr.org/iucr-top/cif/spec/"; }; //optional
    ...
    bool ci_char_traits::eq(char c1, char c2) { return tolower(c1) == tolower(c2); }

    bool ci_char_traits::ne(char c1, char c2) { return tolower(c1) != tolower(c2); }

    bool ci_char_traits::lt(char c1, char c2) { return tolower(c1) < tolower(c2); }

    int ci_char_traits::compare(const char *s1, const char *s2, size_t n) { return strnicmp(s1, s2, n); }
   ...
   */
    ...
    void CIFData::ExtractAll() {
        ...
        if (mDataBlockName == "data_global") { // :KLUDGE: this data block name is used for journal &author information
    ...
    void CIFData::ExtractUnitCell() {
        ...
        if ((mvItem.find("_cell_length_a") != mvItem.end())
            ...
            if ((spgid > 2) && (spgid <= 15)) {// :TODO: monoclinic spg, depending on unique axis....
            }
            if ((spgid > 15) && (spgid <= 142)) {// orthorombic & tetragonal
            ...
            if ((spgid > 74) && (spgid <= 142)) {// Tetragonal, make sure a=b if one is missing
                if (mvLatticePar[1] == 0) mvLatticePar[1] = mvLatticePar[0];
                if (mvLatticePar[0] == 0) mvLatticePar[0] = mvLatticePar[1];
            }
            if ((spgid > 142) && (spgid <= 194)) {// trigonal/ rhomboedric...
                const string::size_type pos = mSpaceGroup->GetHallName().find('R');
                if (pos == std::string::npos) {//rhomboedric cell, a=b=c, alpha=beta=gamma
                    float a = 0;
                ...
                } else {//hexagonal cell, a=b & alpha=beta=pi/2, gamma= 2*pi/3
                    if (mvLatticePar[1] == 0) mvLatticePar[1] = mvLatticePar[0];
                    if (mvLatticePar[0] == 0) mvLatticePar[0] = mvLatticePar[1];
                    if (mvLatticePar[3] == 0) mvLatticePar[3] = M_PI / 2;
                    if (mvLatticePar[4] == 0) mvLatticePar[4] = M_PI / 2;
            ...
            if (mvLatticePar[1] < 1e-6) {
                stringstream ss;
                ss << "CIF Error: missing b lattice parameter - cannot interpret structure ! (in data block:"
                   << mDataBlockName << ")";
                obErrorLog.ThrowError(__FUNCTION__, ss.str(), obError);
            }
            if (mvLatticePar[2] < 1e-6) {
                stringstream ss;
                ss << "CIF Error: missing c lattice parameter - cannot interpret structure ! (in data block:"
                   << mDataBlockName << ")";
                obErrorLog.ThrowError(__FUNCTION__, ss.str(), obError);
            }
    ...
    void CIFData::ExtractSpacegroup() {
        ...
        if (positem != mvItem.end()) {
            ...
            if (positem != mvItem.end()) {
                mSpacegroupSymbolHall = positem->second;
                found = true;
                obErrorLog.ThrowError(__FUNCTION__, "Found spacegroup Hall symbol (with OBSOLETE CIF #1.0 TAG):" +
                                                    mSpacegroupSymbolHall, obDebug);
            }
        }
        ...
        if (positem != mvItem.end()) {
            ...
            found = true;
            obErrorLog.ThrowError(__FUNCTION__, "Found spacegroup Hermann-Mauguin symbol:" + mSpacegroupHermannMauguin,
                                  obDebug);
        } else {
            ...
            if (positem != mvItem.end()) {
                ...
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Found spacegroup Hermann-Mauguin symbol (with OBSOLETE CIF #1.0 TAG):" +
                                      mSpacegroupHermannMauguin, obDebug);
            }
        }
        ...
        if (positem != mvItem.end()) {
            obErrorLog.ThrowError(__FUNCTION__, "Found spacegroup IT_coordinate_system_code:" + positem->second,
                                  obDebug);
            if ((mSpacegroupHermannMauguin.length() > 0) && (positem->second == "1" || positem->second == "2")) {
                ...
                stringstream ss;
                ss << "CIF Error: found DDL2 tag _space_group.IT_coordinate_system_code (" << positem->second << ")"
                   << endl
    ...
    void CIFData::ExtractAtomicPositions() {
        ...
        for (map<set<ci_string>, map<ci_string, vector<string> > >::const_iterator loop = mvLoop.begin();
            ...
            if (mvAtom.size() > 0) {// Got the atoms, get names and symbols
                ...
                if (poslabel != loop->second.end())
                    for (unsigned int i = 0; i < nb; ++i) {
                        ...
                        if (possymbol == loop->second.end()) {// There was no symbol, use the labels to guess it
                            int nbc = 0;
    ...
    void CIFData::ExtractBonds() {
        ...
        for (map<set<ci_string>, map<ci_string, vector<string> > >::const_iterator loop = mvLoop.begin();
            ...
            if ((poslabel1 != loop->second.end()) && (poslabel2 != loop->second.end()) &&
                (posdist != loop->second.end())) {
    ...
    string CIFReadValue(istream &in, char &lastc) {
        ...
        while (in.peek() == '#') {//discard these comments for now
            string tmp;
            getline(in, tmp);
            lastc = '\r';
            while (!isgraph(in.peek())) in.get(lastc);
        ...
        if ((in.peek() == '\'') || (in.peek() == '\"')) {//QuotedString
            char delim;
            in.get(delim);
            value = "";
        ...
        while (!in.eof()) {
            ...
            if ((in.peek() == 'l') || (in.peek() == 'L')) {// loop_
                ...
                while (true) {
                    ...
                    if (ci_string(tmp.c_str()) == "loop_") {//go back and continue
                        in.clear();
                    ...
                    if (tmp.size() >= 5)
                        if (ci_string(tmp.substr(0, 5).c_str()) == "data_") {//go back and continue
                            in.clear();
                            in.seekg(pos0, std::ios::beg);
                            stringstream ss;
                            ss << "END OF LOOP :" << tmp << "," << (char) in.peek() << "," << in.tellg();
                    ...
                    for (unsigned int i = 0; i < tit.size(); ++i) {//Read all values
    ...
    bool CIFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        for (map<string, CIFData>::iterator pos = cif.mvData.begin(); pos != cif.mvData.end(); ++pos)
            if (pos->second.mvAtom.size() > 0) {
                pmol->BeginModify();
                if (pos->second.mvLatticePar.size() == 6) {// We have one unit cell
                ...
                else if (pos->second.mFormula != "") pmol->SetTitle(pos->second.mFormula);
                ...
                for (vector<CIFData::CIFAtom>::const_iterator posat = pos->second.mvAtom.begin();
                     posat != pos->second.mvAtom.end(); ++posat) {
                    ...
                    if (tmpSymbol.size() > nbc) {// Try to find a formal charge in the symbol
                        int charge = 0;
                        int sign = 0;
                        for (unsigned int i = nbc;
                             i < tmpSymbol.size(); ++i) {// Use first number found as formal charge
                ...
                if (!pConv->IsOption("b", OBConversion::INOPTIONS))
                    pmol->ConnectTheDots();
                if (pConv->IsOption("B", OBConversion::INOPTIONS)) {
                    for (vector<CIFData::CIFBond>::const_iterator posbond = pos->second.mvBond.begin(); posbond !=
                                                                                                        pos->second.mvBond.end(); ++posbond) {// Add bonds present in the cif and not detected by ConnectTheDots()
    ...
    bool CIFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        FOR_ATOMS_OF_MOL(atom, *pmol) {
            ...
            if (atom->HasData("_atom_site_occupancy")) {
                occup = (dynamic_cast<OBPairFloatingPoint *> (atom->GetData(
                        "_atom_site_occupancy")))->GetGenericValue();
            } else occup = 1.0;
        ...
        if (pConv->IsOption("g", OBConversion::OUTOPTIONS)) {
            ...
            FOR_BONDS_OF_MOL(bond, *pmol) {
                ...
                if (bond->IsPeriodic()) {
                    ...
                    symmetry_num +=
                            100 * uc[0] + 10 * uc[1] + 1 * uc[2];  // Unit cell directionality vs. 555, per CIF spec
                }
                if (symmetry_num == 555) {
                    sym_key = ".";
                } else {
