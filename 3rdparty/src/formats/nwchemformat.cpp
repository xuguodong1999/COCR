...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.emsl.pnl.gov/docs/nwchem/"; }; //optional
    ...
    */
    void NWChemOutputFormat::ReadOrbitals(istream *ifs, OBMol *molecule) {
        ...
        while (ifs->getline(buffer, BUFF_SIZE)) {
            ...
            else if (strstr(buffer, ORBITAL_SECTION_PATTERN_2) != nullptr &&
                     strstr(buffer, ORBITAL_SECTION_PATTERN_1) != nullptr) {
        ...
        while (ifs->getline(buffer, BUFF_SIZE)) {
            if (strstr(buffer, NEB_BEAD_START_PATTERN) != nullptr) {
                ...
                for (unsigned int i = 0; i < natoms; i++) {
                    ...
                    if (OBElements::GetAtomicNum(vs[1].substr(0, end_of_symbol).c_str()) !=
                        molecule->GetAtom(i + 1)->GetAtomicNum())
                        break;
                    if (current_bead >= nbeads) {
                        cerr << "Current bead out of range: " << current_bead << " of " << nbeads << endl;
                        break;
            ...
            if (strstr(buffer, NBEADS_PATTERN) != nullptr) {
                ...
                while (vsize > 7) {
                    ...
                    for (unsigned int i = 0; i < natoms; i++) {
                        ...
                        //    0                 1     2     3
                        if ((vs.size() < 4) ||
                            (molecule->GetAtom(i + 1)->GetAtomicNum() != OBElements::GetAtomicNum(vs[0].c_str())))
                    ...
                    if (vs.size() <= 1) // blank line
                    {
                        ...
                        molecule->SetEnergies(energies);
                        molecule->SetConformers(beads);
                        unsigned int ts_position = distance(energies.begin(),
                                                            max_element(energies.begin(), energies.end()));
