...
namespace OpenBabel {
    ...
    public:
        ...
        bool matchCandidate(State &state, OBQueryAtom *queryAtom, OBAtom *queriedAtom) {
            ...
            if (DEBUG) {
                cout << "FOUND:  " << queryAtom->GetIndex() << " -> " << queriedAtom->GetIndex() << "       "
                     << state.queryPath.size() << endl;
                cout << "queryDepths:   ";
            ...
            if ((state.query->NumAtoms() - state.queryPath.size() - numT1) >
                (state.queried->NumAtoms() - state.queriedPath.size() - numT2)) {
                Backtrack(state);
                return false;
            }
        ...
        Candidate NextCandidate(State &state, const Candidate &lastCandidate) {
            ...
            std::size_t queryTerminalSize =
                    state.queryDepths.size() - std::count(state.queryDepths.begin(), state.queryDepths.end(), 0);
            std::size_t queriedTerminalSize =
                    state.queriedDepths.size() - std::count(state.queriedDepths.begin(), state.queriedDepths.end(), 0);

            std::size_t mappingSize = state.queryPath.size();

            if (queryTerminalSize > mappingSize && queriedTerminalSize > mappingSize) {
                while (lastQueryAtom < querySize &&
                       (state.queryPathBits.BitIsSet(lastQueryAtom) || !state.queryDepths[lastQueryAtom])) {
                    lastQueryAtom++;
                    lastQueriedAtom = 0;
                }
            } else {
            ...
            if (queryTerminalSize > mappingSize && queriedTerminalSize > mappingSize) {
                while (lastQueriedAtom < queriedSize &&
                       (state.queriedPathBits.BitIsSet(lastQueriedAtom) || !state.queriedDepths[lastQueriedAtom]))
                    lastQueriedAtom++;
            } else {
        ...
        void MapNext(State &state, OBQueryAtom *queryAtom, OBAtom *queriedAtom) {
            ...
            while (!state.abort) {
                ...
                if (DEBUG)
                    cout << yellow << "candidate: " << candidate.queryAtom->GetIndex() << " -> "
                         << candidate.queriedAtom->GetIndex() << normal << endl;
        ...
        void Backtrack(State &state) {
            ...
            std::replace(state.queriedDepths.begin(), state.queriedDepths.end(), depth,
                         static_cast<unsigned int>(0)); // O(n)  n = # vertices in the queried
        }
    ...
    bool FindAutomorphisms(OBMol *mol, Automorphisms &maps, const std::vector<unsigned int> &symClasses,
                           const OBBitVec &mask, std::size_t maxMemory) {
        maps.clear();
        MapAllFunctor functor(maps, maxMemory);
