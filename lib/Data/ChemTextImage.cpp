#include "XGD/Data/ChemTextImage.h"
...
bool CRNNDataGenerator::init(const std::string &_topDir) {
    ...
    if (!std::filesystem::exists(topDir)) {
        if (!std::filesystem::create_directories(topDir)) {
            xgd::serr << "fail to create dir: " << topDir << std::endl;
            exit(-1);
        }
    }
    ...
    if (!std::filesystem::exists(imgPath)) {
        if (!std::filesystem::create_directories(imgPath)) {
            xgd::serr << "fail to create dir: " << imgPath << std::endl;
            exit(-1);
        }
    }
...
bool CRNNDataGenerator::initData() {
    getChemTexts();
    getDictTexts();
    xgd::sout << "chemTexts.size()=" << chemTexts.size() << ", dictTexts.size()="
    ...
    if (availableFontFamilies.empty()) {
        ...
        static std::unordered_set<std::string> whiteList{
                ...
                "华文中宋", "华文仿宋", "华文宋体", "华文新魏", "华文楷体", "华文细黑", "幼圆", "文鼎ＰＬ简中楷",
                "文鼎ＰＬ简报宋",
                "方正姚体", "隶书"};
...
void CRNNDataGenerator::dump(const size_t &_trainNum, const size_t &_testNum) {
    if (!isInited) {
        xgd::serr << "you must call CRNNDataGenerator::init before dump data" << std::endl;
        exit(-1);
    }
...
void CRNNDataGenerator::display() {
    ...
    for (size_t idx = 0; idx <= 500000; idx++) {
        ...
        char a[100];
        sprintf(a, imgKey, idx);
        xgd::sout << "image-key=" << a << std::endl;
        sprintf(a, labelKey, idx);
        xgd::sout << "label-key=" << a << std::endl;
        auto img = CvUtil::BufferToGrayMat(buffer);
...
void CRNNDataGenerator::getDictTexts() {
    ...
    std::string dataPath = XGD_ASSET_DIR;
    dataPath += "/datasets/dictionary/english-words.json";
    dc.loadFromWordDict(dataPath.c_str());
...
//    xgd::sout << textSet.size() << std::endl;
...
//    xgd::sout << "alphabet=" << alphabet << std::endl;
}
    ...
    for (auto &c: _key) {
        if (StdUtil::notExist(alphabetSet, c)) {
            if (std::isalpha(c)) {
                ...
                xgd::serr << "unhandled char: " << c << std::endl;
                exit(-1);
            }
        } else {
            key.push_back(c);
...
void CRNNDataGenerator::getChemTexts() {
    LineTextDataCreator dc;
    std::string dataPath = XGD_ASSET_DIR;
    dataPath += "/../third_party/openbabel-src/openbabel/data/superatom.txt";
    dc.loadFromPattern(dataPath);
    std::unordered_set<std::string> textSet;
    for (auto &text: dc.getWordSet()) {
        if (text.length() <= MAX_TEXT_LENGTH) {
            textSet.insert(text);
