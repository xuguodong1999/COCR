...
OpenBabel::OBMol convertJMolToOBMol(const JMol &_jMol) {
    ...
    // FIXME: we lose geometric information like R/S, CIS/TRANS and delocalized bonds here
    j2oAtomMap.clear();
    o2jAtomMap.clear();
    ...
    auto convert_bond = [&](const size_t &_bid) {
        ...
        if (BondType::DelocalizedBond == bond->getBondType()) {
            obBond->SetBondOrder(5);
        } else {
            obBond->SetBondOrder(fracToBase<int>(bond->asValence()));
        }
    };
...
std::shared_ptr<JMol> convertOBMolToJMol(const OpenBabel::OBMol &_obMol) {
    ...
    FOR_BONDS_OF_MOL(obBond, const_cast<OpenBabel::OBMol &>(_obMol)) {
        ...
        if (5 == obBond->GetBondOrder()) {
            bondType = BondType::DelocalizedBond;
        } else {
            bondType = static_cast<BondType>(obBond->GetBondOrder());
        }
...
//    xgd::sout << "_mol->atomsNum()" << _mol.atomsNum();
    auto obMol = convertJMolToOBMol(_mol);
    runOBForceField(obMol, "uff");
...
//                 << _content.c_str() << " as " << _format.c_str();
    ...
    // TODO: do not drop 3d information in file
