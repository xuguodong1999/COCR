...
void MolOp::randomize(
    ...
    auto replace_line_bond_type = [&](const size_t &_bid) {
        ...
        const frac deltaFrom = ElementUtil::GetElementValence(fromAtom->getElementType())
                               - atomValenceMap[from];
        const frac deltaTo = ElementUtil::GetElementValence(toAtom->getElementType())
                             - atomValenceMap[to];
        if (deltaFrom >= 1 && deltaTo >= 1) {
            frac delta = std::min(deltaFrom, deltaTo);
            std::vector<BondType> sBondCandidates;
            if (delta >= 2) {// triple, double
                sBondCandidates = {BondType::SingleBond, BondType::SingleBond,
                                   BondType::SingleBond, BondType::WaveBond,
                                   BondType::DoubleBond, BondType::TripleBond};
            } else {// double
                sBondCandidates = {BondType::SingleBond, BondType::SingleBond,
                                   BondType::SingleBond, BondType::WaveBond,
                                   BondType::DoubleBond};
            }
            // check if atom has double bonds connected
            ...
            auto check_allow_replace = [&](const size_t &_bid2) -> bool {
                ...
                if (from == bond2->getAtomFrom() || from == bond2->getAtomTo()) {
                    if (BondType::DoubleBond == bond2->getBondType()) {
                        allowReplace = false;
                        return false;
                    }
                }
            ...
            if (allowReplace) {
                ...
                bond->setType(StdUtil::randSelect(sBondCandidates));
    ...
    auto replace_atom_type = [&](const size_t &_aid) -> void {
        ...
        const auto &ele = StdUtil::randSelect(sAtomCandidates);
        if (atomValenceMap[_aid] <= ElementUtil::GetElementValence(ele)) {
    ...
    auto add_group = [&](const size_t &_aid) {// COOH CHO etc
        ...
        if (atomValenceMap[_aid] == frac(1) && ElementType::C == atom->getElementType()) {
            addGroup(_aid);
        }
    };
    ...
    auto replace_bond_with_ring = [&](const size_t &_bid) {
        ...
        if (StdUtil::byProb(0.2) && BondType::SingleBond == bond->getBondType()) {
            size_t from = bond->getAtomFrom(), to = bond->getAtomTo();
            // 避免一个原子挂上好几个环导致排版引擎崩溃
            if (StdUtil::notExist(usedAids, from) && StdUtil::notExist(usedAids, from)) {
                if (_addAromaticRing && StdUtil::byProb(0.5)) {
    ...
    auto clear_invalid_bond = [&](const size_t &_bid) {
        auto bond = mol->getBondById(_bid);
        if (BondType::TripleBond == bond->getBondType()) {
    ...
    auto add_hydrogen = [&](const size_t &_aid) {
        ...
        if (ElementType::C != atom->getElementType()) {// 杂原子较大可能加氢
            if (StdUtil::byProb(_addHydrogenProb + 0.5)) {
                addHs(_aid);
            }
        } else if (StdUtil::byProb(_addHydrogenProb)) {// 碳原子按概率加氢
            addHs(_aid);
        }
    };
    ...
    auto add_wedge_bond = [&](const size_t &_aid) {
        ...
        if (neighborAtomsMap[_aid].size() == 4 && ElementType::C == atom->getElementType()) {
            ...
            if (allowThisWedge) {
                ...
                while (okNeighbor < 2) {
                    auto &bond = StdUtil::randSelect(bonds);
                    if (bond->setFrom(_aid)) {
                        if (okNeighbor == 0)
                            bond->setType(BondType::SolidWedgeBond);
                        else
...
void MolOp::addHs(const size_t &_aid) {
    ...
    auto atom = mol->getAtomById(_aid);
    frac numOfH = ElementUtil::GetElementValence(atom->getElementType()) - valence;
//    xgd::sout<<atom->getAtomicNumber()<<","<<valence<<"," << std::endl;
//    xgd::sout<<numOfH<<std::endl;
    if (numOfH >= 1) {
        int hsNum = fracToBase<int>(numOfH);
        for (int i = 0; i < hsNum; i++) {
            auto h = mol->addAtom(1);
            const size_t &hid = h->getId();
            mol->addBond(hid, _aid, BondType::SingleBond);
...
void MolOp::addGroup(const size_t &_aid) {
    ...
    switch (index) {
        case 0: {// 羰基、酰基、醛基、羧基、巯基
            ...
            newAtoms = {
                    mol->addAtom(StdUtil::randSelect(carbonyl)),
                    mol->addAtom(StdUtil::randSelect(acyl)),
                    mol->addAtom(6)
            };
            newBonds = {
                    mol->addBond(_aid, newAtoms[2]->getId(),
                                 BondType::SingleBond),// 连羧基碳
                    mol->addBond(newAtoms[2]->getId(), newAtoms[1]->getId(),
                                 BondType::SingleBond),// 连羧基氢
                    mol->addBond(newAtoms[2]->getId(), newAtoms[0]->getId(),
                                 BondType::DoubleBond)// 连羧基氧
            };
            break;
        }
        case 1:
...
void MolOp::addAromaticRing(const size_t &_bid) {
    ...
    switch (index) {
        ...
        case 3: {// 呋喃系
            ...
            newBonds.push_back(mol->addBond(from, newAtoms[1 + StdUtil::randInt() % 2]->getId(),
                                            BondType::SingleBond));
            newBonds.push_back(mol->addBond(to, newAtoms[3 + StdUtil::randInt() % 2]->getId(),
                                            BondType::SingleBond));
            break;
        }
        case 4:
        case 5: {// 苯环系
            ...
            aromaticRingBids.push_back({std::move(bondInRing)});
            newBonds.push_back(mol->addBond(from, pos1, BondType::SingleBond));
            newBonds.push_back(mol->addBond(to, pos2, BondType::SingleBond));
            break;
        }
        case 6: {//萘
            std::vector<size_t> atomicNumbers = {6, 6, 6, 7};
            ...
            for (int i = 0; i < reactSiteNum; i++) {
                auto atom = mol->addAtom(StdUtil::randSelect(atomicNumbers));
            ...
            if (poses.size() < 2) {
                size_t pos1 = StdUtil::randInt() % (reactSiteNum / 2);
                size_t pos2 = reactSiteNum / 2 + StdUtil::randInt() % (reactSiteNum / 2);
...
void MolOp::addCommonRing(const size_t &_bid) {
    ...
    auto &nodeNum = StdUtil::randSelect(nodeNums);
    ...
    switch (nodeNum) {
        case 3: {// 三元环
            ...
            newBonds = {
                    ...
                    mol->addBond(newAtoms[1]->getId(), newAtoms[2]->getId()),
                    mol->addBond(newAtoms[2]->getId(), newAtoms[0]->getId()),
                    mol->addBond(from, newAtoms[StdUtil::randInt() % 2]->getId()),
                    mol->addBond(to, newAtoms[StdUtil::randInt() % 2]->getId())
            };
            break;
        }
        case 4: {// 四元环
            ...
            newBonds = {
                    ...
                    mol->addBond(from, newAtoms[StdUtil::randInt() % 3]->getId()),
                    mol->addBond(to, newAtoms[StdUtil::randInt() % 3]->getId())
            };
            break;
        }
        case 5: {// 五元环
            ...
            newBonds = {
                    ...
                    mol->addBond(from, newAtoms[StdUtil::randInt() % 4]->getId()),
                    mol->addBond(to, newAtoms[StdUtil::randInt() % 4]->getId())
            };
            break;
        }
        case 6: {// 六元环
...
//            }
            newBonds = {
                    ...
                    mol->addBond(from, newAtoms[StdUtil::randInt() % 5]->getId()),
                    mol->addBond(to, newAtoms[StdUtil::randInt() % 5]->getId())
            };
            break;
        }
        case 7: {// 七元环
            ...
            newBonds = {
                    ...
                    mol->addBond(from, newAtoms[StdUtil::randInt() % 6]->getId()),
                    mol->addBond(to, newAtoms[StdUtil::randInt() % 6]->getId())
            };
            break;
        }
        case 100: {
            ...
            newBonds = {
                    ...
                    mol->addBond(newAtoms[4]->getId(), newAtoms[5]->getId(),
                                 BondType::DoubleBond),
                    mol->addBond(newAtoms[5]->getId(), newAtoms[0]->getId()),
                    mol->addBond(from, newAtoms[0 + StdUtil::randInt() % 3]->getId()),
                    mol->addBond(to, newAtoms[3 + StdUtil::randInt() % 3]->getId()),
...
void MolOp::updateAtomValenceMap() {
    ...
    auto add_valence4atom = [&](const size_t &_aid, const frac &_valence) {
        if (StdUtil::notExist(atomValenceMap, _aid)) {
    ...
    auto add_valence_from_bond = [&](const size_t &_bid) {
        auto bond = mol->getBondById(_bid);
//        xgd::sout<<bond->asValence()<<std::endl;
...
bool MolOp::markAsDouble(const size_t &_bid) {
    ...
    const auto &oa1 = ElementUtil::GetElementValence(a1->getElementType());
    const auto &oa2 = ElementUtil::GetElementValence(a2->getElementType());
...
void MolOp::updateDoubleBondCounter() {
    ...
    auto add_db_num = [&](const size_t &_bid) -> void {
        auto bond = mol->getBondById(_bid);
        bool isDb = BondType::DoubleBond == bond->getBondType();
        auto a1 = bond->getAtomFrom(), a2 = bond->getAtomTo();
        if (StdUtil::notExist(dbCounter, a1)) {
            dbCounter[a1] = 0;
        }
        if (StdUtil::notExist(dbCounter, a2)) {
            dbCounter[a2] = 0;
        }
        if (isDb) {
            dbCounter[a1] += 1;
