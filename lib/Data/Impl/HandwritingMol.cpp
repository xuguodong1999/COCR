...
#include "XGD/HandWriting/Bond.h"
...
void HwMol::rotateBy(float angle, const point2f &cent) {
    // rotate skeleton, keep text's gravity
    for (auto &sym: mData) {
...
//    return avgSize;
    // 添加弧线转折
//    xgd::sout << "lineBondMap.size()=" << lineBondMap.size() << std::endl;
//    xgd::sout << "avgBondLength=" << avgSize << std::endl;
    for (auto &[bid1, bid2]: lineBondMap) {
        ...
        const auto &[x, y] = mol2d->getPos2D(aid);
        const point2f A = {x, y};
//        xgd::sout << "A=" << A << std::endl;
//        xgd::sout << "bbox1=" << item1->getBoundingBox().value() << std::endl;
//        xgd::sout << "bbox2=" << item2->getBoundingBox().value() << std::endl;
...
//    xgd::sout << "avgSize=" << avgSize << std::endl;
    molOp->updateAtomValenceMap();
    return avgSize;
}
...
void HwMol::dumpAsDarknet(const std::string &_imgPath, const std::string &_labelPath,
    ...
    for (size_t i = 0; i < _repeatTimes; i++) {
        ...
        ofsm.precision(6);
        if (!ofsm.is_open()) {
            xgd::serr << "fail to open " << _labelPath + suffix + ".txt" << std::endl;
            exit(-1);
        }
...
void HwMol::showOnScreen(const size_t &_repeatTimes, bool _showBox) {
    ...
    for (size_t i = 0; i < _repeatTimes; i++) {
        ...
        for (auto &sym: target->mData) {
//            xgd::sout << fullLabels[sym->getItemType()] << std::endl;
            const auto bBox0 = sym->getBoundingBox().value();
            const auto &[tl, br] = bBox0;
            const auto &[x0, y0] = tl;
            const auto &[x1, y1] = br;
...
//        xgd::sout << xBegin << "," << xMid << "," << xEnd << std::endl;
...
//        xgd::sout << "element=" << atom->getElementName() << ",curVal=" << val << std::endl;
        auto newItem = crnnDataGenerator.getRectStr(_freeRect, fracToBase<int>(val), _isLeft);
        if (newItem) {
//            xgd::sout << "!!!!! replace\n";
...
//        xgd::sout << "from=" << from_ << ",to=" << to_ << std::endl;
