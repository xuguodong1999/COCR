...
//    xgd::sout<<dw<<","<<dh<<","<<ret.cols<<","<<ret.rows<<std::endl;
    result.sync();
...
//    xgd::sout << get_bw_tl_br() << std::endl;
//    xgd::sout << get_bw_bl_tr() << std::endl;
...
static cv::Mat convertQImageToMat(const QImage &_img) {
    cv::Mat mat;
    if (_img.isNull()) {
        xgd::serr << "you are converting an empty QImage to cv::Mat" << std::endl;
        exit(-1);
    }
    ...
    switch (_img.format()) {
        ...
        case QImage::Format_Indexed8:
            ...
            break;
        default: {
            xgd::serr << "unImpl image format:" << _img.format() << std::endl;
            exit(-1);
        }
    }
//    xgd::sout << mat.cols << "x" << mat.rows << std::endl;
    return std::move(mat);
}
