...
void JMol::display() {
    ...
    loopAtomVec([&](Atom &atom) {
        xgd::serr << atom.getId() << ":" << atom.getName().c_str() << std::endl;
        if (is2DInfoLatest) {
            xgd::serr << "2d(" << atom.x << "," << atom.y << ")" << std::endl;
        }
        if (is3DInfoLatest) {
            xgd::serr << "3d(" << atom.xx << "," << atom.yy << atom.zz << ")" << std::endl;
        }
    });
    loopBondVec([&](Bond &bond) {
        xgd::serr << bond.getId() << ":" << bond.getBondOrder() << ","
...
void JMol::norm3D(const float &_xx, const float &_yy, const float &_zz,
    ...
    if (!is3DInfoLatest) {
        bool success = generate3D();
        xgd::serr << __FUNCTION__ << "generate3D=" << success << std::endl;
    }
...
int JMol::getNumHydrogen(const id_type &_aid) {
    ...
    // xgd::serr << atom->getName() << numHs << std::endl;
    return numHs;
}
