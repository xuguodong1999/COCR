...
std::pair<atom_t, atom_t> JMol_p::makeAbbType(const TokenType &_abb) {
    xgd::serr << __FUNCTION__ << static_cast<int>(_abb) << std::endl;
    switch (_abb) {
        case TokenType::Me: {
            return makeAlkane(1);
        }
...
//        xgd::sout << "offset=" << offset << std::endl;
        ...
        if (isNumberToken(its->second)) {
            ...
            elementMap[tokenVec.size() - 1] = ElementUtil::convertStringToElementType(subStr);
        }
        it += offset;
//        xgd::sout << subStr << std::endl;
    }
...
std::pair<atom_t, atom_t> JMol_p::extractNoBracketTokens(
    ...
    xgd::serr << __FUNCTION__ << "suffix=" << suffix << std::endl;
            ...
            if (isElementToken(curToken)) {
                ...
                if (i + 1 <= iEnd) {
                    ...
                    if (isNumberToken(nextToken)) {
                        number = numbers[i + 1];
                        xgd::serr << "number=" << number << std::endl;
                        ...
                        if (a1 && a1->getCommonNebNum() > 1) {
                            // 如果遇到可续接的原子，更新主原子信息
                            xgd::serr << "bind" << a1->getName() << std::endl;
                            last_ele = a1;
                        }
                    }
                } else {
                ...
                // 拼接缩略词
                xgd::serr << __FUNCTION__ << "isAbbToken:" << (int) curToken << std::endl;
...
bool ckit_deprecated::JMol_p::tryExpand(const id_type &_aid) {
    ...
    xgd::serr << __FUNCTION__ << inputName.c_str() << std::endl;
    auto opt = interpret(inputName);
    if (!opt) { return false; }
    auto &tokenStruct = opt.value();
    xgd::serr << "atom->isLeftToRight=" << atom->isLeftToRight() << std::endl;
...
//    xgd::serr << "element" << (int) elements[i];
//        } else if (isNumberToken(token)) {
//    xgd::serr << "number" << numbers[i];
//        } else {
//    xgd::serr << "token" << (int) token;
//        }
//    }
    xgd::serr << "**********" << std::endl;
    ...
    if (atom && a_end) {
        ...
        xgd::serr << "bonds.size()=" << bonds.size();
        if (bonds.size() > 1) {
...
//            mol.rebuildAllData();
    ...
    xgd::serr << __FUNCTION__ << "return true";
    return true;
}
...
bool JMol_p::reverseTokens(JMol_p::token_struct &tokenStruct) {
    ...
    for (int i = tokens.size() - 1; i >= 0; i--) {
        ...
        if (isNumberToken(curToken)) {
            ...
            if (numCache != -1 && ElementUtil::GetElementNebNum(elements[i]) == 1) {
    ...
    if (numbers2.size() == numbers.size() &&
        ...
        xgd::serr << __FUNCTION__ << "ret true";
        return true;
    }
    xgd::serr << __FUNCTION__ << "ret false" << numbers2.size() << numbers.size()
              << elements2.size() << elements.size()
              << tokens2.size() << tokens.size();
    return false;
}
