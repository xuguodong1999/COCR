...
bool Equation::intoMatter() {
    ...
    if (!checkFormat()) {
#ifdef TESTCEI
        xgd::sout << "不满足方程的格式：XXX=...=XXX！" << endl;
#endif
        return 0;
    }
    ...
    if (!intoElement()) {
#ifdef TESTCEI
        xgd::sout << "字符串拆分失败！未完成元素级拆分。" << endl;
#endif
        return 0;
    }
    return 1;
...
#ifdef TESTCEI
            xgd::sout << "字串 " << leftMatters[i].name << " 解析失败！" << endl;
#endif
            return 0;
        }
    for (int i = 0; i < (int) rightMatters.size(); i++)
...
#ifdef TESTCEI
            xgd::sout << "字串 " << rightMatters[i].name << " 解析失败！" << endl;
#endif
            return 0;
        }
    //  反应物元素累加、排序、合并
...
#ifdef TESTCEI
            xgd::sout << "无法获得 " << leftMatters[i].name << " 的化合价，配平失败！" << endl;
#endif
            return 0;
        }
    for (i = 0; i < cp; i++)
...
#ifdef TESTCEI
            xgd::sout << "无法获得 " << rightMatters[i].name << " 的化合价，配平失败！" << endl;
#endif
            return 0;
        }
...
//                tmp.b = 1;
                tmp.assign(leftMatters[i].eleList[j].amount, 1);
                redox[i] =
    ...
    for (int i = 0; i < cr + cp; i++)
        printf("%4d", fracToBase<int>(redox[i]));
    xgd::sout << "      ]" << endl;
#endif
    return 1;
}
...
bool Equation::exec(const string &_content) {
    mInput = boost::algorithm::trim_copy(_content);
    return exec();
}
...
bool Equation::exec() {
    if (!intoMatter()) {
#ifdef TESTCEI
        xgd::sout << "字符串拆分失败！未完成物质级拆分。" << endl;
#endif
        return 0;
    }
    if (!balance()) {
#ifdef TESTCEI
        xgd::sout << "方程式配平失败！" << endl;
#endif
        return 0;
    }
    addStateInfo();
