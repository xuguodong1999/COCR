...
std::shared_ptr<HwBond> HwBond::GetHwBond(const BondType &_bondType) {
    switch (_bondType) {
        ...
        case BondType::DelocalizedBond:// only draw skeleton here, write circle on another session
        ...
        default: {
            xgd::serr << "Bond::GetBond: unknown bond type "
...
void HwCircleBond::setVertices(const std::vector<point2f> &_pts) {
    if (_pts.size() < 2) {
        xgd::serr << "pts for circle should be more than 1, but "
...
void HwSingleBond::setVertices(const std::vector<point2f> &_pts) {
    if (_pts.size() != 2) {
        xgd::serr << "HwSingleBond::setVertices: "
                  << _pts.size() << " pts got." << std::endl;
        exit(-1);
    }
    from = _pts[0];
