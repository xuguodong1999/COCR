#include "CouchSymbolLabel.h" // const std::map<std::string, int> sStr2IntMap
...
HwScript HwDataLoader::GetByIntLabel(int label) {
    if (label >= mData.size()) {
        xgd::serr << "label " << label << " out of boundary: [0," <<
                  mData.size() << ")" << std::endl;
        exit(-1);
    }
...
HwScript HwDataLoader::GetShape(const ShapeType &_shapeType) {
    switch (_shapeType) {
        ...
        default: {
            xgd::serr << (int) _shapeType << " not supported! "
...
void HwDataLoader::LoadCouchDataSet(const std::string &COUCH_SYM_PATH) {
    mData.clear();
    std::ifstream ism(COUCH_SYM_PATH, std::ios::in | std::ios::binary);
    if (!ism.is_open()) {
        xgd::serr << "fail to load " << COUCH_SYM_PATH << " with ifstream" << std::endl;
        exit(-1);
    }
    int a, b, c, d, e, iii = 0;
    // 【标签索引】、【宽度】、【长度】、【笔画数目】
...
HwDataLoader &HwDataLoader::getInstance() {
    ...
    if (!e.isDataLoaded) {
        ...
        dataPath += "/datasets/couch-subset/data.dat";
        e.LoadCouchDataSet(dataPath);
        e.isDataLoaded = true;
    }
    return e;
}
