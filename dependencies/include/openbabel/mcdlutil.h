...
namespace OpenBabel {
  ...
  OB_EXPORT int hydrogenValency(int na);
  OB_EXPORT int maxValency(int na);

  //Alternate overloaded methods
  OB_EXPORT int alternate(OBMol * pmol, const int nH[], int bondOrders []);  //This method does not work!
  //Zero-based atomic numeration should be in connection matrix arrays iA1 and iA2-so first atom has indez zero
  OB_EXPORT int alternate(const std::vector<int> aPosition, const std::vector<int> aCharge,
  ...
  OB_EXPORT void generateDiagram(OBMol * pmol);
  //Zero-based atomic numeration should be in connection matrix arrays iA1 and iA2-so first atom has indez zero
  OB_EXPORT void generateDiagram(const std::vector<int> iA1, const std::vector<int> iA2,
                                 std::vector<double>& rx, std::vector<double>& ry, int nAtoms, int nBonds);
  OB_EXPORT void generateDiagram(OBMol * pmol, std::ostream & ofs);  //for testing purposes only
  ...
  OB_EXPORT void equivalenceList(OBMol * pmol, std::vector<int>& eqList);
  ...
  //Fragment addition
  OB_EXPORT void addFragment(OBMol * molecule, OBMol * fragment, int molAN, int fragAN, int molBN,
                             int fragBN, bool isAddition);

  //routines below have no common meaning, but are necessary to process stereo information
  OB_EXPORT void createStereoLists(OBMol * pmol, std::vector<int>& bondStereoList,
                                   std::vector<int>& atomStereoList, std::vector<int>& eqList);
  OB_EXPORT std::string getAtomMCDL(OBMol * pmol, int ntatoms, const std::vector<int> ix,
      const std::vector<int> aNumber, const std::vector<int> atomStereoList, const std::vector<int> eqList);
  OB_EXPORT std::string getBondMCDL(OBMol * pmol, int nbStore, int ntatoms, const std::vector<int> ix,
      const std::vector<int> aNumber, int bonds[MAXBONDS][4], const std::vector<int> bondStereoList,
      const std::vector<int> eqList);
  OB_EXPORT void implementAtomStereo(std::vector<int>& iA1, std::vector<int>& iA2, std::vector<int>& stereoBonds,
                                     const std::vector<double>rx, const std::vector<double> ry, int acount, int bcount, std::string astereo);
  OB_EXPORT void implementBondStereo(const std::vector<int> iA1, const std::vector<int> iA2,
                                     std::vector<double>& rx, std::vector<double>& ry, int acount, int bcount, std::string bstereo);


  OB_EXPORT int groupRedraw(OBMol * pmol, int bondN, int atomN, bool atomNInGroup);
  //int  groupRedrawFrameAtom(OBMol * pmol, int bondN, int atomInFrame);

  OB_EXPORT int  canonizeMCDL(const std::string atomBlock, std::vector<std::string> & structureList);
  OB_EXPORT bool parseFormula(const std::string formulaString, std::vector <int>& enumber, int & valency);

  OB_EXPORT void prepareTest(OBMol * pmol, std::ostream & ofs);
