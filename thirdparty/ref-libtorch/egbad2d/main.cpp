...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, GAN_Encoder &enc, GAN_Generator &gen, GAN_Discriminator &dis,
           std::vector<transforms_Compose > &transform);

void test(po::variables_map &vm, torch::Device &device, GAN_Encoder &enc, GAN_Generator &gen, GAN_Discriminator &dis,
          std::vector<transforms_Compose > &transform);
...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("valid_sigma_max", po::value<float>()->default_value(3.0),
             "maximum value of latent variable for output images in validation")
            ("valid_sigma_inter", po::value<float>()->default_value(0.5),
             "the interval of latent variable for output images in validation")
            ...
            ("test_Lambda", po::value<float>()->default_value(0.1),
             "anomaly score rate between reconstruction and feature matching in test")
            ...
            ("lr_enc", po::value<float>()->default_value(1e-3), "learning rate for encoder")
            ...
            ("E_ltw", po::value<float>()->default_value(0.5),
             "the weight of loss threshold for stable training in encoder : x=0.0 is normal training")
            ("G_ltw", po::value<float>()->default_value(0.5),
             "the weight of loss threshold for stable training in generator : x=0.0 is normal training")
            ("D_ltw", po::value<float>()->default_value(0.5),
             "the weight of loss threshold for stable training in discriminator : x=0.0 is normal training")
            ...
            ("nef", po::value<size_t>()->default_value(64),
             "the number of filters in convolution layer closest to image in encoder")
            ("ngf", po::value<size_t>()->default_value(64),
             "the number of filters in convolution layer closest to image in generator")
            ("ndf", po::value<size_t>()->default_value(64),
             "the number of filters in convolution layer closest to image in discriminator");
...
int main(int argc, const char *argv[]) {
    ...
    GAN_Encoder enc(vm);
    enc->to(device);
    GAN_Generator gen(vm);
    gen->to(device);
    GAN_Discriminator dis(vm);
    dis->to(device);
    ...
    if (vm["train"].as<bool>()) {
        ...
        train(vm, device, enc, gen, dis, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()) {
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, enc, gen, dis, transform);
    }
