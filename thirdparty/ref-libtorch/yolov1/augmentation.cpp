...
// ------------------------------------------------------------------
YOLOAugmentationImpl::YOLOAugmentationImpl(const double jitter_, const double flip_rate_, const double scale_rate_,
                                           const double blur_rate_, const double brightness_rate_,
                                           const double hue_rate_, const double saturation_rate_,
                                           const double shift_rate_, const double crop_rate_) {
    this->jitter = jitter_;
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::deepcopy(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_flip(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                       cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_scale(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                        cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_blur(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                       cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_brightness(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                             cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_hue(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                      cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_saturation(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                             cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_shift(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                        cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
    ...
    for (j = 0; j < height; j++) {
        for (i = 0; i < width; i++) {
            ...
            if ((i_in < 0) || (width <= i_in) || (j_in < 0) || (height <= j_in)) {
                ...
                for (k = 0; k < channels; k++) {
                    data_out1_ptr[j * width * channels + i * channels + k] = data_in1_ptr[j_in * width * channels +
                                                                                          i_in * channels + k];
                }
            }
        }
    ...
    if (std::get<0>(data_in2).numel() > 0) {
        ...
        if (x_min_out.numel() > 0) {
            ...
            coords_out = torch::cat(
                    {cx_out.unsqueeze(1), cy_out.unsqueeze(1), w_out.unsqueeze(1), h_out.unsqueeze(1)}, /*dim=*/
                    1).contiguous();  // coords_out{object,4}
            data_out2 = {ids_out.detach().clone(),
                         coords_out.detach().clone()};  // {object} (ids), {object,4} (coordinates)
        } else {
            data_out2 = {torch::Tensor(), torch::Tensor()};
        }
    } else {
...
// --------------------------------------------------------------------------
void YOLOAugmentationImpl::random_crop(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                       cv::Mat &data_out1, std::tuple<torch::Tensor, torch::Tensor> &data_out2) {
...
// -----------------------------------------------------------------
void
YOLOAugmentationImpl::forward(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2, cv::Mat &data_out1,
