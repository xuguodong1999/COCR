...
test(po::variables_map &vm, torch::Device &device, MC_Discriminator &model, std::vector<transforms_Compose > &transform,
    ...
    dataloader = DataLoader::ImageFolderClassesWithPaths(dataset, /*batch_size_=*/1, /*shuffle_=*/
    ...
    while (dataloader(data)) {
        ...
        std::cout << '<' << std::get<2>(data).at(0) << "> cross-entropy:" << loss.item<float>() << " judge:" << judge
                  << " response:" << response << '(' << class_names.at(response) << ") answer:" << answer << '('
                  << class_names.at(answer) << ')' << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> cross-entropy:" << loss.item<float>() << " judge:" << judge
            << " response:" << response << '(' << class_names.at(response) << ") answer:" << answer << '('
            << class_names.at(answer) << ')' << std::endl;
    ...
    // (8) Average Output
    std::cout << "<All> cross-entropy:" << ave_loss << " accuracy:" << accuracy << " (time:" << ave_time << ')'
              << std::endl;
