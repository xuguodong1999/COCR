...
// Test Function
// ---------------
void test(po::variables_map &vm, torch::Device &device, SegNet &model, std::vector<transforms_Compose > &transformI,
    ...
    dataloader = DataLoader::ImageFolderSegmentWithPaths(dataset, /*batch_size_=*/1, /*shuffle_=*/
                                                         false, /*num_workers_=*/0);
    std::cout << "total test images : " << dataset.size() << std::endl << std::endl;

    // (2) Get Model
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + ".pth";
    ...
    while (dataloader(data)) {
        ...
        for (size_t i = 0; i < std::get<4>(data).size(); i++) {
            answer_mask = torch::full({label.size(0), label.size(1), label.size(2)}, /*value=*/(long int) i,
                                      torch::TensorOptions().dtype(torch::kLong)).to(device);
            total_class_pixel = (label == answer_mask).sum().item<long int>();
            if (total_class_pixel != 0) {
                response_mask = torch::full({label.size(0), label.size(1), label.size(2)}, /*value=*/2,
                                            torch::TensorOptions().dtype(torch::kLong)).to(device);
                correct_per_class = (
                        ((label == output_argmax).to(torch::kLong) + (label == answer_mask).to(torch::kLong)) ==
                        response_mask).sum().item<long int>();
        ...
        std::cout << '<' << std::get<2>(data).at(0) << "> cross-entropy:" << loss.item<float>()
                  << " pixel-wise-accuracy:" << pixel_wise_accuracy << " mean-accuracy:" << mean_accuracy << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> cross-entropy:" << loss.item<float>() << " pixel-wise-accuracy:"
            << pixel_wise_accuracy << " mean-accuracy:" << mean_accuracy << std::endl;
    ...
    // (7) Average Output
    std::cout << "<All> cross-entropy:" << ave_loss << " pixel-wise-accuracy:" << ave_pixel_wise_accuracy
              << " mean-accuracy:" << ave_mean_accuracy << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> cross-entropy:" << ave_loss << " pixel-wise-accuracy:" << ave_pixel_wise_accuracy << " mean-accuracy:"
        << ave_mean_accuracy << " (time:" << ave_time << ')' << std::endl;
