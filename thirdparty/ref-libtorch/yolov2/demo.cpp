...
void demo(po::variables_map &vm, torch::Device &device, YOLOv2 &model, std::vector<transforms_Compose > &transformI,
    ...
    while (cap.read(BGR)) {
        ...
        RGB.copyTo(imageI);
        tensorI = transforms::apply(transformI, imageI).unsqueeze(/*dim=*/0).to(
                device);  // imageI{H_D,W_D,C} ==={Resize,ToTensor,etc.}===> tensorI{1,C,H,W}
        ...
        detect_result = detector(
                tensorO[0]);  // tensorO[0]{G,G,FF} ===> detect_result{ (ids{BB_n}, coords{BB_n,4}, probs{BB_n}) }
        imageO = visualizer::draw_detections_des(tensorD.detach(),
        ...
        ss1.str("");
        ss1.clear(std::stringstream::goodbit);
        ss2.str("");
        ss2.clear(std::stringstream::goodbit);
