...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("SPN_salt_rate", po::value<float>()->default_value(0.5),
             "salt noise probability of occurrence in total noise")
...
int main(int argc, const char *argv[]) {
    ...
    if (vm["GN"].as<bool>()) {
        transformI.push_back(transforms_AddGaussNoise(vm["GN_prob"].as<float>(), vm["GN_mean"].as<float>(),
                                                      vm["GN_std"].as<float>()));
    }
    transformI.push_back(
