...
torch::Tensor jaccard(torch::Tensor _box_a, torch::Tensor _box_b) {
    ...
    //	cout << e.what() << endl;
    //}
    auto max_xy = torch::min(box_a.narrow(1, 2, 2).unsqueeze(1).expand({A, B, 2}),
                             box_b.narrow(1, 2, 2).unsqueeze(0).expand({A, B, 2}));
    auto min_xy = torch::max(box_a.narrow(1, 0, 2).unsqueeze(1).expand({A, B, 2}),
                             box_b.narrow(1, 0, 2).unsqueeze(0).expand({A, B, 2}));

    auto inter = torch::clamp((max_xy - min_xy), 0);
    inter = inter.select(2, 0) * inter.select(2, 1);

    //计算先验框和真实框各自的面积
    auto area_a = ((box_a.select(1, 2) - box_a.select(1, 0)) * (box_a.select(1, 3) - box_a.select(1, 1))).unsqueeze(
            1).expand_as(inter); // [A, B]
    auto area_b = ((box_b.select(1, 2) - box_b.select(1, 0)) * (box_b.select(1, 3) - box_b.select(1, 1))).unsqueeze(
            0).expand_as(inter);  // [A, B]
...
torch::Tensor box_ciou(torch::Tensor b1, torch::Tensor b2) {
    ...
    auto v = (4 / (Pi * Pi)) * torch::pow((torch::atan(b1_wh.select(-1, 0) / b1_wh.select(-1, 1)) -
                                           torch::atan(b2_wh.select(-1, 0) / b2_wh.select(-1, 1))), 2);
...
std::vector<torch::Tensor> YOLOLossImpl::forward(torch::Tensor input, std::vector<torch::Tensor> targets) {
    ...
    auto prediction = input.view({bs, int(num_anchors / 2), bbox_attrs, in_h, in_w}).permute(
    ...
    auto loss_cls = torch::sum(
            BCELoss(pred_cls.index({mask == 1}), smooth_label(tcls.index({mask == 1}), label_smooth, num_classes)) /
            bs);
...
YOLOLossImpl::get_target(std::vector<torch::Tensor> targets, torch::Tensor scaled_anchors, int in_w, int in_h,
    ...
    std::vector<std::vector<int>> anchor_vec_in_vec = {{3, 4, 5},
                                                       {0, 1, 2}};
    ...
    for (int b = 0; b < bs; b++) {
        ...
        //计算真实框的位置
        auto gt_box = torch::Tensor(torch::cat({torch::zeros_like(gws), torch::zeros_like(ghs), gws, ghs}, 1)).to(
                torch::kFloat32);

        //计算出所有先验框的位置
        auto anchor_shapes = torch::Tensor(
                torch::cat({torch::zeros({num_anchors, 2}).to(scaled_anchorsType), torch::Tensor(scaled_anchors)},
                           1)).to(TensorType);
        //计算重合程度
        ...
        for (int i = 0; i < best_ns.sizes()[0]; i++) {
            ...
            if (gj < in_h && gi < in_w) {
                auto best_n = vec_index(anchor_index,
                                        best_ns[i].item().toInt());// (best_ns[i] - subtract_index).item().toInt();
                //判定哪些先验框内部真实的存在物体
...
std::vector<torch::Tensor>
YOLOLossImpl::get_ignore(torch::Tensor prediction, std::vector<torch::Tensor> targets, torch::Tensor scaled_anchors,
                         int in_w, int in_h, torch::Tensor noobj_mask) {
    int bs = targets.size();
    int index = vec_index(feature_length, in_w);
    std::vector<std::vector<int>> anchor_vec_in_vec = {{3, 4, 5},
                                                       {0, 1, 2}};
...
std::vector<torch::Tensor>
non_maximum_suppression(torch::Tensor prediction, int num_classes, float conf_thres, float nms_thres) {
    ...
    for (int image_id = 0; image_id < prediction.sizes()[0]; image_id++) {
        ...
        for (auto c: img_classes) {
            ...
            auto keep = nms_libtorch(detections_class.narrow(-1, 0, 4),
                                     detections_class.select(-1, 4) * detections_class.select(-1, 5), nms_thres);
...
std::vector<int> nms_libtorch(torch::Tensor bboxes, torch::Tensor scores, float thresh) {
    ...
    while (order.numel() > 0) {// torch.numel()返回张量元素个数
        ...
        auto iou =
                inter / (areas[keep.back()] + areas.index({order.narrow(0, 1, order.size(-1) - 1)}) - inter);//[N - 1, ]
