...
// -----------------------------------------------
void datasets::collect(const std::string root, const std::string sub, std::vector<std::string> &paths,
...
// -------------------------------------------------------------------------
datasets::Data1dFolderWithPaths::Data1dFolderWithPaths(const std::string root,
                                                       std::vector<transforms_Compose > &transform_) {
...
void datasets::Data1dFolderWithPaths::get(const size_t idx, std::tuple<torch::Tensor, std::string> &data) {
    ...
    torch::Tensor tensor_out = transforms::applyT(this->transform,
                                                  tensor_in);  // Tensor Data ==={Normalize,etc.}===> Tensor Data
...
// -------------------------------------------------------------------------
datasets::Data1dFolderPairWithPaths::Data1dFolderPairWithPaths(const std::string root1, const std::string root2,
...
// -------------------------------------------------------------------------
void datasets::Data1dFolderPairWithPaths::get(const size_t idx,
                                              std::tuple<torch::Tensor, torch::Tensor, std::string, std::string> &data) {
    ...
    torch::Tensor tensor1_out = transforms::applyT(this->transformI,
                                                   tensor1_in);  // Tensor Data ==={Normalize,etc.}===> Tensor Data
    torch::Tensor tensor2_out = transforms::applyT(this->transformO,
                                                   tensor2_in);  // Tensor Data ==={Normalize,etc.}===> Tensor Data
...
// -------------------------------------------------------------------------
datasets::ImageFolderWithPaths::ImageFolderWithPaths(const std::string root,
                                                     std::vector<transforms_Compose > &transform_) {
    datasets::collect(root, "", this->paths, this->fnames);
    std::sort(this->paths.begin(), this->paths.end());
    std::sort(this->fnames.begin(), this->fnames.end());
...
void datasets::ImageFolderWithPaths::get(const size_t idx, std::tuple<torch::Tensor, std::string> &data) {
    ...
    torch::Tensor image = transforms::apply(this->transform,
                                            image_Mat);  // Mat Image ==={Resize,ToTensor,etc.}===> Tensor Image
    std::string fname = this->fnames.at(idx);
...
// -------------------------------------------------------------------------
datasets::ImageFolderPairWithPaths::ImageFolderPairWithPaths(const std::string root1, const std::string root2,
                                                             std::vector<transforms_Compose > &transformI_,
                                                             std::vector<transforms_Compose > &transformO_) {
...
// -------------------------------------------------------------------------
void datasets::ImageFolderPairWithPaths::get(const size_t idx,
                                             std::tuple<torch::Tensor, torch::Tensor, std::string, std::string> &data) {
    cv::Mat image_Mat1 = datasets::RGB_Loader(this->paths1.at(idx));
    cv::Mat image_Mat2 = datasets::RGB_Loader(this->paths2.at(idx));
    torch::Tensor image1 = transforms::apply(this->transformI,
                                             image_Mat1);  // Mat Image ==={Resize,ToTensor,etc.}===> Tensor Image
    torch::Tensor image2 = transforms::apply(this->transformO,
                                             image_Mat2);  // Mat Image ==={Resize,ToTensor,etc.}===> Tensor Image
    std::string fname1 = this->fnames1.at(idx);
    std::string fname2 = this->fnames2.at(idx);
...
// ----------------------------------------------------------------------------------------
datasets::ImageFolderPairAndRandomSamplingWithPaths::ImageFolderPairAndRandomSamplingWithPaths(const std::string root1,
...
// ------------------------------------------------------------------------------------------
void datasets::ImageFolderPairAndRandomSamplingWithPaths::get(const size_t idx, const size_t idx_rand,
    ...
    torch::Tensor image2 = transforms::apply(this->transformO,
                                             image_Mat2);  // Mat Image ==={Resize,ToTensor,etc.}===> Tensor Image
    torch::Tensor image_rand = transforms::apply(this->transform_rand,
                                                 image_Mat_rand);  // Mat Image ==={Resize,ToTensor,etc.}===> Tensor Image
    std::string fname1 = this->fnames1.at(idx);
    std::string fname2 = this->fnames2.at(idx);
...
// -------------------------------------------------------------------------
void datasets::ImageFolderSegmentWithPaths::get(const size_t idx,
...
// -------------------------------------------------------------------------
datasets::ImageFolderClassesWithPaths::ImageFolderClassesWithPaths(const std::string root,
                                                                   std::vector<transforms_Compose > &transform_,
...
// -------------------------------------------------------------------------
void datasets::ImageFolderClassesWithPaths::get(const size_t idx,
                                                std::tuple<torch::Tensor, torch::Tensor, std::string> &data) {
    cv::Mat image_Mat = datasets::RGB_Loader(this->paths.at(idx));
    torch::Tensor image = transforms::apply(this->transform,
                                            image_Mat);  // Mat Image ==={Resize,ToTensor,etc.}===> Tensor Image
    torch::Tensor class_id = torch::full({}, (long int) this->class_ids.at(idx),
...
// -------------------------------------------------------------------------
datasets::ImageFolderBBWithPaths::ImageFolderBBWithPaths(const std::string root1, const std::string root2,
...
// --------------------------------------------------------------------------
void datasets::ImageFolderBBWithPaths::deepcopy(cv::Mat &data_in1, std::tuple<torch::Tensor, torch::Tensor> &data_in2,
                                                cv::Mat &data_out1,
...
// -------------------------------------------------------------------------
void datasets::ImageFolderBBWithPaths::get(const size_t idx,
