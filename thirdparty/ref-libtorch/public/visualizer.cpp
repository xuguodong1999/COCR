...
// ----------------------------------------------------------
void visualizer::save_image(const torch::Tensor image, const std::string path, const std::pair<float, float> range,
                            const size_t cols, const size_t padding, const size_t bits) {
    ...
    auto mini_batch = image.clamp(/*min=*/range.first, /*max=*/range.second).to(torch::kCPU).chunk(
            mini_batch_size, /*dim=*/0);  // {N,C,H,W} ===> {1,C,H,W} + {1,C,H,W} + ...
    for (auto &tensor: mini_batch) {
        ...
        float_mat = cv::Mat(cv::Size(width, height), mtype_in,
                            tensor_con.data_ptr<float>());  // torch::Tensor ===> cv::Mat
        normal_mat = (float_mat - range.first) /
                     (float) (range.second - range.first);  // [range.first, range.second] ===> [0,1]
        ...
        for (j = 0; j < height; j++) {
            for (i = 0; i < width; i++) {
                for (k = 0; k < sample.elemSize(); k++) {
                    output.data[(j + j_dev) * output.step + (i + i_dev) * output.elemSize() + k] = sample.data[
                            j * sample.step + i * sample.elemSize() + k];
                }
            }
        }
    }
...
// ----------------------------------------------------------
void visualizer::save_label(const torch::Tensor label, const std::string path,
                            const std::vector<std::tuple<unsigned char, unsigned char, unsigned char>> label_palette,
                            const size_t cols, const size_t padding) {
    ...
    i = 0;
    samples = std::vector<cv::Mat>(mini_batch_size);
    auto mini_batch = label.to(torch::kCPU).chunk(mini_batch_size, /*dim=*/
                                                  0);  // {N,1,H,W} ===> {1,1,H,W} + {1,1,H,W} + ...
    for (auto &tensor: mini_batch) {
...
// ----------------------------------------------------------
cv::Mat visualizer::draw_detections(const torch::Tensor image, const std::tuple<torch::Tensor, torch::Tensor> label,
    ...
    if (BB_n > 0) {
        for (size_t b = 0; b < BB_n; b++) {
            ...
            cv::rectangle(sample, cv::Point(x_min, y_min), cv::Point(x_max, y_max), cv::Scalar(B, G, R), /*thickness=*/
                          line_thickness);

            // (4.3) Draw class names
            class_name = class_names.at(id);
            text_size = cv::getTextSize(class_name, cv::FONT_HERSHEY_SIMPLEX, /*fontScale=*/font_size, /*thickness=*/
                                        text_thickness, &baseline);
            cv::rectangle(sample, cv::Point(x_min, y_min), cv::Point(x_min + text_size.width + line_thickness,
                                                                     y_min + text_size.height + line_thickness +
                                                                     baseline), cv::Scalar(B, G, R), /*thickness=*/-1);
            cv::putText(sample, class_name, cv::Point(x_min + line_thickness, y_min + 2 * baseline + line_thickness),
                        cv::FONT_HERSHEY_SIMPLEX, /*fontScale=*/font_size, cv::Scalar(0, 0, 0), /*thickness=*/
                        text_thickness, /*lineType=*/cv::LINE_8);
...
// ----------------------------------------------------------
cv::Mat visualizer::draw_detections_des(const torch::Tensor image, const std::tuple<torch::Tensor, torch::Tensor> label,
    ...
    if (BB_n > 0) {
        ...
        probs_sorted_with_idx = prob.sort(/*dim=*/0, /*descending=*/
                                                  false);  // probs_sorted_with_idx(probs{object}, idx{object})
        ...
        coords_sorted = coords.gather(/*dim=*/0, /*index=*/probs_idx.unsqueeze(/*dim=*/-1).expand(
                {probs_idx.size(0), 4})).contiguous();  // coords_sorted{object,4}
        ...
        for (size_t b = 0; b < BB_n; b++) {
            ...
            line_thickness =
                    line_thickness_min + (size_t) ((double) (line_thickness_max - line_thickness_min) * rate + 0.5);
            text_thickness =
                    text_thickness_min + (size_t) ((double) (text_thickness_max - text_thickness_min) * rate + 0.5);
...
void visualizer::graph::plot(const float base, const std::vector<float> value) {
    ...
    if (this->flag) {
        ...
        for (size_t i = 0; i < this->label.size() - 1; i++) {
            fprintf(gp, "'%s' using 1:%zu ti '%s' with lines,", this->data_fname.c_str(), i + 2,
                    this->label.at(i).c_str());
        }
        fprintf(gp, "'%s' using 1:%zu ti '%s' with lines\n", this->data_fname.c_str(), this->label.size() + 1,
                this->label.at(this->label.size() - 1).c_str());
        pclose(gp);
    }
