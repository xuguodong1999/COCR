...
EncoderImpl::EncoderImpl(po::variables_map &vm) {
    ...
    DownSampling(this->model, vm["nc"].as<size_t>(), feature, /*BN=*/false, /*ReLU=*/
                 true);       // {C,256,256} ===> {F,128,128}
    DownSampling(this->model, feature, feature * 2, /*BN=*/true, /*ReLU=*/
                 true);                    // {F,128,128} ===> {2F,64,64}
    DownSampling(this->model, feature * 2, feature * 4, /*BN=*/true, /*ReLU=*/
                 true);                  // {2F,64,64}  ===> {4F,32,32}
    DownSampling(this->model, feature * 4, feature * 8, /*BN=*/true, /*ReLU=*/
                 true);                  // {4F,32,32}  ===> {8F,16,16}
    for (size_t i = 0; i < num_downs - 5; i++) {
        DownSampling(this->model, feature * 8, feature * 8, /*BN=*/true, /*ReLU=*/
                     true);              // {8F,16,16}  ===> {8F,2,2}
    }
    DownSampling(this->model, feature * 8, vm["nz_c"].as<size_t>(), /*BN=*/false, /*ReLU=*/
                 false);  // {8F,2,2}    ===> {ZC,1,1}
...
DecoderImpl::DecoderImpl(po::variables_map &vm) {
    ...
    UpSampling(this->model, vm["nz_c"].as<size_t>(), feature * 8, /*BN=*/true, /*ReLU=*/
               true);    // {ZC,1,1}     ===> {8F,2,2}
    for (size_t i = 0; i < num_downs - 5; i++) {
...
void EstimationNetworkImpl::estimation(torch::Tensor z, torch::Tensor gamma_ap) {
    ...
    torch::Tensor mu = torch::sum(gamma_ap.unsqueeze(2) * z.unsqueeze(1), /*dim=*/0) /
                       gamma_sum.unsqueeze(1);  // gamma_ap{N,K,1}, z{N,1,Z}, gamma_sum{K,1} ===> mu{K,Z}
    torch::Tensor z_dev = z.unsqueeze(1) - mu.unsqueeze(0);  // z{N,1,Z}, mu{1,K,Z} ===> z_dev{N,K,Z}
    torch::Tensor z_dev_mat =
            z_dev.unsqueeze(3) * z_dev.unsqueeze(2);  // z_dev{N,K,Z,1}, z_dev{N,K,1,Z} ===> z_dev_mat{N,K,Z,Z}
    torch::Tensor sigma = torch::sum(gamma_ap.unsqueeze(-1).unsqueeze(-1) * z_dev_mat, /*dim=*/0) /
                          gamma_sum.unsqueeze(-1).unsqueeze(
                                  -1);  // gamma_ap{N,K,1,1}, z_dev_mat{N,K,Z,Z}, gamma_sum{K,1,1} ===> sigma{K,Z,Z}
    torch::Tensor phi = gamma_sum / gamma_ap.size(0);  // gamma_sum{K} ===> phi{K}

    torch::Tensor sigma_k =
            sigma[0] + (torch::eye(nz) * this->eps).detach().to(z.device());  // sigma[i]{Z,Z} ===> sigma_k{Z,Z}
    ...
    torch::Tensor det_sigma =
            torch::det(2.0 * PI * sigma_k).unsqueeze(0) + this->eps;  // sigma_k{Z,Z} ===> det_sigma{1}
    for (size_t i = 1; i < nk; i++) {
        ...
        sigma_inv = torch::cat({sigma_inv, torch::inverse(sigma_k).unsqueeze(0)}, /*dim=*/
                               0);  // sigma_inv{i,Z,Z} + {1,Z,Z} ===> sigma_inv{i+1,Z,Z}
        det_sigma = torch::cat({det_sigma, torch::det(2.0 * PI * sigma_k).unsqueeze(0) + this->eps}, /*dim=*/
                               0);  // det_sigma{i} + {1} ===> det_sigma{i+1}
    }

    torch::Tensor exp_term_left = torch::sum(z_dev.unsqueeze(3) * sigma_inv.unsqueeze(0), /*dim=*/
                                             2);  // z_dev{N,K,Z,1}, sigma_inv{1,K,Z,Z} ===> exp_term_left{N,K,Z}
    torch::Tensor exp_term_base = -0.5 * torch::sum(exp_term_left * z_dev, /*dim=*/
                                                    2);  // exp_term_left{N,K,Z}, z_dev{N,K,Z} ===> exp_term_base{N,K}
    torch::Tensor max_value = std::get<0>(torch::max(exp_term_base.clamp(/*min=*/0.0), /*dim=*/1, /*keepdim=*/
                                                     true));  // exp_term_base{N,K} ===> max_value{N,1}  (Note: To prevent overflow in "logsumexp" calculation.)
    torch::Tensor exp_term = torch::exp(
            exp_term_base - max_value);  // exp_term_base{N,K}, max_value{N,1} ===> exp_term{N,K}
    torch::Tensor energy = -max_value.squeeze(1) - torch::log(
            torch::sum(phi.unsqueeze(0) * exp_term / torch::sqrt(det_sigma).unsqueeze(0), /*dim=*/1) +
            this->eps);  // max_value{N}, phi{1,K}, exp_term{N,K}, det_sigma{1,1} ===> energy{N}
...
void EstimationNetworkImpl::estimationNVI(torch::Tensor z, torch::Tensor gamma_ap) {
    ...
    torch::Tensor NVI = criterion(gamma_ap.log(), phi.expand(
            {(long int) mini_batch_size, (long int) nk}));  // gamma_ap{N,K}, phi{N,K} ===> NVI{}
...
void EstimationNetworkImpl::estimationGMP(torch::Tensor z, torch::Tensor gamma_ap) {
    ...
    this->mu_sum_keep = (this->mu_sum_keep +
                         mu * gamma_sum.unsqueeze(-1)).detach();  // mu{K,Z}, gamma_sum{K,1} ===> mu_sum_keep{K,Z}
    this->sigma_sum_keep = (this->sigma_sum_keep + sigma * gamma_sum.unsqueeze(-1).unsqueeze(
            -1)).detach();  // sigma{K,Z,Z}, gamma_sum{K,1,1} ===> sigma_sum_keep{K,Z,Z}
...
torch::Tensor EstimationNetworkImpl::estimated_mu() {
    torch::Tensor mu = this->mu_sum_keep /
                       this->gamma_sum_keep.unsqueeze(-1);  // mu_sum_keep{K,Z}, gamma_sum_keep{K,1} ===> mu{K,Z}
    return mu.detach();
}
...
torch::Tensor EstimationNetworkImpl::estimated_sigma() {
    torch::Tensor sigma = this->sigma_sum_keep / this->gamma_sum_keep.unsqueeze(-1).unsqueeze(
            -1);  // sigma_sum_keep{K,Z,Z}, gamma_sum_keep{K,1,1} ===> sigma{K,Z,Z}
    return sigma.detach();
}
...
// ----------------------------------------------------------------------
torch::Tensor
EstimationNetworkImpl::anomaly_score(torch::Tensor z, torch::Tensor mu, torch::Tensor sigma, torch::Tensor phi) {
...
torch::Tensor AbsoluteEuclideanDistance(torch::Tensor x1, torch::Tensor x2, const bool rec_detach) {
    ...
    torch::Tensor out = torch::sqrt(
            torch::sum(torch::pow(x1 - x2, 2.0), /*dim=*/1, /*keepdim=*/true));  // ||x1 - x2||_2
    if (rec_detach) {
        return out.detach();
    }
    return out;
