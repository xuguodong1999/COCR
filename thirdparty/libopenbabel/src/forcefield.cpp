  ...
  **/
  ...
  void OBForceField::PrintVelocities()
  {
    IF_OBFF_LOGLVL_LOW {
      ...
      FOR_ATOMS_OF_MOL (a, _mol) {
        OBFFLog(fmt::format("{:d}\t<{:8.3f}, {:8.3f}, {:8.3f}>\n",
  ...
  int OBForceField::SystematicRotorSearchInitialize(unsigned int geomSteps, bool sampleRingBonds)
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      OBFFLog(fmt::format("  NUMBER OF ROTATABLE BONDS: {:d}\n", rl.Size()));
      ...
      OBFFLog(fmt::format("  NUMBER OF POSSIBLE ROTAMERS: {:d}\n", combinations));
    }
    ...
    IF_OBFF_LOGLVL_LOW {
      OBFFLog(fmt::format("  GENERATED {:d} CONFORMERS\n\n", _mol.NumConformers()));
                                         ...
                                         bool sampleRingBonds)
  {
    ...
    IF_OBFF_LOGLVL_HIGH {
      ...
      for (unsigned int i = 1; i < rotorWeights.size() - 1; ++i) {
        OBFFLog(fmt::format(" Weight: {:d}", i));
        for (unsigned int j = 0; j < rotorWeights[i].size(); ++j) {
          OBFFLog(fmt::format(" {:8.3f}", rotorWeights[i][j]));
        }
        OBFFLog("\n");
      }
    }
  ...
  void OBForceField::DistanceGeometry()
  {
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      for (i=0; i<N; i++) {
        OBFFLog("[");
        for (j=0; j<N; j++) {
          OBFFLog(fmt::format(" {:8.4f} ", matrix[i][j]));
        }
        OBFFLog("]\n");
      }
      OBFFLog("\n");
    ...
    IF_OBFF_LOGLVL_LOW {
      ...
      for (i=0; i<N; i++) {
        OBFFLog(fmt::format("{:8.4f} ", eigenvalues[i]));
      }
      OBFFLog("\n");
      ...
      for (i=0; i<N; i++) {
        OBFFLog("[");
        for (j=0; j<N; j++) {
          OBFFLog(fmt::format(" {:8.4f} ", eigenvectors[i][j]));
        }
        OBFFLog("]\n");
      }
      OBFFLog("\n");
...
//      IF_OBFF_LOGLVL_LOW {
//      OBFFLog(fmt::format("UPDATE VDW PAIRS: {:d} --> {:d} (VDW), {:d} (ELE) \n",
...
//      }
  ...
  void OBForceField::ValidateSteepestDescent(int steps)
  {
    ...
    for (int i = 1; i <= steps; i++) {
      ...
      IF_OBFF_LOGLVL_LOW {
        OBFFLog(fmt::format(" {:4d}    {:8.3f}    {:8.3f}\n", i, e_n2, e_n1));
      }
  ...
  bool OBForceField::SteepestDescentTakeNSteps(int n)
  {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      IF_OBFF_LOGLVL_LOW {
        if (_cstep % 10 == 0) {
          OBFFLog(fmt::format(" {:4d}    {:8.5f}    {:8.5f}\n", _cstep, e_n2, _e_n1));
        }
      }
