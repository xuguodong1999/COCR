...
torch::Tensor SegNetImpl::forward(torch::Tensor x) {
    ...
    for (size_t i = 0; i < this->num_downs; i++) {
        feature = this->decoder[i]->as<UpSamplingImpl>()->forward(feature, indices.at(this->num_downs - i - 1),
                                                                  unpool_sizes.at(this->num_downs - i -
                                                                                  1));  // {8F,8,8} ===> {F,256,256}
    }
...
// ----------------------------------------------------------------------
DownSamplingImpl::DownSamplingImpl(const size_t in_nc, const size_t out_nc, const size_t n_layers,
...
// ----------------------------------------------------------------------
torch::Tensor
UpSamplingImpl::forward(torch::Tensor x, torch::Tensor indices, const std::vector<long int> unpool_sizes) {
