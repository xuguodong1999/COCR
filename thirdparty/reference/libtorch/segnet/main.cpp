...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, SegNet &model, std::vector<transforms_Compose > &transformI,
...
int main(int argc, const char *argv[]) {
    ...
    std::vector<transforms_Compose > transformO{
            transforms_Resize(cv::Size(vm["size"].as<size_t>(), vm["size"].as<size_t>()),
                              cv::INTER_NEAREST),  // {IH,IW,1} ===method{OW,OH}===> {OH,OW,1}
    ...
    Set_Model_Params(vm, segnet, "SegNet");

    // (8.1) Training Phase
    if (vm["train"].as<bool>()) {
        Set_Options(vm, argc, argv, args, "train");
        train(vm, device, segnet, transformI, transformO);
    }
