...
WAE_EncoderImpl::WAE_EncoderImpl(po::variables_map &vm) {
    ...
    this->model->push_back(
            ViewImpl({-1, (long int) (feature * 8 * ns * ns)}));                     // {8F,4,4}    ===> {8F*4*4}
    this->model->push_back(
            nn::Linear(feature * 8 * ns * ns, vm["nz"].as<size_t>()));              // {8F*4*4}    ===> {Z}
...
WAE_DecoderImpl::WAE_DecoderImpl(po::variables_map &vm) {
    ...
    this->model->push_back(
            nn::Linear(vm["nz"].as<size_t>(), feature * 8 * ns * ns));                 // {Z}         ===> {8F*4*4}
    this->model->push_back(
            ViewImpl({-1, (long int) (feature * 8), (long int) ns, (long int) ns}));  // {8F*4*4}    ===> {8F,4,4}
