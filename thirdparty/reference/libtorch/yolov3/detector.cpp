...
// ------------------------------------
YOLODetector::YOLODetector(const std::vector<std::vector<std::tuple<float, float>>> anchors_, const long int class_num_,
                           const float prob_thresh_, const float nms_thresh_) {
    ...
    this->anchors_wh = torch::from_blob(anchors.data(), {scales, 1, 1, this->na, 2},
                                        torch::kFloat).clone();  // anchors_wh{S,1,1,A,2}
...
torch::Tensor YOLODetector::NonMaximumSuppression(torch::Tensor &coord, torch::Tensor &conf) {
    ...
    while (nms_idx_sorted.numel() > 0) {
        ...
        nms_idx_sorted = nms_idx_sorted.split_with_sizes(/*split_sizes=*/{1, nms_idx_sorted.numel() - 1}, /*dim=*/0).at(
                1);  // nms_idx_sorted{num} ===> {num-1}
        left = x_min.gather(/*dim=*/0, /*index=*/nms_idx_sorted).clamp_min(
                x_min[idx].item<float>());  // x_min{object} ===> left{num-1}
        top = y_min.gather(/*dim=*/0, /*index=*/nms_idx_sorted).clamp_min(
                y_min[idx].item<float>());  // y_min{object} ===> top{num-1}
        right = x_max.gather(/*dim=*/0, /*index=*/nms_idx_sorted).clamp_max(
                x_max[idx].item<float>());  // x_max{object} ===> right{num-1}
        bottom = y_max.gather(/*dim=*/0, /*index=*/nms_idx_sorted).clamp_max(
                y_max[idx].item<float>());  // y_max{object} ===> bottom{num-1}
    ...
    nms_idx = torch::from_blob(nms_idx_vec.data(), {(long int) nms_idx_vec.size()}, torch::kInt).to(torch::kLong).to(
            conf.device()).clone();  // {nms-object}
...
YOLODetector::operator()(const std::vector<torch::Tensor> preds, const std::tuple<float, float> image_sizes) {
    ...
    image_size = torch::from_blob(image_size_vec.data(), {1, 1, 1, 2}, torch::kFloat).to(
            device).clone();  // image_size{1,1,1,2}
    ...
    for (size_t i = 0; i < scales; i++) {
        ...
        /*************************************************************************/
        pred_view = pred.view(
                {ng, ng, this->na, this->class_num + 5});  // pred{G,G,A*(CN+5)} ===> pred_view{G,G,A,CN+5}
        pred_split = pred_view.split_with_sizes(/*split_sizes=*/{this->class_num, 2, 2, 1}, /*dim=*/
                                                                3);  // pred_view{G,G,A,CN+5} ===> pred_split({G,G,A,CN}, {G,G,A,2}, {G,G,A,2}, {G,G,A,1})
        ...
        pred_coord = torch::cat({pred_xy, pred_wh}, /*dim=*/
                                3);  // pred_xy{G,G,A,2} + pred_wh{G,G,A,2} ===> pred_coord{G,G,A,4}
        pred_coord_per = pred_coord.permute(
                {3, 0, 1, 2}).contiguous();  // pred_coord{G,G,A,4} ===> pred_coord_per{4,G,G,A}

        // (3.2) Get object mask
        torch::Tensor class_score, id, prob, obj_mask;
        std::tuple<torch::Tensor, torch::Tensor> class_score_with_idx;
        /*************************************************************************/
        class_score_with_idx = pred_class.max(/*dim=*/3, /*keepdim=*/
                                                      false);  // pred_class{G,G,A,CN} ===> class_score_with_idx({G,G,A}, {G,G,A})
        ...
        obj_coord = torch::cat({obj_cx.unsqueeze(/*dim=*/-1), obj_cy.unsqueeze(/*dim=*/-1), obj_w.unsqueeze(/*dim=*/-1),
                                obj_h.unsqueeze(/*dim=*/-1)}, /*dim=*/1);  // obj_coord{object,4}
    ...
    for (size_t i = 1; i < obj_id_vec.size(); i++) {
        obj_ids = torch::cat({obj_ids, obj_id_vec.at(i)}, /*dim=*/
                             0);  // obj_ids{objects} + obj_id_vec[i]{object} ===> obj_ids{objects + object}
        obj_coords = torch::cat({obj_coords, obj_coord_vec.at(i)}, /*dim=*/
                                0);  // obj_coords{objects,4} + obj_coord_vec[i]{object,4} ===> obj_coords{objects + object,4}
        obj_confs = torch::cat({obj_confs, obj_conf_vec.at(i)}, /*dim=*/
                               0);  // obj_confs{objects} + obj_conf_vec[i]{object} ===> obj_confs{objects + object}
        obj_probs = torch::cat({obj_probs, obj_prob_vec.at(i)}, /*dim=*/
                               0);  // obj_probs{objects} + obj_prob_vec[i]{object} ===> obj_probs{objects + object}
    }
    ...
    /*************************************************************************/
    if (obj_ids.numel() > 0) {
        nms_idx = this->NonMaximumSuppression(obj_coords,
                                              obj_confs);  // obj_coords{objects,4}, obj_confs{objects} ===> nms_idx{nms-object}
        final_ids = obj_ids.gather(/*dim=*/0, /*index=*/
                                           nms_idx).contiguous().detach().clone();  // obj_ids{objects} ===> final_ids{nms-object}
        final_coords = obj_coords.gather(/*dim=*/0, /*index=*/nms_idx.unsqueeze(/*dim=*/-1).expand({nms_idx.size(0),
                                                                                                    4})).contiguous().detach().clone();  // obj_coords{objects,4} ===> final_coords{nms-object,4}
        final_probs = obj_probs.gather(/*dim=*/0, /*index=*/
                                               nms_idx).contiguous().detach().clone();  // obj_probs{objects} ===> final_probs{nms-object}
    }

    return {final_ids, final_coords,
            final_probs};  // {nms-object} (ids), {nms-object,4} (coordinates), {nms-object} (probabilities)
