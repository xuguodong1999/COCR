...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, MC_Discriminator &model,
           std::vector<transforms_Compose > &transform, const std::vector<std::string> class_names);

void
test(po::variables_map &vm, torch::Device &device, MC_Discriminator &model, std::vector<transforms_Compose > &transform,
...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("class_list", po::value<std::string>()->default_value("list/ImageNet.txt"),
             "file name in which class names are listed")
            ("class_num", po::value<size_t>()->default_value(1000), "total classes")
            ("size", po::value<size_t>()->default_value(256), "image width and height")
            ...
            ("train_dir", po::value<std::string>()->default_value("train"),
             "training image directory : ./datasets/<dataset>/<train_dir>/<class name>/<image files>")
            ...
            ("valid_dir", po::value<std::string>()->default_value("valid"),
             "validation image directory : ./datasets/<dataset>/<valid_dir>/<class name>/<image files>")
            ...
            ("test_dir", po::value<std::string>()->default_value("test"),
             "test image directory : ./datasets/<dataset>/<test_dir>/<class name>/<image files>")
            ...
            ("BN", po::value<bool>()->default_value(true), "whether to use batch normalization");
...
int main(int argc, const char *argv[]) {
    ...
    std::vector<transforms_Compose > transform{
            ...
            transforms_Normalize(std::vector<float>{0.485, 0.456, 0.406},
                                 std::vector<float>{0.229, 0.224, 0.225})  // Pixel Value Normalization for ImageNet
    };
