...
UNetImpl::UNetImpl(po::variables_map &vm) {
    ...
    blocks = UNetBlockImpl({vm["nc"].as<size_t>(), vm["class_num"].as<size_t>()}, feature, /*submodule_=*/
...
UNetBlockImpl::UNetBlockImpl(const std::pair<size_t, size_t> outside_nc, const size_t inside_nc,
    ...
    if (this->outermost) {  // {IC,256,256} ===> {F,128,128} ===> ... ===> {2F,128,128} ===> {OC,256,256}
        DownSampling(this->model, outside_nc.first, inside_nc, /*BN=*/false, /*ReLU=*/
                     false);  // {IC,256,256} ===> {F,128,128}
        this->model->push_back(submodule);
        UpSampling(this->model, inside_nc * 2, outside_nc.second, /*BN=*/false, /*ReLU=*/
        ...
        DownSampling(this->model, outside_nc.first, inside_nc, /*BN=*/true, /*ReLU=*/
