...
Loss::build_target(std::vector<std::tuple<torch::Tensor, torch::Tensor>> &target) {
    ...
    target_class = torch::full({(long int) mini_batch_size, this->ng, this->ng, this->class_num}, /*value=*/0.0,
                               torch::TensorOptions().dtype(torch::kFloat));  // target_class{N,G,G,CN}
    target_coord = torch::full({(long int) mini_batch_size, this->ng, this->ng, 5}, /*value=*/0.0,
                               torch::TensorOptions().dtype(torch::kFloat));  // target_coord{N,G,G,5}
    ...
    return {target_class.detach().clone(),
            target_coord.detach().clone()};  // target_class{N,G,G,CN}, target_coord{N,G,G,5}
...
torch::Tensor Loss::rescale(torch::Tensor &BBs) {
    ...
    output = torch::cat({x_min.unsqueeze(/*dim=*/0), y_min.unsqueeze(/*dim=*/0), x_max.unsqueeze(/*dim=*/0),
                         y_max.unsqueeze(/*dim=*/0)}, /*dim=*/0);  // output{4,BB,object}
    return output;
}
...
Loss::operator()(torch::Tensor &input, std::vector<std::tuple<torch::Tensor, torch::Tensor>> &target) {
    ...
    /*************************************************************************/
    input_new = input.split_with_sizes(/*split_sizes=*/{this->class_num, this->nb * 5}, /*dim=*/
                                                       3);  // input{N,G,G,FF} ===> input_new({N,G,G,CN}, {N,G,G,BB*5})
    input_class = input_new.at(0);  // input_class{N,G,G,CN}
    input_coord = input_new.at(1);  // input_coord{N,G,G,BB*5}
    input_coord_new = input_coord.view({input_coord.size(0), input_coord.size(1), input_coord.size(2), this->nb,
                                        5});  // input_coord{N,G,G,BB*5} ===> input_coord_new{N,G,G,BB,5}
    input_coord_per = input_coord_new.permute(
            {4, 0, 1, 2, 3}).contiguous();  // input_coord_new{N,G,G,BB,5} ===> input_coord_per{5,N,G,G,BB}
    ...
    target_coord_per = target_coord.permute(
            {3, 0, 1, 2}).contiguous();  // target_coord{N,G,G,5} ===> target_coord_per{5,N,G,G}
    ...
    /*************************************************************************/
    target_conf_coord = target_conf.unsqueeze(/*dim=*/-1).unsqueeze(/*dim=*/
            -1);  // target_conf{N,G,G} ===> target_conf_coord{N,G,G,1,1}
    input_obj_coord_mask = (target_conf_coord > 0.5).expand(
            {target_conf_coord.size(0), target_conf_coord.size(1), target_conf_coord.size(2), this->nb,
             5});  // target_conf_coord{N,G,G,1,1} ===> input_obj_coord_mask{N,G,G,BB,5}
    target_obj_coord_mask = (target_conf_coord > 0.5).expand(
            {target_conf_coord.size(0), target_conf_coord.size(1), target_conf_coord.size(2), 1,
             5});  // target_conf_coord{N,G,G,1,1} ===> target_obj_coord_mask{N,G,G,1,5}
    input_obj_coord = input_coord_new.masked_select(/*mask=*/input_obj_coord_mask).view(
            {-1, this->nb, 5});  // input_coord_new{N,G,G,BB,5} ===> input_obj_coord{object,BB,5}
    target_obj_coord = target_coord_new.masked_select(/*mask=*/target_obj_coord_mask).view(
            {-1, 1, 5});  // target_coord_new{N,G,G,1,5} ===> target_obj_coord{object,1,5}
    obj_flag = (input_obj_coord.numel() > 0);
    if (obj_flag) {
        input_BBs_normalized = input_obj_coord.permute(
                {2, 1, 0}).contiguous();  // input_obj_coord{object,BB,5} ===> input_BBs_normalized{5,BB,object}
        target_BBs_normalized = target_obj_coord.permute(
                {2, 1, 0}).contiguous();  // target_obj_coord{object,1,5} ===> target_BBs_normalized{5,1,object}
        input_BBs = this->rescale(
                input_BBs_normalized);  // input_BBs_normalized{5,BB,object} ===rescale===> input_BBs{4,BB,object}
        target_BBs = this->rescale(
                target_BBs_normalized);  // target_BBs_normalized{5,1,object} ===rescale===> target_BBs{4,1,object}
        IoU = this->compute_iou(input_BBs, target_BBs).squeeze(/*dim=*/
                1).detach().clone();  // input_BBs{4,BB,object}, target_BBs{4,1,object} ===> IoU{BB,object}
        max_IoU_with_idx = IoU.max(/*dim=*/0, /*keepdim=*/
                                           false);  // IoU{BB,object} ===> max_IoU_with_idx(IoU{object}, idx{object})
        ...
        response_idx = max_idx.unsqueeze(/*dim=*/0).unsqueeze(/*dim=*/0).expand(
                {5, 1, max_idx.size(0)});  // max_idx{object} ===> response_idx{5,1,object}
        input_response_coord = input_BBs_normalized.gather(/*dim=*/1, /*index=*/response_idx).squeeze(/*dim=*/
                1);  // input_BBs_normalized{5,BB,object} ===> input_response_coord{5,object}
        target_response_coord = target_BBs_normalized.squeeze(/*dim=*/
                1);  // target_BBs_normalized{5,1,object} ===> target_response_coord{5,object}
    }
    ...
    if (obj_flag) {
        ...
        loss_coord_xy =
                (criterion(input_response_cx, target_response_cx) + criterion(input_response_cy, target_response_cy)) *
                0.5 / (float) mini_batch_size;
    } else {
    ...
    if (obj_flag) {
        ...
        loss_coord_wh = (criterion(input_response_w.sqrt(), target_response_w.sqrt()) +
                         criterion(input_response_h.sqrt(), target_response_h.sqrt())) * 0.5 / (float) mini_batch_size;
    } else {
    ...
    /*************************************************************************/
    target_conf_conf = target_conf.unsqueeze(/*dim=*/-1).expand(
            {target_conf.size(0), target_conf.size(1), target_conf.size(2),
             this->nb});  // target_conf{N,G,G} ===> target_conf_conf{N,G,G,BB}
    noobj_conf_mask = (target_conf_conf < 0.5);  // target_conf_conf{N,G,G,BB} ===> noobj_conf_mask{N,G,G,BB}
    input_noobj_conf = input_conf.masked_select(/*mask=*/
            noobj_conf_mask);  // input_conf{N,G,G,BB} ===> input_noobj_conf{no object confidence}
    target_noobj_conf = target_conf_conf.masked_select(/*mask=*/
            noobj_conf_mask);  // target_conf_conf{N,G,G,BB} ===> target_noobj_conf{no object confidence}
    ...
    /*************************************************************************/
    if (obj_flag) {
        target_conf_class = target_conf.unsqueeze(/*dim=*/-1).expand(
                {target_conf.size(0), target_conf.size(1), target_conf.size(2),
                 this->class_num});  // target_conf{N,G,G} ===> target_conf_class{N,G,G,CN}
        obj_class_mask = (target_conf_class > 0.5);  // target_conf_class{N,G,G,CN} ===> obj_class_mask{N,G,G,CN}
        input_obj_class = input_class.masked_select(/*mask=*/
                obj_class_mask);  // input_class{N,G,G,CN} ===> input_noobj_conf{CN}
        target_obj_class = target_class.masked_select(/*mask=*/
                obj_class_mask);  // target_class{N,G,G,CN} ===> target_noobj_conf{object class}
