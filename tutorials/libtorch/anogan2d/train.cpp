...
valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion,
      GAN_Generator &gen, GAN_Discriminator &dis, const size_t epoch, visualizer::graph &writer,
      visualizer::graph &writer_dis);
...
void train(po::variables_map &vm, torch::Device &device, GAN_Generator &gen, GAN_Discriminator &dis,
    ...
    torch::Tensor gen_loss, dis_loss, dis_real_loss, dis_fake_loss;
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        while (dataloader(mini_batch)) {
            ...
            show_progress->increment(/*loss_value=*/
                    {gen_loss.item<float>(), dis_real_loss.item<float>(), dis_fake_loss.item<float>()});
            ofs << "iters:" << show_progress->get_iters() << '/' << total_iter << ' ' << std::flush;
            ofs << "G:" << gen_loss.item<float>() << "(ave:" << show_progress->get_ave(0) << ") " << std::flush;
            ofs << "D_Real:" << dis_real_loss.item<float>() << "(ave:" << show_progress->get_ave(1) << ") "
                << std::flush;
            ofs << "D_Fake:" << dis_fake_loss.item<float>() << "(ave:" << show_progress->get_ave(2) << ")" << std::endl;
        ...
        train_loss_dis.plot(/*base=*/epoch, /*value=*/
                                     {show_progress->get_ave(1) + show_progress->get_ave(2), show_progress->get_ave(1),
                                      show_progress->get_ave(2)});
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)) {
            valid(vm, valid_dataloader, device, criterion, gen, dis, epoch, valid_loss, valid_loss_dis);
        }
