...
void valid(po::variables_map &vm, DataLoader::ImageFolderPairWithPaths &valid_dataloader, torch::Device &device,
           ...
           const size_t epoch, visualizer::graph &writer, visualizer::graph &writer_gen,
    ...
    while (valid_dataloader(mini_batch)) {
        ...
        label_real = torch::full(
                {dis_fake_out.size(0), dis_fake_out.size(1), dis_fake_out.size(2), dis_fake_out.size(3)}, /*value*/1.0,
                torch::TensorOptions().dtype(torch::kFloat)).to(device);
        label_fake = torch::full(
                {dis_fake_out.size(0), dis_fake_out.size(1), dis_fake_out.size(2), dis_fake_out.size(3)}, /*value*/0.0,
                torch::TensorOptions().dtype(torch::kFloat)).to(device);
