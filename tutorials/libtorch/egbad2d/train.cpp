...
valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion,
      GAN_Encoder &enc, GAN_Generator &gen, GAN_Discriminator &dis, const size_t epoch, visualizer::graph &writer,
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        while (dataloader(mini_batch)) {
            ...
            show_progress->increment(/*loss_value=*/
                    {enc_loss.item<float>(), gen_loss.item<float>(), dis_real_loss.item<float>(),
