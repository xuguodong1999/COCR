...
MC_AlexNetImpl::MC_AlexNetImpl(po::variables_map &vm) {
    ...
    this->features = nn::Sequential(
            nn::Conv2d(nn::Conv2dOptions(/*in_channels=*/vm["nc"].as<size_t>(), /*out_channels=*/96, /*kernel_size=*/
                                                         11).stride(4)),  // {C,227,227} ===> {96,55,55}
            nn::ReLU(nn::ReLUOptions().inplace(true)),
            nn::LocalResponseNorm(nn::LocalResponseNormOptions(/*size=*/5).alpha(0.0001).beta(0.75).k(2.0)),
            nn::MaxPool2d(nn::MaxPool2dOptions(/*kernel_size=*/3).stride(
                    2)),                                                         // {96,55,55} ===> {96,27,27}
            nn::Conv2d(nn::Conv2dOptions(/*in_channels=*/96, /*out_channels=*/256, /*kernel_size=*/5).stride(1).padding(
                    2)),          // {96,27,27} ===> {256,27,27}
            nn::ReLU(nn::ReLUOptions().inplace(true)),
            nn::LocalResponseNorm(nn::LocalResponseNormOptions(/*size=*/5).alpha(0.0001).beta(0.75).k(2.0)),
            nn::MaxPool2d(nn::MaxPool2dOptions(/*kernel_size=*/3).stride(
                    2)),                                                         // {256,27,27} ===> {256,13,13}
            nn::Conv2d(
                    nn::Conv2dOptions(/*in_channels=*/256, /*out_channels=*/384, /*kernel_size=*/3).stride(1).padding(
                            1)),         // {256,13,13} ===> {384,13,13}
            nn::ReLU(nn::ReLUOptions().inplace(true)),
            nn::Conv2d(
                    nn::Conv2dOptions(/*in_channels=*/384, /*out_channels=*/384, /*kernel_size=*/3).stride(1).padding(
                            1)),         // {384,13,13} ===> {384,13,13}
            nn::ReLU(nn::ReLUOptions().inplace(true)),
            nn::Conv2d(
                    nn::Conv2dOptions(/*in_channels=*/384, /*out_channels=*/256, /*kernel_size=*/3).stride(1).padding(
                            1)),         // {384,13,13} ===> {256,13,13}
            nn::ReLU(nn::ReLUOptions().inplace(true)),
            nn::MaxPool2d(nn::MaxPool2dOptions(/*kernel_size=*/3).stride(
                    2))                                                          // {256,13,13} ===> {256,6,6}
    );
    ...
    this->avgpool = nn::Sequential(
            nn::AdaptiveAvgPool2d(nn::AdaptiveAvgPool2dOptions({6, 6})));  // {256,X,X} ===> {256,6,6}
    register_module("avgpool", this->avgpool);

    this->classifier = nn::Sequential(
            nn::Dropout(0.5),
            nn::Linear(/*in_channels=*/256 * 6 * 6, /*out_channels=*/
                                       4096),                      // {256*6*6} ===> {4096}
            nn::ReLU(nn::ReLUOptions().inplace(true)),
            nn::Dropout(0.5),
