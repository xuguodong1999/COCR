...
void show_bbox_coco(cv::Mat image, torch::Tensor bboxes, int nums) {
    ...
    for (int i = 0; i < bboxes.size(0); i = i + 7) {
        cv::rectangle(image, cv::Rect(bbox[i + 0], bbox[i + 1], bbox[i + 2] - bbox[i + 0], bbox[i + 3] - bbox[i + 1]),
                      cv::Scalar(0, 0, 255));
        cv::Point origin;
        origin.x = bbox[i + 0];
        origin.y = bbox[i + 1] + 8;
        cv::putText(image, std::to_string(int(bbox[i + 6])), origin, font_face, font_scale, cv::Scalar(0, 0, 255),
                    thickness, 1, 0);
    }
    delete bbox;
...
int main() {
    ...
    cv::Mat image = cv::imread("~/datasets/\\detection\\val\\images\\2007_005331.jpg");
    Detector detector;
    detector.Initialize(-1, 416, 416, "~/datasets/\\detection\\name.txt");
    //detector.Train("~/datasets/\\detection", ".jpg", 30,
