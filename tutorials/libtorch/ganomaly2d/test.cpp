...
void test(po::variables_map &vm, torch::Device &device, Encoder &enc1, Encoder &enc2, Decoder &dec,
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_enc1.pth";
    torch::load(enc1, path, device);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_enc2.pth";
    torch::load(enc2, path, device);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_dec.pth";
    ...
    while (dataloader(data)) {
        ...
        std::cout << '<' << std::get<1>(data).at(0) << "> con_" << vm["loss_con"].as<std::string>() << ':'
                  << con_loss.item<float>() << " enc_" << vm["loss_enc"].as<std::string>() << ':'
                  << enc_loss.item<float>() << " anomaly_score:" << anomaly_score.item<float>() << std::endl;
        ofs << '<' << std::get<1>(data).at(0) << "> con_" << vm["loss_con"].as<std::string>() << ':'
            << con_loss.item<float>() << " enc_" << vm["loss_enc"].as<std::string>() << ':' << enc_loss.item<float>()
            << " anomaly_score:" << anomaly_score.item<float>() << std::endl;
    ...
    // (7) Average Output
    std::cout << "<All> con_" << vm["loss_con"].as<std::string>() << ':' << ave_con_loss << " enc_"
              << vm["loss_enc"].as<std::string>() << ':' << ave_enc_loss << " anomaly_score:" << ave_anomaly_score
              << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> con_" << vm["loss_con"].as<std::string>() << ':' << ave_con_loss << " enc_"
        << vm["loss_enc"].as<std::string>() << ':' << ave_enc_loss << " anomaly_score:" << ave_anomaly_score
        << " (time:" << ave_time << ')' << std::endl;
