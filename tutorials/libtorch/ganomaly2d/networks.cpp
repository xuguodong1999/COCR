...
EncoderImpl::EncoderImpl(po::variables_map &vm) {
    ...
    size_t feature = vm["ngef"].as<size_t>();
...
torch::Tensor EncoderImpl::forward(torch::Tensor x) {
    torch::Tensor out = this->model->forward(x);  // {C,256,256} ===> {Z,4,4}
    return out;
}
...
DecoderImpl::DecoderImpl(po::variables_map &vm) {
    ...
    size_t feature = vm["ngdf"].as<size_t>();
...
// ----------------------------------------------------------------------
torch::Tensor DecoderImpl::forward(torch::Tensor z) {
    torch::Tensor out = this->model->forward(z);  // {Z,4,4} ===> {C,256,256}
    return out;
}
...
GAN_DiscriminatorImpl::GAN_DiscriminatorImpl(po::variables_map &vm) {
    ...
    DownSampling(this->down, vm["nc"].as<size_t>(), feature, /*BN=*/false, /*LReLU=*/
                 true);  // {C,256,256} ===> {F,128,128}
    DownSampling(this->down, feature, feature * 2, /*BN=*/true, /*LReLU=*/
                 true);               // {F,128,128} ===> {2F,64,64}
    DownSampling(this->down, feature * 2, feature * 4, /*BN=*/true, /*LReLU=*/
                 true);             // {2F,64,64}  ===> {4F,32,32}
    DownSampling(this->down, feature * 4, feature * 8, /*BN=*/true, /*LReLU=*/
                 true);             // {4F,32,32}  ===> {8F,16,16}
    for (size_t i = 0; i < num_downs - 5; i++) {
        DownSampling(this->down, feature * 8, feature * 8, /*BN=*/true, /*LReLU=*/
