...
// --------------------------------------------------------------------
DataLoader::Data1dFolderWithPaths::Data1dFolderWithPaths(datasets::Data1dFolderWithPaths &dataset_,
                                                         const size_t batch_size_, const bool shuffle_,
                                                         const size_t num_workers_, const bool pin_memory_,
...
// -------------------------------------------------------------------------
DataLoader::Data1dFolderPairWithPaths::Data1dFolderPairWithPaths(datasets::Data1dFolderPairWithPaths &dataset_,
                                                                 const size_t batch_size_, const bool shuffle_,
                                                                 const size_t num_workers_, const bool pin_memory_,
                                                                 const bool drop_last_) {
...
// -----------------------------------------------------------------------
bool DataLoader::Data1dFolderPairWithPaths::operator()(
...
// --------------------------------------------------------------------
DataLoader::ImageFolderWithPaths::ImageFolderWithPaths(datasets::ImageFolderWithPaths &dataset_,
                                                       const size_t batch_size_, const bool shuffle_,
                                                       const size_t num_workers_, const bool pin_memory_,
                                                       const bool drop_last_) {
...
// --------------------------------------------------------------------
DataLoader::ImageFolderPairWithPaths::ImageFolderPairWithPaths(datasets::ImageFolderPairWithPaths &dataset_,
                                                               const size_t batch_size_, const bool shuffle_,
                                                               const size_t num_workers_, const bool pin_memory_,
                                                               const bool drop_last_) {
...
// --------------------------------------------------------------------
bool DataLoader::ImageFolderPairWithPaths::operator()(
        std::tuple<torch::Tensor, torch::Tensor, std::vector<std::string>, std::vector<std::string>> &data) {
...
// ------------------------------------------------------------------------------------------
DataLoader::ImageFolderPairAndRandomSamplingWithPaths::ImageFolderPairAndRandomSamplingWithPaths(
        datasets::ImageFolderPairAndRandomSamplingWithPaths &dataset_, const size_t batch_size_, const bool shuffle_,
        const size_t num_workers_, const bool pin_memory_, const bool drop_last_) {
...
// ---------------------------------------------------------------------------------------
bool DataLoader::ImageFolderPairAndRandomSamplingWithPaths::operator()(
        std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, std::vector<std::string>, std::vector<std::string>, std::vector<std::string>> &data) {
...
#pragma omp parallel for
    ...
    data = {data1, data2, data3, data4, data5,
            data6};  // {N,C,H,W} (images1), {N,C,H,W} (images2), {N,C,H,W} (images_rand), {N} (fnames1), {N} (fnames2), {N} (fnames_rand)
    delete[] data_before;
...
// --------------------------------------------------------------------
DataLoader::ImageFolderSegmentWithPaths::ImageFolderSegmentWithPaths(datasets::ImageFolderSegmentWithPaths &dataset_,
                                                                     const size_t batch_size_, const bool shuffle_,
                                                                     const size_t num_workers_, const bool pin_memory_,
                                                                     const bool drop_last_) {
...
// --------------------------------------------------------------------
bool DataLoader::ImageFolderSegmentWithPaths::operator()(
...
#pragma omp parallel for
    ...
    data = {data1, data2, data3, data4,
            data5};  // {N,C,H,W} (images1), {N,H,W} (images2), {N} (fnames1), {N} (fnames2), {L} (label_palette)
    delete[] data_before;
...
// --------------------------------------------------------------------
DataLoader::ImageFolderClassesWithPaths::ImageFolderClassesWithPaths(datasets::ImageFolderClassesWithPaths &dataset_,
                                                                     const size_t batch_size_, const bool shuffle_,
                                                                     const size_t num_workers_, const bool pin_memory_,
                                                                     const bool drop_last_) {
...
// --------------------------------------------------------------------
bool DataLoader::ImageFolderClassesWithPaths::operator()(
...
// --------------------------------------------------------------------
DataLoader::ImageFolderBBWithPaths::ImageFolderBBWithPaths(datasets::ImageFolderBBWithPaths &dataset_,
                                                           const size_t batch_size_, const bool shuffle_,
                                                           const size_t num_workers_, const bool pin_memory_,
                                                           const bool drop_last_) {
...
// --------------------------------------------------------------------
bool DataLoader::ImageFolderBBWithPaths::operator()(
...
#pragma omp parallel for
    ...
    for (i = 1; i < mini_batch_size; i++) {
        ...
        tensor1 = torch::unsqueeze(tensor1, /*dim=*/0);  // {C,H,W} ===> {1,C,H,W}
        data1 = torch::cat({data1, tensor1}, /*dim=*/0);  // {i,C,H,W} + {1,C,H,W} ===> {i+1,C,H,W}
        data2.push_back(std::get<1>(
                group));  // {i, ({BB_n}, {BB_n,4}) } + {1, ({BB_n}, {BB_n,4}) } ===> {i+1, ({BB_n}, {BB_n,4}) }
        data3.push_back(std::get<2>(group));
        data4.push_back(std::get<3>(group));
    }
    data1 = data1.contiguous().detach().clone();
    ...
    data = {data1, data2, data3,
            data4};  // {N,C,H,W} (images), {N, ({BB_n}, {BB_n,4}) } (annotations), {N} (fnames1), {N} (fnames2)
    delete[] data_before;
