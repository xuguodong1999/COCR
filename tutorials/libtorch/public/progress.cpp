...
// ------------------------------------------------------
progress::display::display(const size_t count_max_, const std::pair<size_t, size_t> epoch,
                           const std::vector<std::string> loss_) {
...
progress::display::display(const size_t count_max_, const std::string header1, const std::string header2,
                           const std::vector<std::string> loss_) {
...
void progress::display::increment(const std::vector<float> loss_value) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        switch (i) {
            ...
            case 1:
                ...
                break;
            case 2:
                sec_per_iter = (double) std::chrono::duration_cast<std::chrono::milliseconds>(
                        this->end - this->start).count() * 0.001 / (double) this->count;
    ...
    ss << "[" << elap_min_str << ":" << elap_sec_str << "<" << rem_min_str << ":" << rem_sec_str << ", "
       << sec_per_iter_str << "s/it]";
    right_str = ss.str();
    ...
    for (i = 0; i < 8; i++) {
        ...
        switch (i) {
            ...
            case 2:
                ...
                break;
            case 3:
                sec_per_count = (double) std::chrono::duration_cast<std::chrono::milliseconds>(
                        this->end - this->start).count() * 0.001 / (double) (count - this->count_start);
