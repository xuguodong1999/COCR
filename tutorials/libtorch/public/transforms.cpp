...
void
transforms::forward(std::vector<transforms_Compose > &transform_, T_in &data_in, T_out &data_out, const int count) {
...
template void
transforms::forward<cv::Mat, cv::Mat>(std::vector<transforms_Compose > &transform_, cv::Mat &data_in, cv::Mat &data_out,
                                      const int count);

template void
transforms::forward<cv::Mat, torch::Tensor>(std::vector<transforms_Compose > &transform_, cv::Mat &data_in,
                                            torch::Tensor &data_out, const int count);

template void
transforms::forward<torch::Tensor, cv::Mat>(std::vector<transforms_Compose > &transform_, torch::Tensor &data_in,
                                            cv::Mat &data_out, const int count);

template void
transforms::forward<torch::Tensor, torch::Tensor>(std::vector<transforms_Compose > &transform_, torch::Tensor &data_in,
                                                  torch::Tensor &data_out, const int count);
...
void transforms::Normalize1dImpl::forward(torch::Tensor &data_in, torch::Tensor &data_out) {
    ...
    torch::Tensor data_out_src = (data_in - meanF.to(data_in.device())) /
                                 stdF.to(data_in.device());  // data_in{D}, meanF{*}, stdF{*} ===> data_out_src{D}
...
void transforms::ToTensorImpl::forward(cv::Mat &data_in, torch::Tensor &data_out) {
    ...
    torch::Tensor data_out_src = torch::from_blob(float_mat.data,
                                                  {float_mat.rows, float_mat.cols, float_mat.channels()},
                                                  torch::kFloat);  // {0,1,2} = {H,W,C}
...
void transforms::ToTensorLabelImpl::forward(cv::Mat &data_in, torch::Tensor &data_out) {
    torch::Tensor data_out_src = torch::from_blob(data_in.data, {data_in.rows, data_in.cols, data_in.channels()},
                                                  torch::kInt).to(torch::kLong);  // {0,1,2} = {H,W,C}
    data_out_src = data_out_src.permute({2, 0, 1});  // {0,1,2} = {H,W,C} ===> {0,1,2} = {C,H,W}
...
void transforms::AddRVINoiseImpl::forward(torch::Tensor &data_in, torch::Tensor &data_out) {
    ...
    torch::Tensor noise =
            torch::rand({channels, height, width}).to(data_in.device()) * (this->range.second - this->range.first) +
            this->range.first;
...
// ---------------------------------------------------------------------------
transforms::AddSPNoiseImpl::AddSPNoiseImpl(const float occur_prob_, const float salt_rate_,
                                           const std::pair<float, float> range_) {
    this->occur_prob = occur_prob_;
...
void transforms::AddSPNoiseImpl::forward(torch::Tensor &data_in, torch::Tensor &data_out) {
    ...
    torch::Tensor salt_flag =
            (randu2 < this->salt_rate).to(torch::kFloat).expand({channels, height, width}) * noise_flag;
    torch::Tensor pepper_flag =
            ((randu2 < this->salt_rate) == false).to(torch::kFloat).expand({channels, height, width}) * noise_flag;
...
// ------------------------------------------------------------------------------
transforms::AddGaussNoiseImpl::AddGaussNoiseImpl(const float occur_prob_, const float mean_, const float std_,
                                                 const std::pair<float, float> range_) {
    this->occur_prob = occur_prob_;
    this->mean = mean_;
    this->std = std_;
...
void transforms::AddGaussNoiseImpl::forward(torch::Tensor &data_in, torch::Tensor &data_out) {
    ...
    torch::Tensor data_mix = (data_in + noise * noise_flag).clamp(/*min=*/this->range.first, /*max=*/
                                                                          this->range.second);
    data_out = data_mix.contiguous().detach().clone();
...
transforms::NormalizeImpl::NormalizeImpl(const float mean_, const std::vector<float> std_) {
    this->mean = torch::from_blob((float *) &mean_, {1, 1, 1}, torch::kFloat).clone();  // mean{1,1,1}
    this->std = torch::from_blob((float *) std_.data(), {(long int) std_.size(), 1, 1},
                                 torch::kFloat).clone();  // std{C,1,1}
}

transforms::NormalizeImpl::NormalizeImpl(const std::vector<float> mean_, const float std_) {
    this->mean = torch::from_blob((float *) mean_.data(), {(long int) mean_.size(), 1, 1},
                                  torch::kFloat).clone();  // mean{C,1,1}
    this->std = torch::from_blob((float *) &std_, {1, 1, 1}, torch::kFloat).clone();  // std{1,1,1}
}
...
void transforms::NormalizeImpl::forward(torch::Tensor &data_in, torch::Tensor &data_out) {
    ...
    torch::Tensor data_out_src = (data_in - meanF.to(data_in.device())) /
                                 stdF.to(data_in.device());  // data_in{C,H,W}, meanF{*,1,1}, stdF{*,1,1} ===> data_out_src{C,H,W}
    data_out = data_out_src.contiguous().detach().clone();
