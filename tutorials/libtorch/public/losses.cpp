...
// ----------------------------------------------------
Losses::SSIMLoss::SSIMLoss(const size_t nc_, const torch::Device device, const size_t window_size_,
                           const float gauss_std_, const float c1_base_, const float c2_base_) {
    ...
    auto Gaussian_PDF = [](float mean, float std, float x) {
        return (float) std::exp(-(x - mean) * (x - mean) / (2.0 * std * std));
    };
    ...
    tensor = tensor /
             tensor.sum();                                                       // No Normalize {W} ===> Normalize {W}
    ...
    tensor = tensor.expand({(long int) this->nc, 1, (long int) this->window_size,
                            (long int) this->window_size}).contiguous();  // {1,1,W,W} ===> {C,1,W,W}
...
torch::Tensor Losses::SSIMLoss::Structural_Similarity(torch::Tensor &image1, torch::Tensor &image2) {
    ...
    torch::Tensor mu1 = F::conv2d(image1, this->window,
                                  F::Conv2dFuncOptions().padding(this->window_size / 2).groups(this->nc));
    torch::Tensor mu2 = F::conv2d(image2, this->window,
                                  F::Conv2dFuncOptions().padding(this->window_size / 2).groups(this->nc));
    ...
    torch::Tensor var1 = F::conv2d(image1 * image1, this->window,
                                   F::Conv2dFuncOptions().padding(this->window_size / 2).groups(this->nc)) - mu1_sq;
    torch::Tensor var2 = F::conv2d(image2 * image2, this->window,
                                   F::Conv2dFuncOptions().padding(this->window_size / 2).groups(this->nc)) - mu2_sq;
    torch::Tensor covar = F::conv2d(image1 * image2, this->window,
                                    F::Conv2dFuncOptions().padding(this->window_size / 2).groups(this->nc)) - mu1_mu2;
