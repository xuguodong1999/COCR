...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device,
           torch::nn::BCEWithLogitsLoss &criterion_adv, Loss &criterion_con, Loss &criterion_lat, UNet_Generator &gen,
           GAN_Discriminator &dis, const size_t epoch, visualizer::graph &writer_rec, visualizer::graph &writer_lat,
           visualizer::graph &writer_gan, visualizer::graph &writer_dis);


// -------------------
// Training Function
// -------------------
void train(po::variables_map &vm, torch::Device &device, UNet_Generator &gen, GAN_Discriminator &dis,
    ...
    torch::Tensor image, fake_image, pair;
    torch::Tensor dis_real_feat, dis_fake_feat;
    torch::Tensor dis_real_out, dis_fake_out;
    torch::Tensor gen_loss, adv_loss, con_loss, lat_loss, dis_loss, dis_real_loss, dis_fake_loss;
    ...
    visualizer::graph train_loss_rec, train_loss_lat, train_loss_gan, train_loss_dis;
    visualizer::graph valid_loss_rec, valid_loss_lat, valid_loss_gan, valid_loss_dis;
    ...
    auto gen_optimizer = torch::optim::Adam(gen->parameters(),
                                            torch::optim::AdamOptions(vm["lr_gen"].as<float>()).betas(
    ...
    // (4) Set Loss Function
    auto criterion_adv = torch::nn::BCEWithLogitsLoss(torch::nn::BCEWithLogitsLossOptions().reduction(torch::kMean));
    auto criterion_con = Loss(vm["loss_con"].as<std::string>());
    auto criterion_lat = Loss(vm["loss_lat"].as<std::string>());
    ...
    train_loss_rec = visualizer::graph(path, /*gname_=*/"train_loss_rec", /*label_=*/{"Contextual"});
    train_loss_lat = visualizer::graph(path, /*gname_=*/"train_loss_lat", /*label_=*/{"Latent"});
    ...
    if (vm["valid"].as<bool>()) {
        valid_loss_rec = visualizer::graph(path, /*gname_=*/"valid_loss_rec", /*label_=*/{"Contextual"});
        valid_loss_lat = visualizer::graph(path, /*gname_=*/"valid_loss_lat", /*label_=*/{"Latent"});
        valid_loss_gan = visualizer::graph(path, /*gname_=*/"valid_loss_gan", /*label_=*/
    ...
    if (vm["train_load_epoch"].as<std::string>() == "") {
        ...
        path = checkpoint_dir + "/models/epoch_" + vm["train_load_epoch"].as<std::string>() + "_gen.pth";
        ...
        path = checkpoint_dir + "/optims/epoch_" + vm["train_load_epoch"].as<std::string>() + "_gen.pth";
        torch::load(gen_optimizer, path, device);
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        while (dataloader(mini_batch)) {
            ...
            gen_loss = adv_loss + con_loss + lat_loss;
            ...
            show_progress->increment(/*loss_value=*/
                    {adv_loss.item<float>(), con_loss.item<float>(), lat_loss.item<float>(),
                     dis_real_loss.item<float>(), dis_fake_loss.item<float>()});
            ...
            ofs << "lat:" << lat_loss.item<float>() << "(ave:" << show_progress->get_ave(2) << ") " << std::flush;
            ofs << "D_Real:" << dis_real_loss.item<float>() << "(ave:" << show_progress->get_ave(3) << ") "
                << std::flush;
        ...
        // -----------------------------------
        train_loss_rec.plot(/*base=*/epoch, /*value=*/{show_progress->get_ave(1)});
        train_loss_lat.plot(/*base=*/epoch, /*value=*/{show_progress->get_ave(2)});
        train_loss_gan.plot(/*base=*/epoch, /*value=*/{show_progress->get_ave(0),
                                                       show_progress->get_ave(3) + show_progress->get_ave(4)});
        train_loss_dis.plot(/*base=*/epoch, /*value=*/
                                     {show_progress->get_ave(3) + show_progress->get_ave(4), show_progress->get_ave(3),
                                      show_progress->get_ave(4)});
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)) {
            valid(vm, valid_dataloader, device, criterion_adv, criterion_con, criterion_lat, gen, dis, epoch,
                  valid_loss_rec, valid_loss_lat, valid_loss_gan, valid_loss_dis);
        }
        ...
        if (epoch % vm["save_epoch"].as<size_t>() == 0) {
            path = checkpoint_dir + "/models/epoch_" + std::to_string(epoch) + "_gen.pth";
            torch::save(gen, path);
            path = checkpoint_dir + "/models/epoch_" + std::to_string(epoch) + "_dis.pth";
            torch::save(dis, path);
            path = checkpoint_dir + "/optims/epoch_" + std::to_string(epoch) + "_gen.pth";
            torch::save(gen_optimizer, path);
            path = checkpoint_dir + "/optims/epoch_" + std::to_string(epoch) + "_dis.pth";
            torch::save(dis_optimizer, path);
        }
        path = checkpoint_dir + "/models/epoch_latest_gen.pth";
        torch::save(gen, path);
        path = checkpoint_dir + "/models/epoch_latest_dis.pth";
        torch::save(dis, path);
        path = checkpoint_dir + "/optims/epoch_latest_gen.pth";
        torch::save(gen_optimizer, path);
