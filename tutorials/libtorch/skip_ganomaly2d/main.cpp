...
void test(po::variables_map &vm, torch::Device &device, UNet_Generator &gen, GAN_Discriminator &dis,
          std::vector<transforms_Compose > &transform);
...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("loss_lat", po::value<std::string>()->default_value("l2"),
             "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
            ...
            ("Lambda_con", po::value<float>()->default_value(40.0), "the multiple of contextual loss")
            ("Lambda_lat", po::value<float>()->default_value(1.0), "the multiple of latent loss")
            ("no_dropout", po::value<bool>()->default_value(true), "Dropout off/on");
