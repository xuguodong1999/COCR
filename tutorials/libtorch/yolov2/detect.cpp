...
void detect(po::variables_map &vm, torch::Device &device, YOLOv2 &model, std::vector<transforms_Compose > &transformI,
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["detect_load_epoch"].as<std::string>() + ".pth";
    ...
    // (4) Tensor Forward
    model->eval();
    result_dir = vm["detect_result_dir"].as<std::string>();
    ...
    while (dataloader(data)) {
        ...
        detect_result = detector(
                output[0]);  // output[0]{G,G,FF} ===> detect_result{ (ids{BB_n}, coords{BB_n,4}, probs{BB_n}) }
        ids = std::get<0>(detect_result);  // ids{BB_n}
        coords = std::get<1>(detect_result);  // coords{BB_n,4}
        probs = std::get<2>(detect_result);  // probs{BB_n}
        imageO = visualizer::draw_detections_des(imageD[0].detach(), {ids, coords}, probs, class_names,
