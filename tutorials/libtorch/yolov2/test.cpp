...
// Test Function
// ---------------
void test(po::variables_map &vm, torch::Device &device, YOLOv2 &model, std::vector<transforms_Compose > &transform,
          const std::vector<std::string> class_names, const std::vector<std::tuple<float, float>> anchors) {
    ...
    // (3) Set Loss Function
    auto criterion = Loss(anchors, (long int) vm["class_num"].as<size_t>(), vm["noobj_thresh"].as<float>());
    ...
    while (dataloader(data)) {
        ...
        std::cout << '<' << std::get<2>(data).at(0) << "> coord_xy:" << loss_coord_xy.item<float>() << " coord_wh:"
                  << loss_coord_wh.item<float>() << " conf_o:" << loss_obj.item<float>() << " conf_x:"
                  << loss_noobj.item<float>() << " class:" << loss_class.item<float>() << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> coord_xy:" << loss_coord_xy.item<float>() << " coord_wh:"
            << loss_coord_wh.item<float>() << " conf_o:" << loss_obj.item<float>() << " conf_x:"
            << loss_noobj.item<float>() << " class:" << loss_class.item<float>() << std::endl;
    ...
    // (7) Average Output
    std::cout << "<All> coord_xy:" << ave_loss_coord_xy << " coord_wh:" << ave_loss_coord_wh << " conf_o:"
              << ave_loss_obj << " conf_x:" << ave_loss_noobj << " class:" << ave_loss_class << " (time:" << ave_time
              << ')' << std::endl;
    ofs << "<All> coord_xy:" << ave_loss_coord_xy << " coord_wh:" << ave_loss_coord_wh << " conf_o:" << ave_loss_obj
        << " conf_x:" << ave_loss_noobj << " class:" << ave_loss_class << " (time:" << ave_time << ')' << std::endl;
