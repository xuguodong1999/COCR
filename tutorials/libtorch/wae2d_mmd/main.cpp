...
// Function Prototype
void train(po::variables_map &vm, torch::Device &device, WAE_Encoder &enc, WAE_Decoder &dec,
           std::vector<transforms_Compose > &transform);

void test(po::variables_map &vm, torch::Device &device, WAE_Encoder &enc, WAE_Decoder &dec,
          std::vector<transforms_Compose > &transform);
...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("valid_batch_size", po::value<size_t>()->default_value(4), "validation batch size")
            ...
            ("test_out_dir", po::value<std::string>()->default_value("testO"),
             "test output image directory : ./datasets/<dataset>/<test_out_dir>/<image files>")
            ...
            ("Lambda", po::value<float>()->default_value(0.01), "the multiple of MMD loss");
...
int main(int argc, const char *argv[]) {
    ...
    if (vm["train"].as<bool>()) {
        ...
        train(vm, device, enc, dec, transform);
    }
