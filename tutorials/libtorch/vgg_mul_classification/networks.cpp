...
MC_VGGNetImpl::MC_VGGNetImpl(po::variables_map &vm) {
    ...
    if (n_layers == 11) {
        ...
        cfg = {64, 64, MAX_POOLING, 128, 128, MAX_POOLING, 256, 256, MAX_POOLING, 512, 512, MAX_POOLING, 512, 512,
               MAX_POOLING};
    } else if (n_layers == 16) {
        cfg = {64, 64, MAX_POOLING, 128, 128, MAX_POOLING, 256, 256, 256, MAX_POOLING, 512, 512, 512, MAX_POOLING, 512,
               512, 512, MAX_POOLING};
    } else if (n_layers == 19) {
        cfg = {64, 64, MAX_POOLING, 128, 128, MAX_POOLING, 256, 256, 256, 256, MAX_POOLING, 512, 512, 512, 512,
               MAX_POOLING, 512, 512, 512, 512, MAX_POOLING};
    } else {
    ...
    this->avgpool = nn::Sequential(
            nn::AdaptiveAvgPool2d(nn::AdaptiveAvgPool2dOptions({7, 7})));  // {512,X,X} ===> {512,7,7}
    register_module("avgpool", this->avgpool);

    this->classifier = nn::Sequential(
            nn::Linear(/*in_channels=*/512 * 7 * 7, /*out_channels=*/
                                       4096),                      // {512*7*7} ===> {4096}
...
nn::Sequential make_layers(const size_t nc, const std::vector<long int> cfg, const bool BN) {
    ...
    for (auto v: cfg) {
        if (v == MAX_POOLING) {
            ...
            sq->push_back(nn::Conv2d(
                    nn::Conv2dOptions(/*in_channels=*/in_channels, /*out_channels=*/v, /*kernel_size=*/3).stride(
                            1).padding(1)));
            if (BN) {
