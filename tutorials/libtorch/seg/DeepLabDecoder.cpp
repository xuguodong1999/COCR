...
ASPPImpl::ASPPImpl(int in_channels, int out_channels, std::vector<int> atrous_rates, bool separable) {
    modules->push_back(
            torch::nn::Sequential(torch::nn::Conv2d(conv_options(in_channels, out_channels, 1, 1, 0, 1, false)),
                                  torch::nn::BatchNorm2d(torch::nn::BatchNorm2dOptions(out_channels)),
                                  torch::nn::ReLU()));
...
DeepLabV3PlusDecoderImpl::DeepLabV3PlusDecoderImpl(std::vector<int> encoder_channels, int out_channels,
    ...
    up = torch::nn::Upsample(torch::nn::UpsampleOptions().align_corners(true).scale_factor(
            std::vector<double>({scale_factor, scale_factor})).mode(torch::kBilinear));
