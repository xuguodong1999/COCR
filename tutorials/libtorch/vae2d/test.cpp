...
void test(po::variables_map &vm, torch::Device &device, VariationalAutoEncoder &model,
    ...
    float ave_rec_loss, ave_kld_loss, ave_GT_loss;
    ...
    while (dataloader(data)) {
        ...
        ave_kld_loss += kld.item<float>();
        ave_GT_loss += GT_loss.item<float>();
        ave_time += seconds;

        std::cout << '<' << std::get<2>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << rec.item<float>()
                  << " kld:" << kld.item<float>() << " GT_" << vm["loss"].as<std::string>() << ':'
                  << GT_loss.item<float>() << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << rec.item<float>()
            << " kld:" << kld.item<float>() << " GT_" << vm["loss"].as<std::string>() << ':' << GT_loss.item<float>()
            << std::endl;
    ...
    ave_kld_loss = ave_kld_loss / (float) dataset.size();
    ave_GT_loss = ave_GT_loss / (float) dataset.size();
    ave_time = ave_time / (double) dataset.size();

    // (7) Average Output
    std::cout << "<All> " << vm["loss"].as<std::string>() << ':' << ave_rec_loss << " kld:" << ave_kld_loss << " GT_"
              << vm["loss"].as<std::string>() << ':' << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> " << vm["loss"].as<std::string>() << ':' << ave_rec_loss << " kld:" << ave_kld_loss << " GT_"
        << vm["loss"].as<std::string>() << ':' << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
