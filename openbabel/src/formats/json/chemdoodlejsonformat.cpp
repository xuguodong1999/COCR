...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://web.chemdoodle.com/docs/chemdoodle-json-format"; };
            ...
            for (rapidjson::SizeType i = 0; i < bonds.Size(); i++) {
                ...
                if (bond.HasMember("o") && bond["o"].IsNumber()) {
                    ...
                    if (fabs(bo - 1) < 0.01) {
                        ...
                        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Bond order 0.5 not supported, using 1",
                                              obWarning);
                        order = 1;
                    } else if (fabs(bo - 1.5) < 0.01) {
                        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Bond order 1.5 not supported, using 2",
                                              obWarning);
                        order = 2;
                    } else {
                        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Unsupported bond order, skipping bond",
                                              obWarning);
                        continue;
                    }
