...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://www.ncbi.nlm.nih.gov/data_specs/asn/pcsubstance.asn"; };
    ...
    bool PubChemJSONFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        // Atom elements
        if (!molRoot.HasMember("atoms") || !molRoot["atoms"].HasMember("aid") ||
            !molRoot["atoms"].HasMember("element")) {
        ...
        if (molRoot.HasMember("coords") && molRoot["coords"].IsArray() && molRoot["coords"].Size() > 0 &&
            ...
            if (coords.HasMember("conformers") && coords["conformers"].IsArray() && coords["conformers"].Size() > 0) {
                ...
                if (conf.HasMember("x") && conf["x"].IsArray() && conf["x"].Size() >= cAids.Size() &&
                    conf.HasMember("y") &&
                                ...
                                if (styles[i].IsInt()) {
                                    ...
                                    if (style == 8) {
                                        ...
                                        if (pbond->HasData("style")) {
                                            AnnotationData *data = dynamic_cast<AnnotationData *>(pbond->GetData(
                                                    "style"));
            ...
            if (molRoot.HasMember("stereo") && molRoot["stereo"].IsArray()) {
                for (rapidjson::SizeType i = 0; i < molRoot["stereo"].Size(); i++) {
                    ...
                    if (stereo.HasMember("tetrahedral")) {
                        ...
                        config.refs.push_back(
                                (tet["below"].GetInt() == -1) ? OBStereo::ImplicitRef : tet["below"].GetInt());
                        if ((tet["parity"].IsInt() && tet["parity"].GetInt() == 1) ||
                            ...
                            config.refs.push_back(
                                    (tet["bottom"].GetInt() == -1) ? OBStereo::ImplicitRef : tet["bottom"].GetInt());
                            config.refs.push_back(
                                    (tet["above"].GetInt() == -1) ? OBStereo::ImplicitRef : tet["above"].GetInt());
                        } else if ((tet["parity"].IsInt() && tet["parity"].GetInt() == 2) ||
                                   (tet["parity"].IsString() &&
                                    strcmp(tet["parity"].GetString(), "counterclockwise") == 0)) {
                            config.specified = true;
                        ...
                        config.refs.push_back(
                                (pl["ltop"].GetInt() == -1) ? OBStereo::ImplicitRef : pl["ltop"].GetInt());
                        config.refs.push_back(
                                (pl["rtop"].GetInt() == -1) ? OBStereo::ImplicitRef : pl["rtop"].GetInt());
                        config.refs.push_back(
                                (pl["rbottom"].GetInt() == -1) ? OBStereo::ImplicitRef : pl["rbottom"].GetInt());
                        config.refs.push_back(
                                (pl["lbottom"].GetInt() == -1) ? OBStereo::ImplicitRef : pl["lbottom"].GetInt());
                        if ((pl["parity"].IsInt() && (pl["parity"].GetInt() == 3 || pl["parity"].GetInt() == 255)) ||
                            (pl["parity"].IsString() &&
                             (strcmp(pl["parity"].GetString(), "any") == 0 ||
                              strcmp(pl["parity"].GetString(), "unknown") == 0))) {
                            config.specified = false;
                        } else {
                            config.specified = true;
                        ...
                        config.refs.push_back(
                                (sq["lbelow"].GetInt() == -1) ? OBStereo::ImplicitRef : sq["lbelow"].GetInt());
                        config.refs.push_back(
                                (sq["rbelow"].GetInt() == -1) ? OBStereo::ImplicitRef : sq["rbelow"].GetInt());
                        config.refs.push_back((sq["rabove"].GetInt()) ? OBStereo::ImplicitRef : sq["rabove"].GetInt());
                        config.refs.push_back(
                                (sq["labove"].GetInt() == -1) ? OBStereo::ImplicitRef : sq["labove"].GetInt());
                        if ((sq["parity"].IsInt() && (sq["parity"].GetInt() == 4 || sq["parity"].GetInt() == 255)) ||
                            (sq["parity"].IsString() &&
                             (strcmp(sq["parity"].GetString(), "any") == 0 ||
                              strcmp(sq["parity"].GetString(), "unknown") == 0))) {
                            config.specified = false;
                        } else {
                            config.specified = true;
                            config.shape = OBStereo::ShapeU;
                        ...
                        obErrorLog.ThrowError("PubChemJSONFormat", "Octahedral stereochemistry not implemented",
                                              obWarning);
                        // aids: center, top, bottom, lbelow, rbelow, labove, rabove
                    } else if (stereo.HasMember("bipyramid")) {
                        obErrorLog.ThrowError("PubChemJSONFormat", "Bipyramidal stereochemistry not implemented",
                                              obWarning);
                        // aids: above, below, bottom, center, top, right
                    } else if (stereo.HasMember("tshape")) {
                        obErrorLog.ThrowError("PubChemJSONFormat", "T shape stereochemistry not implemented",
                                              obWarning);
                        // aids: center, top, bottom, above
                    } else if (stereo.HasMember("pentagonal")) {
                        obErrorLog.ThrowError("PubChemJSONFormat", "Pentagonal stereochemistry not implemented",
                                              obWarning);
    ...
    bool PubChemJSONFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (pmol->GetDimension() == 0) {
            obErrorLog.ThrowError("PubChemJSONFormat", "No 2D or 3D coordinates exist. "
                                                       "To generate 2D or 3D coordinates use --gen2D or --gen3D.",
                                  obError);
            return false;
        }
        ...
        if (pConv->IsOption("d", OBConversion::GENOPTIONS)) {
            obErrorLog.ThrowError("PubChemJSONFormat", "Stereo output may be invalid due to implicit hydrogens",
                                  obWarning);
        } else {
            pmol->AddHydrogens();
        }
        ...
        FOR_BONDS_OF_MOL(pbond, pmol) {
            if (facade.HasCisTransStereo(pbond->GetId())) {
                ...
                if (begin && ltop) {
                    ...
                    ct.AddMember("lbottom",
                                 rapidjson::Value((int) cts->GetCisRef(cts->GetTransRef(config.refs[0]))).Move(), al);
