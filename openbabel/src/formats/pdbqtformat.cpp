...
namespace OpenBabel {
    ...
    public:
        ...
        unsigned int UpOne() {
            if (parents.size() >= 2) { return parents.at(parents.size() - 2); }
            return 0;
        }
        ...
        void all_atoms(OBMol &mol) {
            clear();
            rigid_with.insert(0);
            for (unsigned int i = 1; i <= mol.NumAtoms(); i++) { atoms.push_back(i); }
        }
    };
    ...
    public:
        ...
        virtual const char *
        SpecificationURL() { return "http://autodock.scripps.edu/faqs-help/faq/what-is-the-format-of-a-pdbqt-file"; };

        virtual const char *GetMIMEType() { return "chemical/x-pdbqt"; };
    ...
    static void
    OutputGroup(OBMol &mol, ostream &ofs, const vector<int> &group, map<unsigned int, unsigned int> new_indexes,
                bool use_new_indexes);

    static unsigned int AtomsSoFar(const map<unsigned int, branch> &tree, unsigned int depth);

    static bool FindBondedPiece(const vector<int> &root, const vector<int> &branch, unsigned int &root_atom,
                                unsigned int &branch_atom,
                                unsigned int &root_atom_rank, unsigned int &branch_atom_rank, const OBMol &mol,
    ...
    void OutputGroup(OBMol &mol, ostream &ofs, const vector<int> &group, map<unsigned int, unsigned int> new_indexes,
                     bool use_new_indexes) {
    ...
    bool OutputTree(OBConversion *pConv, OBMol &mol, ostream &ofs, map<unsigned int, branch> &tree, unsigned int depth,
        ...
        for (unsigned int i = 1; i < tree.size(); i++) {
            how_many_atoms_move.insert(
                    pair<unsigned int, unsigned int>((*tree.find(i)).second.how_many_atoms_moved, i));
        }
        ...
        if (!preserve_original_index) //generates the new ordering
        {
            ...
            for (unsigned int i = 0; i < tree.size(); i++) {
                if (free_bonds.count(i)) {
                    for (set<unsigned int>::iterator it = (*tree.find(i)).second.rigid_with.begin();
                         it != (*tree.find(i)).second.rigid_with.end(); ++it) {
        ...
        if (!(pConv->IsOption("r", OBConversion::OUTOPTIONS)))
            ofs << "ROOT" << endl;
        for (set<unsigned int>::iterator it = (*tree.find(0)).second.rigid_with.begin();
             it != (*tree.find(0)).second.rigid_with.end(); ++it) {
        ...
        for (unsigned int i = 1; i < tree.size(); i++) {
            ...
            for (vector<unsigned int>::iterator it = (*tree.find(i)).second.parents.end();
                 it != (*tree.find(i)).second.parents.begin();) {
                --it;
            ...
            if (fecund) {
                ...
                for (unsigned int i = 0; i < rigid_fragments.size(); i++) {
                    if (FindBondedPiece((*tree.find(position)).second.atoms, rigid_fragments.at(i),
                        ...
                        sprog.parents = (*tree.find(
                                position)).second.parents; //all parents of the parent are parents too
    ...
    //and if both atoms it connects have at least one other atom bounded to them
    {
        if (the_bond->GetBondOrder() != 1 || the_bond->IsAromatic() ||
            ...
            return false;
        if (((the_bond->GetBeginAtom())->GetExplicitDegree() == 1) ||
            ((the_bond->GetEndAtom())->GetExplicitDegree() == 1)) { return false; }
        return true;
    }
...
//		for (unsigned int i = 0; i < depth; i++) {numberAtoms+= tree.at(i).second.first.size();}
    ...
    bool FindBondedPiece(const vector<int> &root, const vector<int> &branched, unsigned int &root_atom,
                         unsigned int &branch_atom,
                         unsigned int &root_atom_rank, unsigned int &branch_atom_rank, const OBMol &mol,
                         unsigned int &atoms_moved) {
        OBBond *the_bond;
...
//    vector<OBMol> result;
    ...
    bool PDBQTFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if ((pConv->IsOption("c", OBConversion::OUTOPTIONS) != nullptr &&
             pConv->IsOption("r", OBConversion::OUTOPTIONS) != nullptr)
        ...
        for (unsigned int i = 0; i < all_pieces.size(); i++) {
            ...
            char buffer[BUFF_SIZE];
            if (!residue) {
                if (!pConv->IsLast() || pConv->GetOutputIndex() > 1) { // More than one molecule record
                ...
                if (!(pConv->IsOption("r", OBConversion::OUTOPTIONS))) {
                    ...
                    for (rotBondId = 0; rotBondId < nRotBond; rotBondId++) {
                        ...
                        for (bondAtomNum = 0; bondAtomNum < 2; bondAtomNum++) {
                            ...
                            strncpy(type_name,
                                    OBElements::GetSymbol(rotBondTable[rotBondId][bondAtomNum]->GetAtomicNum()),
                                    sizeof(type_name));
                            if (strlen(type_name) > 1)
                                type_name[1] = toupper(type_name[1]);
                            if ((res = rotBondTable[rotBondId][bondAtomNum]->GetResidue()) != nullptr) {
                                snprintf(type_name, 5, "%s",
                                         (char *) res->GetAtomID(rotBondTable[rotBondId][bondAtomNum]).c_str());
...
//        ofs << "USER                           _______ _______ _______ _____ _____    ______ ____" << endl;
            ...
            if (flexible) {
                ...
                best_root_atom = FindFragments(all_pieces.at(i),
                                               rigid_fragments); //the return value is the root atom index
                ...
                for (unsigned int j = 0; j < rigid_fragments.size(); j++) {
                    if (IsIn((rigid_fragments.at(j)), best_root_atom)) {
                        root_piece = j;
                        break;
...
//        ofs << "TER" << endl;
    ...
    /* ATOMFORMAT "(i5,1x,a4,a1,a3,1x,a1,i4,a1,3x,3f8.3,2f6.2,a2,a2)" */
    {
        ...
        if (sbuf.size() > 72) {
            ...
            if ((pdbqt_element == "A ") || (pdbqt_element == " A") || (pdbqt_element == "Z ") ||
                (pdbqt_element == " Z") ||
                (pdbqt_element == "G ") || (pdbqt_element == " G") || (pdbqt_element == "GA") ||
                (pdbqt_element == "J ") ||
                (pdbqt_element == " J") || (pdbqt_element == "Q ") || (pdbqt_element == " Q")) {
                element = "C";
                element += " ";
            } //all these are carbons
            else if (pdbqt_element == "HD") {
                element = "H";
                element += " ";
            } //HD are hydrogens
            else if (pdbqt_element == "HS") {
                element = "H";
                element += " ";
            } //HS are hydrogens
            else if (pdbqt_element == "NA") {
                element = "N";
                element += " ";
            } //NA are nitrogens
            else if (pdbqt_element == "NS") {
                element = "N";
                element += " ";
            } //NS are nitrogens
            else if (pdbqt_element == "OA") {
                element = "O";
                element += " ";
            } //OA are oxygens
            else if (pdbqt_element == "OS") {
                element = "O";
                element += " ";
            } //OS are oxygens
            else if (pdbqt_element == "SA") {
                element = "S";
                element += " ";
            } //SA are sulphurs
