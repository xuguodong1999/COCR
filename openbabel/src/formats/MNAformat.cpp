...
namespace OpenBabel {
    ...
    bool MNAFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (pConv->IsOption("d", OBConversion::GENOPTIONS)) {
            if (pConv->GetOutputIndex() == 1)
                obErrorLog.ThrowError(__FUNCTION__, "MNA includes hydrogens by definition, just be aware of that.",
                                      obInfo);
        ...
        FOR_ATOMS_OF_MOL(atom, mol) ofs << MNAize(&*atom, levels) << endl;
    ...
    string MNAFormat::MNAize(OBAtom *const atom, unsigned level) {
        ...
        if (level != 0) {
            ...
            vector<OBAtom *> nbors;
            FOR_NBORS_OF_ATOM(nbor, atom) nbors.push_back(&*nbor);
