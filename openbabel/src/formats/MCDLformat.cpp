...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *GetMIMEType() { return "chemical/x-MCDL"; }
...
//    return "started"; //passed
        ...
        while (i < naStore) {
            if ((nConn[i] == 1) && (nHydr[i] == 0)) {
                k = -1;
                for (j = 0; j < nbStore; j++)
                    if ((bonds[j][0] == (i + 1)) || (bonds[j][1] == (i + 1))) {
                        k = j;
                        break;
                    }
                if (k >= 0) {
...
//bond stereo getting
        ...
        if (chgflag != 0) {
            ...
            for (i = 0; i < nt - 1; i++)
                for (j = i; j < nt; j++)
                    if ((chgarray[i][0] == chgarray[j][0]) && (chgarray[i][1] > chgarray[j][1])) {
                        n = chgarray[i][1];
                        chgarray[i][1] = chgarray[j][1];
                        chgarray[j][1] = n;
                    }
                    ...
                    for (j = 0; j < nt; j++)
                        if (chgarray[j][0] == i) {
                            if (chgarray[j][1] == 0)
                                ...
                                frag = "" + intToStr(chgarray[j][1]) + "," + intToStr(abs(i)) +
                                       "-";//Math.abs(i)+"-"+chgarray[j][1];
...
//passed
        ...
        for (i = 0; i < nelements; i++) {
            ...
            for (j = 1; j <= n1; j++) {
                ...
                if (sstore.length() > 0)
                    if (parseFormula(sstore, enumber)) { //bf=Formula.parseString(sstore);
                        for (k = 1; k < NELEMMAX; k++)
                            if (enumber[k] > 0)
                                for (m = 1; m <= enumber[k]; m++) {
                ...
                if (charges[nt - 1] != 0) {      //search for appropriate atom to put charge..
                    if (nPrev == acount) {
                        ...
                        if (charges[nt - 1] < 0) {
                            assignCharges(aPosition, iA1, iA2, aCharge, charges, bondOrders, 8, nPrev, nt, acount,
                                          bcount);
                            assignCharges(aPosition, iA1, iA2, aCharge, charges, bondOrders, 7, nPrev, nt, acount,
                                          bcount);
                            assignCharges(aPosition, iA1, iA2, aCharge, charges, bondOrders, 16, nPrev, nt, acount,
                                          bcount);
                            if (charges[nt - 1] != 0) {
                                aCharge[nt - 1] = charges[nt - 1];
                            };
        ...
        for (i = 0; i < bcount; i++)
            if (bondOrders[i] == 1)
                stereoBonds[i] = -1; //flags for bonds, which might be stereo
        ...
        if (pmol != nullptr) {
            ...
            for (i = 0; i < bcount; i++) {
                flags = 0;
                if (stereoBonds[i] == 1) flags = OB_WEDGE_BOND; else if (stereoBonds[i] == 2) flags = OB_HASH_BOND;
    ...
    bool MCDLFormat::parseFormula(const string formulaString, std::vector<int> &enumber) {
        ...
        for (i = 1; i < NELEMMCDL; i++)
            if (strlen(OBElements::GetSymbol(i)) == 2) {
                test = true;
                ...
                while (test) {
                    ...
                    if (n >= 0) {
                        ...
                        k = 1;
                        if (n < value.length())
                            if ((value.at(n) >= '0') && (value.at(n) <= '9')) {
                                n1 = n;
                                n2 = n;
                                while ((n2 < (value.length() - 1)) && (value.at(n2) >= '0') && (value.at(n2) <= '9'))
                                    n2++;
        ...
        for (i = 1; i < NELEMMCDL; i++)
            if (strlen(OBElements::GetSymbol(i)) == 1) {
                test = true;
                asym = OBElements::GetSymbol(i);
                while (test) {
                    test = false;
