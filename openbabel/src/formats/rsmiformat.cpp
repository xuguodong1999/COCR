...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *GetMIMEType() { return "chemical/x-daylight-smiles"; }; // not right, need something else
    ...
    bool SmiReactFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (pos != string::npos) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  rsmiles + " contained a character '" + rsmiles[pos] + "' which is invalid in SMILES",
                                  obError);
            return false;
        }
