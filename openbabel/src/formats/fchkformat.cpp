...
namespace OpenBabel {
    ...
    private :
        ...
        template<class T>
        static bool read_numbers(const char *const,
                                 vector<T> &,
                                 const unsigned int width = 0);

        template<class T>
        static bool read_section(const char *const,
                                 vector<T> &,
                                 const unsigned int,
                                 bool *const,
                                 const char *const,
    ...
    bool FCHKFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (-1 != MxBond) {
            ...
            if (NBond.end() != find_if(NBond.begin(),
                                       NBond.end(),
                                       bind(less_equal<int>(), placeholders::_1, 0)) ||
                NBond.end() != find_if(NBond.begin(),
                                       NBond.end(),
                                       bind(greater<int>(), placeholders::_1, MxBond)) ||
                IBond.end() != find_if(IBond.begin(),
                                       IBond.end(),
                                       bind(less<int>(), placeholders::_1, 0)) ||
                IBond.end() != find_if(IBond.begin(),
                                       IBond.end(),
                                       bind(greater<int>(), placeholders::_1, Natoms))) {
