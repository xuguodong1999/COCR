...
namespace OpenBabel {
    ...
    public:
        ...
        GenBankFormat() { // Copied from the Chemical MIME Page at http://www.ch.ic.ac.uk/chemime/
        ...
        virtual const char *
        SpecificationURL() { return "http://www.ncbi.nlm.nih.gov/Sitemap/samplerecord.html"; } //optional
        ...
        virtual const char *GetMIMEType() { return "chemical/x-genbank"; }

        virtual unsigned int Flags() { return NOTWRITABLE | READONEONLY; }
    ...
    bool GenBankFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        getline(*in, line);
        while (!in->eof()) { // We have a new line in 'line'
            if (!line.compare(0, 6, "LOCUS", 6) || !line.compare(0, 2, "ID ", 2)) {
                if (sequence_type == GenBankFormat::UnknownSequence) { // attempt to determine the sequence type
        ...
        bool rv = ReadFASTASequence(pmol, sequence_type, in,
                                    ...
                                    !pConv->IsOption("s", OBConversion::INOPTIONS), false);
        pmol->EndModify();
        return rv;
    }
