...
// The average molecular masses used by InChI are listed in util.c or the InChI Technical Manual Appendix 1
    ...
    const unsigned int inchi_avg_mass[
            MAX_AVG_MASS + 1] = {0, 1, 4, 7, 9, 11, 12, 14, 16, 19, 20, 23, 24, 27, 28, 31, 32, 35, 40, 39, 40, 45, 48,
                                 51, 52, 55, 56, 59, 59, 64, 65, 70, 73, 75, 79, 80, 84, 85, 88, 89, 91, 93, 96, 98,
                                 101, 103, 106, 108, 112, 115, 119, 122, 128,
                                 127, 131, 133, 137, 139, 140, 141, 144, 145, 150, 152, 157, 159, 163, 165, 167, 169,
                                 173, 175, 178, 181, 184, 186, 190, 192, 195,
                                 197, 201, 204, 207, 209, 209, 210, 222, 223, 226, 227, 232, 231, 238, 237, 244, 243,
                                 247, 247, 251, 252, 257, 258, 259, 260, 261,
    ...
    bool InChIFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (pmol->HasData("inchi") && pConv->IsOption("r") == nullptr && pConv->IsOption("a") == nullptr &&
            ...
            if (mol.GetDimension() == 3 ||
                (mol.GetDimension() == 2 && pConv->IsOption("s", pConv->OUTOPTIONS) != nullptr))
                        ...
                        if (stereo != OBStereo::NotStereo) {
                            switch (stereo) {
                                ...
                                case OBStereo::UnknownDir:
                                    ...
                                    break;
                                default:; // INCHI_BOND_STEREO_NONE
                            }
                        }
