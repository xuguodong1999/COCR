    ...
    bool COFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (vs.size() != 4) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Unable to read Culgi Object File. First line incorrectly formatted or file is empty.",
                                  obWarning);
            return (false);
        }
        if (vs[0] != "@CulgiVersion:") {
            obErrorLog.ThrowError(__FUNCTION__, "Unable to read Culgi Object File. First line incorrectly formatted.",
                                  obWarning);
            return (false);
        }
        ...
        if (!bFoundKey) {
            obErrorLog.ThrowError(__FUNCTION__, "Unable to read Culgi Object File. File does not contain a molecule",
                                  obWarning);
            return (false);
        }
        pmol->SetTitle(name);
        ...
        while (ifs.getline(buffer, BUFF_SIZE)) {
            ...
            if (vs[0] == "atom") {
                if (vs.size() < offset + 9) {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Unable to read Culgi Object File. Atom line appears truncated.", obWarning);
                    return (false);
                }
                ...
                if (vs.size() < 4) {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Unable to read Culgi Object File. Bond line appears truncated.", obWarning);
                    return (false);
                }
                int iniatom = -1;
                int finatom = -1;
                ...
                if (version > 7) {
                    ...
                    if (iniatom < 0 || finatom < 0) {
                        errorMsg << "Unable to read Culgi Object File. Atoms " << vs[0] << " or " << vs[1]
                                 << "not found in atom list.\n";
                ...
                if (vs.size() != 3) {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Unable to read Culgi Object File. Formal charge line appears truncated",
                                          obWarning);
                    return (false);
                }
