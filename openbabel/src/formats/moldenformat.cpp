    ...
    bool OBMoldenFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        while (getline(ifs, lineBuffer)) {
            if (lineBuffer.find("[Atoms]") != string::npos ||
                ...
                if (ecpLines != 0) {
                    cerr << "WARNING: element number given in 3rd column does not agree with element name on "
                         << ecpLines << " lines." << endl
                ...
                while (getline(ifs, lineBuffer)) {
                    ...
                    if (sscanf(lineBuffer.c_str(), "%d", &natoms) == 0 || !natoms) {
                        obErrorLog.ThrowError(__FUNCTION__,
                                              "Problems reading an XYZ geometry: The first line must contain the number of atoms.",
                                              obWarning);
//                return(false);
                    }
...
//                return(false);
                    ...
                    if (fabs(energy) < 1.0e-8) {
                        obErrorLog.ThrowError(__FUNCTION__,
                                              "Problems reading an XYZ geometry: The second line should contain the energy.",
                                              obWarning);
                    }
