...
/* IL_ISOT_ORD, */
                      {"/",  "isotopic",                                "isotopic",      0}, /* isotopic */
/* IL_STER_ORD, */
                      {"/",  "stereo",                                  "stereo",        0}, /* stereo */
                /* items */
/* IL_VERS_ORD, */
                      {"",   "version",                                 "version",       1},
/* IL_FML__ORD, */
                      {"/",  "formula",                                 "formula",       1}, /* basic part formula */
/* IL_CONN_ORD, */
...
/* IL_SP3S_ORD, */
                      {"/t", "sp3",                                     "sp3",           0},
/* IL_INVS_ORD, */
                      {"/m", "sp3:inverted",                            "abs.inverted",  0}, /* mirrored */
/* IL_TYPS_ORD, */
                      {"/s", "type (1=abs, 2=rel, 3=rac)",              "type",          0}, /* stereo type */
                /* isotopic */
/* IL_ATMS_ORD, */
                      {"/i", "atoms",                                   "atoms",         1},
...
/* IL_XCGA_ORD, */
                ...
                /* fixed H only */
/* IL_FMLF_ORD, */
                      {"/f", "formula",                                 "formula",       1}, /* fixed H formula */
/* IL_HFIX_ORD, */
                      {"/h", "H_fixed",                                 "H-fixed",       1}, /* fixed-H */
/* IL_TRNS_ORD, */
                      {"/o", "transposition",                           "transposition", 0}, /* order */
/* IL_REC__ORD, */
...
/* AL_ISOT_ORD, */
                      {"/",     "isotopic",                             "isotopic",            0}, /* isotopic */
/* AL_STER_ORD, */
                      {"/",     "abs_stereo_inverted",                  "stereo.abs.inverted", 0}, /* inv abs sp3 stereo */
/* AL_REVR_ORD, */
...
int OutputINChIXmlStructStartTag(INCHI_IOSTREAM *output_file, char *pStr, int ind /* indent*/,
    ...
    if (bNoStructLabels) {
        ...
        inchi_ios_print(output_file, "%s\n", pStr);
        ret = 1; /*  success */
    } else if (!(szSdfLabel && szSdfLabel[0]) && !(szSdfValue && szSdfValue[0])) {
...
int OutputINChI2(char *pStr, int nStrLen,
    ...
    int bINChIOutputOptions0 =
            bINChIOutputOptions & ~(INCHI_OUT_XML | INCHI_OUT_PLAIN_TEXT | INCHI_OUT_PLAIN_TEXT_COMMENTS);
    ...
    for (i = 0, is = pINChISort; i < num_components; i++, is++) {
        ...
        for (j = TAUT_NON; j < TAUT_NUM; j++) {
                ...
                case OUT_TN: /* x[TAUT_YES]: tautomeric if present otherwise non-tautomeric;
                    ...
                    if (jj < 0) {
                        ...
                        if (bTautAndNonTaut && bTautIsNonTaut &&
                            ...
                            !is->pINChI[jj]->bDeleted && !is->pINChI[jj]->lenTautomer) { ; /* the requested non-tautomeric component is in tautomeric position
                             (is->pINChI[TAUT_YES]);
            ...
            if ((pINChI = is->pINChI[jj]) && pINChI->nNumberOfAtoms > 0) {
                ...
                bCurIsoStereoSp2 =
                        bCurIsoHStereo && pINChI->StereoIsotopic && (pINChI->StereoIsotopic->nNumberOfStereoBonds > 0);
                bCurIsoStereoSp3 = bCurIsoHStereo && pINChI->StereoIsotopic &&
                                   (pINChI->StereoIsotopic->nNumberOfStereoCenters > 0);
                bCurIsoStereoSp3Inv =
                        bCurIsoStereoSp3 && pINChI->StereoIsotopic->nCompInv2Abs; /* inversion changes sp3 stereo */
                ...
                if (bRequestedRelativeStereo) {
                    bCurStereoSp3 =
                            bCurRelative || (bCurStereoSp3 && (pINChI->Stereo->nNumberOfStereoCenters > 1)); /* Fix11 */
                    bCurIsoStereoSp3 = bCurIsoRelative ? bCurIsoStereoSp3 : 0;
                } else {
                    if (bRequestedRacemicStereo) {
                        bCurStereoSp3 = bCurRacemic > 1 ||
                                        (bCurStereoSp3 && (pINChI->Stereo->nNumberOfStereoCenters > 1)); /* Fix11 */
                ...
                if (!(bINChIOutputOptions & INCHI_OUT_NO_AUX_INFO) && (pINChI_Aux = is->pINChI_Aux[jj])) {
                    ...
                    if (bCurIso) {
                        ...
                        if (bCurTaut) {
                            bIsotopicTautEqu[ii] |= (bCurIsoTautEqu = bHasEquString(
...
*/
    ...
    ***************************************************************/
    ...
    if (bOutType == OUT_TN && !bNonTautIsIdenticalToTaut /* added 2004-10-04 Fix16 */
        ...
        if (bOutType == OUT_NONTAUT && bOutputType == OUT_TN &&
            bSecondNonTautPass /* && bTautomeric && bNonTautomeric*/ ) {
    ...
    if (!(bINChIOutputOptions & INCHI_OUT_NO_AUX_INFO)) {
        ...
        if (bXml) {
            ...
            if (INCHI_BAS == iINChI) {
                tot_len = sprintf(pStr,
                                  "AuxInfo="); /* in wINChI window, separate INChI: from AuxInfo: with blank line */
        ...
        if (bIsotopic && !i &&
            ...
            if (bIsotopicAtomEqu[iCurTautMode]) {
                ...
                tot_len = str_AuxIsoEqu(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                                        &bOverflow, bOutType, TAUT_MODE, num_components,
                                        bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
                if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr, bXml ? 0 : -2/*was -1: Fix15*/,
                                bPlainTextTags))
                    goto exit_function;
                inchi_ios_print(output_file, "%s%s", pStr, pLF);
            } else {
    ...
    for (j = *i; j < num_inp_atoms;) {
        ...
        if (at[j].p_parity) {
            ...
            for (n = 0, k = 0; n < MAX_NUM_STEREO_ATOM_NEIGH; n++) {
                ...
                } else if ((int) neigh == j && at[neigh].orig_at_number == at[j].p_orig_at_num[n]) {
            ...
            if (is_ok && b_self <= 1 && b_self + k == MAX_NUM_STEREO_ATOM_NEIGH) {
                ...
                } else if (ATOM_PARITY_ILL_DEF(at[j].p_parity)) {
                    parity = at[j].p_parity;
                } else { ; /* invalid atom parity */
                }
            } else { ;/* add error message here */
            }
        }
...
int WriteOrigBonds(int num_inp_atoms, inp_ATOM *at, int *i, char *szBuf, int buf_len, STRUCT_DATA *sd) {
    ...
    char szCurBonds[7 * MAXVAL +
                    2]; /* num_neigh*(1 byte bond type + 2 bytes for bond parity up to 4 digits per neighbor number) + at the end one ';' */
        ...
        for (kk = 0; kk < at[j].valence; kk++) {
            ...
            if (j2 < j) {
                ...
                if ((chain_len == 1 && chain_len2 == 1) ||  /* regular stereobond */
                    ...
                    chain_len2 = 0;
                } else if (chain_len2 > 1 && j2 > pnxt_atom2) { /* j2 is a cumulene endpoint */
                    int m;
                    pcur_atom = j2;
                    picur2nxt = k2;
                ...
                if (chain_len) {
                    ...
                    if (bWellDef || bHasMetal) {
                        ...
                        if (neigh1 < num_inp_atoms) {
                            bNeighSwitched1 = (neigh1 !=
                                               at[pcur_atom].neighbor[(int) at[pcur_atom].sn_ord[picur_sb_parity_ord]]);
                        } else {
                        ...
                        if (neigh2 < num_inp_atoms) {
                            bNeighSwitched2 = (neigh2 !=
                                               at[pnxt_atom].neighbor[(int) at[pnxt_atom].sn_ord[pinxt_sb_parity_ord]]);
                        } else {
                            AddMOLfileError(sd->pStrErrStruct, "Cannot find 0D stereobond neighbor");
                            /*
                            sd->nStructReadError =  99;
...
int FillOutOrigStruct(ORIG_ATOM_DATA *orig_inp_data, ORIG_STRUCT *pOrigStruct, STRUCT_DATA *sd) {
    ...
    if (pOrigStruct->szAtoms &&
        ...
        i == orig_inp_data->num_inp_atoms) { ; /* success */
    } else {
        return -1;
    }
    /* bonds */
