...
void DeAllocateForNonStereoRemoval(AT_RANK **nAtomNumberCanon1, AT_RANK **nAtomNumberCanon2,
                                   NEIGH_LIST **nl, NEIGH_LIST **nl1, NEIGH_LIST **nl2, AT_RANK **nVisited1,
                                   AT_RANK **nVisited2);
...
int CheckNextSymmNeighborsAndBonds(sp_ATOM *at, AT_RANK cur1, AT_RANK cur2, AT_RANK n1, AT_RANK n2,
                                   ...
                                   AT_RANK *nVisitOrd1, AT_RANK *nVisitOrd2, const AT_RANK *nRank1,
...
int CalculatedPathsParitiesAreIdentical(sp_ATOM *at, int num_atoms, const AT_RANK *nSymmRank,
                                        AT_RANK *nCanonRank, AT_RANK *nAtomNumberCanon, AT_RANK *nAtomNumberCanon1,
                                        AT_RANK *nAtomNumberCanon2,
                                        AT_RANK *nVisited1, AT_RANK *nVisited2,
                                        AT_RANK prev_sb_neigh, AT_RANK cur, AT_RANK next1, AT_RANK next2,
                                        int nNeighMode,
...
int RemoveCalculatedNonStereoBondParities(sp_ATOM *at, int num_atoms, int num_at_tg,
                                          AT_RANK **pRankStack1, AT_RANK **pRankStack2, AT_RANK *nTempRank,
                                          NEIGH_LIST *NeighList,
                                          AT_RANK *nCanonRank, const AT_RANK *nSymmRank,
                                          AT_RANK *nAtomNumberCanon, AT_RANK *nAtomNumberCanon1,
                                          AT_RANK *nAtomNumberCanon2,
 ...
 *
 */
int SortedEquInfoToRanks(const AT_RANK *nSymmRank, AT_RANK *nRank, const AT_RANK *nAtomNumber, int num_atoms,
                         int *bChanged) {
...
/**************************************************************************************/
void SortNeighListsBySymmAndCanonRank(int num_atoms, NEIGH_LIST *NeighList, const AT_RANK *nSymmRank,
...
int DifferentiateRanksBasic(int num_atoms, NEIGH_LIST *NeighList,
    ...
    do {
        *lNumIter += 1;
        ...
        /*  the following call creates pnCurrRank out of pnPrevRank */
        nNumCurrRanks = SetNewRanksFromNeighLists(num_atoms, NeighList, pnPrevRank, pnCurrRank, nAtomNumber,
                                                  bUseAltSort, CompNeighListRanks);
    } while (nNumPrevRanks != nNumCurrRanks || memcmp(pnPrevRank, pnCurrRank, num_atoms * sizeof(pnCurrRank[0])));
    return nNumCurrRanks;
}
...
int parity_of_mapped_atom2(int from_at, int to_at, const sp_ATOM *at, EQ_NEIGH *pEN,
    ...
    num_trans_from = insertions_sort(nNeighNumberFrom, num_neigh, sizeof(nNeighNumberFrom[0]),
    ...
    if (nNumCompNeighborsRanksCountEql) {
        ...
        for (i = 1, r = 0, r1 = nNeighRankFrom[neigh1 = nNeighNumberFrom[0]];
             i < num_neigh; i++, r1 = r2, neigh1 = neigh2) {
...
int map_an_atom2(int num_atoms, int num_max, int at_no1/*from*/, int at_no2/*to*/,
    ...
    nNumTies = NumberOfTies(pRankStack1, pRankStack2, length, at_no1, at_no2, &nNewRank, bAddStack,
                            &bAtFromHasAlreadyBeenMapped);
    ...
    if (mode == MAP_MODE_C2v || mode == MAP_MODE_C2) {
        ...
        for (i = 0; i <= 2; i++) {
            if (i == 0) {
                ...
                nn1 = n2;
                nn2 = n1;
            } else if (mode == MAP_MODE_C2v) {   /* was '=', pointed by WDI */
                /* i = 1 or 2
                 * C2v. Other neighbors must be unmoved: map
            ...
            } else if (mode == MAP_MODE_C2) {  /* was '=', pointed by WDI */
                /* i = 1 or 2
                 * C2.  Other neighbors should be mapped on each other
            ...
            if (nRank10 == nRank20 && nRank1 == nRank2) {
                if (nRank10 == nRank1) { ;/* atoms are already mapped */
                } else {
...
int CreateCheckSymmPaths(sp_ATOM *at, AT_RANK prev1, AT_RANK cur1, AT_RANK prev2, AT_RANK cur2,
    ...
    if (PARITY_WELL_DEF(at[cur1].stereo_atom_parity) &&
        ...
        } else if (*bParitiesInverted != (at[cur1].stereo_atom_parity + at[cur2].stereo_atom_parity) % 2) {
            return 0; /*  Different known in advance parities have wrong "inverted" relation */
        }
    } else if (PARITY_KNOWN(at[cur1].stereo_atom_parity) &&
    ...
    for (k = 1, k1 = 1, k2 = 1; k < at[cur1].valence; k++, k1++, k2++) {
        ...
        if (!nVisited1[n1]) { /*  recursion */
            ...
            pbParitiesInverted = (at[cur1].nRingSystem == at[n1].nRingSystem) ? bParitiesInverted
                                                                              : &bParitiesInvertedZero;
            if (0 >= (ret = CreateCheckSymmPaths(at, cur1, n1, cur2, n2, nAvoidCheckAtom,
                                                 nVisited1, nVisited2, nVisitOrd1, nVisitOrd2,
                                                 nl1, nl2, nRank1, nRank2, nCanonRank, nLength, pbParitiesInverted,
                                                 mode))) {
                return ret;
            }
        }
    ...
    */
    ...
    for (i = 0; i < pCS->nLenLinearCTStereoCarb; i++, nNumComparedCenters += (k > 0)) {
        ...
        if ((nNeighMode == NEIGH_MODE_RING && (i11 != i01) && (i12 != i01)) ||
            ...
            } else if (ATOM_PARITY_WELL_DEF(parity) && ATOM_PARITY_WELL_DEF(parity12)) {
    ...
    for (i = 0; i < pCS->nLenLinearCTStereoDble; i++, nNumComparedBonds += (k > 0)) {
        ...
        i11 = nAtomNumberCanon1[r1 -
                                1]; /*  ord. number of the atom that got canon rank r1 during the parallel traversal */
        i12 = nAtomNumberCanon1[r2 -
                                1]; /*  ord. number of the atom that got canon rank r2 during the parallel traversal */
        ...
        if ((nNeighMode == NEIGH_MODE_RING && (i11 != i01 || i12 != i02) && (i11 != i02 || i12 != i01)) ||
            nNeighMode ==
            NEIGH_MODE_CHAIN                    /*  in NEIGH_MODE_CHAIN case here we always have 2 different atoms */
                ) {
            /*******************************************/
...
int RemoveCalculatedNonStereoBondParities(sp_ATOM *at, int num_atoms, int num_at_tg,
    ...
    for (i1 = 0; i1 < num_atoms && !RETURNED_ERROR(ret_failed); i1++) {
        ...
        for (n1 = 0;
             n1 < MAX_NUM_STEREO_BONDS && !RETURNED_ERROR(ret_failed) && (s2 = at[i1].stereo_bond_neighbor[n1]); n1++) {
            ...
            switch (nNeighMode) {
                ...
                case NEIGH_MODE_RING:
                    ...
                    bSymmNeigh = BreakNeighborsTie(at, num_atoms, num_at_tg, opposite_atom, i1,
                                                   neigh, 0, 1, 0,
                                                   pRankStack1, pRankStack2, nTempRank, NeighList, nSymmRank,
                                                   nCanonRank,
            ...
            if (0 < (ret1 = CreateCheckSymmPaths(at, (AT_RANK) i1, neigh[0], (AT_RANK) i1, neigh[1], nAvoidCheckAtom,
                ...
                0 < (ret2 = CalculatedPathsParitiesAreIdentical(at, num_atoms, nSymmRank,
                                                                nCanonRank, nAtomNumberCanon, nAtomNumberCanon1,
                                                                nAtomNumberCanon2,
                ...
                if (ret2 & (NOT_WELL_DEF_UNKN | NOT_WELL_DEF_UNDF)) {
                    ...
                    if ((PARITY_ILL_DEF(at[i1].stereo_bond_parity[n1]) &&
                         PARITY_VAL(at[i1].stereo_bond_parity[n1]) > new_parity) ||
                        PARITY_CALCULATE(at[i1].stereo_bond_parity[n1])) {
                        /*  set new unknown or undefined parity */
                        SetOneStereoBondIllDefParity(at, i1, /* atom number*/ n1 /* stereo bond ord. number*/,
                                                     new_parity);
                        /*  change in pCS */
...
int RemoveCalculatedNonStereoCenterParities(sp_ATOM *at, int num_atoms, int num_at_tg,
    ...
    for (i = 0; i < num_atoms && !RETURNED_ERROR(ret_failed); i++) {
        ...
        for (j = 0; j < at[i].valence; j++) {
            neigh[j] = nl[i][j +
                             1]; /*  sorting does NOT guarantee that canon. rank of neigh[0] is greater than others */
        }
        /* 
                    ...
                    */
                    ...
                    if (0 < (ret1 = CreateCheckSymmPaths(at, (AT_RANK) i, neigh[j], (AT_RANK) i, neigh[k],
                        ...
                        0 < (ret2 = CalculatedPathsParitiesAreIdentical(at, num_atoms, nSymmRank,
                                                                        nCanonRank, nAtomNumberCanon, nAtomNumberCanon1,
                                                                        nAtomNumberCanon2,
                                                                        nVisited1, nVisited2, (AT_RANK) MAX_ATOMS,
                                                                        (AT_RANK) i,
                        ...
                        if (ret2 & (NOT_WELL_DEF_UNKN | NOT_WELL_DEF_UNDF)) {
                            /*  possibly change the parity to unknown or undefined */
                            int new_parity = (ret2 & NOT_WELL_DEF_UNKN) ? vABParityUnknown /*AB_PARITY_UNKN*/
                                                                        : AB_PARITY_UNDF;
                            if ((PARITY_ILL_DEF(at[i].stereo_atom_parity) &&
                                ...
                                at[i].stereo_atom_parity =
                                        (at[i].stereo_atom_parity ^ PARITY_VAL(at[i].stereo_atom_parity)) |
                                        PARITY_VAL(new_parity);
