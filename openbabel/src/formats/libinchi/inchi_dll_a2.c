...
int CopyLinearCTStereoToINChIStereo(INChI_Stereo *Stereo,
                                    ...
                                    AT_STEREO_DBLE *LinearCTStereoDble, int nLenLinearCTStereoDble, AT_NUMB *pCanonOrd,
                                    AT_RANK *pCanonRank, sp_ATOM *at, int bIsotopic,
                                    AT_STEREO_CARB *LinearCTStereoCarbInv, AT_STEREO_DBLE *LinearCTStereoDbleInv,
...
int NormOneStructureINChI(INCHIGEN_DATA *gendata,
    ...
    */
    ...
    if (prep_inp_data[iINChI].num_components > 1) {
        all_inp_norm_data = (INP_ATOM_DATA2 *) inchi_calloc(prep_inp_data[iINChI].num_components,
                                                            sizeof(all_inp_norm_data[0]));
    }
    ...
    MYREALLOC2(PINChI2, PINChI_Aux2, pINChI2[iINChI], pINChI_Aux2[iINChI], sd->num_components[iINChI],
               cur_prep_inp_data->num_components, k);
    if (k) {
    ...
    genctl->cti[iINChI] = (COMPONENT_TREAT_INFO *) inchi_calloc(sd->num_components[iINChI],
                                                                sizeof(COMPONENT_TREAT_INFO));
...
/* pINChI2[m=iINChI-1][j< prep_inp_data[m].num_components][TAUT_NON] */
    ...
    SetConnectedComponentNumber(inp_cur_data->at, inp_cur_data->num_at,
                                i + 1); /*  normalization alters structure component number */
        ...
        if (cur_is_non_taut + cur_is_taut) {
            ...
            for (j = j1; j <= j2; j++) {
                int bIsotopic = (pINChI[i][j]->nNumberOfIsotopicAtoms ||
                                 ...
                                 (pINChI[i][j]->nPossibleLocationsOfIsotopicH &&
                                  pINChI[i][j]->nPossibleLocationsOfIsotopicH[0] > 1));
                if (j == TAUT_YES) {
...
int Normalization_step(INChI **ppINChI,
    ...
    if (out_norm_data[TAUT_NON]->at && out_norm_data[TAUT_YES]->at && z->at[TAUT_NON] &&
        !z->s[TAUT_YES].nLenLinearCTTautomer) {
...
/*^^^ */
    ...
    if ((z->nUserMode & REQ_MODE_BASIC) &&
        (!out_norm_data[TAUT_NON]->at || !ppINChI[TAUT_NON] || !ppINChI_Aux[TAUT_NON] || !z->at[TAUT_NON])) {
        z->nUserMode ^= REQ_MODE_BASIC;
    }
    if ((z->nUserMode & REQ_MODE_TAUT) &&
        (!out_norm_data[TAUT_YES]->at || !ppINChI[TAUT_YES] || !ppINChI_Aux[TAUT_YES] || !z->at[TAUT_YES])) {
    ...
    if ((z->nUserMode & REQ_MODE_NON_ISO) && !(z->nUserMode & REQ_MODE_ISO)) {
        ...
        if (t_group_info) {
            ...
            if (t_group_info->nIsotopicEndpointAtomNumber) {
                t_group_info->nIsotopicEndpointAtomNumber[0] = inchi_min(1,
                                                                         t_group_info->nIsotopicEndpointAtomNumber[0]);
            }
            memset(t_group_info->num_iso_H, 0, sizeof(t_group_info->num_iso_H));
            memset(t_group_info->tni.nNumRemovedProtonsIsotopic, 0,
                   sizeof(t_group_info->tni.nNumRemovedProtonsIsotopic));
    ...
    for (i = z->n2; i >= z->n1 && !RETURNED_ERROR(ret); i--) {
        ...
        switch (i) {
            ...
            case TAUT_NON: /*  non-tautomeric */
                ...
                z->nMode |= (z->bHasIsotopicAtoms && z->s[TAUT_NON].bMayHaveStereo &&
                             (z->nUserMode & REQ_MODE_ISO_STEREO)) ? CANON_MODE_ISO_STEREO : 0;
                ...
                if ((ret = AllocateCS(pCS, z->num_atoms, z->num_atoms, z->s[TAUT_NON].nLenCT,
                                      z->s[TAUT_NON].nLenCTAtOnly,
            ...
            case TAUT_YES: /*  tautomeric */
                ...
                z->nMode |= (z->bHasIsotopicAtoms && z->s[TAUT_YES].bMayHaveStereo &&
                             (z->nUserMode & REQ_MODE_ISO_STEREO)) ? CANON_MODE_ISO_STEREO : 0;
                z->nMode |= (z->nUserMode & REQ_MODE_NOEQ_STEREO) ? CMODE_NOEQ_STEREO : 0;
                z->nMode |= (z->nUserMode & REQ_MODE_REDNDNT_STEREO) ? CMODE_REDNDNT_STEREO : 0;
                z->nMode |= (z->nUserMode & REQ_MODE_NO_ALT_SBONDS) ? CMODE_NO_ALT_SBONDS : 0;
                ...
                if ((ret = AllocateCS(pCS, z->num_atoms, z->num_at_tg, z->s[TAUT_YES].nLenCT,
                                      z->s[TAUT_YES].nLenCTAtOnly,
        ...
        if (pBCN) {
            ...
            /* old way */
            pCS->NeighList = CreateNeighList(z->num_atoms, i ? z->num_at_tg : z->num_atoms, z->at[i],
                                             pCS->bDoubleBondSquare, pCS->t_group_info);
            pCS->pBCN = NULL;
            ret = Canon_INChI(z->num_atoms, i ? z->num_at_tg : z->num_atoms,
                              z->at[i], pCS, z->nMode, i);
        }
...
/****************************************************************************/
int
CreateCompositeNormAtom(COMP_ATOM_DATA *composite_norm_data, INP_ATOM_DATA2 *all_inp_norm_data, int num_components) {
    int i, j, jj, k, n, m, tot_num_at, tot_num_H, cur_num_at, cur_num_H, nNumRemovedProtons;
    int num_comp[TAUT_NUM + 1], num_taut[TAUT_NUM + 1], num_del[TAUT_NUM + 1], num_at[TAUT_NUM + 1], num_inp_at[
            TAUT_NUM + 1];
            ...
            for (i = 0; i < num_components; i++) {
                ...
                if (jj == TAUT_INI) {
                    if (all_inp_norm_data[i][j].bExists &&
                        all_inp_norm_data[i][j].at_fixed_bonds) {
                        k = j;
                    } else if (all_inp_norm_data[i][ALT_TAUT(j)].bExists &&
                               !all_inp_norm_data[i][ALT_TAUT(j)].bDeleted &&
                               !all_inp_norm_data[i][j].bDeleted) {
                        k = ALT_TAUT(j);
                    } else if (all_inp_norm_data[i][j].bExists) {
                        k = j;
                    } else {
                        continue;
                    }
            ...
            for (i = 0; i < num_components; i++) {
                ...
                if (jj == TAUT_INI) {
                    if (all_inp_norm_data[i][j].bExists && all_inp_norm_data[i][j].at_fixed_bonds) {
                        ...
                        k = ALT_TAUT(j);
                    } else if (all_inp_norm_data[i][j].bExists && !all_inp_norm_data[i][ALT_TAUT(j)].bDeleted) {
                        k = j;
                    } else {
                        continue;
                    }
                ...
                cur_num_at =
                        all_inp_norm_data[i][k].num_at - cur_num_H;  /* number of all but explicit terminal H atoms */
                ...
                memcpy(at, at_from,
                       sizeof(composite_norm_data[0].at[0]) * cur_num_at); /* copy atoms except terminal H */
...
int CreateCompAtomData(COMP_ATOM_DATA *inp_at_data, int num_atoms, int num_components, int bIntermediateTaut) {
    ...
    if ((inp_at_data->at = CreateInpAtom(num_atoms)) &&
        (num_components <= 1 || bIntermediateTaut ||
         (inp_at_data->nOffsetAtAndH = (AT_NUMB *) inchi_calloc(sizeof(inp_at_data->nOffsetAtAndH[0]),
                                                                2 * (num_components + 1))))) {
...
^^^*/
    ...
    if ((pCS->nLenLinearCTStereoCarb > 0 || pCS->nLenLinearCTStereoDble > 0) &&
        ...
        nErrorCode = CopyLinearCTStereoToINChIStereo(Stereo,
                                                     ...
                                                     pCS->LinearCTStereoDble, pCS->nLenLinearCTStereoDble, pCanonOrd,
                                                     pCanonRank, at, 0 /* non-isotopic */
                , pCS->LinearCTStereoCarbInv, pCS->LinearCTStereoDbleInv, pCanonOrdInv, pCanonRankInv);
    ...
    if ((pCS->nLenLinearCTIsotopicStereoCarb > 0 || pCS->nLenLinearCTIsotopicStereoDble > 0) &&
        ...
        nErrorCode = CopyLinearCTStereoToINChIStereo(Stereo,
                                                     ...
                                                     pCS->nLenLinearCTIsotopicStereoDble, pCanonOrd, pCanonRank, at,
                                                     1 /* isotopic */
                , pCS->LinearCTIsotopicStereoCarbInv, pCS->LinearCTIsotopicStereoDbleInv, pCanonOrdInv, pCanonRankInv);
