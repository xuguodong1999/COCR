...
int InChI2Atom(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, const char *szCurHdr, long num_inp,
               StrFromINChI *pStruct, int iComponent, int iAtNoOffset, int bI2A_Flag, int bHasSomeFixedH,
               InpInChI *OneInput) {
...
int MergeStructureComponents(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, long num_inp, char *szCurHdr,
    ...
    if ((num_components = pOneInput->nNumComponents[INCHI_REC][TAUT_NON])) {
        iInchiRec = INCHI_REC;
        iMobileH = TAUT_NON;
    } else if ((num_components = pOneInput->nNumComponents[INCHI_REC][TAUT_YES])) {
        iInchiRec = INCHI_REC;
        iMobileH = TAUT_YES;
    } else if ((num_components = pOneInput->nNumComponents[INCHI_BAS][TAUT_NON])) {
        iInchiRec = INCHI_BAS;
        iMobileH = TAUT_NON;
    } else if ((num_components = pOneInput->nNumComponents[INCHI_BAS][TAUT_YES])) {
        iInchiRec = INCHI_BAS;
        iMobileH = TAUT_YES;
    } else {
...
int AddProtonAndIsoHBalanceToMobHStruct(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
    ...
    for (iInchiRec = 0; iInchiRec < INCHI_NUM; iInchiRec++) {
        for (k = 0; k < pOneInput->nNumComponents[iInchiRec][TAUT_YES]; k++) {
            ...
            if (iInchiRec == INCHI_REC ||
                (iInchiRec == INCHI_BAS && (k1 = pStruct[iInchiRec][TAUT_YES][k].nLink) >= 0)) {
            ...
            } else if (0 <= (k1 = -(1 + pStruct[iInchiRec][TAUT_YES][k].nLink))) {
...
int CompareAllOrigInchiToRevInChI(StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM], InpInChI *pOneInput, int bReqNonTaut,
    ...
    for (iComponent = 0; iComponent < num_components; iComponent++) {
        ...
        iMobileHpStruct =
                ...
                OldPrintCompareOneOrigInchiToRevInChI(pStruct[iInchiRec][bMobileH]+iComponent, pInChI, bMobileH,
                        ...
                        ret = CompareOneOrigInchiToRevInChI(pStruct[iInchiRec][bMobileH] + iComponent, pInChI, bMobileH,
                                                            iComponent,
    ...
    if (iMobileH == TAUT_YES) {
        if (pOneInput->nNumProtons[iInchiRec][iMobileH].pNumProtons) {
            ...
            if (nNumRemovedProtons.nNumRemovedProtons !=
                pOneInput->nNumProtons[iInchiRec][iMobileH].nNumRemovedProtons) {
            ...
            for (i = 0; i < NUM_H_ISOTOPES; i++) {
                if (nNumRemovedProtons.nNumRemovedIsotopicH[i] !=
                    pOneInput->nNumProtons[iInchiRec][TAUT_YES].nNumRemovedIsotopicH[i]) {
...
int CompareAllDisconnectedOrigInchiToRevInChI(StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM],
    ...
    */
    ...
    if (!nNumNonTaut1 ==
        !nNumNonTaut2) { ; /* difference if(nNumNonTaut1 != nNumNonTaut2) will be caught in InChI comparison */
    } else if (nNumNonTaut1) {
    ...
    if (ret == _IS_OKAY || ret == _IS_WARNING) {
        ...
        if (pStruct->RevInChI.num_components[i]) {
            ...
            if (b == b0 && b == TAUT_NON) {
                ...
                /* compare removed H */
                if (pStruct->nNumRemovedProtonsMobHInChI !=
                    pStruct->RevInChI.pINChI_Aux[i][0][TAUT_YES]->nNumRemovedProtons) {
                    CompareInchiFlags[TAUT_YES] |= INCHIDIFF_MOBH_PROTONS;
                }
            }
            memset(nCurRemovedProtons, 0, sizeof(*nCurRemovedProtons));
...
/*************************************************************************************/
INCHI_MODE
CompareReversedStereoINChI3(INChI_Stereo *s1/* InChI from reversed struct */, INChI_Stereo *s2 /* input InChI */,
                            ICR *picr) {
    int ret = 0;
    ...
    if (s1 && s2 && (s2->nCompInv2Abs != 2) && s1->nCompInv2Abs != s2->nCompInv2Abs && s1->nCompInv2Abs &&
        s2->nCompInv2Abs) {
...
int AddOneMsg(char *szMsg, int used_len, int tot_len, const char *szAddMsg, const char *szDelim) {
    ...
    if (len + len_delim + used_len < tot_len) {
        ...
        used_len += len;
    } else if ((len_to_copy = (tot_len - used_len - len_delim - (int) sizeof(ellip))) > 10) {
        if (len_delim) {
            strcpy(szMsg + used_len, szDelim);
            used_len += len_delim;
        }
                            ...
                            for (n = 0; CompareInchiMsgsGroup[n].nGroupID != IDGRP_ZERO; n++) {
                                if (CompareInchiMsgsGroup[n].nGroupID == CompareInchiMsgs[k].nGroupID) {
                                    ...
                                    if (iCurGrpIdx != iPrevGrpIdx) {
                                        ...
                                        len = AddOneMsg(szMsg, len, nLenMsg,
                                                        CompareInchiMsgsGroup[iCurGrpIdx].szGroupName, NULL);
                                    }
                                    len = AddOneMsg(szMsg, len, nLenMsg, CompareInchiMsgs[k].szMsg,
                                                    iCurGrpIdx == iPrevGrpIdx ? "," : NULL);
