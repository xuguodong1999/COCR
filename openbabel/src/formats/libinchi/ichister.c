...
static int save_a_stereo_bond(int z_prod, int result_action,
                              int at1, int ord1, AT_NUMB *stereo_bond_neighbor1, S_CHAR *stereo_bond_ord1,
                              S_CHAR *stereo_bond_z_prod1, S_CHAR *stereo_bond_parity1,
                              int at2, int ord2, AT_NUMB *stereo_bond_neighbor2, S_CHAR *stereo_bond_ord2,
                              S_CHAR *stereo_bond_z_prod2, S_CHAR *stereo_bond_parity2);
...
static double triple_prod_and_min_abs_sine2(double at_coord[][3], double central_at_coord[], int bAddedExplicitNeighbor,
                                            double *min_sine, int *bAmbiguous);
...
int GetHalfStereobond0DParity(inp_ATOM *at, int cur_at, AT_NUMB nSbNeighOrigAtNumb[], int nNumExplictAttachments,
                              int bond_parity, int nFlag);

int GetStereocenter0DParity(inp_ATOM *at, int cur_at, int j1, AT_NUMB nSbNeighOrigAtNumb[], int nFlag);

int
GetSbNeighOrigAtNumb(inp_ATOM *at, int cur_at, inp_ATOM *at_removed_H, int num_removed_H, AT_NUMB nSbNeighOrigAtNumb[]);
...
double get_z_coord(inp_ATOM *at, int cur_atom, int neigh_no, int *nType, int bPointedEdgeStereo) {
    ...
    } else if (stereo_type == STEREO_SNGL_EITHER &&
...
/*	const double one_pi = 2.0*atan2(1.0 , 0.0 ); */
    ...
    if (num_Up == nNumNeigh) {
        ...
        for (i = 0; i < nNumNeigh || bPrev_Up; i++) {
            ...
            } else if (bPrev_Up) { /* end of contiguous sequence of Up bonds */
...
/*************************************************************/
double triple_prod_and_min_abs_sine2(double at_coord[][3], double central_at_coord[], int bAddedExplicitNeighbor,
                                     double *min_sine, int *bAmbiguous) {
...
int bIsOxide(inp_ATOM *at, int cur_at) {
    ...
    for (i = 0; i < a->valence; i++) {
        ...
        } else if (bond_type == BOND_TAUTOM || bond_type == BOND_ALT12NS) {
            an = at + (int) a->neighbor[i];
            if (1 == an->valence &&
                2 == get_endpoint_valence(an->el_number)) {
                return 1;
    ...
    for (m = 0, last_parity = 0; m < MAX_NUM_STEREO_BONDS && at[cur_at].sb_parity[m]; m++) {
        ...
        if (0 <= at[cur_at].sb_ord[m] && at[cur_at].sb_ord[m] < at[cur_at].valence &&
            ...
            for (i = 0; i < nNumExplictAttachments; i++) {
                ...
                } else if (nNextSbAtOrigNumb == nSbNeighOrigAtNumb[i]) {
        ...
        if (!last_parity) {
        ...
        } else if (last_parity != cur_parity && cur_parity) {
            ...
            } else if (ATOM_PARITY_WELL_DEF(cur_parity)) {
    ...
    if (chain_length % 2) {
        ...
        */
        ...
        if (bWrong_z_dir1 && bWrong_z_dir2) {
            goto set_default;
        } else if (bWrong_z_dir1 || bWrong_z_dir2) {
        ...
        switch (k) {
            case 0:
                for (j = 0; j < num_eH; j++, nNumExplictAttachments++) {
                    ...
                    z = -get_z_coord(at, (at_removed_H - at) + next, 0 /*neighbor #*/, &nType,
            ...
            case 1:
                for (j = 0; j < at[cur_at].valence; j++, nNumExplictAttachments++) {
                    ...
                    z = get_z_coord(at, cur_at, j /*neighbor #*/, &nType,
                                    (bPointedEdgeStereo & PES_BIT_POINT_EDGE_STEREO));
                    switch (nType) {
                        case ZTYPE_EITHER:
                            num_either_single++; /*  bond in "Either" direction. */
...
int can_be_a_stereo_bond_with_isotopic_H(inp_ATOM *at, int cur_at, INCHI_MODE nMode) {
    ...
    if (0 == at[cur_at].num_H && 0 == at[cur_at].charge && 0 == at[cur_at].radical &&
        ...
        if (2 == at[cur_at].valence && 3 == at[cur_at].chem_bonds_valence) {
            type_N = 1;
        } else if (3 == at[cur_at].valence && 5 == at[cur_at].chem_bonds_valence) {
    ...
    for (i = 0, num_stereo_bonds = 0; i < at[cur_at].valence; i++) {
        ...
        if (bond_type == BOND_ALTERN) {
            ...
            if (cur_at > next_at)
                bFound = 1;
        } else if (bond_type != BOND_SINGLE && bond_type != BOND_TAUTOM) {
            num_wrong_bonds_1++;
        ...
        if (bFound) {
            ...
            if (0 == at[next_at].num_H && 0 == at[next_at].charge && 0 == at[next_at].radical &&
                ...
                if (2 == at[next_at].valence && 3 == at[next_at].chem_bonds_valence) {
                    type_N_next = 1; /* -N= */
                } else if (3 == at[next_at].valence && 5 == at[next_at].chem_bonds_valence) {
            ...
            if (j < at[next_at].valence ||                  /* at[next_at] has a wrong bond type*/
                ...
                bFound = 0;
            } else if (2 < num_2s_next) {
                bFound = 0;
            } else if (2 == num_2s_next) {
                if (2 == at[next_at].valence) { ; /* only one double bond permitted except cumulenes */
#if (N_V_STEREOBONDS == 1)
                } else if (1 == (num_2s_hetero_next[0] | num_2s_hetero_next[1]) &&
...
#define AB_UNKNOWN  0x20
    ...
    else if (nParity == AB_PARITY_EVEN)
...
int set_stereo_bonds_parity(sp_ATOM *out_at, inp_ATOM *at, int at_1, inp_ATOM *at_removed_H, int num_removed_H,
    ...
    if (0 == at[at_1].num_H && 0 == at[at_1].charge && 0 == at[at_1].radical &&
        ...
        if (2 == at[at_1].valence && 3 == at[at_1].chem_bonds_valence) {
            type_N = 1;
        } else if (3 == at[at_1].valence && 5 == at[at_1].chem_bonds_valence) {
            type_N = 2; /* unfortunately includes >N# */
        }
    }
#endif
        ...
        if (bFound) {
            ...
            if (0 == at[at_2].num_H && 0 == at[at_2].charge && 0 == at[at_2].radical &&
                ...
                if (2 == at[at_2].valence && 3 == at[at_2].chem_bonds_valence) {
                    type_N_next = 1; /* -N= */
                } else if (3 == at[at_2].valence && 5 == at[at_2].chem_bonds_valence) {
                    type_N_next = 2; /* unfortunately includes >N# */
                }
            }
#endif
...
int GetStereocenter0DParity(inp_ATOM *at, int cur_at, int j1, AT_NUMB nSbNeighOrigAtNumb[], int nFlag) {
    ...
    if (at[cur_at].p_parity && (j1 == MAX_NUM_STEREO_ATOM_NEIGH - 1 || j1 == MAX_NUM_STEREO_ATOM_NEIGH)) {
        ...
        num_trans_inp = insertions_sort(nInpNeighOrigAtNumb, MAX_NUM_STEREO_ATOM_NEIGH, sizeof(nInpNeighOrigAtNumb[0]),
                                        comp_AT_NUMB);
        ...
        if (j1 == MAX_NUM_STEREO_ATOM_NEIGH -
                  1) { ; /*num_trans_neigh += j1;*/ /* the lone pair or implicit H is implicitly at the top of the list */
        }
...
int set_stereo_atom_parity(sp_ATOM *out_at, inp_ATOM *at, int cur_at, inp_ATOM *at_removed_H,
        ...
        switch (k) {
            ...
            case 0:
                ...
                for (j = 0; j < num_explicit_H; j++, j1++) {
                    ...
                    z = -get_z_coord(at, (at_removed_H - at) + next_at, 0 /*neighbor #*/, &nType,
                                     -(bPointedEdgeStereo & PES_BIT_POINT_EDGE_STEREO));
                    switch (nType) {
                        case ZTYPE_EITHER:
    ...
    if (j1 == MAX_NUM_STEREO_ATOM_NEIGH - 1) {
        ...
        } else if (nMustHaveNumNeigh == MAX_NUM_STEREO_ATOM_NEIGH) {
    ...
    if (bAddExplicitNeighbor) {
        ...
        if (parity == vABParityUnknown /*AB_PARITY_UNKN*/ ) {
        ...
        } else if (num_z == 0 || are_3_vect_in_one_plane(at_coord, MIN_SINE)) {
            ...
            */
            ...
            if (z < MIN_BOND_LEN || rmin / rmax < MIN_SINE) {
                ...
                if (AB_PARITY_NONE ==
                    (parity = GetStereocenter0DParity(at, cur_at, j1, nSbNeighOrigAtNumb, FlagSC_0D))) {
                    parity = AB_PARITY_UNDF;
                }
                goto exit_function;
            }
    ...
    if (b2D) {
        ...
        n2DTetrahedralAmbiguity = Get2DTetrahedralAmbiguity(at_coord, bAddExplicitNeighbor,
    ...
    */
     ...
     ********************************************************/
    triple_product = triple_prod_and_min_abs_sine2(&at_coord[1], at_coord_center, bAddExplicitNeighbor, &min_sine,
                                                   &bAmbiguous);
    /*
     * check for tetrahedral ambiguity -- leave it out for now
     */
    if (fabs(triple_product) > ZERO_FLOAT &&
        (min_sine > MIN_SINE || (fabs(min_sine) > ZERO_FLOAT && (n2DTetrahedralAmbiguity & T2D_OKAY)))) {
    ...
    /*  non-isotopic parity */
    if (at[cur_at].num_H > 1 || parity <= 0); /*  no non-isotopic parity */
    else
    ...
    ***********************************************************************************/
    ...
    if (nMaxNumStereoAtoms || nMaxNumStereoBonds) {
        for (i = 0, num_stereo = 0; i < num_at; i++) {
            ...
            } else if ((num = can_be_a_stereo_bond_with_isotopic_H(at, i, nMode))) { /*  accept cumulenes */
...
int ReconcileCmlIncidentBondParities(inp_ATOM *at, int cur_atom, int prev_atom, S_CHAR *visited, int bDisconnected) {
    ...
    for (i = 0; i < MAX_NUM_STEREO_BONDS && at[cur_atom].sb_parity[i]; i++) {
        ...
        } else if (cur_parity != 2 - (cur_order_parity + cur_sb_parity) % 2) {
        ...
        } else if (nxt_parity != 2 - (nxt_order_parity + nxt_sb_parity) % 2) {
...
/*****************************************************************/
int get_opposite_sb_atom(inp_ATOM *at, int cur_atom, int icur2nxt, int *pnxt_atom, int *pinxt2cur,
