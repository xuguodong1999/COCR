...
int AddBondsPos(inp_ATOM *atom, T_BONDPOS *BondPosTmp, int nNumBondPosTmp, T_BONDPOS *BondPos, int nMaxNumBondPos,
                int nNumBondPos);

int
AddEndPoints(T_ENDPOINT *EndPointTmp, int nNumNewEndPoint, T_ENDPOINT *EndPoint, int nMaxNumEndPoint, int nNumEndPoint);
...
int bIsCenterPointStrict(inp_ATOM *atom, int iat) {
    if (atom[iat].valence == atom[iat].chem_bonds_valence) {
        ...
        if (endpoint_valence &&
            ((endpoint_valence > atom[iat].valence && /* added a check for negative charge or H 3-31-03 */
    ...
    while (nLenDfsPath >= nMinLenDfsPath) {
        ...
        if (j < atom[cur_at = (int) DfsPath[nLenDfsPath].at_no].valence) {
            ...
            if (nxt_at == nDoNotTouchAtom1 ||
                nxt_at == nDoNotTouchAtom2) { ; /*  ignore */
            } else if (nDfsPathPos[nxt_at]) {
            ...
            } else if (!(*CheckCenterPoint)(atom, nxt_at)) { ; /*  cannot advance to a non-centerpoint; ignore */
            } else if (nLenDfsPath < nCycleLen) {
                /*  advance */
                nLenDfsPath++;
                cur_at = nxt_at;
                DfsPath[nLenDfsPath].at_no = cur_at;
    ...
    while (nLenDfsPath >= nMinLenDfsPath) {
        ...
        if (j < atom[cur_at = (int) DfsPath[nLenDfsPath].at_no].valence) {
            ...
            if (!(*CheckCenterPointPath)(atom, DfsPath, nLenDfsPath, j,
                                         pBNS, pBD, num_atoms)) { ; /*  cannot advance to a non-centerpoint; ignore */
            } else if (nLenDfsPath < nCycleLen) {
                /*  advance */
                nLenDfsPath++;
                cur_at = nxt_at;
...
int are_alt_bonds(U_CHAR *bonds, int len) {
    ...
    for (i = 1; i < len; i++) {
        ...
        if (next_bond) {
            ...
            return 0;
        } else if (bonds[i] == BOND_SINGLE) {
            next_bond = BOND_DOUBLE;
            continue;
        } else if (bonds[i] == BOND_DOUBLE) {
...
#define PATH_LEN 4
    ...
    nxt_at = DfsPath[middle_pos = (nLenDfsPath + 1) /
                                  2].at_no;  /*  must have tautomeric neighbor -OH or =O or -NH2 or =NH */
...
#define PATH_LEN 4
    ...
    num_taut_endpoints =
            (0 != atom[n1_at].endpoint) + (0 != atom[n2_at].endpoint); /*  if both N atoms already are endpoints */
    /*
