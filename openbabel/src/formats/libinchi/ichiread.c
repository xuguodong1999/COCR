    ...
    for (iINChI = 0, num_diff = 0; iINChI < INCHI_NUM; iINChI++) {
        for (iTaut = TAUT_NON; iTaut < TAUT_NUM; iTaut++) {
            for (iComp = 0; iComp < OneInput->nNumComponents[iINChI][iTaut]; iComp++) {
                ...
                if (!pINChI->nNumberOfAtoms || pINChI->bDeleted || !pINChI->szHillFormula ||
                    !pINChI->szHillFormula[0]) {
                    continue;
                }
                szHillFormulaOld = pINChI->szHillFormula;
                pINChI->szHillFormula = AllocateAndFillHillFormula(pINChI);
                num_diff += !pINChI->szHillFormula || !pINChI->szHillFormula[0] ||
                            strcmp(pINChI->szHillFormula, szHillFormulaOld);
                ...
                if (!bInputInStdFormat)   /* Input InChI is a non-standard one  */
                {
                    ...
                    if (szCurHdr && szCurHdr[0])
                        inchi_ios_eprint(pLog,
                                         "Warning: forced conversion to non-standard InChI for non-std input, %s\n",
                                         szCurHdr);
                    else
                        inchi_ios_eprint(pLog,
                                         "Warning: forced conversion to non-standard InChI for non-std input, Structure %ld\n",
                                         num_inp);
                }
            }
            ...
            */
            ...
            if (bInChI2Structure && !bInChI2InChI) {
                ...
                if (!OneInput.nNumProtons[INCHI_BAS][TAUT_YES].pNumProtons &&
                    ...
                    ret = AddProtonAndIsoHBalanceToMobHStruct(ip, sd, num_inp, bHasSomeFixedH, szCurHdr, pStruct,
                                                              &OneInput);
                    if (ret < 0) {
...
-------------------------------------------------*/
                ...
                if (szMessage) {
                    ...
                    if (bHasMetal && nInitLenMessage < (len = (int) strlen(szMessage))) {
                        ...
                        memmove(szMessage + shift, szMessage + nInitLenMessage,
                                (len - nInitLenMessage) * sizeof(szMessage[0]));
            ...
            if (nReadStatus == RI_ERR_EOF && nErr == 0 && pState == 0 && !strHdr) {
                ...
                inchi_ios_eprint(pLog, "\n%s %s (%d) in %s (%d)\n", strHdr ? strHdr : szHdrSimulation,
                                 getInchiErrName(nErr), nErr, getInchiStateReadErr(pState), pState);
                num_errors++;
                num_processed++;
            }
            if (strHdr) {
...
int OutputInChIAsRequested(INCHI_IOSTREAM *pOut, INCHI_IOSTREAM *pLog,
    ...
    *sd = *sd_inp;
    ...
    if (num_components[INCHI_BAS]) {
        MYREALLOC2(PINChI2, PINChI_Aux2, pINChI[INCHI_BAS], pINChI_Aux[INCHI_BAS], num_components[INCHI_BAS],
                   num_components[INCHI_BAS], k1);
    }
    if (num_components[INCHI_REC]) {
        MYREALLOC2(PINChI2, PINChI_Aux2, pINChI[INCHI_REC], pINChI_Aux[INCHI_REC], num_components[INCHI_REC],
                   num_components[INCHI_REC], k2);
    }
                    ...
                    if (k < OneInput->nNumComponents[iINChI][j]) {
                        ...
                        *pINChI[iINChI][k][j] = OneInput->pInpInChI[iINChI][j][k];
                        ...
                        if (j == TAUT_YES && pINChI[iINChI][k][j] && pINChI[iINChI][k][j]->nNumberOfAtoms &&
                            ...
                            pINChI[iINChI][k][j]->nNum_H_fixed = (S_CHAR *) inchi_calloc(
                                    pINChI[iINChI][k][j]->nNumberOfAtoms + 1, sizeof(pINChI[0][0][0]->nNum_H_fixed[0]));
                        }
    ...
    for (k1 = 0, k2 = (bReqSplitOutputInChI ? k1 + 1 : nNumOutputComponents);
         k1 < k2 && k1 < nNumOutputComponents; k1 = k2, k2++) {
        ...
        if (bReqSplitOutputInChI) {
            ...
            for (j = 0; j < TAUT_NUM && sd->num_components[iINChI]; j++) {
                for (k = k1; k < k2; k++) {
                    ...
                    int cur_is_in_non_taut = (pINChI[iINChI][k][TAUT_NON] &&
                                              pINChI[iINChI][k][TAUT_NON]->nNumberOfAtoms > 0);
                    int cur_is_in_taut = (pINChI[iINChI][k][TAUT_YES] &&
                                          pINChI[iINChI][k][TAUT_YES]->nNumberOfAtoms > 0);
...
int DetectInpInchiCreationOptions(InpInChI *pOneInput, int *bHasReconnected, int *bHasMetal,
    ...
    *bHasMetal = 0;
    ...
    if (!num_known_SB && num_unk_und_SB) { ; /* full SUU option or SB part of it */
    } else {
        nModeFlagsValue |= REQ_MODE_SB_IGN_ALL_UU; /* ignore Unknown/Undefind SB if no well-defined SB exist */
    }

    if (!num_known_SC && num_unk_und_SC) { ; /* full SUU option or SB part of it */
    } else {
...
int SetProtonsAndXchgIsoH(int bInChI2Structure, int bReqSplitOutputInChI, int bReqProtonsForEachComponent,
    ...
    for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
        ...
        } else if (!bReqSplitOutputInChI) {
            nModeProtonIsoExchgH[iINChI] = bAvailableProtonsForEachComponent ?
                                           MODE_PIXH_ADD_TO_EACH :
                                           MODE_PIXH_ADD_TO_FIRST;
        } else if (!bAvailableProtonsForEachComponent) {
...
/******************************************************************************************/
int GetInChIFormulaNumH(INChI *pInChI, int *nNumH) {  /* get number of H including bridging hydrogen atoms */
    const char *p, *q;
    *nNumH = 0;
        ...
        for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
            ...
            for (j = 0; j < TAUT_NUM; j++) {
                ...
                for (bIsotopic = bStereoType = bStereoTypeAlt = 0; bIsotopic < 2; bIsotopic++) {
                    ...
                    nFlagsAlt =
                            bStereoTypeAlt == 2 ? INCHI_FLAG_REL_STEREO : bStereoTypeAlt == 3 ? INCHI_FLAG_RAC_STEREO
                                                                                              : 0;
                }
                ...
                for (k = 0; k < nNumComponents[iINChI][j]; k++) {
                    ...
                    if (pInpInChI[iINChI][j]) {
                        ...
                        INChI *pInChI_Alt = (k < nNumComponents[iINChI][jAlt] &&
                                             ...
                                             pInpInChI[iINChI][jAlt][k].nNumberOfAtoms) ? &pInpInChI[iINChI][jAlt][k]
                                                                                        : NULL;

                        if (nFlags) {
                            pInChI->nFlags |= nFlags;
                        } else if (j == TAUT_NON && !nFlags && nFlagsAlt) {
                            pInChI->nFlags |= nFlagsAlt;
                        }
                        /**** add empty immobile H (nNum_H) if it is missing ****/
                        if (!pInChI->nNum_H &&
                            !(pInChI->nNum_H = (S_CHAR *) inchi_calloc(pInChI->nNumberOfAtoms + 1,
                                                                       sizeof(pInChI->nNum_H[0])))) {
                            ret2 = RI_ERR_ALLOC;
                            goto exit_function;
                        }
                        ...
                        if (!pInChI->nConnTable) {
                            ...
                            if (j == TAUT_NON && k < nNumComponents[iINChI][TAUT_YES] &&
                                ...
                                if (!(pInChI->nConnTable = (AT_NUMB *) inchi_calloc(lenCT + 1,
                                                                                    sizeof(pInChI->nConnTable[0])))) {
                                    ret2 = RI_ERR_ALLOC;
                                    goto exit_function;
                                }
                                ...
                                if (j == TAUT_YES && pInChI->nNumberOfAtoms > 1) {
                                    *pState = IST_MOBILE_H_CONNECTIONS +
                                              (iINChI == INCHI_REC ? IST_HAPPENED_IN_RECMET : 0);
                                    ret2 = RI_ERR_SYNTAX;
                                    goto exit_function;
                                }
                                if (!(pInChI->nConnTable = (AT_NUMB *) inchi_calloc(pInChI->nNumberOfAtoms + 1,
                                                                                    sizeof(pInChI->nConnTable[0])))) {
                                    ret2 = RI_ERR_ALLOC;
                                    goto exit_function;
                                }
                        ...
                        } else if (pInChI->nConnTable && !pInChI->lenConnTable && pInChI->nNumberOfAtoms == 1) {
                            pInChI->nConnTable[0] = 1;
                            pInChI->lenConnTable = 1;
                        }
                            ...
                            if (bIso) {
                                if (pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters &&
                                    ...
                                    if (0 > (ret2 = CopySegment(pInChI, pInChI, CPY_SP3, 1, 0)) ||
                                        ((!pInChI->StereoIsotopic->nCompInv2Abs ||
                                          NO_VALUE_INT == pInChI->StereoIsotopic->nCompInv2Abs) &&
                                ...
                                if (pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters &&
                                    ...
                                    if ((nFlags & (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO)) &&
                                        ...
                                        pInChI->Stereo->nCompInv2Abs = s[iINChI][j][0] > 0 ? 2 : 0;
                                        pInChI->StereoIsotopic->nCompInv2Abs = s[iINChI][j][1] > 0 ? 2 : 0;
                                    } else if (!pInChI->StereoIsotopic->nCompInv2Abs ||
                                               NO_VALUE_INT == pInChI->StereoIsotopic->nCompInv2Abs) {
                        ...
                        if (j == TAUT_NON) {
                            if (pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters &&
                                ...
                                if (!s[iINChI][j][0] && s[iINChI][jAlt][0] > 0 &&
                                    /* /sN is not present in F and is present in M */
                                    ...
                                    if (pInChI_Alt->Stereo->nCompInv2Abs == 0 &&
                                        /*  M: /m. ; means no /m for this component */
                                        pInChI->Stereo->nCompInv2Abs == NO_VALUE_INT &&
                                        /*  F: no /m segment for all components */
                                        pInChI_Alt->StereoIsotopic &&                               /*  MI: present */
                                        pInChI_Alt->StereoIsotopic->nCompInv2Abs != 0 &&
                                        pInChI_Alt->StereoIsotopic->nCompInv2Abs != NO_VALUE_INT &&
                                        /* MI:    /m0 or /m1  */
                                        !s[iINChI][j][0] && !s[iINChI][j][1] &&
                                        /* F, FI: no /s       */
                                        s[iINChI][jAlt][0] == 1 &&
                                        s[iINChI][jAlt][1] == 1          /* M, MI: /s1 and /s1 */
                                            ) {
                                    ...
                                    if (pInChI_Alt->Stereo->nCompInv2Abs == NO_VALUE_INT &&
                                        ...
                                        int bIso = pInChI->nNumberOfIsotopicAtoms ||
                                                   ...
                                                   (pInChI_Alt && pInChI_Alt->nNumberOfIsotopicAtoms);
                                        if (bIso) {
                                            if (!pInChI_Alt->StereoIsotopic &&
                                                /* create zero/NULL-initialized pInChI_Alt->StereoIsotopic */
                                            ...
                                            pInChI->Stereo->nCompInv2Abs = NO_VALUE_INT +
                                                                           1; /* FI: Stereo->CompInv2Abs=0, StereoIsotopic->CompInv2Abs=1 or -1 */
                                        } else {
                                ...
                                if (!s[iINChI][j][0] && !s[iINChI][jAlt][0] && /* /sN in Fixed-H isotopic only */
                                    ...
                                    !(pInChI->StereoIsotopic && pInChI->StereoIsotopic->nNumberOfStereoCenters) &&
                                    /*!(pInChI_Alt && pInChI_Alt->Stereo && pInChI_Alt->Stereo->nNumberOfStereoCenters) &&*/
                                    !(pInChI_Alt && pInChI_Alt->StereoIsotopic &&
                                      pInChI_Alt->StereoIsotopic->nNumberOfStereoCenters)) {
                                    pInChI->Stereo->nCompInv2Abs = NO_VALUE_INT +
                                                                   1; /* Stereo->CompInv2Abs=0, StereoIsotopic->CompInv2Abs=1 or -1 */
                                } else {
                                    pInChI->Stereo->nCompInv2Abs = s[iINChI][j][0] > 0 ? 2 : 0;
                                }
                            }
                        ...
                        if (j == TAUT_NON && k < nNumComponents[iINChI][TAUT_YES]) {
                            ...
                            if (pAltInChI->Stereo && pAltInChI->Stereo->nNumberOfStereoCenters &&
                                ...
                                if ((pInChI->Stereo->nCompInv2Abs == NO_VALUE_INT) &&
                                    ...
                                    if (nNumComponents[iINChI][j] > 1 &&
                                        ...
                                        if (bIso) {
                                            ...
                                            if ((!pInChI->StereoIsotopic &&
                                                ...
                                                || (!pAltInChI->StereoIsotopic &&
                                                    0 > (ret2 = CopySegment(pAltInChI, pAltInChI, CPY_SP3_M, 1, -1)))) {
                                                goto exit_function;
                                            }
                                    ...
                                    } else if ((s[iINChI][jAlt][0] > 0 || s[iINChI][j][0] > 0) && s[iINChI][j][0] >= 0)
                                        pInChI->Stereo->nCompInv2Abs = 2;
                                    else
                                    ...
                                    if (bIso && bNo_InChI_t && bNo_InChI_m /* no /t... or /mN in Fixed-H  */ &&
                                        !nFlags &&
                                        !(pAltInChI->StereoIsotopic && pAltInChI->StereoIsotopic->t_parity) &&
                                        !(pInChI->StereoIsotopic && pInChI->StereoIsotopic->t_parity) &&
                                    ...
                                    if (!(pInChI->nFlags & (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO))) {
                                        pInChI->nFlags |= ((nFlagsAlt | nFlags) &
                            ...
                            if (bIso) {
                                if (!(pInChI->Stereo && pInChI->Stereo->t_parity) &&
                                    /* !FH_Stereo */
                                    !(pAltInChI->Stereo && pAltInChI->Stereo->t_parity) &&
                                    /* !MH_Stereo */
                                    (pAltInChI->StereoIsotopic && pAltInChI->StereoIsotopic->nNumberOfStereoCenters) &&
                                    /*  MH_IsoStereo */
                                    (!pInChI->StereoIsotopic ||
                                     !pInChI->StereoIsotopic->t_parity)) {                  /* !FH_IsoStereo */
                                ...
                                if (!(pInChI->Stereo && pInChI->Stereo->t_parity) &&
                                    ...
                                    (pInChI->StereoIsotopic && pInChI->StereoIsotopic->nNumberOfStereoCenters) &&
                                    /*  FH_IsoStereo /t */
                                    pAltInChI->StereoIsotopic->nCompInv2Abs &&
                                    /*  MH_IsoStereo /m */
                                    (!pInChI->StereoIsotopic->nCompInv2Abs ||
                                     NO_VALUE_INT == pInChI->StereoIsotopic->nCompInv2Abs)) { /*  !FH_IsoStereo /m */
                                    /* added 02-09-2006 */
                                ...
                                if ((pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters) &&
                                    ...
                                    /* !MH_IsoStereo /t */
                                    !(pInChI->StereoIsotopic &&
                                      pInChI->StereoIsotopic->t_parity)) {          /* !FH_IsoStereo /t */
                                    ...
                                    if (pAltInChI->StereoIsotopic && pAltInChI->StereoIsotopic->nCompInv2Abs &&
                                        /* MH_IsoStereo /m */
                                        bNo_InChI_t &&
                                        NO_VALUE_INT != pAltInChI->StereoIsotopic->nCompInv2Abs &&
                                        /* undef FH_IsoStereo /m */
                                        !(pInChI->StereoIsotopic &&
                                          NO_VALUE_INT != pInChI->StereoIsotopic->nCompInv2Abs)) {
                                        if (0 > (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP3_M, 1, 1))) {
                                            goto exit_function;
                                    ...
                                    if (pInChI->StereoIsotopic->nCompInv2Abs == NO_VALUE_INT &&
                                        ...
                                        } else if ((s[iINChI][j][0] > 0 || s[iINChI][j][1] > 0) &&
                                                   s[iINChI][j][1] >= 0) /* ??? != NO_VALUE_INT ??? */
                                            pInChI->StereoIsotopic->nCompInv2Abs = 2;
                                        else
                    ...
                    if (pInpInChI[iINChI][j]) {
                        ...
                        if (pInChI->Stereo && pInChI->Stereo->nCompInv2Abs == NO_VALUE_INT) {
                            if (pInChI->nFlags & (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO) &&
                                ...
                                pInChI->Stereo->nCompInv2Abs = (s[iINChI][j][0] > 0 /*|| s[iINChI][j][1]>0*/) ? 2
                                                                                                              : 0; /* we do not know the real value */
                            } else {
                                pInChI->Stereo->nCompInv2Abs = 0;
                            }
                        }
                        if (pInChI->StereoIsotopic && pInChI->StereoIsotopic->nCompInv2Abs == NO_VALUE_INT) {
                            if (pInChI->nFlags & (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO) &&
                                pInChI->StereoIsotopic->nNumberOfStereoCenters) {
                                pInChI->StereoIsotopic->nCompInv2Abs =
                                        s[iINChI][j][1] > 0 ? 2 : 0; /* we do not know the real value */
                            } else {
                                pInChI->StereoIsotopic->nCompInv2Abs = 0;
                            }
                        }
...
/* (missed repeating /s in FI after F for multi-component case) */
            if (nNumComponents[iINChI][TAUT_NON] > 1)                     /* if multi-component */
                if (!s[iINChI][TAUT_YES][0] && !s[iINChI][TAUT_YES][1])/* if no /s in M, MI */
                    if ((s[iINChI][TAUT_NON][0] > 1) &&
                        (s[iINChI][TAUT_NON][1] > 1))     /* if /srel/srac in both F, FI */
                    ...
                    if (nRemovedCharge || nFoundRemovedIsoH) {
                        ...
                        if (pInChI[TAUT_YES] && bExists[TAUT_YES]) {
                            ...
                            if (!pInChI[TAUT_YES]->nTautomer) {
                                pInChI[TAUT_YES]->nTautomer = (AT_NUMB *) inchi_calloc(pInChI[TAUT_YES]->lenTautomer,
                                                                                       sizeof(pInChI[0]->nTautomer[0]));
                            }
                        }
                    }
                }
            ...
            } else if (nNumComponents[iINChI][TAUT_YES] > nNumComponents[iINChI][TAUT_NON]) {
...
/************************************************************************************/
int ParseAuxSegmentNumbers(const char *str, int bMobileH, INChI *pInpInChI[], int ppnNumComponents[], int state,
                           int *pbAbc) {
...
/***********************************************************************************************************/
int
ParseAuxSegmentReverseXYZ(const char *str, int bMobileH, XYZ_COORD **ppXYZ, INChI *pInpInChI[], int ppnNumComponents[],
                          int state) {
                ...
                switch (j) {
                    ...
                    case TAUT_NON:
                        ...
                        if (!pInChI->nPossibleLocationsOfIsotopicH) {
                            ...
                            } else if (pAltInChI->nPossibleLocationsOfIsotopicH[numAt[jj]]) {
        ...
        for (j = 0; j < TAUT_NUM; j++) {
            for (k = 0; k < nNumComponents[iINChI][j]; k++) {
                ...
                if (iINChI == INCHI_BAS && num_at == 1 &&
                    ...
                    (int) pInChI->nPossibleLocationsOfIsotopicH[0] - 1 >=
                    nLenXYZ) { ; /* a single atom H disconnected from a metal atom has no coordinates */
                } else {
    ...
    do {
        ...
        switch (state) {
            case AST_VERSION:
                ...
                break;
            case AST_MOBILE_H_NUMBERS:
                ret = ParseAuxSegmentNumbers(pLine->str, bMobileH, pInpInChI[bReconn], nNumComponents[bReconn], state,
                                             &bAbc);
            ...
            case AST_MOBILE_H_SP3_INV:
                ...
                break;
            case AST_MOBILE_H_SP3_INV_NUMBERS:
                ret = ParseAuxSegmentSp3InvNumbers(pLine->str, bMobileH, pInpInChI[bReconn], nNumComponents[bReconn],
                                                   state);
            ...
            case AST_MOBILE_H_ISO_LAYER_FORK:
                ...
                } else if (!memicmp(pLine->str, "F:", 2)) {
                    state = AST_FIXED_H_NUMBERS;
                    bMobileH = TAUT_NON;
                } else if ( /*bReconn == INCHI_BAS &&*/ !memicmp(pLine->str, "CRV:", 4)) {
                    state = AST_REVERSE_INFO_CRV;
                } else if (bReconn == INCHI_BAS && !memicmp(pLine->str, "rA:", 3)) {
                    state = AST_REVERSE_INFO_ATOMS;
                } else if (bReconn == INCHI_BAS && !memicmp(pLine->str, "R:", 3)) {
            ...
            case AST_FIXED_H_ISO_SP3_INV_NUMBERS:
                ...
                break;
            case AST_REVERSE_INFO_CRV:
                ret = ParseAuxSegmentReverseCRV(pLine->str, bMobileH, pInpInChI[bReconn], nNumComponents[bReconn],
                                                state);
                /* state = (bReconn == INCHI_BAS)? AST_REVERSE_INFO_ATOMS : AST_RECONNECTED_LAYER_FORK;*/
                state = AST_REVERSE_INFO_ATOMS;
                break;
            case AST_REVERSE_INFO_ATOMS:
                ret = ParseAuxSegmentReverseAtoms(pLine->str, bMobileH, pInpInChI[bReconn], nNumComponents[bReconn],
                                                  state);
                state = AST_REVERSE_INFO_BONDS;
                break;
            case AST_REVERSE_INFO_BONDS:
                ret = ParseAuxSegmentReverseBonds(pLine->str, bMobileH, pInpInChI[bReconn], nNumComponents[bReconn],
                                                  state);
                state = AST_REVERSE_INFO_XYZ;
                break;
            case AST_REVERSE_INFO_XYZ:
                ret = ParseAuxSegmentReverseXYZ(pLine->str, bMobileH, &pXYZ, pInpInChI[bReconn],
                                                nNumComponents[bReconn], state);
        ...
        switch (state) {
            ...
            case IST_MOBILE_H_CONNECTIONS:   /* /c */
                ret = ParseSegmentConnections(pLine->str, bMobileH, &pInpInChI[bReconn][bMobileH],
                                              &nNumComponents[bReconn][bMobileH], &bAbc);
                state = IST_MOBILE_H;
                break;
            case IST_MOBILE_H:               /* /h */
            ...
            case IST_MOBILE_H_SP3_S:       /* /s */
                ret = ParseSegmentSp3s(pLine->str, bMobileH, pInpInChI[bReconn], s[bReconn], nNumComponents[bReconn],
                                       state);
            ...
            case IST_MOBILE_H_ISO_LAYER_FORK:
                ...
                } else if (!ret && state != IST_MOBILE_H_ISO_ATOMS) {
            ...
            case IST_MOBILE_H_ISO_ATOMS:   /* i */
                ret = ParseSegmentIsoAtoms(pLine->str, bMobileH, pInpInChI[bReconn], nNumComponents[bReconn], state,
                                           &bAbc);
                state = IST_MOBILE_H_ISO_EXCH_H;
                break;
            case IST_MOBILE_H_ISO_EXCH_H:  /* /i/h */
                ret = ParseSegmentIsoExchgH(pLine->str, bMobileH, nNumProtons[bReconn], nNumComponents[bReconn], state,
                                            &bAbc);
            ...
            case IST_FIXED_H_ISO_LAYER_FORK:
                ...
                } else if (!ret && state != IST_FIXED_H_ISO_ATOMS) {
                    len = strlen(pLine->str);
                    if (len > 1) {
                        memmove(pLine->str, pLine->str + 1, len);
                    } else {
...
/****************************************************************************************/
int ParseSegmentIsoExchgH(const char *str, int bMobileH, REM_PROTONS nNumProtons[], int pnNumComponents[], int state,
                          int *pbAbc) {
...
int ParseSegmentPerm(const char *str, int bMobileH, INChI *pInpInChI[], int ppnNumComponents[], int state, int *pbAbc) {
    ...
    while (pStart < pEnd) {
        ...
        if (*pbAbc == 1) {
            ...
            for (p = pPermStart, iComponent2 = numTrans = 0;
                 p < pPermEnd; iComponent2 = iComponent1, p = q + (*q == ',')) {
                /* get first atom number */
                if (!isdigit(UCINT *p)) {
                    ret = RI_ERR_SYNTAX;
                    goto exit_function;
...
/****************************************************************************************/
int
ParseSegmentIsoAtoms(const char *str, int bMobileH, INChI *pInpInChI[], int ppnNumComponents[], int state, int *pbAbc) {
...
/****************************************************************************************/
int ParseSegmentSp3s(const char *str, int bMobileH, INChI *pInpInChI[], int s[TAUT_NUM][2], int ppnNumComponents[],
                     int state) {
    /* Pass 1: count bonds and find actual numbers of  atom */
    ...
    if (str[0] != 'm') {
        ...
        if (!bIsSp3LayerNotEmpty(pInpInChI, bMobileHFrom, bIsoFrom, ppnNumComponents[/*bMobileH*/ bMobileHFrom])) {
            ...
            if ((!bMobileHFrom && bIsoFrom) || (bMobileHFrom && !bIsoFrom)) {
                ...
                bMobileHFrom = TAUT_YES;
                bIsoFrom = 0;
                if (!bIsSp3LayerNotEmpty(pInpInChI, bMobileHFrom, bIsoFrom,
                                         ppnNumComponents[bMobileHFrom /*bMobileH*/])) {
                    return 0;
                }
            } else {
                return 0;
    ...
    while (1) {
        ...
        if (((isdigit(*pStart) &&
            ...
            for (i = 0; i < val; i++) {
                ...
                if (bIsoFrom >= 0) {
                    INChI_Stereo *pStereoTo = bIsoTo ? pInChI[iComponent + i].StereoIsotopic : pInChI[iComponent +
                                                                                                      i].Stereo;
                    if (pStereoTo) {
            ...
            for (p = pStart, iAtom = 0; p < pEnd; iAtom++) {
                if ((nAtom1 = (AT_NUMB) inchi_strtol(p, &p, base)) &&
                    (atomParity = (int) inchi_strtol(p, &p, 10),
                            AB_MIN_KNOWN_PARITY <= atomParity && atomParity <= AB_MAX_KNOWN_PARITY)) { ; /* okay */
                } else {
                    ret = RI_ERR_SYNTAX; /* syntax error */
                    goto exit_function;
                }
            ...
            for (p = pStart, iBond = 0; p < pEnd; iBond++) {
                ...
                if ((nAtom1 = (AT_NUMB) inchi_strtol(p, &p, base)) &&
                    ...
                    (bondParity = (int) inchi_strtol(p, &p, 10),
                            AB_MIN_KNOWN_PARITY <= bondParity && bondParity <= AB_MAX_KNOWN_PARITY)) { ; /* okay */
                } else {
                    ret = RI_ERR_SYNTAX; /* syntax error */
                    goto exit_function;
                }
...
int ParseSegmentProtons(const char *str, int bMobileH, REM_PROTONS nNumProtons[], int ppnNumComponents[]) {
    ...
    while (1) {
        ...
        } else if (pStart[0] == '-' && isdigit(UCINT pStart[1])) {
            val = -(int) inchi_strtol(pStart + 1, &q, 10);
        } else {
            ret = RI_ERR_SYNTAX; /* syntax error */
            goto exit_function;
...
#define nNum_H(ICOMPONENT) ((bMobileH==TAUT_YES)? pInChI[ICOMPONENT].nNum_H : pInChI[ICOMPONENT].nNum_H_fixed)
    ...
    while (1) {
        ...
        for (i = 0; i < mpy_component; i++) {
            if (bMobileH == TAUT_NON) {
                ...
                if (iComponent + i < pnNumComponents[nAltMobileH]) {
                    len = inchi_max(pInChI[iComponent + i].nNumberOfAtoms, pAltInChI[iComponent + i].nNumberOfAtoms) +
                          1;
                } else {
                ...
                if (iComponent + i < pnNumComponents[nAltMobileH]) {
                    ...
                    if (pInChI[iComponent + i].nAtom && len2) {
                        /* check */
                        if (memcmp(pInChI[iComponent + i].nAtom, pAltInChI[iComponent + i].nAtom,
                                   len2 * sizeof(pInChI[0].nAtom[0]))) {
                            ret = RI_ERR_SYNTAX; /* syntax error */
                            goto exit_function;
                        }
                    }
                    ...
                    if (pInChI[iComponent + i].nNumberOfAtoms < pAltInChI[iComponent + i].nNumberOfAtoms) {
                        ...
                        if (!(pInChI[iComponent + i].nAtom = (U_CHAR *) inchi_calloc(len,
                                                                                     sizeof(pInChI[0].nAtom[0])))) {
                            ret = RI_ERR_ALLOC; /* allocation error */
                            goto exit_function;
                        }
                        if (len > 1) {
                            memcpy(pInChI[iComponent + i].nAtom, pAltInChI[iComponent + i].nAtom,
                                   (len - 1) * sizeof(pInChI[0].nAtom[0]));
                        }
                ...
                while (p < pEnd) {
                    ...
                    if (!tg_alloc_len) {
                        ...
                        */
                        ...
                        for (i = 0; i < mpy_component; i++) {
                            pInChI[iComponent + i].nTautomer = (AT_NUMB *) inchi_calloc(tg_alloc_len + 1,
                    ...
                    iTGroup++;
                    pInChI[iComponent].nTautomer[tg_pos_Tautomer] =
                            lenTautomer - tg_pos_Tautomer - 1; /* length of the rest of the t-group */
                    ...
                    switch (*p) {
                        ...
                        case ',':
                            switch (state) {
                                ...
                                */
                                        tg_alloc_len =
                                                ((3 + INCHI_T_NUM_MOVABLE) * pInChI[iComponent].nNumberOfAtoms) / 2 + 1;
                                        for (i = 0; i < mpy_component; i++) {
                                            pInChI[iComponent + i].nTautomer = (AT_NUMB *) inchi_calloc(
                                                    tg_alloc_len + 1, sizeof(pInChI->nTautomer[0]));
                                            if (!pInChI[iComponent + i].nTautomer) {
                                                ret = RI_ERR_ALLOC; /* allocation error */
                                                goto exit_function;
                                            }
        ...
        for (i = 1; i < mpy_component; i++) {
            memcpy(nNum_H(iComponent + i), nNum_H(iComponent),
                   pInChI[iComponent + i].nNumberOfAtoms * sizeof(nNum_H(0)[0]));
            /*
...
#define LAST_AT_LEN 256
    ...
    if (str[0] != 'c') {
        if (!pInChI && !*pnNumComponents) {
            ...
            pInChI[iComponent].szHillFormula = (char *) inchi_calloc(lenFormula + 1,
                                                                     sizeof(pInChI[0].szHillFormula[0]));
    ...
    while (1) {
        ...
        for (i = 0; i < mpy_component; i++) {
            ...
            } else if (j > nNumAtoms && (lenConnTable || j != 1)) {
                ret = RI_ERR_SYNTAX; /* syntax error */
                goto exit_function;
            }
            ...
            pInChI[iComponent + i].nConnTable = (AT_NUMB *) inchi_calloc(lenConnTable + 1,
                                                                         sizeof(pInChI[0].nConnTable[0]));
            if (!pInChI[iComponent + i].nConnTable) {
                ret = RI_ERR_ALLOC; /* allocation failure */
                goto exit_function;
            }
        ...
        if (*pbAbc == 1) {
            ...
            while (p < pEnd) {
                ...
                if (isupper(UCINT *p)) {
                    ...
                    } else if (bPrevVersion == 0 && isdigit(*p)) {
                        ...
                        if (isdigit(c) && (curAtom = (AT_NUMB) inchi_strtol(p - 1, &q, 10))) {
                            ...
                            switch (state) {
                                ...
                                case ')':
                                    ...
                                    if ((ret = AddLinkedBond(last_atom[--num_open], curAtom, (AT_NUMB) nNumAtoms,
                                                             pLB))) {
                                        goto exit_function;
                                    }
                                    last_atom[num_open] = curAtom;
                                ...
                                case ',':
                                    ...
                                    if ((ret = AddLinkedBond(last_atom[num_open - 1], curAtom, (AT_NUMB) nNumAtoms,
                                                             pLB))) {
                                        goto exit_function;
                                    }
                                    last_atom[num_open] = curAtom;
        ...
        for (i = 1; i < mpy_component; i++) {
            ...
            */
            ...
            memcpy(pInChI[iComponent + i].nConnTable, pInChI[iComponent].nConnTable,
                   lenConnTable * sizeof(pInChI[0].nConnTable[0]));
        }
        ...
        if (!*pStart || islower(UCINT *pStart)) {
            ...
            if (bMobileH == TAUT_NON && 0 < (nNumComponents = pnNumComponents[nAltMobileH])) {
                ...
                for (i = 0; i < nNumComponents; i++) {
                    ...
                    if ((pInpInChI[bMobileH][i].nAtom = (U_CHAR *) inchi_malloc(
                            (len + 1) * sizeof(pInpInChI[0][0].nAtom[0])))) {
    ...
    while (1) {
        ...
        for (i = 0; i < mpy_component; i++) {
            ...
            if (!i) {
                ...
                pInChI[iComponent + i].nAtom = (U_CHAR *) inchi_malloc(
                        (nNumAtomsAndH + 1) * sizeof(pInChI[0].nAtom[0]));
                if (!pInChI[iComponent + i].nAtom)
        ...
        if (stereoFrom || bIsotopicFrom < 0) {
            if (SegmentType == CPY_SP2) {
                if (bIsotopicFrom < 0 ||
                    ...
                    /* allocate sp2 stereo */
                    if (!(pstereoTo[0]->b_parity = (S_CHAR *) inchi_calloc(len + 1,
                                                                           sizeof(pstereoTo[0]->b_parity[0]))) ||
                        !(pstereoTo[0]->nBondAtom1 = (AT_NUMB *) inchi_calloc(len + 1,
                                                                              sizeof(pstereoTo[0]->nBondAtom1[0]))) ||
                        !(pstereoTo[0]->nBondAtom2 = (AT_NUMB *) inchi_calloc(len + 1,
                                                                              sizeof(pstereoTo[0]->nBondAtom2[0])))) {
                        /* cleanup */
                    ...
                    if (bIsotopicFrom >= 0 && len) {
                        memcpy(pstereoTo[0]->b_parity, stereoFrom->b_parity,
                               (len + 1) * sizeof(pstereoTo[0]->b_parity[0]));
                        memcpy(pstereoTo[0]->nBondAtom1, stereoFrom->nBondAtom1,
                               (len + 1) * sizeof(pstereoTo[0]->nBondAtom1[0]));
                        memcpy(pstereoTo[0]->nBondAtom2, stereoFrom->nBondAtom2,
                               (len + 1) * sizeof(pstereoTo[0]->nBondAtom2[0]));
                    }
                ...
                if (bIsotopicFrom < 0 ||
                    ...
                    /* allocate sp3 stereo */
                    if (!(pstereoTo[0]->t_parity = (S_CHAR *) inchi_calloc(len + 1,
                                                                           sizeof(pstereoTo[0]->b_parity[0]))) ||
                        !(pstereoTo[0]->nNumber = (AT_NUMB *) inchi_calloc(len + 1,
                                                                           sizeof(pstereoTo[0]->nBondAtom1[0])))) {
                        /* cleanup */
                    ...
                    /* copy stereo */
                    if (bIsotopicFrom >= 0 && len) {
                        memcpy(pstereoTo[0]->t_parity, stereoFrom->t_parity,
                               (len + 1) * sizeof(pstereoTo[0]->t_parity[0]));
                        memcpy(pstereoTo[0]->nNumber, stereoFrom->nNumber,
                               (len + 1) * sizeof(pstereoTo[0]->nNumber[0]));
                    }
