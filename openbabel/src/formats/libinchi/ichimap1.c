...
int All_SC_Same(AT_RANK canon_rank1, /*  canonical number */
    ...
    for (i1 = 1; i1 <= iMax1 && r1 == pRankStack2[0][s1 = (int) pRankStack2[1][iMax1 - i1]]; i1++) {
        ...
        } else if (stereo_atom_parity != PARITY_VAL(at[s1].stereo_atom_parity)) {
...
int CompareLinCtStereoDble(AT_STEREO_DBLE *LinearCTStereoDble1, int nLenLinearCTStereoDble1,
    ...
    } else if (LinearCTStereoDble1 && nLenLinearCTStereoDble1 > 0) {
        ret = 1;
    } else if (LinearCTStereoDble2 && nLenLinearCTStereoDble2 > 0) {
        ret = -1;
    }
    return ret;
}
...
int CompareLinCtStereoCarb(AT_STEREO_CARB *LinearCTStereoCarb1, int nLenLinearCTStereoCarb1,
    ...
    } else if (LinearCTStereoCarb1 && nLenLinearCTStereoCarb1 > 0) {
        ret = 1;
    } else if (LinearCTStereoCarb2 && nLenLinearCTStereoCarb2 > 0) {
        ret = -1;
    }
...
int All_SB_Same(AT_RANK canon_rank1, AT_RANK canon_rank2, /*  canonical numbers */
    ...
    for (i1 = 1; i1 <= iMax1 && r1 == pRankStack2[0][s1 = (int) pRankStack2[1][iMax1 - i1]]; i1++) {
        ...
        for (k1 = 0, s2 = 0, bNotFound = 1;
             ...
             (bNotFound = (r2 !=
                           pRankStack2[0][--s2])); k1++); /* continue until the 1st at[s2] (to which at[n2] may be mapped) have been found */
        if (!bNotFound) {
                        ...
                        for (i2 = 1; i2 <= iMax2 && r2 == pRankStack1[0][n2 = (int) pRankStack1[1][iMax2 - i2]]; i2++) {
                            if (cumulene_len) {
                                ...
                                for (m = 0; m < at[n1].valence; m++) {
                                    for (prev = n1, len = 0, next = (int) at[n1].neighbor[m];
                                         len < cumulene_len; len++) {
                                        if (at[next].valence == 2 && !at[next].num_H) {
                                            j = ((int) at[next].neighbor[0] == prev);
                                            prev = next;
                                            next = at[next].neighbor[j];
...
int CurTreeReAlloc(CUR_TREE *cur_tree) {
    if (cur_tree) {
        if (cur_tree->tree && cur_tree->max_len > 0 && cur_tree->incr_len > 0) {
            ...
            if ((cur_tree->tree = (AT_NUMB *) inchi_calloc(cur_tree->max_len + cur_tree->incr_len,
                                                           sizeof(cur_tree->tree[0])))) {
...
/**************************************************************************************/
void CurTreeKeepLastAtomsOnly(CUR_TREE *cur_tree, int tpos,
                              int shift) {   /*  on first entry: shift = 1; other values may occur in subsequent recursion */
    ...
    if (cur_tree && cur_tree->tree && (cur_length_pos = cur_tree->cur_len - shift) > tpos) {
        if (cur_tree->tree[cur_length_pos] > 2) {
            ...
            shift += (int) cur_tree->tree[cur_length_pos] +
                     1; /*  cur_tree->cur_len - (previous segment length position) */
        }
