    ...
    bool OBGaussianCubeFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (!ifs.getline(buffer, BUFF_SIZE)) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problem reading the Gaussian cube file: cannot read the first line (title/comments).",
                                  obWarning);
            return false;
        }
        ...
        if (!ifs.getline(buffer, BUFF_SIZE)) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problem reading the Gaussian cube file: cannot read the second line (title/comments).",
                                  obWarning);
            return false;
        }
        ...
        if (!ifs.getline(buffer, BUFF_SIZE)) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problem reading the Gaussian cube file: cannot read the third line. This should contain the number of atoms and the position of the origin of the cube.",
                                  obWarning);
            return false;
        }
        vector<string> vs;
        ...
        if (vs.size() < 4) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problem reading the Gassian cube file: The third line must contain the number of atoms and the origin of the cube.",
                                  obWarning);
            return false;
        }
        char *endptr;
...
//    cerr << "Number of atoms: " << nAtoms << " Orig: " << vs.at(0) << endl;
        ...
        if (negAtoms) {
            ...
            if (!ifs.getline(buffer, BUFF_SIZE)) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Problem reading the Gaussian cube file: cannot read the line between the atom data and cube data.",
                                      obError);
                return false;
            }
