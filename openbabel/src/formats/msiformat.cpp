...
namespace OpenBabel {
    ...
    public:
        ...
        virtual const char *SpecificationURL() { return "http://openbabel.org/wiki/MSI_format"; }; //optional

        virtual const char *GetMIMEType() { return "chemical/x-msi-msi"; };
    ...
    bool MSIFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
        ...
        if (!EQn(buffer, "# MSI CERIUS2 DataModel File", 28)) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Problems reading an MSI file: The first line must contain the MSI header.",
                                  obWarning);
            return (false);
        }
