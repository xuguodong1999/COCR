...
namespace OpenBabel {
    ...
    bool OBStericConformerFilter::IsGood(const OBMol &mol, const RotorKey &key, double *conformer) {
        ...
        for (a1 = 0; a1 < numAtoms; ++a1) {
            for (a2 = a1 + 1; a2 < numAtoms; ++a2) {
                ...
                if (!m_check_hydrogens &&
                    (atom1->GetAtomicNum() == OBElements::Hydrogen || atom2->GetAtomicNum() == OBElements::Hydrogen))
                    continue;
    ...
    bool
    OBConformerSearch::Setup(const OBMol &mol, int numConformers, int numChildren, int mutability, int convergence) {
        int nb_rotors = 0;
        // copy some variables
        m_mol = mol;
    ...
    void OBConformerSearch::Search() {
        ...
        if (m_logstream != nullptr) {
            (*m_logstream) << std::endl << "=====> Starting conformers search with a Genetic Algorithm <====="
                           << std::endl;
            if (use_sharing) {
            ...
            (*m_logstream) << "Will stop after " << m_convergence << " generations without improvement." << std::endl
                           << std::endl;
        }
        if (use_sharing)
            score_population();
    ...
    */
    ...
    OBConformerSearch::local_opt() {
        ...
        score_population();
        opt_score = vscores[0];
        if ((max_flag && (opt_score > backup_scores[0])) ||
            (!max_flag && (opt_score < backup_scores[0]))) {                // Found a better conformer
        ...
        if (flag_improved) { // Replace best one
    ...
    OBConformerSearch::reproduce(RotorKey &key1, RotorKey &key2) {
        ...
        if (flag_crossover) {
            ...
            for (i = 1; i < key1.size(); i++) {
                if (((OBRandom *) d)->NextInt() % 2) { // Copy parent1 to offspring 1
    ...
    */
    ...
    OBConformerSearch::share_fitness() {
        ...
        for (k = 0; k < pop_size; k++) {
            ...
            if (iniche == dynamic_niches.size()) {  // Could not find a niche for this one
                if (dynamic_niches.size() < nb_niches) { // Create a new niche.
                    iniche = dynamic_niches.size();
                    dynamic_niches.resize(iniche + 1);
                    dynamic_niches[iniche].push_back(i);
                } else { // Do it the hard way: compute sharing factor with all the population
                    sh_count = 0.0;
    ...
    OBConformerSearch::sharing_generation() {
        ...
        while (ninsert < half_pop) {
            ...
            for (i = 0; i < pop_size; i++) {
                ...
                if (dist_min > dist_max) {            // Find the most distant to its clostest neighbor
                    imax = i;
                    dist_max = dist_min;
                }
            }
