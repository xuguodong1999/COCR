...
namespace OpenBabel {
    ...
    **/
    ...
    OBMolBondBFSIter::OBMolBondBFSIter(OBMol *mol, int StartIndex) :
        ...
        for (OBAtomBondIter b(_ptr->GetBeginAtom()); b; ++b) { // Loop thru all bonds attached to the start of the bond
        ...
        for (OBAtomBondIter b(_ptr->GetEndAtom()); b; ++b) { // Loop thru all bonds attached to the end of the bond
            if (_notVisited[b->GetIdx()]) { // Don't revisit the initial bond
                _queue.push(&(*b));
                _depth[b->GetIdx()] = 2;
                _notVisited.SetBitOff(b->GetIdx());
    ...
    OBMolBondBFSIter &OBMolBondBFSIter::operator++() {
        ...
        if (_ptr) {
            for (OBAtomBondIter b(
                    _ptr->GetBeginAtom()); b; ++b) { // Loop thru all bonds attached to the start of the bond
