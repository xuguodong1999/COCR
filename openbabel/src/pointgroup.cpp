...
#define PointGroupsCount (sizeof(PointGroups)/sizeof(POINT_GROUP))
     ...
     */
        ...
        init_axis_parameters(double a[3], double b[3], double c[3]) {
            ...
            if (rab <= ToleranceSame || rbc <= ToleranceSame || rac <= ToleranceSame) {
                StatEarly++;
                if (verbose > 0)
                    printf("    rotation is underdefined by these points: %8.3f %8.3f %8.3f\n", rab, rbc, rac);
                return nullptr;
            }
            rab = (rab + rbc) / 2;
           ...
           */
                    ...
                    if (r > 5 * TolerancePrimary) { /* It's Ok to use CenterOfSomething */
                        if ((axis = init_c2_axis(i, j, CenterOfSomething)) != nullptr) {
                            NormalAxesCount++;
                            NormalAxes = (SYMMETRY_ELEMENT **) realloc(NormalAxes,
                    ...
                    for (k = 0; k < _mol->NumAtoms(); k++) {
                        ...
                        for (l = 0; l < _mol->NumAtoms(); l++) {
                            ...
                            if ((axis = init_c2_axis(i, j, center)) != nullptr) {
                                NormalAxesCount++;
                                NormalAxes = (SYMMETRY_ELEMENT **) realloc(NormalAxes, sizeof(SYMMETRY_ELEMENT *) *
                                                                                       NormalAxesCount);
                                if (NormalAxes == nullptr) {
                                    perror("Out of memory in find_c2_axes");
                                    free(distances);
                ...
                for (j = i + 1; j < _mol->NumAtoms(); j++) {
                    for (k = 0; k < _mol->NumAtoms(); k++) {
                        if ((axis = init_improper_axis(i, j, k)) != nullptr) {
                            ImproperAxesCount++;
                            ImproperAxes = (SYMMETRY_ELEMENT **) realloc(ImproperAxes, sizeof(SYMMETRY_ELEMENT *) *
                                                                                       ImproperAxesCount);
     ...
     */
        ...
        report_symmetry_elements_brief(void) {
            int i;
            char *symmetry_code = (char *) calloc(1, 10 * (PlanesCount + NormalAxesCount + ImproperAxesCount +
            ...
            if (PlanesCount + NormalAxesCount + ImproperAxesCount + InversionCentersCount == 0) {
                ...
                for (i = MaxAxisOrder; i >= 2; i--) {
                    if (NormalAxesCounts[i] == 1) {
                        snprintf(buf, 100, "(C%d) ", i);
                        strcat(symmetry_code, buf);
                    }
                    if (NormalAxesCounts[i] > 1) {
                        snprintf(buf, 100, "%d*(C%d) ", NormalAxesCounts[i], i);
                        strcat(symmetry_code, buf);
                    }
                }
                for (i = MaxAxisOrder; i >= 2; i--) {
                    if (ImproperAxesCounts[i] == 1) {
                        snprintf(buf, 100, "(S%d) ", i);
                        strcat(symmetry_code, buf);
                    }
                    if (ImproperAxesCounts[i] > 1) {
                        snprintf(buf, 100, "%d*(S%d) ", ImproperAxesCounts[i], i);
                        strcat(symmetry_code, buf);
                    }
                }
                if (PlanesCount == 1) strcat(symmetry_code, "(sigma) ");
                if (PlanesCount > 1) {
                    snprintf(buf, 100, "%d*(sigma) ", PlanesCount);
                    strcat(symmetry_code, buf);
                }
                //        printf( "%s\n", symmetry_code ) ;
            }
