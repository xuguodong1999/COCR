...
namespace OpenBabel {
  ...
  */
    ...
    static void
    CreateNewClassVector(vector<pair<OBAtom *, unsigned int> > &vp1, vector<pair<OBAtom *, unsigned int> > &vp2) {
        int m, id;
        OBAtom *nbr;
        vector<OBBond *>::iterator j;
...
#undef  OBBondIncrement
    ...
    bool OBMol::MakeDativeBonds() {
        ...
        for (patom = BeginAtom(ai); patom; patom = NextAtom(ai)) //all atoms
        {
            if (patom->GetAtomicNum() ==
                OBElements::Nitrogen // || patom->GetAtomicNum() == OBElements::Phosphorus) not phosphorus!
                && (patom->GetExplicitValence() == 5 ||
                    (patom->GetExplicitValence() == 4 && patom->GetFormalCharge() == 0))) {
  ...
  */
    bool OBMol::ConvertZeroBonds() {
        ...
        const int BLOCKS[113] = {0, 1, 2, 1, 1, 2, 2, 2, 2, 2, 2, 1, 1, 2, 2, 2, 2, 2, 2, 1, 1, 3, 3, 3, 3, 3, 3, 3, 3,
                                 3,
                                 3, 2, 2, 2, 2, 2, 2, 1, 1, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2, 1, 1, 4, 4,
                                 4,
                                 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2, 1, 1,
                                 4,
    ...
    bool OBMol::CopySubstructure(OBMol &newmol, OBBitVec *atoms, OBBitVec *excludebonds, unsigned int correctvalence,
        ...
        for (data = stereoData.begin(); data != stereoData.end(); ++data) {
            if (static_cast<OBStereoBase *>(*data)->GetType() == OBStereo::CisTrans) {
                ...
                newcfg.begin = cfg.begin == OBStereo::ImplicitRef ? OBStereo::ImplicitRef : AtomMap[GetAtomById(
                        cfg.begin)]->GetId();
                newcfg.end = cfg.end == OBStereo::ImplicitRef ? OBStereo::ImplicitRef : AtomMap[GetAtomById(
                        cfg.end)]->GetId();
                OBStereo::Refs refs;
                for (OBStereo::RefIter ri = cfg.refs.begin(); ri != cfg.refs.end(); ++ri) {
                    OBStereo::Ref ref =
                            *ri == OBStereo::ImplicitRef ? OBStereo::ImplicitRef : AtomMap[GetAtomById(*ri)]->GetId();
                    refs.push_back(ref);
                }
                newcfg.refs = refs;
                ...
                newcfg.from = cfg.from == OBStereo::ImplicitRef ? OBStereo::ImplicitRef : AtomMap[GetAtomById(
                        cfg.from)]->GetId();
                OBStereo::Refs refs;
                for (OBStereo::RefIter ri = cfg.refs.begin(); ri != cfg.refs.end(); ++ri) {
                    OBStereo::Ref ref =
