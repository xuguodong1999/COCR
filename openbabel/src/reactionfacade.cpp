...
namespace OpenBabel {
    ...
    void OBReactionFacadePrivate::FindComponents() {
        ...
        for (std::set<unsigned int>::iterator sit = reactant_components.begin();
             sit != reactant_components.end(); ++sit)
        ...
        for (std::set<unsigned int>::iterator sit = unassigned_components.begin();
             sit != unassigned_components.end(); ++sit)
    ...
    bool OBReactionFacadePrivate::IsValid() {
        if (!_mol->IsReaction()) {
            obErrorLog.ThrowError(__FUNCTION__, "The molecule is not marked as a reaction. Use SetIsReaction().",
                                  obWarning);
            return false;
        }
        FOR_ATOMS_OF_MOL(atom, _mol) {
            ...
            if (!data) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "The molecule contains an atom that is missing a reaction component Id. Use SetComponentId().",
                                      obWarning);
                return false;
            }
            ...
            if (!pi) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "A reaction component Id has been stored using a data type that is not an OBPairInteger.",
                                      obWarning);
                return false;
            }
            val = pi->GetGenericValue();
            if (val <= 0) {
                obErrorLog.ThrowError(__FUNCTION__, "Reaction component Ids should all be non-zero positive integers.",
                                      obWarning);
                return false;
            }
            ...
            if (!data) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "The molecule contains an atom that is missing reaction role information. Use SetRole().",
                                      obWarning);
                return false;
            }
            pi = dynamic_cast<OBPairInteger *>(data);
            if (!pi) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Reaction role information has been stored using a data type that is not an OBPairInteger.",
                                      obWarning);
                return false;
            }
            val = pi->GetGenericValue();
            if (val < 0 || val > 3) {
                obErrorLog.ThrowError(__FUNCTION__, "Reaction roles should be in the range 0 to 3 inclusive.",
                                      obWarning);
                return false;
            }
        }
        ...
        while (iter) { // for each connected component
            ...
            do { // for each atom in connected component
                ...
                if (my_rxncomp != rxncomp) {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "The molecule contains a connected component that contains atoms with different reaction component Ids. All atoms in a particular connected component should have the same value.",
                                          obWarning);
                    return false;
                }
