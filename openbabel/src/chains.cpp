...
namespace OpenBabel {
   ...
   */
    static Template Peptide[MAXPEPTIDE] = {
            ...
            /* NTer  */
                           {0x0002, 7, 1, 0x0030, 0,      0,      0}, //!< NTre
            /* NPro  */
                           {0x0004, 7, 3, 0x0030, 0x0100, -6,     0}, //!< NPro
            /* NPT   */
                           {0x0008, 7, 2, 0x0030, -6,     0,      0}, //!< NPT
            /* CA    */
                           {0x0010, 6, 3, 0x000F, 0x0700, -6,     0}, //!< CA
            /* CAGly */
                           {0x0020, 6, 2, 0x0003, 0x0700, 0,      0}, //!< CAGly
            /* C     */
                           {0x0100, 6, 3, 0x0030, 0x1000, 0x0005, 0}, //!< C
            /* CTer  */
                           {0x0200, 6, 2, 0x0030, 0x1000, 0,      0}, //!< CTer
            /* COXT  */
                           {0x0400, 6, 3, 0x0030, 0x1000, 0x2000, 0}, //!< COXT
            /* O     */
                           {0x1000, 8, 1, 0x0700, 0,      0,      0}, //!< O
            /* OXT   */
                           {0x2000, 8, 1, 0x0400, 0,      0,      0}  //!< OXT
    };
...
#define ATOMMAX        68
   ...
   */
    ...
    static ByteCode *AllocateByteCode(int type) {
        ...
        result = new ByteCode;
        if (!result) {
            obErrorLog.ThrowError(__FUNCTION__, "Unable to allocate byte codes for biomolecule residue perception.",
                                  obError);
            return (result);
        }
        result->type = type;
    ...
    static void FatalMemoryError(void) {
        obErrorLog.ThrowError(__FUNCTION__, "Unable to allocate memory for biomolecule residue / chain perception.",
                              obError);
    }
