...
// debug function
void print_sym_classes(const std::string &label,
                       const std::vector<std::pair<OpenBabel::OBAtom *, unsigned int> > &atom_sym_classes) {
    cout << label << ": ";
...
namespace OpenBabel {
    ...
    OBGraphSym::OBGraphSym(OBMol *pmol, const OBBitVec *frag_atoms) : d(new OBGraphSymPrivate) {
        ...
        if (frag_atoms) {
            ...
            FOR_ATOMS_OF_MOL(a, d->_pmol)d->_frag_atoms.SetBitOn(a->GetIdx());
        }
    }
    ...
    inline bool
    ComparePairFirst(const std::pair<OBAtom *, unsigned int> &a, const std::pair<OBAtom *, unsigned int> &b) {
        return (a.first->GetIdx() < b.first->GetIdx());
    }

    inline bool
    ComparePairSecond(const std::pair<OBAtom *, unsigned int> &a, const std::pair<OBAtom *, unsigned int> &b) {
