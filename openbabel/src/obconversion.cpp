...
*/
        ...
        Count = -1;
        EndNumber = StartNumber = 0;
        pOb1 = nullptr; //leave tidy
    ...
    int OBConversion::FullConvert(std::vector<std::string> &FileList, std::string &OutputFileName,
...
#endif
        {
            ...
            if (IsOption("t", GENOPTIONS)) {
                ...
                if (HasMultipleOutputFiles) {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Cannot have multiple output files and also concatenate input files (-t option)",
                                          obError);
                    return 0;
                }
            ...
            if (FileList.empty()) {
                pIs = nullptr;
                if (HasMultipleOutputFiles) {
                    obErrorLog.ThrowError(__FUNCTION__, "Cannot use multiple output files without an input file",
                                          obError);
                    return 0;
                }
            } else {
