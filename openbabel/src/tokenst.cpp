...
namespace OpenBabel {
  ...
  **/
    std::string OpenDatafile(std::istringstream &ifs, const std::string &filename) {
        std::string res_name = ":/obdata/" + filename;
        QFile f(res_name.c_str());
        if (!f.open(QIODevice::ReadOnly)) {
            return (""); // error
        }
        ifs.clear();
        ifs.str(f.readAll().toStdString());
