...
namespace OpenBabel {
    ...
    */
        ...
        while (ifs.getline(buffer, BUFF_SIZE)) {
            ...
            if (vs.size() != 12) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Format error in eqeqIonizations.txt. Each data row should have exactly 12 elements.",
                                      obError);
                return false;
            }
    ...
    bool EQEqCharges::ComputeCharges(OBMol &mol) {
        ...
        for (i = 0; i < N; i++) {
            ...
            if (_ionizations[a][c + 1] == -1 || _ionizations[a][c] == -1 || a > TABLE_OF_ELEMENTS_SIZE) {
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Insufficient ionization data for atoms in the given molecule. Update `data/eqeqIonizations.txt` with missing information and re-run this function.",
                                      obError);
                return false;
            }
    ...
    double EQEqCharges::GetPeriodicEwaldJij(double J_i, double J_j, vector3 dx, bool isSameAtom, matrix3x3 unitcell,
                                            matrix3x3 fourier, double cellVolume, int numNeighbors[]) {
        int u, v, w;
