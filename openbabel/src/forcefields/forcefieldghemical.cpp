...
namespace OpenBabel {
    ...
    bool OBForceFieldGhemical::SetupCalculations() {
        ...
        FOR_TORSIONS_OF_MOL(t, _mol) {
            ...
            parameter = GetParameterGhemical(torsiontype, a->GetType(), b->GetType(), c->GetType(), d->GetType(),
                                             _fftorsionparams);
            if (parameter == nullptr) {
                parameter = GetParameterGhemical(torsiontype, "FFFF", b->GetType(), c->GetType(), d->GetType(),
                                                 _fftorsionparams);
                if (parameter == nullptr) {
                    parameter = GetParameterGhemical(torsiontype, a->GetType(), b->GetType(), c->GetType(), "FFFF",
                                                     _fftorsionparams);
                    if (parameter == nullptr) {
                        parameter = GetParameterGhemical(torsiontype, "FFFF", b->GetType(), c->GetType(), "FFFF",
                                                         _fftorsionparams);
                        if (parameter == nullptr) {
                            torsioncalc.V = 0.0;
                            torsioncalc.s = 1.0;
...
//      ifs.close();
    ...
    bool OBForceFieldGhemical::SetPartialCharges() {
        ...
        FOR_ATOMS_OF_MOL (atom, _mol)atom->SetPartialCharge(0.0);
        ...
        FOR_BONDS_OF_MOL (bond, _mol) {
            ...
            for (unsigned int idx = 0; idx < _ffchargeparams.size(); ++idx) {
                if (((_a == _ffchargeparams[idx]._a) && (_b == _ffchargeparams[idx]._b)) &&
                    (bondtype == _ffchargeparams[idx]._ipar[0])) {
                ...
                } else if (((_a == _ffchargeparams[idx]._b) && (_b == _ffchargeparams[idx]._a)) &&
                           (bondtype == _ffchargeparams[idx]._ipar[0])) {
                    a->SetPartialCharge(a->GetPartialCharge() + _ffchargeparams[idx]._dpar[0]);
                    b->SetPartialCharge(b->GetPartialCharge() - _ffchargeparams[idx]._dpar[0]);
                }
    ...
    OBFFParameter *
    OBForceFieldGhemical::GetParameterGhemical(int type, const char *a, const char *b, const char *c, const char *d,
