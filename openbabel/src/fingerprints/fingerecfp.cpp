...
/// \brief Fingerprint based on ECFP
    ...
    public:
        ...
        virtual std::string DescribeBits(const std::vector<unsigned int> fp, bool bSet = true) { return _ss.str(); }
...
{ \
  ...
  c -= a; c -= b; c ^= (b>>15); \
    ...
    static unsigned int ECFPHash(unsigned char *ptr, unsigned int length) {
        ...
        switch (len) {
            case 11:
                c += ((unsigned int) ptr[10] << 24);
            case 10:
                c += ((unsigned int) ptr[9] << 16);
            case 9:
                c += ((unsigned int) ptr[8] << 8);
            case 8:
                b += ((unsigned int) ptr[7] << 24);
            case 7:
                b += ((unsigned int) ptr[6] << 16);
            case 6:
                b += ((unsigned int) ptr[5] << 8);
            case 5:
                b += ptr[4];
            case 4:
                a += ((unsigned int) ptr[3] << 24);
            case 3:
                a += ((unsigned int) ptr[2] << 16);
            case 2:
                a += ((unsigned int) ptr[1] << 8);
            case 1:
                a += ptr[0];
        }
        mix32(a, b, c);
