...
namespace OpenBabel {
    ...
    bool containsAtLeast_2true_2paraAssemblies(OBAtom *ligandAtom, OBAtom *atom, const OBStereoUnitSet &units,
                                               const std::vector<OBBitVec> &mergedRings);
    ...
    int classifyTetrahedralNbrSymClasses(const std::vector<unsigned int> &symClasses, OBAtom *atom) {
        ...
        FOR_NBORS_OF_ATOM (nbr, atom)nbrClasses.push_back(symClasses.at(nbr->GetIndex()));
    ...
    */
    ...
    struct StereoRing {
        ...
        struct ParaBond {
            ...
            ParaBond(unsigned long _id, unsigned int _inIdx, unsigned int _outIdx) : id(_id), inIdx(_inIdx),
                                                                                     outIdx(_outIdx) {}
        ...
        for (std::size_t i = 0; i < rings.size(); ++i) {
            ...
            for (std::size_t j = 0; j < rings[i].paraAtoms.size(); ++j) {
                ...
                if (paraAtom.outsideNbrs.size() == 1) {
                    ...
                    if (symmetry_classes[paraAtom.outsideNbrs[0]->GetIndex()] !=
                        symmetry_classes[paraAtom.outsideNbrs[1]->GetIndex()]) {
                        // they are different
                        //cout << "OK: " << __LINE__ << endl;
                        return true;
                    } else {
            ...
            for (std::size_t j = 0; j < rings[i].paraBonds.size(); ++j) {
                ...
                if (paraBond.outsideNbrs.size() == 1) {
                    ...
                    if (paraBond.outsideNbrs.size() != 2)
                        continue;
                    // two ring substituents, need to check for topological difference
                    if (symmetry_classes[paraBond.outsideNbrs[0]->GetIndex()] !=
                        symmetry_classes[paraBond.outsideNbrs[1]->GetIndex()]) {
                        // they are different
                        //cout << "OK: " << __LINE__ << endl;
                        return true;
                    } else {
                    ...
                    if (paraAtom.outsideNbrs.size() == 2) {
                        if (symmetry_classes[paraAtom.outsideNbrs[0]->GetIndex()] ==
                            ...
                            for (std::size_t k = 0; k < rings[i].paraAtoms.size(); ++k) {
                                ...
                                if (paraAtom.inIdx == paraAtom2.outIdx &&
                                    paraAtom.insideNbrs == paraAtom2.outsideNbrs) {
                                    isSpiro = true;
                                    if (ApplyRule1(paraAtom2, symmetry_classes, rings, visitedRings, units,
                                                   stereoAtoms))
                                        isStereoUnit = true;
                                }
                            }
                    ...
                    if (paraBond.outsideNbrs.size() == 2) {
                        if (symmetry_classes[paraBond.outsideNbrs[0]->GetIndex()] ==
                            ...
                            for (std::size_t k = 0; k < rings[i].paraBonds.size(); ++k) {
                                ...
                                if (paraBond.inIdx == paraBond2.outIdx &&
                                    paraBond.insideNbrs == paraBond2.outsideNbrs) {
                                    isSpiro = true;
                                    if (ApplyRule1(paraBond2, symmetry_classes, rings, visitedRings, units,
                                                   stereoAtoms))
                                        isStereoUnit = true;
                                }
                            }
...
// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    void findDuplicatedSymmetryClasses(OBAtom *atom, const std::vector<unsigned int> &symClasses,
        ...
        std::vector<unsigned int> nbrSymClasses;
        FOR_NBORS_OF_ATOM (nbr, atom)nbrSymClasses.push_back(symClasses.at(nbr->GetIndex()));
        ...
        for (OBStereoUnitSet::const_iterator u2 = units.begin(); u2 != units.end(); ++u2) {
            if ((*u2).type == OBStereo::Tetrahedral) {
                ...
                if (ligand.BitIsSet(begin->GetId()) || ligand.BitIsSet(end->GetId())) {
                    if ((*u2).para) {
                        for (std::size_t ringIdx = 0; ringIdx < mergedRings.size(); ++ringIdx) {
                            if (mergedRings.at(ringIdx).BitIsSet(begin->GetIdx()) ||
                                mergedRings.at(ringIdx).BitIsSet(end->GetIdx())) {
                                if (std::find(ringIndices.begin(), ringIndices.end(), ringIdx) == ringIndices.end()) {
                                    ringIndices.push_back(ringIdx);
                                }
                            }
    ...
    std::vector<OBCisTransStereo *> CisTransFrom0D(OBMol *mol,
        ...
        for (i = bonds.begin(); i != bonds.end(); ++i) {
            ...
            if (ring && ring->Size() <= IMPLICIT_CIS_RING_SIZE) {
                ...
                for (int i = 0; i < 2; ++i) {
                    if (config.refs[i * 2] != OBStereo::ImplicitRef &&
                        ring->IsMember(mol->GetAtomById(config.refs[i * 2])))
    ...
    std::vector<OBCisTransStereo *> CisTransFrom3D(OBMol *mol,
        ...
        for (i = bonds.begin(); i != bonds.end(); ++i) {
            ...
            if (uc) {
                ...
                tor02 = CalcTorsionAngle(begin->GetVector() + bondVecs[0], begin->GetVector(), end->GetVector(),
                                         end->GetVector() + bondVecs[2]);
                tor03 = CalcTorsionAngle(begin->GetVector() + bondVecs[0], begin->GetVector(), end->GetVector(),
                                         end->GetVector() + bondVecs[3]);
                tor12 = CalcTorsionAngle(begin->GetVector() + bondVecs[1], begin->GetVector(), end->GetVector(),
                                         end->GetVector() + bondVecs[2]);
                tor13 = CalcTorsionAngle(begin->GetVector() + bondVecs[1], begin->GetVector(), end->GetVector(),
                                         end->GetVector() + bondVecs[3]);
            }
            ...
            if (std::abs(tor02) < 90.0 && std::abs(tor03) > 90.0) {
                ...
                if (std::abs(tor12) < 90.0 || std::abs(tor13) > 90.0) {
                    obErrorLog.ThrowError(__FUNCTION__,
                                          "Could not determine cis/trans from 3D coordinates, using unspecified",
                                          obInfo);
                    config.specified = false;
                }
                ...
                obErrorLog.ThrowError(__FUNCTION__,
                                      "Could not determine cis/trans from 3D coordinates, using unspecified", obInfo);
                config.shape = OBStereo::ShapeU;
                config.specified = false;
            }
        ...
        for (i = centers.begin(); i != centers.end(); ++i) {
            ...
            if (!config.specified || (config.specified && config.winding == OBStereo::UnknownWinding)) {
                ...
                if (hashAtoms.size() == 4 || wedgeAtoms.size() == 4) {
                    ...
                    if (AngleOrder(order[0]->GetVector(), order[1]->GetVector(), nbrs[1]->GetVector(),
                                   center->GetVector()))
                        order.push_back(nbrs[1]);
                    else
                        order.insert(order.begin() + 1, nbrs[1]);
                    if (AngleOrder(order[0]->GetVector(), order[2]->GetVector(), nbrs[2]->GetVector(),
                                   center->GetVector()))
                        order.push_back(nbrs[2]);
                    else {
                        if (AngleOrder(order[0]->GetVector(), order[1]->GetVector(), nbrs[2]->GetVector(),
                                       center->GetVector()))
                    ...
                    if (planeAtoms.size() == 2 && wedgeAtoms.size() == 1) { // Two planes, 1 wedge, 1 hash
                        if (order[2] == hashAtoms[0]) { // The wedge and hash are opposite
                            double angle = GetAngle(order[1], center,
                                                    order[3]); // The anticlockwise angle between the plane atoms
                            if (angle > -133 && angle < 133) { // This value is from the InChI TechMan Figure 9
                                if (angle > 0) { // Change to three planes and the hash bond
                                    std::rotate(order.begin(), order.begin() + 2,
                                                order.end()); // Change the order so that it begins with the hash bond
                                    wedge = false;
                ...
                if (hashAtoms.size() == 0 || wedgeAtoms.size() == 0) {
                    ...
                    if (planeAtoms.size() == 2) { // Two planes, 1 stereo
                        double angle = GetAngle(order[1], center,
                                                order[2]); // The anticlockwise angle between the plane atoms
...
//         obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obError);
    ...
    std::vector<OBCisTransStereo *> CisTransFrom2D(OBMol *mol,
                                                   const OBStereoUnitSet &stereoUnits,
                                                   const std::map<OBBond *, enum OBStereo::BondDirection> *updown,
                                                   bool addToMol) {
        std::vector<OBCisTransStereo *> configs;
        ...
        for (std::vector<OBGenericData *>::iterator data = vdata.begin(); data != vdata.end(); ++data)
            if (((OBStereoBase *) *data)->GetType() == OBStereo::Tetrahedral) {
                ...
                if (cfg.specified) {
                    ...
                    vector<OBAtom *> nbrs;
                    FOR_NBORS_OF_ATOM(a, center)nbrs.push_back(&*a);
                    ...
                    if (chosen ==
                        nullptr) { // There is a remote possibility of this but let's worry about 99.9% of cases first
                        obErrorLog.ThrowError(__FUNCTION__,
                    ...
                    if (cfg.winding != OBStereo::UnknownWinding) {
                        ...
                        if (test_cfg.from != OBStereo::ImplicitRef) {
                            OBStereo::RefIter ri = std::find(test_cfg.refs.begin(), test_cfg.refs.end(),
                                                             (unsigned long) OBStereo::ImplicitRef);
                        ...
                        if (implicit) {
                            ...
                            if (uc)
                                anticlockwise_order = AngleOrder(
                                        uc->UnwrapCartesianNear(mol.GetAtomById(test_cfg.refs[0])->GetVector(),
                                                                center_coord),
                                        uc->UnwrapCartesianNear(mol.GetAtomById(test_cfg.refs[1])->GetVector(),
                                                                center_coord),
                                        uc->UnwrapCartesianNear(mol.GetAtomById(test_cfg.refs[2])->GetVector(),
                                                                center_coord),
                                        center_coord
                                );
                            else
                            ...
                            double angle = GetAngle(mol.GetAtomById(test_cfg.refs[0]), center,
                                                    mol.GetAtomById(test_cfg.refs[2]));
                            if ((angle < 0 && anticlockwise_order) ||
                                (angle > 0 && !anticlockwise_order)) // Is the stereobond in the bigger angle?
    ...
    void StereoRefToImplicit(OBMol &mol, OBStereo::Ref atomId) {
        ...
        const auto equal_to_atomId = std::bind(equal_to<OBStereo::Ref>(), placeholders::_1, atomId);
        ...
        for (std::vector<OBGenericData *>::iterator data = vdata.begin(); data != vdata.end(); ++data) {
            ...
            if (datatype == OBStereo::CisTrans) {
                ...
                replace_if(ct_cfg.refs.begin(), ct_cfg.refs.end(), equal_to_atomId,
                ...
                replace_if(ts_cfg.refs.begin(), ts_cfg.refs.end(), equal_to_atomId,
                           (OBStereo::Ref) OBStereo::ImplicitRef);
                ts->SetConfig(ts_cfg);
            }
        }
    ...
    void ImplicitRefToStereo(OBMol &mol, OBStereo::Ref centerId, OBStereo::Ref newId) {
        // The following is for use in replace_if(...) below
        const auto equal_to_implicitRef = std::bind(equal_to<OBStereo::Ref>(), placeholders::_1,
                                                    (OBStereo::Ref) OBStereo::ImplicitRef);
        ...
        for (std::vector<OBGenericData *>::iterator data = vdata.begin(); data != vdata.end(); ++data) {
            ...
            if (datatype != OBStereo::CisTrans && datatype != OBStereo::Tetrahedral) {
                // Maybe I should just unset the stereochemistry if this happens?
                obErrorLog.ThrowError(__FUNCTION__,
                                      "This function should be updated to handle additional stereo types.\nSome stereochemistry objects may contain implicit refs to hydrogens which need to be converted to explicit.",
                                      obWarning);
                continue;
            }
            ...
            if (datatype == OBStereo::CisTrans) {
                ...
                if (ct_cfg.begin == centerId || ct_cfg.end == centerId) {
                    ...
                    if (ct_cfg.begin == centerId)
                        replace_if(ct_cfg.refs.begin(), ct_cfg.refs.begin() + 2, equal_to_implicitRef,
                                   (OBStereo::Ref) newId);
                    if (ct_cfg.end == centerId)
                        replace_if(ct_cfg.refs.begin() + 2, ct_cfg.refs.end(), equal_to_implicitRef,
                                   (OBStereo::Ref) newId);
                    ct->SetConfig(ct_cfg);
                }
            } else if (datatype == OBStereo::Tetrahedral) {
