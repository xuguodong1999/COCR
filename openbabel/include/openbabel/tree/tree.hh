...
template<typename iter, class BinaryPredicate>
bool
tree<T, tree_node_allocator>::equal(const iter &one_, const iter &two, const iter &three_, BinaryPredicate fun) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::sibling(const iterator_base &it, unsigned int num) {
    tree_node *tmp;
    if (it.node->parent == 0) {
...
typename tree<T, tree_node_allocator>::leaf_iterator &tree<T, tree_node_allocator>::leaf_iterator::operator++() {
    ...
    if (this->node->first_child != nullptr) { // current node is no longer leaf (children got added)
        ...
        while (this->node->next_sibling == nullptr) {
            ...
            this->node = this->node->parent;
            if (top_node != nullptr && this->node == top_node) return *this;
        }
        this->node = this->node->next_sibling;
        while (this->node->first_child)
            this->node = this->node->first_child;
