...
void test(po::variables_map &vm, torch::Device &device, Encoder &enc1, Encoder &enc2, Decoder &dec,
          std::vector<transforms_Compose > &transform);
...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("loss_adv", po::value<std::string>()->default_value("l2"),
             "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
            ("loss_con", po::value<std::string>()->default_value("l1"),
             "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
            ("loss_enc", po::value<std::string>()->default_value("l2"),
             "l1 (mean absolute error), l2 (mean squared error), ssim (structural similarity), etc.")
            ...
            ("lr_dec", po::value<float>()->default_value(1e-3), "learning rate for decoder")
            ...
            ("ngef", po::value<size_t>()->default_value(64),
             "the number of filters in convolution layer closest to image in encoder")
            ("ngdf", po::value<size_t>()->default_value(64),
             "the number of filters in convolution layer closest to image in decoder")
            ("ndf", po::value<size_t>()->default_value(64),
             "the number of filters in convolution layer closest to image in discriminator")
            ("Lambda_adv", po::value<float>()->default_value(1.0), "the multiple of adversarial loss")
            ("Lambda_con", po::value<float>()->default_value(50.0), "the multiple of contextual loss")
            ("Lambda_enc", po::value<float>()->default_value(1.0), "the multiple of encoder loss");
...
int main(int argc, const char *argv[]) {
    ...
    Set_Model_Params(vm, enc1, "Encoder1");
    Set_Model_Params(vm, enc2, "Encoder2");
    ...
    if (vm["train"].as<bool>()) {
        ...
        train(vm, device, enc1, enc2, dec, dis, transform);
    }

    // (8.2) Test Phase
    if (vm["test"].as<bool>()) {
        Set_Options(vm, argc, argv, args, "test");
        test(vm, device, enc1, enc2, dec, transform);
    }
