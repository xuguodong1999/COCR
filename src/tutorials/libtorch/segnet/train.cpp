...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderSegmentWithPaths &valid_dataloader, torch::Device &device,
           Loss &criterion, SegNet &model, const size_t epoch, visualizer::graph &writer);
...
void train(po::variables_map &vm, torch::Device &device, SegNet &model, std::vector<transforms_Compose > &transformI,
    ...
    dataloader = DataLoader::ImageFolderSegmentWithPaths(dataset, vm["batch_size"].as<size_t>(), /*shuffle_=*/
    ...
    if (vm["valid"].as<bool>()) {
        ...
        valid_dataset = datasets::ImageFolderSegmentWithPaths(valid_input_dir, valid_output_dir, transformI,
                                                              transformO);
        valid_dataloader = DataLoader::ImageFolderSegmentWithPaths(valid_dataset,
