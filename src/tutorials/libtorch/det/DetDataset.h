...
public:
    // Constructor
    DetDataset(std::vector<std::string> images, std::vector<std::string> labels, std::vector<std::string> class_names,
               bool istrain = true,
        ...
        width = width_;
        height = height_;
        hflipProb = hflip_prob;
        vflipProb = vflip_prob;
    ...
    torch::data::Example<> get(size_t index) override {
        ...
        torch::Tensor img_tensor = torch::from_blob(m_data.image.data, {m_data.image.rows, m_data.image.cols, 3},
                                                    torch::kByte).permute({2, 0, 1}); // Channels x Height x Width
