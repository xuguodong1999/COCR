...
void test(po::variables_map &vm, torch::Device &device, Encoder &enc, Decoder &dec, EstimationNetwork &est,
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_gmp.dat";
    load_params(path, mu, sigma, phi);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_est.pth";
    ...
    while (dataloader(data)) {
        ...
        std::cout << '<' << std::get<1>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << loss.item<float>()
                  << " anomaly_score:" << score << std::endl;
        ofs << '<' << std::get<1>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << loss.item<float>()
            << " anomaly_score:" << score << std::endl;
    ...
    // (7) Average Output
    std::cout << "<All> " << vm["loss"].as<std::string>() << ':' << ave_loss << " anomaly_score:" << ave_anomaly_score
              << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> " << vm["loss"].as<std::string>() << ':' << ave_loss << " anomaly_score:" << ave_anomaly_score
        << " (time:" << ave_time << ')' << std::endl;
