...
// Function Prototype
void valid(po::variables_map &vm, DataLoader::ImageFolderPairWithPaths &valid_dataloader, torch::Device &device,
           Loss &criterion_GAN, torch::nn::L1Loss &criterion_L1, UNet_Generator &gen, PatchGAN_Discriminator &dis,
           const size_t epoch, visualizer::graph &writer, visualizer::graph &writer_gen, visualizer::graph &writer_dis);


// -------------------
// Training Function
// -------------------
void train(po::variables_map &vm, torch::Device &device, UNet_Generator &gen, PatchGAN_Discriminator &dis,
           std::vector<transforms_Compose > &transformI, std::vector<transforms_Compose > &transformO) {
    ...
    dataloader = DataLoader::ImageFolderPairWithPaths(dataset, vm["batch_size"].as<size_t>(), /*shuffle_=*/
    ...
    if (vm["valid"].as<bool>()) {
        ...
        valid_dataloader = DataLoader::ImageFolderPairWithPaths(valid_dataset,
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        while (dataloader(mini_batch)) {
            ...
            label_real = torch::full(
                    {dis_fake_out.size(0), dis_fake_out.size(1), dis_fake_out.size(2), dis_fake_out.size(3)}, /*value*/
                    1.0, torch::TensorOptions().dtype(torch::kFloat)).to(device);
            label_fake = torch::full(
                    {dis_fake_out.size(0), dis_fake_out.size(1), dis_fake_out.size(2), dis_fake_out.size(3)}, /*value*/
                    0.0, torch::TensorOptions().dtype(torch::kFloat)).to(device);
            ...
            show_progress->increment(/*loss_value=*/
                    {G_GAN_loss.item<float>(), G_L1_loss.item<float>(), dis_real_loss.item<float>(),
        ...
        // -----------------------------------
        train_loss.plot(/*base=*/epoch, /*value=*/{show_progress->get_ave(0) + show_progress->get_ave(1),
                                                   show_progress->get_ave(2) + show_progress->get_ave(3)});
        train_loss_gen.plot(/*base=*/epoch, /*value=*/
                                     {show_progress->get_ave(0) + show_progress->get_ave(1), show_progress->get_ave(0),
                                      show_progress->get_ave(1)});
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)) {
            valid(vm, valid_dataloader, device, criterion_GAN, criterion_L1, gen, dis, epoch, valid_loss,
                  valid_loss_gen, valid_loss_dis);
        }
