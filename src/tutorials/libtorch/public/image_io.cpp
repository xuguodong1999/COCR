...
namespace image_io {
    namespace {
        ...
        torch::Tensor make_grid(torch::Tensor tensor, int64_t nrow, int64_t padding,
                                bool normalize, const std::vector<double> &range, bool scale_each,
                                torch::Scalar pad_value) {
            if (tensor.dim() == 2) {
    ...
    torch::Tensor load_image(const std::string &file_path,
        ...
        if (shape.empty()) {
            return transform(torch::from_blob(image_raw.get(),
                                              {height, width, depth}, torch::kUInt8).clone().to(
                    torch::kFloat32).permute({2, 0, 1}).div_(255));
        }
        ...
        return transform(torch::from_blob(image_resized_buffer.data(),
                                          {new_height, new_width, depth}, torch::kUInt8).clone().to(
                torch::kFloat32).permute({2, 0, 1}).div_(255));
    }
}  // namespace image_io
