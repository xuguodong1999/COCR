...
MC_DiscriminatorImpl::MC_DiscriminatorImpl(po::variables_map &vm) {
    ...
    Convolution(this->features, /*in_nc=*/vm["nc"].as<size_t>(), /*out_nc=*/feature, /*ksize=*/4, /*stride=*/2, /*pad=*/
                1, /*BN=*/false, /*LReLU=*/true);  // {C,256,256} ===> {F,128,128}
    Convolution(this->features, /*in_nc=*/feature, /*out_nc=*/feature * 2, /*ksize=*/4, /*stride=*/2, /*pad=*/1, /*BN=*/
                BN, /*LReLU=*/true);                 // {F,128,128} ===> {2F,64,64}
    Convolution(this->features, /*in_nc=*/feature * 2, /*out_nc=*/feature * 4, /*ksize=*/4, /*stride=*/2, /*pad=*/
                1, /*BN=*/BN, /*LReLU=*/true);               // {2F,64,64}  ===> {4F,32,32}
    Convolution(this->features, /*in_nc=*/feature * 4, /*out_nc=*/feature * 8, /*ksize=*/4, /*stride=*/2, /*pad=*/
                1, /*BN=*/BN, /*LReLU=*/true);               // {4F,32,32}  ===> {8F,16,16}
    for (size_t i = 0; i < num_downs - 5; i++) {
        Convolution(this->features, /*in_nc=*/feature * 8, /*out_nc=*/feature * 8, /*ksize=*/4, /*stride=*/2, /*pad=*/
                    1, /*BN=*/BN, /*LReLU=*/true);           // {8F,16,16}  ===> {8F,2,2}
    }
    register_module("features", this->features);

    // 2th Blocks  {8F,X,X} ===> {8F,2,2}
    this->avgpool = nn::Sequential(nn::AdaptiveAvgPool2d(nn::AdaptiveAvgPool2dOptions(
            {2, 2})));                                                           // {8F,X,X}    ===> {8F,2,2}
    register_module("avgpool", this->avgpool);

    // 3th Blocks  {8F,2,2} ===> {CN,1,1}
    Convolution(this->classifier, /*in_nc=*/feature * 8, /*out_nc=*/class_num, /*ksize=*/2, /*stride=*/1, /*pad=*/
                0, /*BN=*/false, /*LReLU=*/false);         // {8F,2,2}    ===> {CN,1,1}
