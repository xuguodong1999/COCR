...
AnomalyScore(torch::Tensor image, torch::Tensor fake_image, GAN_Discriminator &dis, const float Lambda = 0.1);
...
void test(po::variables_map &vm, torch::Device &device, GAN_Generator &gen, GAN_Discriminator &dis,
    ...
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_gen.pth";
    torch::load(gen, path, device);
    path = "checkpoints/" + vm["dataset"].as<std::string>() + "/models/epoch_" +
           vm["test_load_epoch"].as<std::string>() + "_dis.pth";
    ...
    auto z_optimizer = torch::optim::Adam({z}, torch::optim::AdamOptions(vm["lr_z"].as<float>()).betas(
    ...
    while (dataloader(data)) {
        ...
        show_progress = new progress::display(/*count_max_=*/search_epoch, /*header1=*/
                                                             std::get<1>(data).at(0), /*header2=*/"|", /*loss_=*/
                                                             {"loss", "res", "dis"});
        ...
        for (size_t i = 0; i < search_epoch; i++) {
            ...
            show_progress->increment(/*loss_value=*/
                    {loss.item<float>(), res_loss.item<float>(), dis_loss.item<float>()});
        }
        output = gen->forward(z);
        anomaly_score_with_alpha = AnomalyScore(image, output, dis, vm["test_Lambda"].as<float>());
        ...
        ofs << '<' << std::get<1>(data).at(0) << "> anomaly_score:" << anomaly_score.item<float>() << " res:"
            << res_loss.item<float>() << " dis:" << dis_loss.item<float>() << " (time:" << ave_time << ')' << std::endl;
    ...
    // (7) Average Output
    std::cout << "<All> anomaly_score:" << ave_anomaly_score << " res:" << ave_res_loss << " dis:" << ave_dis_loss
              << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> anomaly_score:" << ave_anomaly_score << " res:" << ave_res_loss << " dis:" << ave_dis_loss
        << " (time:" << ave_time << ')' << std::endl;
...
// ---------------------------------------------
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
AnomalyScore(torch::Tensor image, torch::Tensor fake_image, GAN_Discriminator &dis, const float Lambda) {
