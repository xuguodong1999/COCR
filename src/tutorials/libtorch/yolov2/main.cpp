...
void detect(po::variables_map &vm, torch::Device &device, YOLOv2 &model, std::vector<transforms_Compose > &transformI,
            std::vector<transforms_Compose > &transformD, const std::vector<std::string> class_names,
            const std::vector<std::tuple<float, float>> anchors);

void demo(po::variables_map &vm, torch::Device &device, YOLOv2 &model, std::vector<transforms_Compose > &transformI,
          std::vector<transforms_Compose > &transformD, const std::vector<std::string> class_names,
          const std::vector<std::tuple<float, float>> anchors);
...
po::options_description parse_arguments() {
    ...
    args.add_options()
            ...
            ("noobj_thresh", po::value<float>()->default_value(0.6),
...
int main(int argc, const char *argv[]) {
    ...
    std::vector<std::tuple<float, float>> anchors = Set_Anchors(vm["anchor_list"].as<std::string>(),
                                                                vm["na"].as<size_t>());
