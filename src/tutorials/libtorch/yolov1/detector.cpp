...
// ------------------------------------
YOLODetector::YOLODetector(const long int class_num_, const long int ng_, const long int nb_, const float prob_thresh_,
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> YOLODetector::operator()(const torch::Tensor pred) {
    ...
    /*************************************************************************/
    pred_new = pred.split_with_sizes(/*split_sizes=*/{this->class_num, this->nb * 5}, /*dim=*/
                                                     2);  // pred{G,G,FF} ===> pred_new({G,G,CN}, {G,G,BB*5})
    pred_class = pred_new.at(0);  // pred_class{G,G,CN}
    pred_coord = pred_new.at(1);  // pred_coord{G,G,BB*5}
    pred_coord_new = pred_coord.view(
            {this->ng, this->ng, this->nb, 5});  // pred_coord{G,G,BB*5} ===> pred_coord_new{G,G,BB,5}
    pred_coord_per = pred_coord_new.permute(
            {3, 0, 1, 2}).contiguous();  // pred_coord_new{G,G,BB,5} ===> pred_coord_per{5,G,G,BB}
    class_score_with_idx = pred_class.max(/*dim=*/2, /*keepdim=*/
                                                  true);  // pred_class{G,G,CN} ===> class_score_with_idx({G,G,1}, {G,G,1})
    ...
    /*************************************************************************/
    arange = torch::arange(/*start=*/0.0, /*end=*/(float) this->ng, /*step=*/1.0,
                                     torch::TensorOptions().dtype(torch::kFloat)).to(
            pred.device());  // arange{G} = [0,1,2,...,G-1]
    x0 = arange.unsqueeze(/*dim=*/0).unsqueeze(/*dim=*/-1).expand(
            {this->ng, this->ng, this->nb});  // arange{G} ===> {1,G,1} ===> x0{G,G,BB}
    y0 = arange.unsqueeze(/*dim=*/-1).unsqueeze(/*dim=*/-1).expand(
            {this->ng, this->ng, this->nb});  // arange{G} ===> {G,1,1} ===> y0{G,G,BB}
    ...
    obj_cx_normalized = pred_coord_per[0].masked_select(/*mask=*/
            obj_mask);  // pred_coord_per{5,G,G,BB} ===> obj_cx_normalized{object}
    obj_cy_normalized = pred_coord_per[1].masked_select(/*mask=*/
            obj_mask);  // pred_coord_per{5,G,G,BB} ===> obj_cy_normalized{object}
    obj_cx = obj_cx_normalized / (float) this->ng +
             obj_x0 / (float) this->ng;  // obj_cx_normalized{object} ===> obj_cx{object}
    obj_cy = obj_cy_normalized / (float) this->ng +
             obj_y0 / (float) this->ng;  // obj_cy_normalized{object} ===> obj_cy{object}
    ...
    /*************************************************************************/
    if (obj_ids.numel() > 0) {
        nms_idx = this->NonMaximumSuppression(obj_coord,
                                              obj_conf);  // obj_coord{object,4}, obj_conf{object} ===> nms_idx{nms-object}
        final_ids = obj_ids.gather(/*dim=*/0, /*index=*/
                                           nms_idx).contiguous().detach().clone();  // obj_ids{object} ===> final_ids{nms-object}
        final_coord = obj_coord.gather(/*dim=*/0, /*index=*/nms_idx.unsqueeze(/*dim=*/-1).expand({nms_idx.size(0),
                                                                                                  4})).contiguous().detach().clone();  // obj_coord{object,4} ===> final_coord{nms-object,4}
        final_prob = obj_prob.gather(/*dim=*/0, /*index=*/
                                             nms_idx).contiguous().detach().clone();  // obj_prob{object} ===> final_prob{nms-object}
    }

    return {final_ids, final_coord,
            final_prob};  // {nms-object} (ids), {nms-object,4} (coordinates), {nms-object} (probabilities)
