...
// Test Function
// ---------------
void test(po::variables_map &vm, torch::Device &device, ConvolutionalAutoEncoder &model,
    ...
    std::string result_dir, result_in_dir, result_out_dir;
    ...
    dataloader = DataLoader::ImageFolderPairWithPaths(dataset, /*batch_size_=*/1, /*shuffle_=*/false, /*num_workers_=*/
                                                      0);
    ...
    result_in_dir = result_dir + "/input";
    fs::create_directories(result_in_dir);
    result_out_dir = result_dir + "/output";
    fs::create_directories(result_out_dir);
    ...
    while (dataloader(data)) {
        ...
        std::cout << '<' << std::get<2>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << loss.item<float>()
                  << " GT_" << vm["loss"].as<std::string>() << ':' << GT_loss.item<float>() << std::endl;
        ofs << '<' << std::get<2>(data).at(0) << "> " << vm["loss"].as<std::string>() << ':' << loss.item<float>()
            << " GT_" << vm["loss"].as<std::string>() << ':' << GT_loss.item<float>() << std::endl;

        fname = result_in_dir + '/' + std::get<2>(data).at(0);
        visualizer::save_image(imageI.detach(), fname, /*range=*/output_range, /*cols=*/1, /*padding=*/0);
        fname = result_out_dir + '/' + std::get<3>(data).at(0);
    ...
    // (7) Average Output
    std::cout << "<All> " << vm["loss"].as<std::string>() << ':' << ave_loss << " GT_" << vm["loss"].as<std::string>()
              << ':' << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
    ofs << "<All> " << vm["loss"].as<std::string>() << ':' << ave_loss << " GT_" << vm["loss"].as<std::string>() << ':'
        << ave_GT_loss << " (time:" << ave_time << ')' << std::endl;
