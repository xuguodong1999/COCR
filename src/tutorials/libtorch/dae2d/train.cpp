...
void valid(po::variables_map &vm, DataLoader::ImageFolderPairWithPaths &valid_dataloader, torch::Device &device,
           Loss &criterion, ConvolutionalAutoEncoder &model, const size_t epoch, visualizer::graph &writer);
...
void train(po::variables_map &vm, torch::Device &device, ConvolutionalAutoEncoder &model,
    ...
    torch::Tensor loss, imageI, imageO, output, pair;
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        while (dataloader(mini_batch)) {
            ...
            if (iter % save_sample_iter == 1) {
                ...
                pair = torch::cat({imageI, output, imageO}, /*dim=*/0);
