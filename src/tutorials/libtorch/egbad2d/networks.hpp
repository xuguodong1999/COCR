...
// ----------------------------------------------------------
// struct{GAN_EncoderImpl}(nn::Module)
// ----------------------------------------------------------
struct GAN_EncoderImpl : nn::Module {
private:
    nn::Sequential model;
public:
    ...
    GAN_EncoderImpl(po::variables_map &vm);
...
struct GAN_DiscriminatorImpl : nn::Module {
private:
    nn::Sequential x_down, z_down, features, classifier;
public:
    GAN_DiscriminatorImpl() {}

    GAN_DiscriminatorImpl(po::variables_map &vm);

    std::pair<torch::Tensor, torch::Tensor> forward(torch::Tensor x, torch::Tensor z);
};

TORCH_MODULE(GAN_Encoder);
