...
__global__ void
backward_maxpool_depth_layer_kernel(int n, int w, int h, int c, int batch, float *delta, float *prev_delta,
                                    int *indexes) {
    int id = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
    if (id >= n) return;
...
__global__ void
forward_maxpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride_x, int stride_y, int size, int pad,
...
__global__ void
backward_maxpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride_x, int stride_y, int size, int pad,
...
extern "C" void forward_maxpool_layer_gpu(maxpool_layer layer, network_state state) {
...
#endif
    {
        ...
        forward_maxpool_layer_kernel << < cuda_gridsize(
                n), BLOCK, 0, get_cuda_stream() >> > (n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size, layer.pad, state.input, layer.output_gpu, layer.indexes_gpu);
        CHECK_CUDA(cudaPeekAtLastError());

        if (layer.maxpool_zero_nonmax) {
            forward_zero_nonmax_kernel << <
            cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> > (n, state.input, layer.output_gpu);
            CHECK_CUDA(cudaPeekAtLastError());
        }
    }
    ...
    if (layer.antialiasing) {
        ...
        simple_copy_ongpu(layer.input_layer->outputs * layer.input_layer->batch, layer.input_layer->output_gpu,
                          layer.output_gpu);
    }
}
...
extern "C" void backward_maxpool_layer_gpu(maxpool_layer layer, network_state state) {
    if (layer.antialiasing) {
        ...
        //if (!state.train) s.index = state.index;  // don't use TC for training (especially without cuda_convert_f32_to_f16() )
        simple_copy_ongpu(layer.input_layer->outputs * layer.input_layer->batch, layer.delta_gpu,
                          layer.input_layer->delta_gpu);
    ...
    if (layer.maxpool_depth) {
        ...
        backward_maxpool_depth_layer_kernel << < cuda_gridsize(
                n), BLOCK, 0, get_cuda_stream() >> >(n, layer.w, layer.h, layer.c, layer.batch, layer.delta_gpu, state.delta, layer.indexes_gpu);
        CHECK_CUDA(cudaPeekAtLastError());
        return;
    }
    ...
    backward_maxpool_layer_kernel<<<cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >>>(n, layer.h, layer.w, layer.c,
                                                                                      layer.stride_x, layer.stride_y,
                                                                                      layer.size, layer.pad,
                                                                                      layer.delta_gpu, state.delta,
                                                                                      layer.indexes_gpu);
    CHECK_CUDA(cudaPeekAtLastError());

    if (layer.maxpool_zero_nonmax) {
        backward_zero_nonmax_kernel << <
        cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> > (n, layer.indexes_gpu, state.delta);
        CHECK_CUDA(cudaPeekAtLastError());
    }
}
...
__global__ void
forward_local_avgpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride_x, int stride_y, int size, int pad,
...
__global__ void
backward_local_avgpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride_x, int stride_y, int size, int pad,
...
extern "C" void forward_local_avgpool_layer_gpu(maxpool_layer layer, network_state state) {
...
#endif
    {
        ...
        forward_local_avgpool_layer_kernel << < cuda_gridsize(
                n), BLOCK, 0, get_cuda_stream() >> > (n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size, layer.pad, state.input, layer.output_gpu);
        CHECK_CUDA(cudaPeekAtLastError());
    }
}
...
extern "C" void backward_local_avgpool_layer_gpu(maxpool_layer layer, network_state state) {
    ...
    backward_local_avgpool_layer_kernel << < cuda_gridsize(
            n), BLOCK, 0, get_cuda_stream() >> >(n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size, layer.pad, layer.delta_gpu, state.delta);
    CHECK_CUDA(cudaPeekAtLastError());
}
