...
void softmax_cpu(float *input, int n, int batch, int batch_offset, int groups, int group_offset, int stride, float temp,
    ...
    for (b = 0; b < batch; ++b) {
        for (g = 0; g < groups; ++g) {
            softmax(input + b * batch_offset + g * group_offset, n, temp, output + b * batch_offset + g * group_offset,
                    stride);
        }
    }
}
...
void get_embedding(float *src, int src_w, int src_h, int src_c, int embedding_size, int cur_w, int cur_h, int cur_n,
    ...
    for (i = 0; i < embedding_size; ++i) {
        const int src_index =
                cur_b * (src_c * src_h * src_w) + cur_n * (embedding_size * src_h * src_w) + i * src_h * src_w +
                cur_h * (src_w) + cur_w;
...
void grad_contrastive_loss_positive_f(size_t i, int *class_ids, int *labels, size_t num_of_samples, float **z,
    ...
    if (N == 0 || temperature == 0 || vec_len == 0) {
        fprintf(stderr,
                " Error: N == 0 || temperature == 0 || vec_len == 0. N=%f, temperature=%f, vec_len=%f, labels[i] = %d \n",
...
void grad_contrastive_loss_positive(size_t i, int *labels, size_t num_of_samples, float **z, unsigned int feature_size,
    ...
    for (j = 0; j < num_of_samples; ++j) {
        ...
        if (i != j && labels[i] == labels[j]) {
            ...
            const float P = p_constrastive[i * num_of_samples +
                                           j];   // P_constrastive(i, j, labels, num_of_samples, z, feature_size, temperature, cos_sim);
            //const float custom_pos_mult = 1 - sim;
        ...
        if (i != j && labels[i] == labels[j]) {
            ...
            for (k = 0; k < num_of_samples; ++k) {
                ...
                if (k != i && k != j && labels[k] >= 0) {
                    const float sim = cos_sim[i * num_of_samples +
                                              k];        // cosine_similarity(z[i], z[k], feature_size);
                    const float P = p_constrastive[i * num_of_samples +
                                                   k];   // P_constrastive(i, k, labels, num_of_samples, z, feature_size, temperature, cos_sim);
                    //const float custom_pos_mult = 1 + sim;
