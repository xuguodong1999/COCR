 ...
 * CH3COOH,COOH,...,etc.
 */
class Text : public ShapeInterface {
public:
    std::vector<Shape> shapes;
    std::string name;
public:
    static std::shared_ptr<Text> GetSymbol(const std::string &_symbolType = "");

    Text() : name("Symbol") { isRotateAllowed = false; }

    Text(const NString &name) {
