...
void demoText() {
    ...
    while (true) {
        cv::Mat img1 = cv::Mat(hhh, www, CV_8UC3, WHITE);
        Text hehe(makeNotedStringByChar("1234567890"));
...
void demoRichText() {
    ...
    while (true) {
        cv::Mat img1 = cv::Mat(hhh, www, CV_8UC3, WHITE);
        Text::setSAngleK((rand() % 20 - 10) / 100.0);

        Text::setSNormSizeK((rand() % 10 - 5) / 100.0 + 1.0);
        Text::setSNormOffsetKx((rand() % 10 - 5) / 100.0);
        Text::setSNormOffsetKy((rand() % 10 - 5) / 100.0);

        Text::setSSubSizeK((rand() % 10 - 15) / 100.0 + 1.0);
        Text::setSSubOffsetKx((rand() % 10 - 5) / 100.0);
        Text::setSSubOffsetKy((rand() % 10 - 5) / 100.0);
        Text hehe(makeNotedString(
//                "CuSO4(5H2O)",
...
//        auto positiveSym = Symbol::GetSymbol("positive");
        auto positiveSym = Text::GetSymbol("negative");
