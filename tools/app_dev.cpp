...
cocr::OCRManager makeOCRManager(const std::string &_modelDir) {
    /// detector
...
//    }
    ...
    /// manager
    cocr::OCRManager ocrManager(detector, recognizer, corrector, composer);
    return ocrManager;
}
...
//    std::vector<int> testcases = {7};
    ...
    while (true) {
        ...
        idx %= testcases.size();
        auto testImg = cocr::convertQImageToMat(image);
...
//        cv::erode(image, image, cv::Mat());
        int angle = 2;
        if (rand() % 2) { angle = -2; }
        image = cocr::rotateCvMat(image, angle);
...
std::shared_ptr<cocr::JMol> getTestMol() {
...
std::shared_ptr<cocr::JMol> getTestMol_CC0C() {
    using namespace cocr;
    auto mol = std::make_shared<JMolAdapter>();
...
int testMolFormat() {
    ...
    std::cout << std::endl;
    std::shared_ptr<cocr::JMol> mol = std::make_shared<cocr::JMolAdapter>();
    mol->readAsSMI("c1ccccc1");
    std::cout << "**********************" << std::endl;
