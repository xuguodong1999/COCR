...
void forward_backward_network_gpu(network net, float *x, float *y) {
    ...
    for (i = 0; i < net.n; ++i) {
        ...
        if (net.cudnn_half) {
            if (l.type == CONVOLUTIONAL && l.weights_gpu && l.weights_gpu16) {
                ...
                cuda_convert_f32_to_f16(l.input_layer->weights_gpu, l.input_layer->nweights,
                ...
                cuda_convert_f32_to_f16(l.output_layer->weights_gpu, l.output_layer->nweights,
    ...
    if (*(state.net.total_bbox) > 0)
        fprintf(stderr, " total_bbox = %d, rewritten_bbox = %f %% \n", *(state.net.total_bbox),
                100 * (float) *(state.net.rewritten_bbox) / *(state.net.total_bbox));
}
...
float train_networks(network *nets, int n, data d, int interval) {
    ...
    *nets[0].seen = nets[0].batch * nets[0].subdivisions * get_current_iteration(
            nets[0]); // remove this line, when you will save to weights-file both: seen & cur_iteration
