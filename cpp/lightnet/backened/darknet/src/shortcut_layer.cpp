...
layer make_shortcut_layer(int batch, int n, int *input_layers, int *input_sizes, int w, int h, int c,
                          float **layers_output, float **layers_delta, float **layers_output_gpu,
                          float **layers_delta_gpu, WEIGHTS_TYPE_T weights_type,
...
void resize_shortcut_layer(layer *l, int w, int h, network *net) {
    ...
    if (l->activation == SWISH || l->activation == MISH)
        l->activation_input = (float *) realloc(l->activation_input, l->batch * l->outputs * sizeof(float));
...
#pragma omp parallel for
        ...
        shortcut_multilayer_cpu(l.outputs * l.batch, l.outputs, l.batch, l.n, l.input_sizes, l.layers_output, l.output,
                                state.input, l.weights, l.nweights, l.weights_normalization);
    }
...
void forward_shortcut_layer_gpu(const layer l, network_state state) {
    ...
    //else
    {
        shortcut_multilayer_gpu(l.outputs, l.batch, l.n, l.input_sizes_gpu, l.layers_output_gpu, l.output_gpu,
                                state.input, l.weights_gpu, l.nweights, l.weights_normalization);
    }
...
void backward_shortcut_layer_gpu(const layer l, network_state state) {
    ...
    backward_shortcut_multilayer_gpu(l.outputs, l.batch, l.n, l.input_sizes_gpu, l.layers_delta_gpu, state.delta,
                                     l.delta_gpu,
...
void
update_shortcut_layer_gpu(layer l, int batch, float learning_rate_init, float momentum, float decay, float loss_scale) {
    if (l.nweights > 0) {
