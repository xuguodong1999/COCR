...
void convolution_2d(int w, int h, int ksize, int n, int c, int pad, int stride,
    ...
    __m256i all256_sing1 = _mm256_set_epi32(0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000,
...
static inline void xnor_avx2_popcnt(__m256i a_bit256, __m256i b_bit256, __m256i *count_sum) {
    ...
    c_bit256 = _mm256_andnot_si256(xor256,
                                   c_bit256);  // can be optimized - we can do other NOT for wegihts once and do not do this NOT
...
void gemm_nn_custom_bin_mean_transposed(int M, int N, int K, float ALPHA_UNUSED,
    ...
    //for (i = 0; i < M; ++i)
#pragma omp parallel for
    for (i = 0; i < (M / 2) * 2; i += 2) {   // l.n - filters [16 - 55 - 1024]
    ...
    for (i = (M / 2) * 2; i < M; i += 1) {
        float mean_val = mean_arr[i];
        int j, k;
        for (j = 0; j < N; j += 1) { // out_h*out_w - one channel output size [169 - 173056]
            const int bit_step = 256;
            __m256i count_sum = _mm256_set1_epi8(0);
...
void activate_array_cpu_custom(float *x, const int n, const ACTIVATION a) {
    ...
    if (a == LINEAR) {}
        ...
        if (is_fma_avx2()) {
            __m256i all256_sing1 = _mm256_set_epi32(0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000,
            ...
            for (i = 0; i < n - 8; i += 8) {
                ...
                __m256i sign256 = _mm256_and_si256(_mm256_castps_si256(src256),
                                                   all256_sing1); // check sign in 8 x 32-bit floats

                __m256 result256 = _mm256_blendv_ps(src256, mult256,
                                                    _mm256_castsi256_ps(sign256)); // (sign>0) ? src : mult;
...
void gemm_ongpu(int TA, int TB, int M, int N, int K, float ALPHA,
    ...
    cudaError_t status = (cudaError_t) cublasSgemm(handle, (TB ? CUBLAS_OP_T : CUBLAS_OP_N),
                                                   (TA ? CUBLAS_OP_T : CUBLAS_OP_N), N, M, K, &ALPHA, B_gpu, ldb, A_gpu,
                                                   lda, &BETA, C_gpu, ldc);
    CHECK_CUDA(status);
}
