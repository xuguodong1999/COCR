...
#pragma omp parallel for
        for (i = 0; i < size; ++i) {
            ...
            state.delta[input_index] +=
                    l.delta[i] * from_output[i];// / l.out_c; // l.delta * from  (should be divided by l.out_c?)
...
#pragma omp parallel for
        for (i = 0; i < size; ++i) {
            state.delta[i / channel_size] += l.delta[i] *
                                             from_output[i];// / channel_size; // l.delta * from  (should be divided by channel_size?)
...
void forward_scale_channels_layer_gpu(const layer l, network_state state) {
    ...
    scale_channels_gpu(state.net.layers[l.index].output_gpu, size, channel_size, batch_size, l.scale_wh, state.input,
                       l.output_gpu);
...
void backward_scale_channels_layer_gpu(const layer l, network_state state) {
    ...
    backward_scale_channels_gpu(l.delta_gpu, size, channel_size, batch_size, l.scale_wh, state.input, from_delta,
