...
box
get_gaussian_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride,
...
float delta_gaussian_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w,
                              int h, float *delta,
                              float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss, float uc_normalizer,
                              int accumulate, YOLO_POINT yolo_point, float max_delta) {
...
void
delta_gaussian_yolo_class(float *output, float *delta, int index, int class_id, int classes, int stride, float *avg_cat,
                          float label_smooth_eps, float *classes_multipliers, float cls_normalizer) {
    int n;
    if (delta[index]) {
        float y_true = 1;
...
int compare_gaussian_yolo_class(float *output, int classes, int class_index, int stride, float objectness, int class_id,
                                float conf_thresh) {
    int j;
    for (j = 0; j < classes; ++j) {
...
void forward_gaussian_yolo_layer(const layer l, network_state state) {
    ...
    for (b = 0; b < l.batch; ++b) {
        for (j = 0; j < l.h; ++j) {
            for (i = 0; i < l.w; ++i) {
                for (n = 0; n < l.n; ++n) {
                    ...
                    box pred = get_gaussian_yolo_box(l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h,
                                                     state.net.w, state.net.h, l.w * l.h, l.yolo_point);
                    ...
                    for (t = 0; t < l.max_boxes; ++t) {
                        ...
                        if (class_id >= l.classes) {
                            ...
                            printf(" truth.x = %f, truth.y = %f, truth.w = %f, truth.h = %f, class_id = %d \n", truth.x,
                                   truth.y, truth.w, truth.h, class_id);
                        ...
                        int class_id_match = compare_gaussian_yolo_class(l.output, l.classes, class_index, l.w * l.h,
                                                                         objectness, class_id, 0.25f);
                    ...
                    if (best_match_iou > l.ignore_thresh) {
                        const float iou_multiplier = best_match_iou *
                                                     best_match_iou;// (best_match_iou - l.ignore_thresh) / (1.0 - l.ignore_thresh);
                    ...
                    if (best_iou > l.truth_thresh) {
                        const float iou_multiplier =
                                best_iou * best_iou;// (best_iou - l.truth_thresh) / (1.0 - l.truth_thresh);
                        if (l.objectness_smooth)
                            l.delta[obj_index] = l.obj_normalizer * (iou_multiplier - l.output[obj_index]);
                        ...
                        if (l.objectness_smooth)
                            l.delta[class_index + stride * class_id] = class_multiplier * (iou_multiplier -
                                                                                           l.output[class_index +
                                                                                                    stride * class_id]);
                        box truth = float_to_box_stride(state.truth + best_t * (4 + 1) + b * l.truths, 1);
                        delta_gaussian_yolo_box(truth, l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h,
                                                state.net.w, state.net.h, l.delta, (2 - truth.w * truth.h), l.w * l.h,
                                                l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1,
                                                l.yolo_point, l.max_delta);
                    }
                }
            }
        }
        for (t = 0; t < l.max_boxes; ++t) {
            ...
            if (mask_n >= 0) {
                ...
                const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
                float iou = delta_gaussian_yolo_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h,
                                                    state.net.w, state.net.h, l.delta, (2 - truth.w * truth.h),
                                                    l.w * l.h, l.iou_normalizer * class_multiplier, l.iou_loss,
            ...
            for (n = 0; n < l.total; ++n) {
                ...
                if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f) {
                    ...
                    if (iou > l.iou_thresh) {
                        ...
                        int box_index = entry_gaussian_index(l, b, mask_n * l.w * l.h + j * l.w + i, 0);
                        const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
                        float iou = delta_gaussian_yolo_box(truth, l.output, l.biases, n, box_index, i, j, l.w, l.h,
                                                            state.net.w, state.net.h, l.delta, (2 - truth.w * truth.h),
                                                            l.w * l.h, l.iou_normalizer * class_multiplier, l.iou_loss,
                                                            l.uc_normalizer, 1, l.yolo_point, l.max_delta);
                        ...
                        int class_index = entry_gaussian_index(l, b, mask_n * l.w * l.h + j * l.w + i, 9);
                        delta_gaussian_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w * l.h,
                                                  &avg_cat, l.label_smooth_eps, l.classes_multipliers,
                                                  l.cls_normalizer);
    ...
    *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
    ...
    fprintf(stderr,
            "Region %d Avg IOU: %f, Class: %f, Obj: %f, No Obj: %f, .5R: %f, .75R: %f,  count: %d, class_loss = %.2f, iou_loss = %.2f, uc_loss = %.2f, total_loss = %.2f \n",
            state.index, avg_iou / count, avg_cat / class_count, avg_obj / count,
...
int get_gaussian_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative,
                                 detection *dets, int letter) {
    int i, j, n;
    ...
    for (i = 0; i < l.w * l.h; ++i) {
        ...
        for (n = 0; n < l.n; ++n) {
            ...
            if (objectness > thresh) {
                ...
                dets[count].bbox = get_gaussian_yolo_box(predictions, l.biases, l.mask[n], box_index, col, row, l.w,
                                                         l.h, netw, neth, l.w * l.h, l.yolo_point);
                ...
                for (j = 0; j < l.classes; ++j) {
                    ...
                    float uc_aver =
                            (dets[count].uc[0] + dets[count].uc[1] + dets[count].uc[2] + dets[count].uc[3]) / 4.0;
