...
void forward_softmax_layer(const softmax_layer l, network_state net) {
    if (l.softmax_tree) {
        ...
        softmax_cpu(net.input, l.inputs / l.groups, l.batch, l.inputs, l.groups, l.inputs / l.groups, 1, l.temperature,
                    l.output);
    }
...
void forward_softmax_layer_gpu(const softmax_layer l, network_state net) {
    if (l.softmax_tree) {
        ...
        */
        ...
        if (l.spatial) {
            softmax_gpu_new_api(net.input, l.c, l.batch * l.c, l.inputs / l.c, l.w * l.h, 1, l.w * l.h, 1,
                                l.output_gpu);
        } else {
            softmax_gpu_new_api(net.input, l.inputs / l.groups, l.batch, l.inputs, l.groups, l.inputs / l.groups, 1,
                                l.temperature, l.output_gpu);
        }
    }
    if (net.truth && !l.noloss) {
...
contrastive_layer make_contrastive_layer(int batch, int w, int h, int c, int classes, int inputs, layer *yolo_layer) {
    ...
    fprintf(stderr, "contrastive %4d x%4d x%4d x emb_size %4d x batch: %4d  classes = %4d, step = %4d \n", w, h, l.n,
            l.embedding_size, batch, l.classes, step);
...
void forward_contrastive_layer(contrastive_layer l, network_state state) {
                        ...
                        for (n2 = 0; n2 < l.n; ++n2) {
                            for (h2 = 0; h2 < l.h; ++h2) {
                                for (w2 = 0; w2 < l.w; ++w2) {
                                    ...
                                    // calc good sim
                                    if (l.labels[z_index] == l.labels[z_index2] &&
                                    ...
                                    if ((contrast_p_index + 1) >= contrast_p_size) {
                                        ...
                                        contrast_p = (contrastive_params *) xrealloc(contrast_p, contrast_p_size *
                                                                                                 sizeof(contrastive_params));
                                    }
    ...
    */
    ...
    if (l.detection) {
        ...
        int *labels = NULL;
        if (contr_size > 2) {
            cuda_push_array((float *) l.contrast_p_gpu, (float *) contrast_p,
            ...
            cuda_pull_array((float *) l.contrast_p_gpu, (float *) contrast_p,
                            contr_size * sizeof(contrastive_params) / 4);
        }
#else   // GPU
        int k;
                            ...
                            for (n2 = 0; n2 < l.n; ++n2) {
                                for (h2 = 0; h2 < l.h; ++h2) {
                                    for (w2 = 0; w2 < l.w; ++w2) {
                                        ...
                                        float P = -10;
                                        if (l.detection) {
                                            P = P_constrastive_f(z_index, z_index2, l.labels, z, l.embedding_size,
                                                                 l.temperature, contrast_p, contr_size);
                                        } else {
                                            P = P_constrastive(z_index, z_index2, l.labels, step, z, l.embedding_size,
                                                               l.temperature, l.cos_sim, l.exp_cos_sim);
            ...
            for (h = 0; h < l.h; ++h) {
                for (w = 0; w < l.w; ++w) {
                    ...
                    const int delta_index =
                            b * l.embedding_size * l.n * l.h * l.w + n * l.embedding_size * l.h * l.w + h * l.w + w;
                    ...
                    if (l.detection) {
                        ...
                        // positive
                        grad_contrastive_loss_positive_f(z_index, l.class_ids, l.labels, step, z, l.embedding_size,
                                                         l.temperature, l.delta + delta_index, wh, contrast_p,
                                                         contr_size);

                        // negative
                        grad_contrastive_loss_negative_f(z_index, l.class_ids, l.labels, step, z, l.embedding_size,
                                                         l.temperature, l.delta + delta_index, wh, contrast_p,
                                                         contr_size, l.contrastive_neg_max);
                    } else {
                        // classifier

                        // positive
                        grad_contrastive_loss_positive(z_index, l.labels, step, z, l.embedding_size, l.temperature,
                                                       l.cos_sim, l.p_constrastive, l.delta + delta_index, wh);

                        // negative
                        grad_contrastive_loss_negative(z_index, l.labels, step, z, l.embedding_size, l.temperature,
                                                       l.cos_sim, l.p_constrastive, l.delta + delta_index, wh);
                    }
