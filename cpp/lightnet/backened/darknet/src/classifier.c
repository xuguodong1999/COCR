...
float
validate_classifier_single(char *datacfg, char *filename, char *weightfile, network *existing_net, int topk_custom);
...
train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dontuse_opencv,
                 int dont_show, int mjpeg_port, int calc_topk, int show_imgs, char *chart_path) {
    int i;
    ...
    while (get_current_batch(net) < net.max_batches || net.max_batches == 0) {
        ...
        int calc_topk_for_each =
                iter_topk + 2 * train_images_num / (net.batch * net.subdivisions);  // calculate TOPk for each 2 Epochs
        ...
        printf("%d, %.3f: %f, %f avg, %f rate, %lf seconds, %ld images, %f hours left\n", get_current_batch(net),
               (float) (*net.seen) / train_images_num, loss, avg_loss, get_current_rate(net), sec(clock() - time),
               *net.seen, avg_time);
#ifdef OPENCV
        if (net.contrastive) {
            float cur_con_acc = -1;
        ...
        if (!dontuse_opencv)
            draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, i, net.max_batches, topk,
                            draw_precision, topk_buff, avg_contrastive_acc / 100, dont_show, mjpeg_port, avg_time);
#endif  // OPENCV
...
void run_classifier(int argc, char **argv) {
    ...
    if (0 == strcmp(argv[2], "predict")) predict_classifier(data, cfg, weights, filename, top);
        ...
        train_classifier(data, cfg, weights, gpus, ngpus, clear, dontuse_opencv, dont_show, mjpeg_port, calc_topk,
                         show_imgs, chart_path);
    else if (0 == strcmp(argv[2], "demo"))
        demo_classifier(data, cfg, weights, cam_index, filename, benchmark, benchmark_layers);
