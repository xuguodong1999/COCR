...
float *cuda_make_array_pinned_preallocated(float *x, size_t n) {
    ...
    if (!x_cpu) {
        if (allocation_size > pinned_block_size / 2) {
            ...
            if (status != cudaSuccess)
                fprintf(stderr, " Can't allocate CUDA-pinned memory on CPU-RAM (pre-allocated memory is over too) \n");
            CHECK_CUDA(status);
        } else {
            ...
            cudaError_t status = cudaHostAlloc((void **) &pinned_ptr[pinned_block_id], pinned_block_size,
