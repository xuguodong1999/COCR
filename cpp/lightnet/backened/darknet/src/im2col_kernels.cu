...
__device__ void transpose8rS32_reversed_diagonale(unsigned char *A, unsigned char *B, int m, int n) {
    ...
    B[7 * n] = reverse_byte_CUDA(x >> 24);
    B[6 * n] = reverse_byte_CUDA(x >> 16);
    B[5 * n] = reverse_byte_CUDA(x >> 8);
    B[4 * n] = reverse_byte_CUDA(x);
    B[3 * n] = reverse_byte_CUDA(y >> 24);
    B[2 * n] = reverse_byte_CUDA(y >> 16);
    B[1 * n] = reverse_byte_CUDA(y >> 8);
    B[0 * n] = reverse_byte_CUDA(y);
...
void transpose_bin_gpu(unsigned char *A, unsigned char *B, const int n, const int m,
    ...
    num_blocks32, BLOCK_TRANSPOSE32, 0, get_cuda_stream() >> >((uint32_t *) A, (uint32_t *) B, n, m, lda, ldb, block_size);
...
__global__ void
transpose_uint32_kernel(uint32_t *src, uint32_t *dst, int src_h, int src_w, int src_align, int dst_align) {
...
void transpose_uint32_gpu(uint32_t *src, uint32_t *dst, int src_h, int src_w, int src_align, int dst_align) {
    ...
    num_blocks, BLOCK, 0, get_cuda_stream() >> >(src, dst, src_h, src_w, src_align, dst_align);
    CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
...
__global__ void
transpose_uint32_kernel_2(uint32_t *src, uint32_t *dst, int src_h, int src_w, int src_align, int dst_align) {
...
void transpose_uint32_gpu_2(uint32_t *src, uint32_t *dst, int src_h, int src_w, int src_align, int dst_align) {
    ...
    num_blocks, TRANS_BLOCK, 0, get_cuda_stream() >> >(src, dst, src_h, src_w, src_align, dst_align);
    CHECK_CUDA(cudaPeekAtLastError());
}
// --------------------------------
    ...
    {    // out_h*out_w - one channel output size [169 - 173056]
        ...
        if (i < M)  //if (i < M)  // l.n - filters [16 - 55 - 1024]
        {
            ...
            for (; k < K; k += 128)  // l.size*l.size*l.c - one filter size [27 - 144 - 9216]
            {
                ...
                int i_d = lane_id / 4;
                int j_d = lane_id / 4;
                uint32_t a_val = *(uint32_t *) (A + ((i + i_d) * lda + (k + k_d * 32)) /
                                                    8); // wmma::load_matrix_sync(a_frag, (uint32_t *)(A + A_cur_index), lda);

                for (int c_x = 0; c_x < 2; c_x++) {
                    uint32_t b_val = *(uint32_t *) (B + ((c_x * 8 + j + j_d) * ldb + (k + k_d * 32)) /
                                                        8); // wmma::load_matrix_sync(b_frag, (uint32_t *)(B + B_cur_index), ldb);
...
#pragma UNROLL
            ...
            if (k_d == 0) {
                for (int c_x = 0; c_x < 2; c_x++) {
                    for (int local_j = 0; local_j < 8; ++local_j) {
                        C_s[warp_id * WMMA_M * WMMA_N + i_d * WMMA_N + local_j +
                            WMMA_M * WMMA_N * 32 * c_x] = accum_c_val[local_j + c_x * 8];
                    }
                }
            }
...
// A (weights) in the shared_memory - GOOD
        __global__ void gemm_nn_custom_bin_mean_transposed_gpu_kernel(int M, int N, int K,
                                                                      ...
                                                                      unsigned char *B, int ldb,
                                                                      float *C, int ldc, float *mean_arr,
                                                                      float *bias_arr, int leaky_activation,
                                                                      float *shortcut_in_gpu, float *shortcut_out_gpu) {
            // total 57%
            int index = blockIdx.x * blockDim.x + threadIdx.x;
...
// --------------------------------
        ...
        __global__ void
        convolve_gpu_kernel(float *input, float *weights, float *output, int in_w, int in_h, int in_c, int n, int size,
                            int pad) {
            int index = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        void convolve_gpu(float *input, float *weights, float *output, int in_w, int in_h, int in_c, int n, int size,
                          int pad) {
            ...
            convolve_gpu_kernel << <
            num_blocks, BLOCK, 0, get_cuda_stream() >> > (input, weights, output, in_w, in_h, in_c, n, size, pad);
            CHECK_CUDA(cudaPeekAtLastError());
        }
...
*/
        ...
        __global__ void
        convolve_bin_gpu_kernel(float *input, float *weights, float *output, int in_w, int in_h, int in_c, int n,
                                int size, int pad, int new_lda, float *mean_arr_gpu) {
            int index = blockIdx.x * blockDim.x + threadIdx.x;
            ...
            //if (fil < n)    // (1-6 for one BLOCK)
            {
                ...
                __shared__ uint32_t weights_shared[
                        3 * 3 * 1024 * 6 / 32 + 1];  // 7 KB (6 filters) - use (new_lda) for size calculation
                ...
                for (int tmp_fil = min_fil; tmp_fil <= max_fil; tmp_fil++) {
                    for (int s = threadIdx.x; s < weights_size; s += blockDim.x) {
                        ...
                        weights_shared[s + (tmp_fil - min_fil) * new_lda / 32] = ((uint32_t *) weights)[
                                tmp_fil * new_lda / 32 + s];
                    }
                }
                __syncthreads();
                    ...
                    if (fil < n)    // (1-6 for one BLOCK)
                    {
                        ...
                        for (f_y = 0; f_y < size; ++f_y) {
                            ...
                            for (f_x = 0; f_x < size; ++f_x) {
                                ...
                                //int weights_index = fil*in_c*size*size + chan*size*size + f_y*size + f_x;
                                int weights_shared_index =
                                        (fil - min_fil) * new_lda + chan * size * size + f_y * size + f_x;
                                ...
                                //int input_index = input_pre_index + input_y*in_w + input_x;
                                int input_shared_index = /*input_pre_index +*/
                                        input_y * in_w + input_x + add_input_index;
        ...
        void convolve_bin_gpu(float *input, float *weights, float *output, int in_w, int in_h, int in_c, int n,
            ...
            num_blocks, BLOCK, 0, get_cuda_stream() >> > (input, weights, output, in_w, in_h, in_c, n, size, pad, new_lda, mean_arr_gpu);
            CHECK_CUDA(cudaPeekAtLastError());
        }
