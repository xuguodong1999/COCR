...
//            std::copy(g.dat.begin(), g.dat.end(), dat);
        ...
        virtual void dfsWrapper(traverse, traverse, traverse) = 0;

        virtual void bfsWrapper(traverse, traverse, traverse) = 0;

        virtual std::string toString() = 0;

        friend inline std::istream &operator>>(std::istream &in, Graph<T> &g) {
            T x, y;
            while (in >> (long long &) x >> (long long &) y) {
                g.add(x, y);
            }
            return in;
        }
    ...
    public:
         ...
         */
        void dfsWrapper(traverse before, traverse in, traverse after) override {
            initNodeState();
         ...
         */
        void bfsWrapper(traverse enterQueue, traverse in, traverse after) override {
            initNodeState();
