...
void valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device,
           Loss &criterion_adv, Loss &criterion_con, Loss &criterion_enc, torch::nn::BCEWithLogitsLoss &criterion_dis,
           Encoder &enc1, Encoder &enc2, Decoder &dec, GAN_Discriminator &dis, const size_t epoch,
           visualizer::graph &writer_rec, visualizer::graph &writer_gan, visualizer::graph &writer_dis);
...
// -------------------
void
train(po::variables_map &vm, torch::Device &device, Encoder &enc1, Encoder &enc2, Decoder &dec, GAN_Discriminator &dis,
    ...
    auto enc1_optimizer = torch::optim::Adam(enc1->parameters(),
                                             torch::optim::AdamOptions(vm["lr_enc"].as<float>()).betas(
                                                     {vm["beta1"].as<float>(), vm["beta2"].as<float>()}));
    auto enc2_optimizer = torch::optim::Adam(enc2->parameters(),
                                             torch::optim::AdamOptions(vm["lr_enc"].as<float>()).betas(
                                                     {vm["beta1"].as<float>(), vm["beta2"].as<float>()}));
    ...
    if (vm["train_load_epoch"].as<std::string>() == "") {
        ...
        path = checkpoint_dir + "/models/epoch_" + vm["train_load_epoch"].as<std::string>() + "_enc1.pth";
        torch::load(enc1, path, device);
        path = checkpoint_dir + "/models/epoch_" + vm["train_load_epoch"].as<std::string>() + "_enc2.pth";
        torch::load(enc2, path, device);
        ...
        path = checkpoint_dir + "/optims/epoch_" + vm["train_load_epoch"].as<std::string>() + "_enc1.pth";
        torch::load(enc1_optimizer, path, device);
        path = checkpoint_dir + "/optims/epoch_" + vm["train_load_epoch"].as<std::string>() + "_enc2.pth";
        torch::load(enc2_optimizer, path, device);
    ...
    for (epoch = start_epoch; epoch <= total_epoch; epoch++) {
        ...
        while (dataloader(mini_batch)) {
            ...
            show_progress->increment(/*loss_value=*/
                    {adv_loss.item<float>(), con_loss.item<float>(), enc_loss.item<float>(),
        ...
        if (vm["valid"].as<bool>() && ((epoch - 1) % vm["valid_freq"].as<size_t>() == 0)) {
            valid(vm, valid_dataloader, device, criterion_adv, criterion_con, criterion_enc, criterion_dis, enc1, enc2,
                  dec, dis, epoch, valid_loss_rec, valid_loss_gan, valid_loss_dis);
        }
        ...
        if (epoch % vm["save_epoch"].as<size_t>() == 0) {
            path = checkpoint_dir + "/models/epoch_" + std::to_string(epoch) + "_enc1.pth";
            torch::save(enc1, path);
            path = checkpoint_dir + "/models/epoch_" + std::to_string(epoch) + "_enc2.pth";
            torch::save(enc2, path);
            ...
            torch::save(dis, path);
            path = checkpoint_dir + "/optims/epoch_" + std::to_string(epoch) + "_enc1.pth";
            torch::save(enc1_optimizer, path);
            path = checkpoint_dir + "/optims/epoch_" + std::to_string(epoch) + "_enc2.pth";
            torch::save(enc2_optimizer, path);
        ...
        path = checkpoint_dir + "/models/epoch_latest_enc1.pth";
        torch::save(enc1, path);
        path = checkpoint_dir + "/models/epoch_latest_enc2.pth";
        torch::save(enc2, path);
        path = checkpoint_dir + "/models/epoch_latest_dec.pth";
        torch::save(dec, path);
        path = checkpoint_dir + "/models/epoch_latest_dis.pth";
        torch::save(dis, path);
        path = checkpoint_dir + "/optims/epoch_latest_enc1.pth";
        torch::save(enc1_optimizer, path);
        path = checkpoint_dir + "/optims/epoch_latest_enc2.pth";
        torch::save(enc2_optimizer, path);
