...
valid(po::variables_map &vm, DataLoader::ImageFolderWithPaths &valid_dataloader, torch::Device &device, Loss &criterion,
    ...
    torch::Tensor enc_loss, gen_loss, dis_real_loss, dis_fake_loss;
    ...
    total_enc_loss = 0.0;
    total_gen_loss = 0.0;
    total_dis_real_loss = 0.0;
    total_dis_fake_loss = 0.0;
    ...
    writer.plot(/*base=*/epoch, /*value=*/{ave_enc_loss, ave_gen_loss, ave_dis_real_loss + ave_dis_fake_loss});
    writer_dis.plot(/*base=*/epoch, /*value=*/
                             {ave_dis_real_loss + ave_dis_fake_loss, ave_dis_real_loss, ave_dis_fake_loss});

    // (4) Image Generation
    max_counter = (int) (vm["valid_sigma_max"].as<float>() / vm["valid_sigma_inter"].as<float>() * 2) + 1;
    z_input = torch::full({1, (long int) vm["nz"].as<size_t>()}, /*value=*/-vm["valid_sigma_max"].as<float>(),
                          torch::TensorOptions().dtype(torch::kFloat)).to(device);
    outputs = gen->forward(z_input);
    for (size_t i = 1; i < max_counter; i++) {
        value = -vm["valid_sigma_max"].as<float>() + (float) i * vm["valid_sigma_inter"].as<float>();
        z_input = torch::full({1, (long int) vm["nz"].as<size_t>()}, /*value=*/value,
                              torch::TensorOptions().dtype(torch::kFloat)).to(device);
        output = gen->forward(z_input);
