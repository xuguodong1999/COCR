        ...
        namespace CaseNLL {
            ...
            public:
                ...
                Case1EvalUnit(TOperHandle1 grad,
                        ...
                        : m_grad(std::move(grad)), m_hotPos(hotPos), m_handlePre(std::move(handlePre)),
            ...
            public:
                ...
                Case2EvalUnit(TOperHandle1 grad,
                              ...
                              EvalHandle <Matrix<ElementType, DeviceType>> evalOutput)
                        : m_grad(grad), m_handleTar(std::move(handleTar)), m_handlePre(std::move(handlePre)),
                          m_evalOutput(std::move(evalOutput)) {}
            ...
            struct Calculator {
                ...
                public:
                    Helper(TEvalRes &evalRes, const TOperator1 &oper1,
                           TOutHandle outHandle, TSoftmaxResHandle softmaxRes)
                            : m_evalRes(evalRes), m_oper1(oper1), m_outHandle(std::move(outHandle)),
                              m_dataPtr(m_outHandle.DataPtr()), m_softmaxRes(std::move(softmaxRes)) {}

                    void Eval() {
                        if constexpr ((IsDynamic<decltype(m_oper1.Operand2())>) &&
                                      (IsMatrix<typename TEvalRes::DataType>)) {
                            if (auto ptr = m_oper1.Operand2().template TypeCast<OneHotVector<ElementType, DeviceType>>()) {
                                ...
                                EvalUnit unit(std::move(handle1), ptr->HotPos(), std::move(m_softmaxRes),
                                              std::move(m_outHandle));
                ...
                private:
                    void GenCase() {
                        ...
                        EvalUnit unit(std::move(handle1), std::move(handle2), std::move(m_softmaxRes),
                                      std::move(m_outHandle));
        ...
        namespace CaseGen {
            ...
            public:
                ...
                EvalUnit(TOperHandle1 oper1,
                         TOperHandle2 oper2,
                         EvalHandle <Matrix<ElementType, DeviceType>> evalOutput)
                        : m_oper1(std::move(oper1)), m_oper2(std::move(oper2)), m_evalOutput(evalOutput) {}
            ...
            public:
                ...
                EvalUnit(TOperHandle1 oper1,
                         TOperHandle2 oper2,
                         EvalHandle <Batch<ElementType, DeviceType, CategoryTags::Matrix>> evalOutput)
                        : m_oper1(std::move(oper1)), m_oper2(std::move(oper2)), m_evalOutput(std::move(evalOutput)) {}
