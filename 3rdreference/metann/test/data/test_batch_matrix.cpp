...
namespace {
    void test_batch_matrix1() {
        ...
        REQUIRE(data.AvailableForWrite());
        REQUIRE(data.BatchNum() == 10);
        REQUIRE(data.RowNum() == 13);
        REQUIRE(data.ColNum() == 35);
        ...
        for (size_t i = 0; i < 10; ++i) {
            for (size_t j = 0; j < 13; ++j) {
                for (size_t k = 0; k < 35; ++k) {
                    REQUIRE(data[i](j, k) == (int) (i * 1000 + j * 100 + k));
                }
            }
        }
        ...
        REQUIRE(!data.AvailableForWrite());
        REQUIRE(!data2.AvailableForWrite());
        REQUIRE(data2.BatchNum() == 10);
        REQUIRE(data2.RowNum() == 4);
        REQUIRE(data2.ColNum() == 11);

        for (size_t i = 0; i < 10; ++i) {
            for (size_t j = 3; j < 7; ++j) {
                for (size_t k = 11; k < 22; ++k) {
                    REQUIRE(data2[i](j - 3, k - 11) == (int) (i * 1000 + j * 100 + k));
                }
            }
        }
        cout << "done" << endl;
    ...
    void test_batch_matrix2() {
        ...
        REQUIRE(rm1.RowNum() == 4);
        REQUIRE(rm1.ColNum() == 5);
        REQUIRE(rm1.BatchNum() == 3);
