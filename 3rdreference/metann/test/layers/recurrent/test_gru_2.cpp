...
namespace {
    ...
    void comp(const Batch<float, DeviceTags::CPU, CategoryTags::Matrix> &v1,
        ...
        REQUIRE(v1.RowNum() == v2.RowNum());
        REQUIRE(v1.ColNum() == v2.ColNum());
        REQUIRE(v1.BatchNum() == v2.BatchNum());
    ...
    void test_gru_with_bptt_2() {
        std::cout << "test gru case 2 ..." << std::endl;
        using GRUKernel = InjectPolicy<RecurrentLayer, PUpdate, PFeedbackOutput, PBatchMode>;
        ...
        vector<Batch<float, DeviceTags::CPU, CategoryTags::Matrix>> input;
        for (size_t i = 0; i < 3; ++i)
            input.emplace_back(2, 1, 4);
        id = 0;
        for (size_t i = 0; i < 2; ++i) {
            for (size_t j = 0; j < 4; ++j) {
                input[0].SetValue(i, 0, j, x_np_0[id]);
                input[1].SetValue(i, 0, j, x_np_1[id]);
                input[2].SetValue(i, 0, j, x_np_2[id]);
                ++id;
            }
        }

        vector<Batch<float, DeviceTags::CPU, CategoryTags::Matrix>> outputCheck;
        for (size_t i = 0; i < 3; ++i)
            outputCheck.emplace_back(2, 1, 6);
        id = 0;
        for (size_t i = 0; i < 2; ++i) {
            for (size_t j = 0; j < 6; ++j) {
                outputCheck[0].SetValue(i, 0, j, fres_0[id]);
                outputCheck[1].SetValue(i, 0, j, fres_1[id]);
                outputCheck[2].SetValue(i, 0, j, fres_2[id]);
                ++id;
            }
        }

        vector<Batch<float, DeviceTags::CPU, CategoryTags::Matrix>> output;

        auto gru0 = GRUKernel::InputType::Create()
                .Set<LayerIO>(input[0])
                .Set<RnnLayerHiddenBefore>(MakeDuplicate(2, MakeTrivalMatrix<float, DeviceTags::CPU>(1, 6, 1)));
        output.push_back(Evaluate(gru.FeedForward(gru0).Get<LayerIO>()));
        cout << "\tCompare for forward 0: ", comp(Evaluate(output[0]), outputCheck[0]);

        for (size_t i = 1; i < 3; ++i) {
            auto gru1 = GRUKernel::InputType::Create()
                    .Set<LayerIO>(input[i]);
            output.push_back(Evaluate(gru.FeedForward(gru1).Get<LayerIO>()));
            cout << "\tCompare for forward " << i << ": ", comp(Evaluate(output.back()), outputCheck[i]);
        }

        for (int i = 2; i >= 0; --i) {
            auto grad_gru = gru.FeedBackward(LayerIO::Create().Set<LayerIO>(output[i] * Scalar<int>(-1)));
        }

        GradCollector<float, DeviceTags::CPU> grad_collector;
        gru.GradCollect(grad_collector);
        REQUIRE(grad_collector.size() == 6);
        ...
        std::cout << "test gru case 2 done" << std::endl;
    }
}

void test_gru_2() {
    test_gru_with_bptt_2();
}
