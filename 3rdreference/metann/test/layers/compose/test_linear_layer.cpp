...
namespace {
    void test_linear_layer1() {
        ...
        REQUIRE(fabs(out(0, 0) - 0.75f) < 0.00001);
        REQUIRE(fabs(out(0, 1) - 0.91f) < 0.00001);
        REQUIRE(fabs(out(0, 2) - 1.07f) < 0.00001);
    ...
    void test_linear_layer2() {
        ...
        for (auto &p: grad_collector) {
            ...
            if (w.RowNum() == w1.RowNum()) {
                ...
                REQUIRE(tmp.RowNum() == info.RowNum());
                REQUIRE(tmp.ColNum() == info.ColNum());

                for (size_t i = 0; i < tmp.RowNum(); ++i) {
                    for (size_t j = 0; j < tmp.ColNum(); ++j) {
                        REQUIRE(fabs(info(i, j) - tmp(i, j)) < 0.0001f);
                    }
                }
                ...
                for (size_t i = 0; i < info.RowNum(); ++i) {
                    for (size_t j = 0; j < info.ColNum(); ++j) {
                        REQUIRE(fabs(info(i, j) - g(i, j)) < 0.0001f);
                    }
                }
            } else {
                REQUIRE(false);
    ...
    void test_linear_layer4() {
        ...
        auto w = (*grad_collector.begin()).weight;
        REQUIRE(w == params.rbegin()->second);
