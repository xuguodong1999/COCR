...
namespace {
    void test_abs_layer1() {
        ...
        REQUIRE(res.RowNum() == 4);
        REQUIRE(res.ColNum() == 5);

        for (size_t i = 0; i < 4; ++i) {
            for (size_t j = 0; j < 5; ++j) {
                auto check = fabs(in(i, j));
                REQUIRE(fabs(res(i, j) - check) < 0.0001);
            }
        }
    ...
    void test_abs_layer2() {
        ...
        for (size_t i = 0; i < 4; ++i) {
            for (size_t j = 0; j < 5; ++j) {
                ...
                REQUIRE(fabs(fb(i, j) - check) < 0.0001);
            }
        }
        ...
        for (size_t loop_count = 9; loop_count >= 1; --loop_count) {
            ...
            for (size_t i = 0; i < loop_count; ++i) {
                for (size_t j = 0; j < 3; ++j) {
                    ...
                    REQUIRE(fabs(fb(i, j) - aim) < 0.00001f);
                }
            }
        }
    ...
    void test_abs_layer4() {
        ...
        REQUIRE(fabs(x_out_eval(0, 0) - 0) <= 0.00001);
        REQUIRE(fabs(x_out_eval(0, 1) - 2) <= 0.00001);
        REQUIRE(fabs(x_out_eval(0, 2) - 3) <= 0.00001);
        REQUIRE(fabs(x_out_eval(0, 3) - 4) <= 0.00001);
        ...
        REQUIRE(fabs(y_out_eval(0, 0) - 0) <= 0.00001);
        REQUIRE(fabs(y_out_eval(0, 1) + 5) <= 0.00001);
        REQUIRE(fabs(y_out_eval(0, 2) - 7) <= 0.00001);
        REQUIRE(fabs(y_out_eval(0, 3) + 3) <= 0.00001);
