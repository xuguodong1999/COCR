...
namespace {
    void test_tanh_layer1() {
        ...
        REQUIRE(fabs(res(0, 0) - tanh(-0.27f)) < 0.001);
        REQUIRE(fabs(res(1, 0) - tanh(-0.41f)) < 0.001);
    ...
    void test_tanh_layer2() {
        ...
        REQUIRE(fabs(fb(0, 0) - 0.1f * (1 - tanh(-0.27f) * tanh(-0.27f))) < 0.001);
        REQUIRE(fabs(fb(1, 0) - 0.3f * (1 - tanh(-0.41f) * tanh(-0.41f))) < 0.001);
        ...
        for (size_t loop_count = 1; loop_count < 10; ++loop_count) {
            ...
            for (size_t i = 0; i < loop_count; ++i) {
                for (size_t j = 0; j < 3; ++j) {
                    REQUIRE(fabs(res(i, j) - tanh(in(i, j))) < 0.0001);
                }
            }
        }
