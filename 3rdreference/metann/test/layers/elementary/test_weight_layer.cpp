...
namespace {
    void test_weight_layer1() {
        ...
        REQUIRE(fabs(res(0, 0) + 0.27f) < 0.001);
        REQUIRE(fabs(res(0, 1) + 0.41f) < 0.001);
        ...
        REQUIRE(params.find("root") != params.end());
    ...
    void test_weight_layer2() {
        ...
        auto out = layer.FeedForward(wi);
        auto res = Evaluate(out.Get<LayerIO>());
        REQUIRE(fabs(res(0, 0) + 0.027f) < 0.001);
        REQUIRE(fabs(res(0, 1) + 0.041f) < 0.001);
        ...
        REQUIRE(grad_collector.size() == 1);

        auto w1 = (*grad_collector.begin()).weight;
        auto info_g = Evaluate(Collapse((*grad_collector.begin()).grad));

        REQUIRE(fabs(w1(0, 0) + 0.27f) < 0.001);
        REQUIRE(fabs(w1(0, 1) + 0.41f) < 0.001);

        REQUIRE(fabs(info_g(0, 0) + 0.00495f) < 0.001);
        REQUIRE(fabs(info_g(0, 1) + 0.00997f) < 0.001);
    ...
    void test_weight_layer3() {
        ...
        auto out = layer.FeedForward(wi);
        auto res = Evaluate(out.Get<LayerIO>());
        REQUIRE(fabs(res(0, 0) - 1.0996f) < 0.001);
        REQUIRE(fabs(res(0, 1) - 3.1047f) < 0.001);
        ...
        REQUIRE(fabs(fbOut(0, 0) + 0.07055) < 0.001);
        REQUIRE(fabs(fbOut(0, 1) + 0.041408f) < 0.001);
        ...
        REQUIRE(fabs(w1(0, 0) - 1.1) < 0.001);
        REQUIRE(fabs(w1(1, 0) - 0.1) < 0.001);
        REQUIRE(fabs(w1(0, 1) - 3.1) < 0.001);
        REQUIRE(fabs(w1(1, 1) - 1.17) < 0.001);

        REQUIRE(fabs(info_g(0, 0) - 0.0468531) < 0.001);
        REQUIRE(fabs(info_g(1, 0) - 0.00031423) < 0.001);
        REQUIRE(fabs(info_g(0, 1) + 0.0393606) < 0.001);
        REQUIRE(fabs(info_g(1, 1) + 0.00026398) < 0.001);
    ...
    void test_weight_layer4() {
        ...
        for (int loop_count = 0; loop_count < 10; ++loop_count) {
            ...
            REQUIRE(res.RowNum() == 1);
            REQUIRE(res.ColNum() == 4);
            auto c = handle2.Data();

            for (size_t i = 0; i < 4; ++i) {
                REQUIRE(fabs(res(0, i) - c(0, i)) <= 0.0001f);
            }
        }
        ...
        for (int loop_count = 9; loop_count >= 0; --loop_count) {
            ...
            REQUIRE(fbOut.RowNum() == 1);
            REQUIRE(fbOut.ColNum() == 8);

            for (size_t i = 0; i < 8; ++i) {
                REQUIRE(fabs(fbOut(0, i) - aimFbout(0, i)) < 0.0001);
            }
        }
        ...
        for (size_t i = 0; i < 8; ++i) {
            for (size_t j = 0; j < 4; ++j) {
                REQUIRE(fabs(aim(i, j) - info_g(i, j)) < 0.0001f);
            }
        }
