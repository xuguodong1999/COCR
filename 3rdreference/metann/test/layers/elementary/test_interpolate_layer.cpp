...
namespace {
    void test_interpolate_layer1() {
        ...
        delta.SetValue(0, 0, 0.3f);
        delta.SetValue(0, 1, 0.6f);
        delta.SetValue(0, 2, 0.9f);
        delta.SetValue(1, 0, 0.4f);
        delta.SetValue(1, 1, 0.1f);
        delta.SetValue(1, 2, 0.7f);
        ...
        REQUIRE(fabs(res(0, 0) - 0.17f) < 0.001);
        REQUIRE(fabs(res(0, 1) - 0.24f) < 0.001);
        REQUIRE(fabs(res(0, 2) - 0.31f) < 0.001);
        REQUIRE(fabs(res(1, 0) - 0.46f) < 0.001);
        REQUIRE(fabs(res(1, 1) - 0.59f) < 0.001);
        REQUIRE(fabs(res(1, 2) - 0.63f) < 0.001);
    ...
    void test_interpolate_layer2() {
        ...
        grad.SetValue(0, 0, 0.2f);
        grad.SetValue(0, 1, 0.5f);
        grad.SetValue(0, 2, 0.8f);
        grad.SetValue(1, 0, 0.7f);
        grad.SetValue(1, 1, 0.6f);
        grad.SetValue(1, 2, 0.3f);

        auto out_grad = layer.FeedBackward(LayerIO::Create().Set<LayerIO>(grad));

        auto r = Evaluate(out_grad.Get<InterpolateLayerWeight1>());
        REQUIRE(fabs(r(0, 0) - 0.06f) < 0.001);
        REQUIRE(fabs(r(0, 1) - 0.30f) < 0.001);
        REQUIRE(fabs(r(0, 2) - 0.72f) < 0.001);
        REQUIRE(fabs(r(1, 0) - 0.28f) < 0.001);
        REQUIRE(fabs(r(1, 1) - 0.06f) < 0.001);
        REQUIRE(fabs(r(1, 2) - 0.21f) < 0.001);

        r = Evaluate(out_grad.Get<InterpolateLayerWeight2>());
        REQUIRE(fabs(r(0, 0) - 0.14f) < 0.001);
        REQUIRE(fabs(r(0, 1) - 0.20f) < 0.001);
        REQUIRE(fabs(r(0, 2) - 0.08f) < 0.001);
        REQUIRE(fabs(r(1, 0) - 0.42f) < 0.001);
        REQUIRE(fabs(r(1, 1) - 0.54f) < 0.001);
        REQUIRE(fabs(r(1, 2) - 0.09f) < 0.001);

        r = Evaluate(out_grad.Get<InterpolateLayerLambda>());
        REQUIRE(fabs(r(0, 0) + 0.02f) < 0.001);
        REQUIRE(fabs(r(0, 1) + 0.05f) < 0.001);
        REQUIRE(fabs(r(0, 2) + 0.08f) < 0.001);
        REQUIRE(fabs(r(1, 0) + 0.07f) < 0.001);
        REQUIRE(fabs(r(1, 1) + 0.06f) < 0.001);
        REQUIRE(fabs(r(1, 2) + 0.03f) < 0.001);
    ...
    void test_interpolate_layer3() {
        ...
        for (size_t loop_count = 9; loop_count >= 1; --loop_count) {
            ...
            auto i2 = op2.back();
            op2.pop_back();
            auto delta = opdelta.back();
            opdelta.pop_back();
            for (size_t i = 0; i < loop_count; ++i) {
                for (size_t j = 0; j < 3; ++j) {
                    REQUIRE(fabs(r1(i, j) - grad(i, j) * delta(i, j)) < 0.00001f);
                    REQUIRE(fabs(r2(i, j) - grad(i, j) * (1 - delta(i, j))) < 0.00001f);
                    REQUIRE(fabs(rlambda(i, j) - grad(i, j) * (i1(i, j) - i2(i, j))) < 0.00001f);
                }
            }
        }
