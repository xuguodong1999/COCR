...
namespace {
    ...
    void test_change_policy1() {
        ...
        static_assert(
                is_same<check1, PolicyContainer<PBatchMode, SubPolicyContainer<Tag1, PNoBatchMode>, PEnableBptt>>::value,
                "Check Error");

        using check2 = ChangePolicy<PNoBatchMode, input>;
        static_assert(is_same<check2, PolicyContainer<SubPolicyContainer<Tag1, PNoBatchMode>, PNoBatchMode>>::value,
                      "Check Error");
        cout << "done" << endl;
    }
}
